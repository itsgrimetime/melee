...
mkdir -p "$OUTPUT_DIR"
OUTPUT_ABS="$(cd "$OUTPUT_DIR" && pwd)/$(basename "$3")"
MELEE_ROOT="${MELEE_ROOT:-/home/discord/permuter-work/melee}"
cd "$MELEE_ROOT"
...
"$MELEE_ROOT/tools/mwcc_debug/bin/wibo" "$MELEE_ROOT/build/compilers/GC/1.2.5n/mwcceppc_debug.exe" -Cpp_exceptions off -proc gekko -fp hard -fp_contract on -O4,p -enum int -nodefaults -inline auto -c "$STAGE" -o "$OUTPUT_ABS"
