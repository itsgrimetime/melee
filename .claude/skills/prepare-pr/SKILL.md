...
Open upstream PRs as drafts and always attach the `ai-assisted` label. Every
PR we open against doldecomp/melee with AI help must carry it from creation;
never rely on a maintainer to add it.

```bash
git push -u origin pr/<topic>
gh pr create --draft -R doldecomp/melee --base master \
  --head itsgrimetime:pr/<topic> --label ai-assisted \
  --title "..." --body "..."
```

If a PR was opened without the label, fix it before requesting review:

```bash
gh pr edit <number> -R doldecomp/melee --add-label ai-assisted
```
...
3. Affected modules
4. Push command
5. PR URL, confirming the `ai-assisted` label is set
