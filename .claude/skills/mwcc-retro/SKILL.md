...
The three `probe-backend-*` commands plus `backend-candidate` are the issue
#1240 preview toolset. Their v1 artifacts are diagnostic partial/candidate
evidence, not proof-bearing production traces. Do not treat a successful probe
as proof promotion; `backend-candidate --one-pass --trace-version v2` must remain
fail-closed until the exact lifetime-proof registry is independently promoted.
