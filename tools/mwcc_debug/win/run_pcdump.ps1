...
#                            testing)
#   MWCC_DEBUG_STAGE_SOURCE_STDIN
#                           (set to 1 to read staged source bytes from stdin
#                            and temporarily write them to the requested
#                            src-relative TU before compiling)
#   MWCC_DEBUG_STAGE_SOURCE_FILE
#                           (path to staged source bytes already uploaded to
#                            the remote host; used instead of stdin when set)
#   MWCC_DEBUG_STAGE_SOURCE_DELETE
#                           (set to 1 to delete MWCC_DEBUG_STAGE_SOURCE_FILE
#                            after the bytes have been read)
#   MWCC_DEBUG_STAGE_SOURCE_LABEL
#                           (optional local retained-source label for
#                            diagnostics when source staging is enabled)
#
# Output contract:
...
$stagedSourceOriginalBytes = $null
$stagedSourceActive = $false
$srcAbs = $null

# --- Stage source, install DLL, run, emit, and clean up under the lock ---
try {
    ...
    if ($env:MWCC_DEBUG_STAGE_SOURCE_STDIN -eq "1" -or $env:MWCC_DEBUG_STAGE_SOURCE_FILE) {
        $stageLabel = if ($env:MWCC_DEBUG_STAGE_SOURCE_LABEL) {
            $env:MWCC_DEBUG_STAGE_SOURCE_LABEL
        } else {
            if ($env:MWCC_DEBUG_STAGE_SOURCE_FILE) { $env:MWCC_DEBUG_STAGE_SOURCE_FILE } else { "<stdin>" }
        }
        $stageTransport = "stdin"
        $stageBuffer = $null
        if ($env:MWCC_DEBUG_STAGE_SOURCE_FILE) {
            $stageTransport = "file"
            $stageSourceFile = $env:MWCC_DEBUG_STAGE_SOURCE_FILE
            if (-not (Test-Path $stageSourceFile)) {
                Fail "staged source file not found: $stageSourceFile" 66
            }
            $stageBytes = [System.IO.File]::ReadAllBytes($stageSourceFile)
            if ($env:MWCC_DEBUG_STAGE_SOURCE_DELETE -eq "1") {
                Remove-Item -Force $stageSourceFile -ErrorAction SilentlyContinue
            }
        } else {
            $stdinStream = [Console]::OpenStandardInput()
            $stageBuffer = New-Object System.IO.MemoryStream
            $copyTask = $stdinStream.CopyToAsync($stageBuffer)
            if (-not $copyTask.Wait([TimeSpan]::FromSeconds($timeoutSecs))) {
                $stageBuffer.Dispose()
                Fail "stdin source staging timed out after ${timeoutSecs}s before compile" 124
            }
            $stageBytes = $stageBuffer.ToArray()
            $stageBuffer.Dispose()
        }
        if ($stageBytes.Length -eq 0) {
            Fail "$stageTransport source staging was requested but no source bytes were provided" 66
        }
        $sha256 = [System.Security.Cryptography.SHA256]::Create()
        try {
            $stageHash = [BitConverter]::ToString($sha256.ComputeHash($stageBytes)).Replace("-", "").ToLowerInvariant()
        } finally {
            $sha256.Dispose()
        }
        $stagedSourceOriginalBytes = [System.IO.File]::ReadAllBytes($srcAbs)
        $stagedSourceActive = $true
        [System.IO.File]::WriteAllBytes($srcAbs, $stageBytes)
        Write-Err "[mwcc_debug] staged source from $stageTransport label=$stageLabel into $srcRel ($($stageBytes.Length) bytes sha256=$stageHash)"
    }
    ...
    if ($stagedSourceActive -and $null -ne $stagedSourceOriginalBytes -and $srcAbs) {
        [System.IO.File]::WriteAllBytes($srcAbs, $stagedSourceOriginalBytes)
        Write-Err "[mwcc_debug] restored staged source $srcRel"
    }
