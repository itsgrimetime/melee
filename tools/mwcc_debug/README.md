...
- `melee-agent debug dump local <file.c> --trace-cse --function <fn>` emits
  grepable `IRO_CommonSub: replaced node 439 with 431` lines without vetoing
  the replacement, so the node IDs can be fed back into `--force-no-cse`.
