...
// MWCC_DEBUG_TRACE_CSE=1
//   Log every replacement as "IRO_CommonSub: replaced node X with Y" without
//   vetoing any replacement. Scope with MWCC_DEBUG_TRACE_CSE_FUNCTION.
//
...
static int g_trace_cse_enabled = 0;
static char g_trace_cse_scope_fn[FUNCNAME_BUF_LEN] = {0};
static int g_trace_cse_scope_fn_set = 0;
static int g_trace_cse_scope_fn_truncated = 0;
...
static void parse_force_no_cse_from_env(void)
{
    ...
    g_trace_cse_enabled = 0;
    g_trace_cse_scope_fn_set = 0;
    g_trace_cse_scope_fn_truncated = 0;
    ...
    len = GetEnvironmentVariableA(
        ...
        g_trace_cse_scope_fn, sizeof(g_trace_cse_scope_fn));
    if (len >= sizeof(g_trace_cse_scope_fn))
    {
        g_trace_cse_scope_fn_truncated = 1;
        g_trace_cse_scope_fn_set = 0;
    }
    else
    {
        g_trace_cse_scope_fn_set = (len > 0) ? 1 : 0;
    }
    if (!g_trace_cse_scope_fn_set)
        g_trace_cse_scope_fn[0] = '\0';

    len = GetEnvironmentVariableA(
        "MWCC_DEBUG_TRACE_CSE", trace_buf, sizeof(trace_buf));
    g_trace_cse_enabled =
        (len > 0 && len < sizeof(trace_buf) && trace_buf[0] != '0') ? 1 : 0;
...
static int trace_cse_scope_matches(void)
{
    const char *fn;
    if (!g_trace_cse_scope_fn_set) return 1;
    fn = force_no_cse_current_function_name();
    if (!fn) return 0;
    return str_eq(fn, g_trace_cse_scope_fn);
}
...
static int g_coalesce_class_filter = -1;
...
static void parse_coalesce_overrides_from_env(void)
{
    ...
    // Read optional register-class filter once at parse time. Empty keeps
    // legacy behavior. Values are MWCC class ids: 0=GPR, 1=FPR.
    g_coalesce_class_filter = -1;
    len = GetEnvironmentVariableA(
        "MWCC_DEBUG_FORCE_COALESCE_CLASS", buf, sizeof(buf));
    if (len > 0 && len < sizeof(buf) && buf[0] >= '0' && buf[0] <= '9') {
        g_coalesce_class_filter = (int)(buf[0] - '0');
    }
...
static void forced_common_subs_impl(int allow_veto, int trace_replacements)
{
    void *block;
            ...
            if (node_ref && (U32_AT(node, 0x02) & 8))
            {
                ...
                if (sub_holder)
                {
                    ...
                    if (PTR_AT(sub_holder, 0x06) != 0)
                    {
                        ...
                        if (allow_veto
                            && force_no_cse_rule_matches(at_id, with_id))
                        {
                            const char *fn =
                        ...
                        else
                        {
                            ...
                            if (trace_replacements
                                && mwcc_debug_output_active())
                            {
                                const char *fn =
                                    force_no_cse_current_function_name();
                                debug_printf(
                                    "IRO_CommonSub: replaced node %d "
                                    "with %d (fn=%s)\n",
                                    at_id, with_id, fn ? fn : "<unknown>");
                            }
                            debug_printf(
...
static void __cdecl hook_common_subs(void)
{
    ...
    if (g_force_no_cse_env_truncated
        ...
        || g_trace_cse_scope_fn_truncated
    ...
    allow_veto = (g_n_force_no_cse_rules != 0 && force_no_cse_scope_matches());
    trace_replacements = (g_trace_cse_enabled && trace_cse_scope_matches());

    if (!allow_veto && !trace_replacements)
    {
        ((fn_t)common_subs_trampoline)();
        return;
    }

    forced_common_subs_impl(allow_veto, trace_replacements);
}
...
static void __cdecl hook_real_coalesce(unsigned int rclass, unsigned int n_virtuals)
{
    ...
    forced_count = 0;
    {
        ...
        if (!scope_skip && g_coalesce_class_filter >= 0
            && (int)rclass != g_coalesce_class_filter) {
            scope_skip = 1;
            if (PCFILE && DEBUG_GUARD) {
                debug_printf("[FORCE_COALESCE] coalesce class skip "
                             "(class=%d, scope=%d)\n",
                             rclass, g_coalesce_class_filter);
            }
        }
