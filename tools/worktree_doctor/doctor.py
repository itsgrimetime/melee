...
class Doctor:
    ...
    def check_tooling_overlay(self) -> None:
        for rel_path in TOOLING_FILES:
            ...
            matches_master = _utils.matches_master(rel_path, path)
            if matches_master is None:
            ...
            if self.fix and not has_local_changes:
                ...
                restored = restore_from_master(rel_path, path)
            if restored:
                self.ok(f"refreshed stale tooling from master: {rel_path}")
            elif has_local_changes:
                self.warn(
                    f"tooling file differs from master and has local changes: {rel_path}",
                    "review local edits; worktree-doctor will not overwrite dirty tooling",
                )
            else:
                self.fail(
