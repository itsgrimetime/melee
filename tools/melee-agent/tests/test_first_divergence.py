...
def test_report_to_dict_serializes_allocator_and_source_fields():
    fact = fd.AllocatorFact(
        class_id=1, ig_idx=46, case=fd.DivergenceCase.C_DISPENSE_ORDER,
        iter_idx=12, baseline_reg=2, target_reg=26,
        coalesced_nodes=(), coalesced_root=None, coalesced_root_phys=None,
        blocker_ig=None, blocker_dependency=False,
        working_mask=frozenset({0, 1, 2}), cap_hit=False,
        earlier_unmapped_warning=True, local_target="shift simplify order")
    source = fd.SourceIdea(
        ig_idx=46,
        var_name=None,
        confidence=None,
        alternates=("row_offset",),
        ideas=("try same-line expression spelling",),
        first_def="B7: lfd f46,@1539(r1)",
        ...
        candidate_spans=("src/melee/mn/mndiagram.c:42 row_offset",),
    )

    payload = fd.report_to_dict(fd.FirstDivergenceReport(fact=fact, source=source))

    assert payload["kind"] == "allocator-first-divergence"
    assert payload["fact"]["case"] == "C"
    assert payload["fact"]["baseline_reg_name"] == "f2"
    assert payload["fact"]["target_reg_name"] == "f26"
    assert payload["fact"]["working_mask"] == [0, 1, 2]
    assert payload["source"]["first_def"] == "B7: lfd f46,@1539(r1)"
    assert payload["source"]["candidate_spans"] == [
        "src/melee/mn/mndiagram.c:42 row_offset"
    ]
