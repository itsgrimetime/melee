...
from src.mwcc_debug import frame_reservations as frame_reservations_mod
...
def test_parameter_word_count_model_separates_non_parameter_low_gap() -> None:
    model = frame_reservations_mod._parameter_word_count_model(
        current_floor={"start": 8, "end": 60, "size": 52, "word_count": 13},
        expected_floor={"start": 8, "end": 40, "size": 32, "word_count": 8},
        floor_delta_bytes=8,
        first_shifted_stack_home={
            "symbol": "sp28+12",
            "current_offset": 60,
            "expected_offset": 52,
            "offset_delta": 8,
        },
        measurement_source="first-shifted-symbolic-stack-home",
    )
    ...
    assert model == {
        ...
        "current_low_unused_words": 13,
        "expected_low_unused_words": 8,
        "current_low_unused_non_parameter_words": 3,
        "expected_low_object_after_parameter_floor_words": 3,
        "sizing_boundary": {
            "symbol": "sp28+12",
            "current_offset": 60,
            "expected_offset": 52,
            "offset_delta": 8,
            "offset_delta_words": 2,
        },
        "reason": (
            "first shifted stack home moves by 8 bytes (2 words), so the "
            "observed outgoing parameter floor is modeled as 10 words vs "
            "8 target words; 3 current low-unused word(s) sit above that "
            "parameter-area delta and correspond to the target low object/gap "
            "before sp28+12"
        ),
    }
...
def test_frame_reservation_reports_outgoing_parameter_floor_delta_without_frame_growth() -> None:
    pcdump = textwrap.dedent("""\
        Starting function fn_80000000
        FINAL CODE AFTER INSTRUCTION SCHEDULING
        fn_80000000
    ...
    current_asm = textwrap.dedent("""\
        .fn fn_80000000, global
        /* 80000000 */    stwu    r1, -0x98(r1)
        /* 80000004 */    lfs     f1, 0x50(r1)
        /* 80000008 */    stw     r3, 0x54(r1)
        /* 8000000c */    stw     r4, 0x58(r1)
        /* 80000010 */    stfd    f31, 0x90(r1)
        /* 80000014 */    addi    r1, r1, 0x98
        .endfn fn_80000000
    """)
    expected_asm = textwrap.dedent("""\
        .fn fn_80000000, global
        /* 80000000 */    stwu    r1, -0x98(r1)
        /* 80000004 */    addi    r5, r1, 0x48
        /* 80000008 */    lfs     f1, 0x48(r1)
        /* 8000000c */    stw     r3, 0x4c(r1)
        /* 80000010 */    stw     r4, 0x50(r1)
        /* 80000014 */    stfd    f31, 0x90(r1)
        /* 80000018 */    addi    r1, r1, 0x98
        .endfn fn_80000000
    """)
    ...
    assert report["frame_delta"] == 0
    floor = report["outgoing_parameter_area_floor"]
    assert floor["status"] == "current-floor-larger"
    assert floor["origin"] == "reserved-outgoing-parameter-area-floor"
    assert floor["current_floor"] == {
        "start": 8,
        "end": 0x50,
        "size": 0x48,
        "word_count": 18,
    ...
    assert floor["floor_delta_bytes"] == 8
    assert floor["floor_delta_words"] == 2
    assert floor["unused_range_delta_bytes"] == 8
    assert floor["first_shifted_stack_home"] == {
        "symbol": "sp48",
        "current_offset": 0x50,
        "expected_offset": 0x48,
        "offset_delta": 8,
    }
    assert floor["shifted_stack_home_count"] == 3
    assert floor["current_accesses_in_excess_floor"] == []
    assert floor["call_attribution"]["status"] == "unattributed-reserved-floor"
    assert floor["parameter_word_count_model"] == {
        "status": "computed",
        "measurement_source": "first-shifted-symbolic-stack-home",
        "floor_start": 8,
        "expected_parameter_words": 16,
        "current_parameter_words": 18,
        "extra_parameter_words": 2,
        "current_low_unused_words": 18,
        "expected_low_unused_words": 16,
        "current_low_unused_non_parameter_words": 0,
        "expected_low_object_after_parameter_floor_words": 0,
        "sizing_boundary": {
            "symbol": "sp48",
            "current_offset": 0x50,
            "expected_offset": 0x48,
        ...
        "reason": (
            "first shifted stack home moves by 8 bytes (2 words), so the "
            "observed outgoing parameter floor is modeled as 18 words vs "
            "16 target words; no extra current low-unused words remain "
            "outside that parameter-area delta"
        ),
    }
    assert "outgoing parameter area floor" in report["summary"]
    assert "8 bytes (2 words) larger" in report["summary"]
    assert "parameter words 18 vs 16" in report["summary"]


def test_frame_reservation_attributes_same_frame_local_area_to_source_array() -> None:
    pcdump = textwrap.dedent("""\
        Starting function fn_80000000
        FINAL CODE AFTER INSTRUCTION SCHEDULING
        fn_80000000
    ...
    expected_asm = textwrap.dedent("""\
        .fn fn_80000000, global
        /* 80000000 */    stwu    r1, -0x218(r1)
        /* 80000004 */    addi    r4, r1, 0x18
        /* 80000008 */    addi    r5, r1, 0x18
        /* 8000000c */    stfd    f30, 0x1f8(r1)
        /* 80000010 */    stfd    f31, 0x210(r1)
        /* 80000014 */    addi    r1, r1, 0x218
        .endfn fn_80000000
    """)
    ...
    report = analyze_frame_reservations(
        ...
        source_function_names=("fn_80000000",),
    )

    divergence = report["frame_first_divergence"]
    assert divergence["status"] == "diverged"
    attribution = divergence["source_attribution"]
    assert attribution["status"] == "source-object-attributed"
    assert attribution["identity_kind"] == "source-local-array-size-range"
    primary = attribution["primary_source_object"]
    assert primary["symbol"] == "totals"
    assert primary["type"] == "u32"
    assert primary["element_count"] == 0x78
    assert primary["element_size"] == 4
    assert primary["size"] == 480
    assert primary["expected_offset"] == 0x18
    assert primary["expected_range"] == {
        "start": 0x18,
        "end": 0x1F8,
        "size": 480,
    }
    assert any("tp = totals" in item["text"] for item in primary["address_evidence"])
    assert divergence["cause_hypothesis"]["kind"] == (
        "local-area-vs-outgoing-floor-divergence"
    )
    assert divergence["verdict"]["status"] == "source-reachable-candidate"
    plan_commands = [
        item["command"]
        for item in divergence["frame_transform_probe_plan"]["suggested_commands"]
    ]
    assert any("indexed_byte_address_temp_steering" in cmd for cmd in plan_commands)
    floor = report["outgoing_parameter_area_floor"]
    assert floor["local_area_attribution"]["primary_source_object"]["symbol"] == "totals"


def test_frame_reservation_resolves_symbolic_addi_local_area_as_current_source_object() -> None:
    pcdump = textwrap.dedent("""\
        Starting function fn_80000000
        FINAL CODE AFTER INSTRUCTION SCHEDULING
        fn_80000000
    ...
    current_asm = textwrap.dedent("""\
        .fn fn_80000000, global
        /* 80000000 */    stwu    r1, -0x218(r1)
        /* 80000004 */    addi    r25, r1, 0x18
        /* 80000008 */    stw     r28, 0(r25)
        /* 8000000c */    addi    r30, r1, 0x18
        /* 80000010 */    lwzx    r0, r30, r0
        /* 80000014 */    lmw     r25, 0x1f8(r1)
        /* 80000018 */    addi    r1, r1, 0x218
        .endfn fn_80000000
    """)
    expected_asm = textwrap.dedent("""\
        .fn fn_80000000, global
        /* 80000000 */    stwu    r1, -0x218(r1)
        /* 80000004 */    addi    r25, r1, 0x18
        /* 80000008 */    addi    r30, r1, 0x18
        /* 8000000c */    lmw     r25, 0x1f8(r1)
        /* 80000010 */    addi    r1, r1, 0x218
        .endfn fn_80000000
    """)
    ...
    current_objects = report["current"]["stack_objects"]
    totals_object = next(
        item
        for item in current_objects
        if item.get("kind") == "address-taken-local-or-temporary"
    )
    assert totals_object["start"] == 0x18
    assert totals_object["end"] == 0x1F8
    assert totals_object["source_symbols"] == ["totals"]
    assert totals_object["expected_source_offsets"] == {"totals": 0x18}
    assert totals_object["first_access"]["operands"] == "r25,r1,totals"
    assert totals_object["first_access"]["resolved_operands"] == "r25,r1,24"
    derived_operands = {
        item["operands"] for item in totals_object["derived_symbolic_accesses"]
    }
    assert "r28,0(r25)" in derived_operands
    assert "r0,(r30,r0)" in derived_operands
    assert report["outgoing_parameter_area_floor"] is None
    assert (
        report["frame_first_divergence"].get("cause_hypothesis", {}).get("kind")
        != "local-area-vs-outgoing-floor-divergence"
    )


def test_frame_reservation_invalidates_symbolic_base_after_register_clobber() -> None:
    pcdump = textwrap.dedent("""\
        Starting function fn_80000000
        FINAL CODE AFTER INSTRUCTION SCHEDULING
        fn_80000000
    ...
    current_asm = textwrap.dedent("""\
        ...
        /* 80000004 */    addi    r25, r1, 0x18
        /* 80000008 */    li      r25, 0
        /* 8000000c */    stw     r28, 0(r25)
        /* 80000010 */    lmw     r25, 0x1f8(r1)
        /* 80000014 */    addi    r1, r1, 0x218
        .endfn fn_80000000
    """)
    ...
    totals_object = next(
        item
        for item in current_objects
        if item.get("source_symbols") == ["totals"]
    )
    assert "derived_symbolic_accesses" not in totals_object
    assert totals_object["access_count"] == 1
    assert totals_object["opcodes"] == ["addi"]


def test_frame_reservation_invalidates_symbolic_bases_after_multi_register_clobber() -> None:
    pcdump = textwrap.dedent("""\
        Starting function fn_80000000
        FINAL CODE AFTER INSTRUCTION SCHEDULING
        fn_80000000
    ...
    current_asm = textwrap.dedent("""\
        .fn fn_80000000, global
        /* 80000000 */    stwu    r1, -0x218(r1)
        /* 80000004 */    addi    r26, r1, 0x18
        /* 80000008 */    lmw     r25, 0x1f8(r1)
        /* 8000000c */    stw     r28, 0(r26)
        /* 80000010 */    addi    r1, r1, 0x218
        .endfn fn_80000000
    """)
    ...
    totals_object = next(
        item
        for item in report["current"]["stack_objects"]
        if item.get("source_symbols") == ["totals"]
    )
    assert "derived_symbolic_accesses" not in totals_object
    assert totals_object["access_count"] == 1
...
def test_frame_reservation_invalidates_volatile_symbolic_base_after_call() -> None:
    pcdump = textwrap.dedent("""\
        Starting function fn_80000000
        FINAL CODE AFTER INSTRUCTION SCHEDULING
        fn_80000000
    ...
    current_asm = textwrap.dedent("""\
        .fn fn_80000000, global
        /* 80000000 */    stwu    r1, -0x218(r1)
        /* 80000004 */    addi    r3, r1, 0x18
        /* 80000008 */    bl      helper
        /* 8000000c */    stw     r28, 0(r3)
        /* 80000010 */    lmw     r25, 0x1f8(r1)
        /* 80000014 */    addi    r1, r1, 0x218
        .endfn fn_80000000
    """)
...
def test_frame_reservation_leaves_unresolved_symbolic_addi_unmaterialized() -> None:
    pcdump = textwrap.dedent("""\
        Starting function fn_80000000
        FINAL CODE AFTER INSTRUCTION SCHEDULING
        fn_80000000
    ...
    assert all(
        item.get("source_symbols") != ["totals"]
        for item in report["current"]["stack_objects"]
    )


def test_frame_transform_scores_attributed_local_area_before_frame_size() -> None:
    pcdump = textwrap.dedent("""\
        Starting function fn_80000000
        FINAL CODE AFTER INSTRUCTION SCHEDULING
        fn_80000000
    ...
    expected_asm = textwrap.dedent("""\
        ...
        /* 80000000 */    stwu    r1, -0x218(r1)
        /* 80000004 */    addi    r4, r1, 0x18
        /* 80000008 */    stfd    f30, 0x1f8(r1)
        /* 8000000c */    stfd    f31, 0x210(r1)
        /* 80000010 */    addi    r1, r1, 0x218
        .endfn fn_80000000
    """)
    source = textwrap.dedent("""\
    ...
    neutral_frame = analyze_frame_from_asm_text(textwrap.dedent("""\
        .fn fn_80000000, global
        /* 80000000 */    stwu    r1, -0x218(r1)
        /* 80000004 */    stfd    f31, 0x210(r1)
        /* 80000008 */    addi    r1, r1, 0x218
        .endfn fn_80000000
    """))
    local_area_frame = analyze_frame_from_asm_text(expected_asm)

    evaluation = evaluate_frame_transform_probe_results(
        report,
        [
            {
                "label": "neutral-same-frame",
                "operator": "frame-local-dematerialize",
                "status": "ok",
                "frame": neutral_frame,
                "frame_size": neutral_frame["frame_size"],
                "match_percent": 80.0,
            },
            {
                "label": "indexed-byte-local-area",
                "operator": "transform-corpus:indexed_byte_address_temp_steering",
                "status": "ok",
                "frame": local_area_frame,
                "frame_size": local_area_frame["frame_size"],
                "match_percent": 75.0,
                "provenance": {
                    "target_assignments": ["ig34->r27", "ig44->r25"],
                },
            },
        ],
    )

    assert evaluation["verdict"] == "source-reachable-local-area-transform"
    assert evaluation["best_variant"]["label"] == "indexed-byte-local-area"
    assert evaluation["best_variant"]["target_local_area_fixed"] is True
    assert evaluation["best_variant"]["transform_force_phys_target_count"] == 2
    assert evaluation["stop_condition"]["kind"] == "validated-local-area-transform"


def test_frame_transform_terminal_summary_names_unresolved_low_frame_gap() -> None:
    pcdump = textwrap.dedent("""\
        Starting function fn_80000000
        FINAL CODE AFTER INSTRUCTION SCHEDULING
        fn_80000000
    ...
    assert evaluation["verdict"] == "local-area-transform-ceiling-candidate"
    assert evaluation["stop_condition"]["kind"] == "bounded-local-area-transform-ceiling"
    assert evaluation["stop_condition"]["source_object_symbol"] == "totals"
    assert "totals" in evaluation["stop_condition"]["reason"]
    summary = evaluation["terminal_summary"]
    assert summary["status"] == "blocked"
    assert summary["blocker"] == "no-source-actionable-low-frame-lever"
    assert summary["low_frame_gap"]["source_object_symbol"] == "totals"
    assert summary["low_frame_gap"]["baseline_remaining_bytes"] == 480
...
def test_frame_reservation_marks_stack_home_reorder_downstream_when_registers_dominate() -> None:
    pcdump = textwrap.dedent("""\
        Starting function fn_80000000
        AFTER REGISTER COLORING
        fn_80000000
    ...
    report = analyze_frame_reservations(
        pcdump,
        "fn_80000000",
        expected_asm_text=expected,
        current_asm_text=current,
    )

    cascade = report["register_coloring_cascade"]
    assert cascade["status"] == "dominant"
    assert cascade["verdict"] == "dominant-register-coloring-cascade"
    assert cascade["register_only_paired_line_count"] == 11
    assert cascade["stack_slot_paired_line_count"] == 2
    assert cascade["first_frame_model_change_after_register_coloring"] is True
    assert cascade["first_register_only_mismatch"]["expected"].endswith("li r29, 0")
    guidance = report["current"]["stack_home_reorder_guidance"]
    assert guidance["priority"] == "secondary-register-coloring-dominates"
    assert "downstream" in guidance["dominant_residual"]["reason"]
...
def test_frame_transform_evaluation_scores_parameter_word_reduction() -> None:
    report = {
    ...
    unchanged = {
        ...
        "objective": {
            "frame_after": 240,
            "outgoing_parameter_words": 10,
        },
    }
    fixed = {
        "label": "materialized-call",
        "operator": "transform-corpus:outgoing_parameter_area_shape",
        "status": "ok",
        "match_percent": 99.2,
        "objective": {
            "frame_after": 240,
            "outgoing_parameter_words": 8,
        },
    }

    evaluation = evaluate_frame_transform_probe_results(report, [unchanged, fixed])

    assert evaluation["verdict"] == "source-reachable-parameter-area-transform"
    assert evaluation["best_variant"]["label"] == "materialized-call"
    assert evaluation["best_variant"]["candidate_parameter_words"] == 8
    assert evaluation["best_variant"]["remaining_parameter_word_delta"] == 0
    assert evaluation["best_variant"]["parameter_word_improvement"] == 2
    assert evaluation["stop_condition"] == {
        "status": "satisfied",
        "kind": "validated-parameter-area-transform",
        "reason": (
            "probe materialized-call moves outgoing parameter area to expected "
            "8 word(s)"
        ),
        "variant_label": "materialized-call",
        "remaining_parameter_word_delta": 0,
        "parameter_word_improvement": 2,
    }


def test_frame_transform_evaluation_reports_partial_parameter_word_reduction() -> None:
    report = {
        "current": {"frame_size": 240},
        "expected": {"frame_size": 240},
        "frame_delta": 0,
        "outgoing_parameter_area_floor": {
            "parameter_word_count_model": {
                "status": "computed",
                "expected_parameter_words": 8,
                "current_parameter_words": 13,
                "extra_parameter_words": 5,
            },
        },
    }
    partial = {
    ...
    evaluation = evaluate_frame_transform_probe_results(report, [partial])

    assert evaluation["verdict"] == (
        "partial-source-reachable-parameter-area-transform"
    )
    assert evaluation["stop_condition"] == {
        "status": "partial",
        "kind": "partial-parameter-area-transform",
        "reason": (
            "probe materialized-call reduces absolute outgoing parameter-area "
            "word delta by 3 word(s)"
        ),
        "variant_label": "materialized-call",
        "remaining_parameter_word_delta": -2,
        "parameter_word_improvement": 3,
    }


def test_frame_transform_parameter_family_terminal_proof_keeps_retained_artifacts() -> None:
    report = {
        "current": {"frame_size": 240},
        "expected": {"frame_size": 240},
        "frame_delta": 0,
    ...
    unchanged = {
        "label": "bounded-source-probe",
        "operator": "transform-corpus:outgoing_parameter_area_shape",
        "status": "ok",
        "source_retained": "/tmp/bounded-source-probe.c",
        "pcdump_path": "/tmp/bounded-source-probe.pcdump.txt",
        "source_hunks": [{"label": "bounded-source-probe", "old_start": 1}],
        "checkdiff": {"match_percent": 95.0, "normalized_diff_lines": 20},
        "objective": {
            "frame_after": 240,
            "outgoing_parameter_words": 10,
        },
    ...
    evaluation = evaluate_frame_transform_probe_results(report, [unrelated, unchanged])

    assert evaluation["verdict"] == "parameter-area-transform-ceiling-candidate"
    family = evaluation["terminal_summary"]["outgoing_parameter_area_family"]
    assert family["status"] == "exhausted-no-improvement"
    assert family["generated_probe_count"] == 1
    assert family["measured_probe_count"] == 1
    assert "source-level lifetime/order model" in family["handoff"]
    retained = family["best_variant"]
    assert retained["source_retained"] == "/tmp/bounded-source-probe.c"
    assert retained["pcdump_path"] == "/tmp/bounded-source-probe.pcdump.txt"
    assert retained["source_hunks"] == [{"label": "bounded-source-probe", "old_start": 1}]
    assert retained["checkdiff"] == {
        "match_percent": 95.0,
        "normalized_diff_lines": 20,
    }


def test_frame_transform_evaluation_recognizes_compiled_parameter_area_fix() -> None:
    expected_frame = {
        "frame_size": 240,
        "unused_ranges": [{"start": 8, "end": 40, "size": 32}],
    }
    report = {
        "current": {"frame_size": 240},
    ...
    fixed = {
        "label": "compiled-materialized-call",
        "operator": "transform-corpus:outgoing_parameter_area_shape",
        "status": "ok",
        "frame": {
            "frame_size": 240,
    ...
    evaluation = evaluate_frame_transform_probe_results(report, [fixed])

    assert evaluation["verdict"] == "source-reachable-parameter-area-transform"
    assert evaluation["best_variant"]["candidate_parameter_words"] == 8
    assert evaluation["stop_condition"]["kind"] == (
        "validated-parameter-area-transform"
    )


def test_parameter_area_ceiling_requires_parameter_area_operator() -> None:
    report = {
        "current": {"frame_size": 240},
        "expected": {"frame_size": 240},
        "frame_delta": 0,
    ...
    evaluation = evaluate_frame_transform_probe_results(report, [unrelated])
