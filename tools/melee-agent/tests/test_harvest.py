...
def _row(
    ...
    *,
    ...
    frame_evidence: str | None = None,
    frame_probe_status: str | None = None,
    frame_match_relevance: str | None = None,
    frame_next_command: str = "",
    next_command: str = "",
) -> dict[str, str]:
    has_frame_workflow = bool(frame_closability_tier) or headline_tool in {
    ...
    return {
        ...
        "frame_evidence": (
            ...
            else ("pcdump-attributed" if has_frame_workflow else "")
        ),
        "frame_probe_status": (
            frame_probe_status
            if frame_probe_status is not None
            else ("materializable" if has_frame_workflow else "")
        ),
        "frame_match_relevance": (
            frame_match_relevance
            if frame_match_relevance is not None
            else ("unknown" if has_frame_workflow else "")
        ),
...
def test_attempt_ledger_no_longer_demotes_queue_rows(
    tmp_path: Path,
) -> None:
    repo_root = _repo_with_source(tmp_path)
    queue = tmp_path / "queues" / "indexed-struct-pointer.tsv"
    ...
    # The ledger no longer demotes or filters the "blocked" function: both rows
    # are returned, in queue order, and neither is treated as a terminal
    # dead-end attempt.
    assert [row.function for row in rows] == ["blocked_fn", "ready_fn"]
    assert all(row.terminal_attempt_status == "" for row in rows)
    assert preview["counts"]["terminal_attempt_rows"] == 0

    # The previously-"blocked" function keeps its original actionability and a
    # runnable harness — it is not routed away from agents.
    blocked_row = next(row for row in rows if row.function == "blocked_fn")
    assert blocked_row.source_actionability == "current-tools-indexed-pointer"
    assert select_harness(blocked_row) is not None
...
def test_allocator_force_vector_no_match_rebuckets_already_satisfied(
    ...
    assert "by_tier" not in ledger["summary"]
...
def test_ledger_summary_counts_status_harness_frame_probe_status_and_blocker() -> None:
    results = [
        {
            "status": "validated",
            "harness": "frame-transform-search",
        ...
        },
        {
            ...
            "frame_probe_status": "needs-attribution",
...
def test_legacy_frame_preview_is_normalized_without_closability_copy(
    tmp_path: Path,
) -> None:
    repo_root = tmp_path / "repo"
    repo_root.mkdir()
    ...
    legacy["frame_cause"] = "stack-object-offset-shift"
    legacy["frame_verdict"] = "checkdiff-only"
    legacy["frame_attribution_status"] = "checkdiff-only"
    _write_queue(queue, [legacy])
    ...
    assert sample["frame_evidence"] == "checkdiff-only"
    assert sample["frame_probe_status"] == "needs-attribution"
    assert sample["frame_match_relevance"] == "match-neutral"
    assert sample["source_actionability"] == "diagnostic-only"
    assert "frame_closability_tier" not in sample
    assert "#362" not in json.dumps(preview)
    assert "reorder-gated-362" not in json.dumps(preview)


def test_harvest_normalizes_shared_legacy_frame_compatibility_cases(
    tmp_path: Path,
) -> None:
    repo_root = tmp_path / "repo"
    repo_root.mkdir()
    queue = tmp_path / "queue.tsv"
    _write_queue(queue, [case["input"] for case in LEGACY_FRAME_COMPATIBILITY])

    preview = preview_harvest_queue(
        queue,
        work_bucket="stack-local-layout",
        repo_root=repo_root,
        sample_limit=len(LEGACY_FRAME_COMPATIBILITY),
    )
    ...
    by_function = {row["function"]: row for row in preview["sample"]}
    request_by_function = {request.function: request for request in requests}
    for case in LEGACY_FRAME_COMPATIBILITY:
        function = case["input"]["function"]
        normalized = by_function[function]
        for key, expected in case["expected"].items():
            assert normalized[key] == expected, (case["case"], key)
        assert "frame_closability_tier" not in normalized
        request = request_by_function[function]
        assert request.frame_closability_tier == ""
    assert select_harness(request_by_function["legacy_inspect_only"]) is None
    assert select_harness(request_by_function["legacy_materializable"]) == (
        "frame-transform-search"
    )
...
def test_frame_probe_status_routes_only_materializable_work() -> None:
    base = dict(
        function="demo_fn",
        work_bucket="stack-local-layout",
        match_percent=99.0,
    ...
    materializable = HarvestRequest(
        **base,
        frame_probe_status="materializable",
    )
    terminal = HarvestRequest(
        **base,
        frame_probe_status="terminal-no-safe-lever",
    )

    assert select_harness(materializable) == "lifetime-layout"
    assert select_harness(terminal) is None
