...
def test_coalesced_alias_node_reports_coalesced_away() -> None:
    ...
    report = analyze_lifetime_pressure(
        facts,
        parse_force_phys_spec("40:31", default_class_id=0),
    )
...
def test_backend_trace_fixture_maps_to_allocator_facts() -> None:
    ...
    assert nodes[33].color_status == "colored"
    assert nodes[33].color_decision_ref == "gpr-c1"
    assert nodes[40].color_status == "coalesced_alias"
    assert nodes[40].coalesced_into == 32
    assert nodes[40].assigned_phys == nodes[32].assigned_phys
    assert mappings[0]["alias"] == 40
    assert mappings[0]["root"] == 32
    assert "root_phys" in mappings[0]
    assert decisions["gpr-c0"].id == "gpr-c0"
    assert decisions["gpr-c0"].provenance == "colorgraph"
    assert any(
