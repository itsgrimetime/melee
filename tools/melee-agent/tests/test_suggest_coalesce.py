...
def test_cli_smoke_invokes_command() -> None:
    ...
    proc = subprocess.run(
        ["python", "-m", "src.cli", "debug", "suggest", "coalesce", "--help"],
...
def test_cli_rejects_both_pair_and_discover() -> None:
    ...
    proc = subprocess.run(
        ["python", "-m", "src.cli", "debug", "suggest", "coalesce",
         "-f", "any_fn", "-V", "53=3", "--discover"],
        cwd=pathlib.Path(__file__).parent.parent,  # tools/melee-agent
        capture_output=True, text=True, timeout=15,
    )
    assert proc.returncode != 0
    assert "exactly one of --pair / --trace-copy-json / --discover" in proc.stderr
...
def test_cli_pair_mode_accepts_fpr_pair_and_uses_fpr_facts(
    tmp_path: pathlib.Path,
) -> None:
    pcdump = tmp_path / "fpr.pcdump.txt"
    pcdump.write_text(FPR_COLLIDING_PCDUMP)
    ...
    assert pair["from"] == 46
    assert pair["to"] == 56
    assert pair["ir_facts"]["from"]["first_def"]["opcode"] == "fsubs"
    assert pair["ir_facts"]["to"]["first_def"]["opcode"] == "fmadds"
    assert pair["ir_facts"]["to"]["first_def"]["opcode"] != "lwz"
    assert "f46" in pair["preflight"]["reasons"][0]


def test_cli_pair_mode_derives_fpr_pair_from_trace_copy_json(
    tmp_path: pathlib.Path,
) -> None:
    pcdump = tmp_path / "fpr.pcdump.txt"
    ...
    assert payload["register_class"] == "fpr"
    assert payload["pairs"][0]["from"] == 46
    assert payload["pairs"][0]["to"] == 56


def test_cli_trace_copy_json_honors_top_level_register_class(
    tmp_path: pathlib.Path,
) -> None:
    pcdump = tmp_path / "fpr.pcdump.txt"
    trace_copy = tmp_path / "trace-copy.json"
    ...
    trace_copy.write_text(json.dumps({
        ...
        "from_virtual": 56,
        "to_virtual": 46,
        "from_mapping": {"assigned_reg": 0, "ig_idx": 56},
        "to_mapping": {"assigned_reg": 1, "ig_idx": 46},
    }))
...
def test_cli_trace_copy_json_rejects_register_class_conflict(
    tmp_path: pathlib.Path,
) -> None:
    pcdump = tmp_path / "fpr.pcdump.txt"
    trace_copy = tmp_path / "trace-copy.json"
    ...
    trace_copy.write_text(json.dumps({
        ...
        "from_virtual": 56,
        "to_virtual": 46,
        "from_mapping": {"class_id": 0, "assigned_reg": 0, "ig_idx": 56},
        "to_mapping": {"class_id": 0, "assigned_reg": 1, "ig_idx": 46},
    }))
    ...
    assert "register_class conflicts" in result.stderr
...
def test_render_text_includes_all_pair_fields() -> None:
    ...
    assert "try:" in out
    assert "Catalog: debug util patterns alias-split" in out
...
def test_render_outputs_source_shape_summary_for_trace_copy_mode() -> None:
    report = Report(
        function="test_fn",
        mode="pair",
        ...
        pairs=[PairReport(
            from_virt=64,
            to_virt=34,
            ir_facts={"from": {"virtual": 64, "is_phys": False},
                      "to":   {"virtual": 34, "is_phys": False}},
            suggestions=[],
        )],
        source_shape_summary={
            "kind": "call-return-use-shape-continuation",
            "status": "source-shape-probe-required",
            "source_expression": "GetNameText(post_ceiling_j_name)",
            "assigned_local": "post_ceiling_j_text",
            "candidate_families": ["direct-use", "duplicate-call"],
            "next_command": "melee-agent debug coalesce-search --trace-copy-json x",
        },
    )

    parsed = json.loads(render_json(report))
    assert parsed["source_shape_summary"]["kind"] == (
        "call-return-use-shape-continuation"
    )
    text = render_text(report)
    assert "Source-shape continuation:" in text
    assert "GetNameText(post_ceiling_j_name)" in text
    assert "direct-use, duplicate-call" in text
