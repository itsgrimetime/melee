...
def test_trace_copy_detects_pcode_only_fpr_call_argument_fmr() -> None:
    report = trace_copy_lifetime(
        PCDUMP_FPR_CALL_ARG_COPY,
        "mnDiagram_DrawCellNumber",
        from_virtual=34,
        to_virtual=1,
        reg_class="fpr",
    ...
    assert report.status == "copy-found"
    assert report.register_class == "fpr"
    assert report.copy_opcode == "fmr"
    assert report.from_token == "f34"
    assert report.to_token == "f1"
    assert report.first_copy is not None
    assert report.first_copy.opcode == "fmr"
    assert report.first_copy.operands == "f1,f34"
    assert report.from_mapping.status == "pcode-only"


def test_list_copy_lifetimes_discovers_fpr_physical_argument_copy() -> None:
    reports = list_copy_lifetimes(
        PCDUMP_FPR_CALL_ARG_COPY,
        "mnDiagram_DrawCellNumber",
        involving=34,
    ...
    assert reports[0].copy_opcode == "fmr"


def test_trace_copy_cli_renders_fpr_tokens(tmp_path: pathlib.Path) -> None:
    pcdump = tmp_path / "pcdump.txt"
    pcdump.write_text(PCDUMP_FPR_CALL_ARG_COPY)

    proc = subprocess.run(
        [
            "python", "-m", "src.cli", "debug", "inspect", "trace-copy",
    ...
    assert proc.returncode == 0
    assert "Copy:     f1 <- f34" in proc.stdout
    assert "fmr f1,f34" in proc.stdout
    assert "mr r1,r34" not in proc.stdout


def test_trace_copy_cli_defaults_to_retained_source_sibling(
    tmp_path: pathlib.Path,
) -> None:
    pcdump = tmp_path / "candidate.pcdump.txt"
    source = tmp_path / "candidate.c"
    pcdump.write_text(PCDUMP_CALL_RETURN_CHAIN)
    source.write_text(SOURCE_CALL_RETURN_CHAIN)

    proc = subprocess.run(
        [
            "python", "-m", "src.cli", "debug", "inspect", "trace-copy",
            "--function", "fn_80000002",
            "--from", "r43",
            "--to", "r40",
            "--json",
            str(pcdump),
    ...
    origin = payload["to_mapping"]["call_return_origin"]
    assert origin["source_file"] == str(source)
    assert origin["source_line"] == 3
    assert origin["expression"] == "helper_fn(entity)"
    assert origin["assigned_local"] == "result"
