...
def test_match_virtual_treats_expected_addi_zero_as_precolor_mr() -> None:
    expected_ist = AsmInstruction(
        opcode="addi",
        operands="r31, r6, 0x0",
        regs=[("r", 31), ("r", 6)],
...
def test_match_virtual_for_expected_def_supports_fpr_destinations() -> None:
    expected_ist = AsmInstruction(
        opcode="fmr", operands="f31, f1", regs=[("f", 31), ("f", 1)]
    )
    pre = Pass(name="AFTER PEEPHOLE FORWARD")
    block = Block(index=0, succ=[], pred=[], labels=["L0"])
    block.instructions = [
        _make_ist("fmr", "f44, f1", [("f", 44), ("f", 1)]),
        _make_ist("fmr", "f3, f2", [("f", 3), ("f", 2)]),
    ]
    pre.blocks.append(block)
    ...
    assert result is not None
    assert result.virtual == 44
    assert result.ig_idx == 44
    assert result.instruction_index == 0
