...
def test_mutate_type_change_splits_combined_c89_declaration() -> None:
    """`int i, j;` should be rewritten without changing `j`'s type."""
    source = textwrap.dedent("""\
        void f(void) {
            int i, j;
            i = j;
    ...
    """)
    result = mutate_type_change(source, "f", "i", "long")
    assert "long i;" in result
    assert "int j;" in result
    assert "long i, j;" not in result


def test_mutate_type_change_preserves_combined_decl_indentation() -> None:
    source = textwrap.dedent("""\
        void f(void)
        {
            int i, j;
...
def test_mutate_type_change_splits_second_combined_declarator() -> None:
    """Changing the second name in `int i = 0, j;` preserves the first."""
    source = "void f(void) { int i = 0, j; i = j; }"
    result = mutate_type_change(source, "f", "j", "long")
    assert "int i = 0;" in result
    assert "long j;" in result
...
def test_mutate_anti_coalesce_copy_before_use_adds_volatile_roundtrip() -> None:
    source = textwrap.dedent("""\
    ...
    result = mutate_anti_coalesce_copy_before_use(
        source,
        "f",
        "jobj",
        at_stmt_index=2,
    ...
    assert "HSD_JObj* jobj_split;" in result
    assert "volatile HSD_JObj* jobj_split_barrier;" in result
    assert (
        "jobj_split = jobj;\n"
        "    jobj_split_barrier = jobj_split;\n"
        "    jobj_split = jobj_split_barrier;\n"
        "    HSD_JObjSetMtxDirtySub(jobj_split);"
    ) in result
    assert "HSD_JObjSetTranslateX(jobj, 1.0f);" in result
