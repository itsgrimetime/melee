...
def test_skill_capabilities_can_exclude_untracked_local_skills(tmp_path):
    skills_dir = tmp_path / ".claude" / "skills"
    tracked_dir = skills_dir / "tracked-skill"
    local_dir = skills_dir / "local-skill"
    tracked_dir.mkdir(parents=True)
    local_dir.mkdir()
    (tracked_dir / "SKILL.md").write_text("# Tracked skill\n")
    (local_dir / "SKILL.md").write_text("# Local skill\n")
    subprocess.run(["git", "init", "-q"], cwd=tmp_path, check=True)
    subprocess.run(
        ["git", "add", ".claude/skills/tracked-skill/SKILL.md"],
        cwd=tmp_path,
        check=True,
    )

    all_names = {c.name for c in cap.skill_capabilities(tmp_path)}
    tracked_names = {
        c.name for c in cap.skill_capabilities(tmp_path, tracked_only=True)
    }

    assert all_names == {"tracked-skill", "local-skill"}
    assert tracked_names == {"tracked-skill"}
...
def test_search_relevance_regression():
    ...
    assert "debug search plan-transforms" in [
        c.name for c in cap.run_search("retained Case-C sensitivity", REPO)
    ]
    assert "debug search plan-transforms" in [
        c.name
        for c in cap.run_search("retained IG44 window-order continuation", REPO)
    ]
    field_load_names = [
        c.name for c in cap.run_search("field load source order", REPO)
    ]
    assert "debug select-order-search" in field_load_names
    assert "debug search plan-transforms" in field_load_names
    assert "debug search retained-frontiers" in [
        c.name
        for c in cap.run_search("post ceiling retained frontier triage", REPO)
    ]
    assert "debug search retained-frontiers" in [
        c.name
        for c in cap.run_search("alternate retained frontier ranking", REPO)
    ]
    assert "debug search retained-frontiers" in [
        c.name
        for c in cap.run_search("meta ceiling synthesis", REPO)
    ]
    assert "debug search source-model-synthesis" in [
        c.name
        for c in cap.run_search("sort source model synthesis", REPO)
    ]
    assert "debug search source-model-synthesis" in [
        c.name
        for c in cap.run_search("sort post cross tu source hypothesis", REPO)
    ]
    assert "debug search source-model-synthesis" in [
        c.name
        for c in cap.run_search(
    ...
    assert "debug search source-model-synthesis" in [
        c.name
        for c in cap.run_search("post meta ceiling source family", REPO)
    ]
    assert "debug search source-model-synthesis" in [
        c.name
        for c in cap.run_search(
    ...
    assert "debug search source-family-continuation" in [
        c.name
        for c in cap.run_search("post meta source family continuation", REPO)
    ]
    assert "debug solve allocator-ceiling" in [
        c.name
        for c in cap.run_search("all known frontiers exhausted", REPO)
    ]
    stack_clean = [
        c.name
        for c in cap.run_search("stack clean no anchor continuation", REPO)
    ]
    assert "debug search source-family-continuation" in stack_clean
    assert "debug inspect stack-homes" in stack_clean
    assert "debug suggest expression-interferer-repair" in stack_clean
    assert "debug suggest protected-expression-reconcile" in stack_clean
    assert "debug suggest inline-boundary-continuation" in stack_clean
    dimension = [
        c.name
        for c in cap.run_search(
    ...
    assert "debug search retained-frontiers" in dimension
    assert "debug solve allocator-ceiling" in dimension
    final_family = [
        c.name
        for c in cap.run_search(
    ...
    assert "debug search source-family-continuation" in final_family
    assert "debug search retained-frontiers" in final_family
    post_stack_clean = [
        c.name
        for c in cap.run_search(
            "post stack clean no anchor source family",
            REPO,
        )
    ]
    assert "debug search source-model-synthesis" in post_stack_clean
    assert "debug search retained-frontiers" in post_stack_clean
    assert "debug search post-source-context-next-dimension" in post_stack_clean
    assert "debug solve allocator-ceiling" in post_stack_clean
    for query in (
        "draw post row offset owner expression lifetime",
        "draw-no-modeled-source-actionable-family-after-post-row-offset-owner-split",
    ):
        names = [c.name for c in cap.run_search(query, REPO)]
        assert "debug search source-model-synthesis" in names
        assert "debug search source-family-continuation" in names
        assert "debug search retained-frontiers" in names
        assert "debug solve allocator-ceiling" in names
