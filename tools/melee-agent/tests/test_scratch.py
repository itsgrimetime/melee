...
class TestBuildStrippedContextMultiline:
    """Regression tests for _build_stripped_context (the helper shared by the
    local and --production scratch-create paths).

    Bug: the inline line-based stripper treated any line containing
    ``funcname(`` that did not end in ``;`` as a function-DEFINITION opener.
    Multi-line declarations and multi-line CALL sites also match that shape, so
    the stripper mangled the prototype and corrupted the calling function's
    body (leaving dangling fragments) -> the context no longer compiled and the
    production scratch could not build.

    The fix delegates to the tree-sitter-backed _strip_target_function, which
    removes only the actual definition.
    """

    def _run(self, tmp_path, monkeypatch, context, func_name="mnDiagram_DrawCellNumber"):
        import subprocess
        from types import SimpleNamespace

        from src.cli.scratch import _build_stripped_context

        # Lay the crafted context where _build_stripped_context expects a .ctx.
        ctx_dir = tmp_path / "build" / "GALE01" / "src" / "melee" / "mn"
        ctx_dir.mkdir(parents=True)
        ctx_path = ctx_dir / "mndiagram.ctx"
        ctx_path.write_text(context)

        # Don't actually run ninja - pretend the .ctx is already up to date.
    ...
    def test_strips_definition_but_preserves_decl_and_multiline_calls(self, tmp_path, monkeypatch):
        context = (
            "extern void mnDiagram_DrawCellNumber(void* gobj, u8 col, u8 row,\n"
            "                                     int value);\n"
            "\n"
            ...
            "    int i;\n"
            "    for (i = 0; i < value; i++) {\n"
            "        sentinel_body_call(i);\n"
            "    }\n"
            "}\n"
            "\n"
            "void mnDiagram_FillGridCells(void* gobj)\n"
            "{\n"
            "    int row = 0;\n"
            "    do {\n"
            "        mnDiagram_DrawCellNumber(gobj, (u8) 0,\n"
            "                                 (u8) row, 5);\n"
            "        if (row == 7) {\n"
            "            mnDiagram_DrawCellNumber(\n"
            "                gobj, (u8) 1, (u8) row,\n"
            "                lookup((u8) row)->vals[row]);\n"
            "        }\n"
            "        row += 1;\n"
            "    } while (row <= 10);\n"
            "}\n"
        ...
        result = self._run(tmp_path, monkeypatch, context)

        # The definition body must be removed (the whole point of stripping).
        assert "sentinel_body_call(i)" not in result

        # The multi-line prototype must be preserved (callers need the decl).
        assert "int value);" in result

        # The calling function must remain fully intact - no mangling.
        assert "void mnDiagram_FillGridCells(void* gobj)" in result
        # First call: opener line and its continuation both survive verbatim.
        assert "mnDiagram_DrawCellNumber(gobj, (u8) 0," in result
        assert "(u8) row, 5);" in result
        # Second call (opener on its own line): the arg lines must survive too.
        assert "gobj, (u8) 1, (u8) row," in result
        assert "lookup((u8) row)->vals[row]);" in result

        # Exactly the prototype + two call sites remain (definition opener gone).
        assert result.count("mnDiagram_DrawCellNumber(") == 3
