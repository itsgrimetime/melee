...
def test_mwcc_debug_source_avoids_duplicate_helpers_and_builtin_collisions() -> None:
    text = (REPO_ROOT / "tools" / "mwcc_debug" / "mwcc_debug.c").read_text(
        encoding="utf-8",
    )

    assert len(re.findall(r"^static int mwcc_debug_output_active\(", text, re.MULTILINE)) == 1
    assert re.search(
        r"^void \*memcpy\(void \*dst, const void \*src, __SIZE_TYPE__ n\)",
        text,
        re.MULTILINE,
    )
    assert not re.search(
        r"^void \*memcpy\(void \*dst, const void \*src, unsigned long n\)",
        text,
        re.MULTILINE,
    )
...
def test_dump_remote_force_remat_default_output_skips_cache(
    ...
    class FakePopen:
        ...
        def communicate(self, input=None, timeout=None):
            chunks = []
            while chunk := self.stdout.read(65536):
                chunks.append(chunk)
