...
def test_draw_post_alternate_source_context_real_source_preserves_loop_inputs(
    tmp_path: Path,
) -> None:
    source_path = Path(
        "/Users/mike/.codex/worktrees/eeff/melee/src/melee/mn/mndiagram.c"
    )
    ...
    original_span = synthesis.find_function(
        source_text, "mnDiagram_DrawCellNumber"
...
def test_sort_unbounded_tu_data_ownership_real_source_declaration_order_guard(
    ...
    for row in candidates:
        ...
        assert "typedef struct mnDiagram_804A0750_t" in text
        assert "typedef struct mnDiagram_804A076C_t" in text
        assert "mnDiagram_804A0750_t mnDiagram_804A0750;" in text
        assert "mnDiagram_804A076C_t mnDiagram_804A076C;" in text
        sort_index = text.index("void mnDiagram_8023FC28(void)")
        assert text.index("mnDiagram_804A0750_t mnDiagram_804A0750;") < sort_index
        assert text.index("mnDiagram_804A076C_t mnDiagram_804A076C;") < sort_index
        post_meta_index = text.index("mnDiagram_PostMetaSortUnbounded")
        assert post_meta_index < sort_index
        assert text.count("mnDiagram_804A0750_t mnDiagram_804A0750;") == 1
        assert text.count("mnDiagram_804A076C_t mnDiagram_804A076C;") == 1
        if "mnDiagram_PostMetaSortUnboundedNamesOwner" in text:
            assert text.index("mnDiagram_PostMetaSortUnboundedNamesOwner") < (
                text.index("mnDiagram_GetNameByIndex")
            )
