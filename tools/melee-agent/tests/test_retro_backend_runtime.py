...
def _stub_backend_map_static_discovery(request: pytest.FixtureRequest) -> None:
    """Keep map-probe orchestration tests independent of a local retail compiler."""
    test_name = getattr(request.node, "originalname", request.node.name)
    if not test_name.startswith("test_run_backend_map_probe_"):
        return
