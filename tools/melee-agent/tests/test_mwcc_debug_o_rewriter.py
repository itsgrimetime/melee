...
_GM_1884_TARGET_O = Path(__file__).parent.parent.parent.parent / \
    "build" / "GALE01" / "obj" / "melee" / "gm" / "gm_1884.o"
...
def test_find_named_sdata2_symbols_by_value_omits_duplicate_values() -> None:
    """A value lookup must not pick arbitrary aliases after a TU split."""
    gm_1832_by_value = find_named_sdata2_symbols_by_value(_GM_1832_TARGET_O)
    gm_1884_by_value = find_named_sdata2_symbols_by_value(_GM_1884_TARGET_O)

    assert MAGIC_S32 not in gm_1832_by_value
    assert MAGIC_U32 not in gm_1884_by_value
