...
def _provide_bootstrap_wibo(request: pytest.FixtureRequest) -> None:
    """Keep bootstrap unit tests independent of the developer's local toolchain."""
    test_name = getattr(request.node, "originalname", request.node.name)
    if not test_name.startswith("test_debug_permute_bootstrap_"):
        return
...
def _inline_boundary_report(source: Path) -> dict:
    return {
        "run_id": "cli-test",
        "targets": [
            {
...
def _void_inline_boundary_report(source: Path) -> dict:
    return {
        "run_id": "cli-void-test",
        "targets": [
            {
        ...
        "records": [
            {
                ...
                "error": "nontrivial argument would be duplicated",
            },
        ],
    }


def _flat_inline_boundary_score(candidate: dict) -> dict:
    return {
...
def _real_inline_boundary_score(pcdump_path: Path) -> dict:
    return {
        "score": 130,
        "target_score": {
            "total": 130.0,
        ...
        "structural_guard": {"accepted": True, "shape_preserved": True},
...
def test_representative_grouped_command_help_works() -> None:
    commands = [
        ...
        ["debug", "suggest", "inline-boundary-continuation", "--help"],
...
def test_inline_boundary_continuation_cli_writes_probes_and_hints(
    tmp_path: Path,
) -> None:
    source = tmp_path / "mndiagram.c"
    source.write_text(INLINE_BOUNDARY_SOURCE, encoding="utf-8")
    report = tmp_path / "inline_leverage.json"
    report.write_text(json.dumps(_inline_boundary_report(source)), encoding="utf-8")
    ...
    assert (out_dir / "inline_boundary_target.json").is_file()
    for candidate in payload["candidates"]:
        path = Path(candidate["path"])
        assert path.is_file()
        hint = candidate["score_source"]
        assert hint["status"] == "ready"
        assert "--expression-reg-class gpr" in hint["command"]
        assert "--checkdiff-guard" in hint["command"]


def test_inline_boundary_continuation_cli_accepts_void_duplication_seed(
    tmp_path: Path,
) -> None:
    source = tmp_path / "mndiagram3.c"
    source.write_text(VOID_INLINE_BOUNDARY_SOURCE, encoding="utf-8")
    report = tmp_path / "inline_leverage_void.json"
    report.write_text(json.dumps(_void_inline_boundary_report(source)), encoding="utf-8")
    out_dir = tmp_path / "void_probes"
    ...
    payload = json.loads(result.stdout)
    assert payload["status"] == "ok"
    assert payload["selected_record"]["continuation_seed"] == (
        "void_nontrivial_argument_duplication"
    )
    assert {
        candidate["dimension_id"] for candidate in payload["candidates"]
    } == {
    ...
    for candidate in payload["candidates"]:
        ...
        assert "--full-unit-source" in candidate["score_source"]["command"]
        assert "--retain-pcdump" in candidate["score_source"]["command"]


def test_inline_boundary_continuation_cli_emits_terminal_proof(
    tmp_path: Path,
) -> None:
    source = tmp_path / "mndiagram.c"
    source.write_text(INLINE_BOUNDARY_SOURCE, encoding="utf-8")
    ...
    for candidate in generated_payload["candidates"]:
        path = tmp_path / f"{candidate['candidate_order']}_score.json"
        path.write_text(
            json.dumps(_flat_inline_boundary_score(candidate)),
            encoding="utf-8",
        )
        score_args.extend(["--score-json", str(path)])
    ...
    terminal = payload["terminal_frontier"]
    assert terminal["family_id"] == "inline-leverage-helper-boundary-continuation"
    assert terminal["inline_name"] == "mnDiagram_SumNameKOs"
...
def test_inline_boundary_continuation_cli_terminal_proof_correlates_real_score_source_filenames(
    tmp_path: Path,
) -> None:
    source = tmp_path / "mndiagram.c"
    source.write_text(INLINE_BOUNDARY_SOURCE, encoding="utf-8")
    ...
    generated_payload = json.loads(generated.stdout)
    candidate_ids = {
        candidate["candidate_id"] for candidate in generated_payload["candidates"]
    }
    ...
    for candidate in generated_payload["candidates"]:
        ...
        score_path.write_text(
            json.dumps(
                _real_inline_boundary_score(
                    candidate_path.with_suffix(".pcdump.txt"),
                )
            ),
            encoding="utf-8",
        )
        score_args.extend(["--score-json", str(score_path)])
    ...
    assert payload["status"] == "terminal"
    terminal = payload["terminal_frontier"]
    assert terminal["candidate_count"] == 6
    assert terminal["scored_count"] == 6
    assert {
        row["dimension_id"] for row in terminal["exhausted_dimensions"]
    } == {
        "signature",
    ...
    assert {
        row["candidate_id"] for row in terminal["ranked_candidates"]
    } <= candidate_ids
    assert [row["candidate_id"] for row in payload["ranked_scores"]] == [
        candidate["candidate_id"] for candidate in generated_payload["candidates"]
    ]
    assert [row["rank_score"]["candidate_order"] for row in payload["ranked_scores"]] == [
        0,
        1,
        2,
        3,
...
def test_inline_boundary_continuation_cli_terminal_proof_accepts_checkdiff_files(
    tmp_path: Path,
) -> None:
    source = tmp_path / "mndiagram.c"
    source.write_text(INLINE_BOUNDARY_SOURCE, encoding="utf-8")
    ...
    for candidate in generated_payload["candidates"]:
        candidate_path = Path(candidate["path"])
        checkdiff_path = candidate_path.with_suffix(".checkdiff.json")
        checkdiff_path.write_text(
            json.dumps({
                "function": "mnDiagram_SortNamesByKOs",
                "match": False,
        ...
        score_args.extend(["--score-json", str(checkdiff_path)])
    ...
    terminal = payload["terminal_frontier"]
    assert terminal["candidate_count"] == 6
    assert terminal["ranked_candidates"][0]["checkdiff"]["fuzzy_match_percent"] == 90.0
    assert terminal["ranked_candidates"][0]["checkdiff"]["classification"] == (
        "signature-type-mismatch"
    )


def test_inline_boundary_continuation_cli_correlates_score_by_pcdump_path(
    tmp_path: Path,
) -> None:
    source = tmp_path / "mndiagram.c"
    source.write_text(INLINE_BOUNDARY_SOURCE, encoding="utf-8")
    ...
    score_args: list[str] = []
    for index, candidate in enumerate(generated_payload["candidates"]):
        candidate_path = Path(candidate["path"])
        score_path = tmp_path / f"score_{index}.json"
        score_path.write_text(
            json.dumps(
                _real_inline_boundary_score(
                    candidate_path.with_suffix(".pcdump.txt"),
    ...
    payload = json.loads(result.stdout)
    assert payload["status"] == "terminal"
    assert payload["terminal_frontier"]["candidate_count"] == 6
    assert payload["terminal_frontier"]["scored_count"] == 6


def test_inline_boundary_continuation_cli_partial_real_scores_remains_open(
    tmp_path: Path,
) -> None:
    source = tmp_path / "mndiagram.c"
    source.write_text(INLINE_BOUNDARY_SOURCE, encoding="utf-8")
    ...
    score_args: list[str] = []
    for candidate in generated_payload["candidates"][:5]:
        candidate_path = Path(candidate["path"])
        score_path = candidate_path.with_suffix(".score.json")
        score_path.write_text(
            json.dumps(
    ...
    payload = json.loads(result.stdout)
    assert payload["status"] == "ok"
    assert "terminal_frontier" not in payload
    assert len(payload["ranked_scores"]) == 5


def test_shape_guard_from_checkdiff_payload_accepts_normalized_structural_match() -> None:
    payload = {
        "classification": {
    ...
    guard = debug_cli._shape_guard_from_checkdiff_payload(payload)

    assert guard["accepted"] is True
    assert guard["shape_preserved"] is True
    assert guard["classification_primary"] == "normalized-structural-match"
    assert guard["normalized_diff_lines"] == 0
    assert guard["frame_delta"] == 0
    assert guard["rejection_reason"] is None


def test_shape_guard_from_checkdiff_payload_rejects_structural_drift() -> None:
    payload = {
        "classification": {
    ...
    assert guard["shape_preserved"] is False
    assert guard["classification_primary"] == "inline-boundary-toolchain-artifact"
    assert guard["normalized_diff_lines"] == 6
    assert guard["frame_delta"] == 16
    assert guard["rejection_reason"] == (
...
def test_control_flow_shape_search_help_works() -> None:
    ...
    assert "--suggestions-json" in result.output
    assert "--baseline-checkdiff-json" in result.output
    assert "--score-match-percent" in result.output
    assert "--compile-probes" in result.output
...
def test_control_flow_shape_search_json_scores_generated_candidate(
    ...
    def fake_score(
        ...
        *,
        ...
        include_structural_guard=False,
    ):
        assert include_structural_guard is True
        return debug_cli._SourceCandidateRealScore(
            ...
            checkdiff_payload={
                ...
                "structural_truth_gate": {"normalized_diff_lines": 0},
                "structural": {"opcode_similarity": 1.0},
    ...
    assert payload["probes"][0]["source_hunks"]
    ...
    assert variant["source_hunks"]
    pcdump_path = Path(variant["pcdump_path"])
    assert pcdump_path.exists()
    assert pcdump_path.read_text() == "pcdump text"
    assert variant["checkdiff"] == {
        "match": True,
        "classification": {"primary": "control-flow-source-shape"},
        "structural_truth_gate": {"normalized_diff_lines": 0},
        "structural": {"opcode_similarity": 1.0},
        "frame": {"current_frame_size": 32, "target_frame_size": 32},
    }
    assert variant["probe"]["provenance"]["kind"] == "control-flow-shape"


def test_control_flow_shape_search_json_reports_no_baseline_exhausted_proof(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    melee_root = tmp_path / "repo"
    ...
    monkeypatch.setattr(
        debug_cli,
        "_control_flow_compile_source_variant",
        lambda *args, **kwargs: "pcdump text",
        raising=False,
    )
    monkeypatch.setattr(
        debug_cli,
        ...
        lambda *args, **kwargs: debug_cli._SourceCandidateRealScore(
            ...
            checkdiff_payload={
                "match": False,
                "classification": {"primary": "control-flow-source-shape"},
                "structural_truth_gate": {"normalized_diff_lines": 2},
            },
        ),
    )
    ...
    assert result.exit_code == 0, result.output
    payload = json.loads(result.output)
    assert payload["blocker"] == "no-control-flow-shape-candidate"
    assert payload["probe_count"] == 1
    assert payload["probes"][0]["label"] == "loop-init-outside-for-0"
    assert payload["probes"][0]["provenance"]["family_id"] == (
        "loop-init/loop-peel-unroll"
    )
    proof = next(
        item
        for item in payload["terminal_proofs"]
        if item["terminal_blocker"] == "control-flow-shape-candidates-exhausted"
    )
    assert "baseline" not in proof
    assert proof["family_id"] == "loop-init/loop-peel-unroll"
    assert proof["candidate_count"] == 1
    assert proof["scored_count"] == 1
    assert proof["best_candidate"]["source_hunks"]
    assert Path(proof["best_candidate"]["source_retained"]).exists()
    assert Path(proof["best_candidate"]["pcdump_path"]).read_text() == "pcdump text"
    assert proof["retained_evidence"]["source_hunk_count"] > 0
    assert "source-level loop-init spelling" in proof["next_handoff"]
    assert "baseline checkdiff" in proof["next_handoff"]


def test_control_flow_shape_search_json_scores_suggestion_family_candidates(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    melee_root = tmp_path / "repo"
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: debug_cli._SourceCandidateRealScore(
            ...
            checkdiff_payload={
                "match": False,
                "classification": {"primary": "control-flow-source-shape"},
                "structural_truth_gate": {"normalized_diff_lines": 7},
                "structural": {"opcode_similarity": 0.9},
            },
        ),
    )
    ...
    assert result.exit_code == 0, result.output
    payload = json.loads(result.output)
    assert payload["family_results"][0]["family_id"] == (
        "pointer-walk-loop/pointer-walk-indexed-shape"
    )
    assert payload["family_results"][0]["status"] == "materialized"
    assert payload["probes"][0]["source_hunks"]
    variant = payload["variants"][0]
    assert variant["family_id"] == "pointer-walk-loop/pointer-walk-indexed-shape"
    assert variant["suggestion_kind"] == "pointer-walk-indexed-shape"
    assert variant["source_hunks"]
    assert Path(variant["source_retained"]).exists()
    assert Path(variant["pcdump_path"]).read_text() == "pcdump text"
    assert variant["checkdiff"]["structural_truth_gate"] == {
...
def _helper_u8_index_table_source() -> str:
    return textwrap.dedent(
        ...
        void fn_80000000(u8* sorted, int start, int rank)
        {
            ...
            ptr = sorted + start;
            ptr = ptr + 0x1C;
            value = sorted[start + 0x1C];
            use(value);
            value = sorted[rank + 0x1C];
            use(value);
...
def _write_control_flow_suggestions(path: Path) -> None:
    path.write_text(
        json.dumps(
            {
                "function": "fn_80000000",
...
def test_control_flow_shape_search_json_materializes_helper_u8_index_table_family(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    melee_root = tmp_path / "repo"
    source = melee_root / "src" / "melee" / "mn" / "demo.c"
    source.parent.mkdir(parents=True)
    source.write_text(_helper_u8_index_table_source())
    suggestions = tmp_path / "suggestions.json"
    _write_control_flow_suggestions(suggestions)
    monkeypatch.setattr(debug_cli, "DEFAULT_MELEE_ROOT", melee_root)
    monkeypatch.setattr(
        debug_cli,
        "_find_unit_for_function",
    ...
    assert payload["probe_count"] > 0
    assert payload["probes"][0]["source_hunks"]
    assert "u8-index-table" in payload["probes"][0]["provenance"]["anchors"]
    assert any(
...
def test_control_flow_shape_search_json_reports_exhausted_candidate_proof_for_baseline(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    melee_root = tmp_path / "repo"
        ...
        lambda *args, **kwargs: debug_cli._SourceCandidateRealScore(
            ...
            checkdiff_payload={
                ...
                "classification": {
                    ...
                    "structural_truth_gate": {"normalized_diff_lines": 8},
                },
            },
        ),
    )
    ...
    assert proof["candidate_count"] >= 8
    assert proof["scored_count"] >= 8
    ...
    assert Path(proof["best_candidate"]["source_retained"]).exists()
    assert Path(proof["best_candidate"]["pcdump_path"]).read_text() == "pcdump text"
    assert proof["best_candidate"]["checkdiff_delta"] == {
        "match_percent": 1.0,
        "normalized_diff_lines": 5,
    }
    assert any(
        item["owner_function"] == "get_visible_name"
        and item["anchor_kind"] == "u8-index-table-helper-return"
        and item["source_retained"]
        and item["pcdump_path"]
        for item in proof["candidate_summaries"]
    )
    assert "source-level pointer-walk-loop spelling" in proof["next_handoff"]


def test_control_flow_shape_search_json_reports_terminal_proofs_for_unmaterialized_families(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    import src.mwcc_debug.control_flow_shape as control_flow_shape
    ...
    proof = {
        "family_id": "loop-init/loop-peel-unroll",
        "operator": "loop-init",
        "suggestion_kind": "loop-peel-unroll",
        "terminal_blocker": "simple-counted-loop-not-found",
        "terminal_reason": "no simple counted loop",
    }
    ...
    assert result.exit_code == 0, result.output
    payload = json.loads(result.output)
    assert payload["terminal_proofs"] == [proof]
    assert payload["stop_condition"]["kind"] == "terminal"


def test_control_flow_shape_search_json_compares_baseline_checkdiff(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    melee_root = tmp_path / "repo"
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: debug_cli._SourceCandidateRealScore(
            ...
            checkdiff_payload={
                "match": False,
                "fuzzy_match_percent": 92.10,
                "classification": {
                    "primary": "control-flow-source-shape",
                    "structural_truth_gate": {"normalized_diff_lines": 52},
                },
            },
        ),
    )
    ...
    variant = payload["variants"][0]
    assert variant["checkdiff_delta"]["match_percent"] == pytest.approx(-0.26562)
...
def test_debug_suggest_control_flow_shape_preflights_source_materialization(
    ...
    assert suggestion["source_materialization"]["status"] == "terminal"
    assert suggestion["source_materialization"]["operator"] == "loop-init"
    assert suggestion["source_materialization"]["terminal_proof"][
        "terminal_blocker"
    ] == "simple-counted-loop-not-found"
...
def test_indexed_struct_search_json_scores_generated_candidate(
    ...
    assert variant["match_percent"] == 100.0
    assert variant["final_match_percent"] == 100.0
    assert variant["target_score"] == {
        "mode": "match-percent",
        "source": "indexed-struct-search",
        "matched": 1,
        "targeted": 1,
        "candidate_match_percent": 100.0,
...
def test_indexed_struct_search_json_terminal_proof_for_scored_generated_probe(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    melee_root = tmp_path / "repo"
    ...
    monkeypatch.setattr(
        debug_cli,
        "_indexed_struct_compile_source_variant",
        lambda diff_input, *, function, melee_root, timeout: "pcdump text",
    )
    monkeypatch.setattr(
        debug_cli,
        "_score_source_candidate_real_tree",
        lambda path, **kwargs: debug_cli._SourceCandidateRealScore(97.5, None),
    )
    ...
    proof = payload["terminal_proof"]
    assert proof["kind"] == "indexed-struct-pointer-materialization-exhausted"
    assert proof["exhausted_families"] == ["indexed-struct-pointer"]
    assert proof["evaluated_candidate_count"] == 1
    retained = proof["retained_candidates"][0]
    assert retained["source_retained"].endswith(".c")
    assert retained["pcdump_path"].endswith(".pcdump.txt")
    ...
    assert retained["target_score"] == {
        ...
        "matched": 0,
        "targeted": 1,
        "candidate_match_percent": 97.5,
        "match_percent_error": None,
    }
    assert "frame/layout lever" in proof["next_source_handoff"]
...
def test_indexed_struct_search_json_scans_source_without_checkdiff_hint(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    source = tmp_path / "demo.c"
...
def test_indexed_struct_search_json_reports_unvalidated_candidate(
    ...
    assert payload["variants"][0]["target_score"]["matched"] == 0
    assert payload["terminal_proof"]["retained_candidates"][0]["source_retained"] == (
        str(source)
    )
...
def test_frame_transform_search_auto_bridges_outgoing_parameter_family(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    import src.cli.debug.mutate as mutate_cli
    ...
    report = {
        "current": {"frame_size": 240},
        "expected": {"frame_size": 240},
        "frame_delta": 0,
        "outgoing_parameter_area_floor": {
            "status": "current-floor-larger",
            "floor_delta_bytes": 8,
            "current_accesses_in_excess_floor": [],
            "parameter_word_count_model": {
                "status": "computed",
                "expected_parameter_words": 8,
                "current_parameter_words": 10,
                "extra_parameter_words": 2,
            },
        },
    }

    def fake_append_transform_corpus_probes(
        probes,
        *,
        source_text,
        function,
        ...
        seen.update({
            "include": include,
            "families": families,
            "default_families": tuple(default_families),
        ...
        if include:
            probes.append(
                LifetimeLayoutProbe(
                    label="transform-corpus-outgoing_parameter_area_shape-0",
                    operator="transform-corpus:outgoing_parameter_area_shape",
                    description="synthetic outgoing parameter source bridge",
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: "expected asm",
    )
    monkeypatch.setattr(debug_cli, "_frame_report_aliases", lambda *args: ("fn_80000000", []))
    monkeypatch.setattr(debug_cli, "_frame_source_context", lambda *args, **kwargs: {})
    monkeypatch.setattr(
        debug_cli,
        "_find_unit_for_function",
    ...
    monkeypatch.setattr(
        ...
        fake_append_transform_corpus_probes,
    )
    monkeypatch.setattr(
        mutate_cli,
    ...
    payload = json.loads(result.stdout)
    assert payload["outgoing_floor_bridge"] == {
        "status": "active",
        "families": ["outgoing_parameter_area_shape"],
        "requested_probe_budget": 8,
        "generated_probe_count": 1,
    }
    assert seen["include"] is True
    assert seen["families"] is None
    assert seen["default_families"] == ("outgoing_parameter_area_shape",)
    assert seen["unit"] == "melee/mn/sample"
    probe = payload["probes"][0]
    assert probe["operator"] == "transform-corpus:outgoing_parameter_area_shape"
    assert probe["source_retained"].endswith(
        "transform-corpus-outgoing_parameter_area_shape-0.c"
    )
    assert probe["source_hunks"]
    hunks = payload["source_hunks_by_candidate"]
    assert "transform-corpus-outgoing_parameter_area_shape-0" in hunks
...
def test_frame_reservations_missing_function_lists_small_dump_symbols(
    ...
    assert result.exit_code == 3
    assert "function 'mnDiagram3_MissingInput' not found in pcdump" in result.stderr
    assert "Functions in this dump:" in result.stderr
    assert "fn_802461BC" in result.stderr
    assert "semantic alias" in result.stderr


def _write_frame_alias_report(melee_root: Path) -> None:
    report_json = melee_root / "build" / "GALE01" / "report.json"
    report_json.parent.mkdir(parents=True)
    report_json.write_text(json.dumps({
        "units": [
            {
                "name": "main/melee/mn/mndiagram3",
                "functions": [
                    {
                        "name": "mnDiagram3_HandleInput",
                        "metadata": {"virtual_address": str(0x802461BC)},
                    }
                ],
            }
        ]
...
def test_frame_reservations_resolves_canonical_name_to_fn_addr_pcdump_alias(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _write_frame_alias_report(tmp_path)
    pcdump = tmp_path / "pcdump.txt"
    pcdump.write_text(_frame_alias_pcdump())
    monkeypatch.setattr(debug_cli, "DEFAULT_MELEE_ROOT", tmp_path)
    ...
    payload = json.loads(result.stdout)
    assert payload["function"] == "mnDiagram3_HandleInput"
    assert payload["function_aliases"]["pcdump_function"] == "fn_802461BC"
    assert payload["function_aliases"]["report_function"] == "mnDiagram3_HandleInput"


def test_frame_reservations_resolves_fn_addr_to_canonical_report_cache(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _write_frame_alias_report(tmp_path)
    source = tmp_path / "src" / "melee" / "mn" / "mndiagram3.c"
    source.parent.mkdir(parents=True)
    source.write_text("void mnDiagram3_HandleInput(void) {}\n")
    cache = (
        tmp_path
        / "build"
        / "mwcc_debug_cache"
    ...
    cache.write_text(_frame_alias_pcdump())
    monkeypatch.setattr(debug_cli, "DEFAULT_MELEE_ROOT", tmp_path)
    ...
    assert result.exit_code == 0, result.stderr
    payload = json.loads(result.stdout)
    assert payload["function"] == "fn_802461BC"
    assert payload["function_aliases"]["pcdump_function"] == "fn_802461BC"
    assert payload["function_aliases"]["report_function"] == "mnDiagram3_HandleInput"
...
def test_frame_reservations_cli_text_leads_with_register_cascade_when_dominant(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    pcdump = tmp_path / "pcdump.txt"
    ...
    expected.write_text(textwrap.dedent("""\
        ...
        /* 80000000 */    stwu r1, -80(r1)
        /* 80000004 */    li r29, 0
        /* 80000008 */    addi r26, r3, 0
        /* 8000000c */    lwz r3, 44(r3)
        /* 80000010 */    add r29, r29, r26
        /* 80000014 */    mr r31, r29
        /* 80000018 */    addi r26, r26, 1
        /* 8000001c */    add r30, r31, r26
        /* 80000020 */    mr r25, r30
        /* 80000024 */    add r29, r25, r31
        /* 80000028 */    mr r26, r29
        /* 8000002c */    add r31, r26, r25
        /* 80000030 */    stfs f1, 48(r1)
        /* 80000034 */    stfs f2, 52(r1)
        /* 80000038 */    addi r1, r1, 80
        .endfn fn_80000000
    """))
    current_asm = textwrap.dedent("""\
        +000: 94 21 ff b0 \tstwu    r1,-80(r1)
        +004: 3b 80 00 00 \tli      r28,0
        +008: 38 63 00 00 \taddi    r27,r3,0
        +00c: 80 64 00 2c \tlwz     r4,44(r4)
        +010: 7f 9c da 14 \tadd     r28,r28,r27
        +014: 7f 9e e3 78 \tmr      r30,r28
        +018: 3b 7b 00 01 \taddi    r27,r27,1
        +01c: 7f be da 14 \tadd     r29,r30,r27
        +020: 7f a6 eb 78 \tmr      r6,r29
        +024: 7f 86 f2 14 \tadd     r28,r6,r30
        +028: 7f 9b e3 78 \tmr      r27,r28
        +02c: 7f db 32 14 \tadd     r30,r27,r6
        +030: d0 21 00 40 \tstfs    f1,64(r1)
        +034: d0 41 00 44 \tstfs    f2,68(r1)
        +038: 38 21 00 50 \taddi    r1,r1,80
    """)
    monkeypatch.setattr(
        debug_cli,
        "_read_frame_reservation_current_asm",
    ...
    assert result.exit_code == 0, result.stdout
    out = result.stdout
    assert "dominant residual: register-coloring cascade" in out
    assert "register-only paired lines: 11; stack-slot paired lines: 2" in out
    assert "stack-home reorder is likely downstream" in out
    assert out.index("dominant residual") < out.index("target stack-home offsets")
    assert out.index("dominant residual") < out.index("reorder verdict")
...
def test_target_score_dump_expression_score_flags_virtual_renumber_false_hit(
    tmp_path: Path,
) -> None:
    ...
    expression = payload["expression_score"]
    assert expression["matched"] == 0
    assert expression["false_positive_virtual_id_hit_count"] == 1
    digit_temp = expression["virtuals"]["33"]
    assert digit_temp["virtual_id_matched"] is True
    assert digit_temp["virtual_id_false_positive"] is True
    assert digit_temp["candidate_virtual"] is None
    assert digit_temp["actual"] is None
    assert digit_temp["expected"] == 26

    anchored_target = tmp_path / "target-with-anchor.json"
    anchored_target.write_text(json.dumps({
        "function": "fn_80000000",
        "virtuals": {"32": 28, "33": 26},
        "expression_anchors": {
            "33": {
                "signature": {
                    ...
                    "opcode": "fsubs",
                    "operands": "<dst>,f52,f51",
                },
            },
        },
    }))
    anchored_result = runner.invoke(
        app,
        [
            "debug",
            "target",
    ...
    assert anchored_result.exit_code == 0, (
        anchored_result.stdout + anchored_result.stderr
    )
    anchored_payload = json.loads(anchored_result.stdout)
    anchored_expression = anchored_payload["expression_score"]
    assert anchored_expression["derived_from_baseline"] is False
    assert anchored_expression["virtuals"]["33"]["candidate_virtual"] is None
    assert anchored_expression["false_positive_virtual_id_hit_count"] == 1
...
def _remote_staging_ack(source_bytes: bytes) -> bytes:
    return (
        "[mwcc_debug] staged source from stdin "
        f"sha256={hashlib.sha256(source_bytes).hexdigest()}\n"
    ).encode("utf-8")


def test_dump_remote_retained_source_stages_stdin_through_unit_source(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    melee_root = tmp_path / "melee"
    ...
    retained_text = "void fn_80000000(void) { retained(); }\n"
    retained.write_text(retained_text, encoding="utf-8")
    output = tmp_path / "pcdump.txt"
    captured: dict[str, object] = {}
    ...
    class FakePopen:
        ...
        def communicate(self, input=None):
            ...
            return b"pcdump fixture", _remote_staging_ack(input or b"")

    def fake_resolve(path, *, label="source file"):
        if str(path) == str(retained):
            return "build/diagnostics/case/candidate.c"
        if str(path) in {str(source), "src/melee/mn/sample.c"}:
            return "src/melee/mn/sample.c"
        return str(path)
    ...
    assert result.exit_code == 0, result.stdout + result.stderr
    assert output.read_text(encoding="utf-8") == "pcdump fixture"
    remote_cmd = captured["cmd"][2]  # type: ignore[index]
    assert 'set "MWCC_DEBUG_STAGE_SOURCE_STDIN=1"' in remote_cmd
    assert (
        'set "MWCC_DEBUG_STAGE_SOURCE_LABEL=build/diagnostics/case/candidate.c"'
        in remote_cmd
    )
    assert remote_cmd.endswith(
        r"C:\Users\mikes\code\mwcc_debug\run_pcdump.ps1 src/melee/mn/sample.c"
    )
    assert captured["stdin_arg"] == subprocess.PIPE
    assert captured["stdin"].getvalue() == retained_text.encode("utf-8")  # type: ignore[union-attr]


def test_run_remote_pcdump_large_retained_source_uses_scp_staging(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    retained = tmp_path / "candidate.c"
    retained_bytes = (
        b"void fn_80000000(void) { retained(); }\n"
        + b"/* padding */\n" * 7000
    )
    ...
    class FakePopen:
        ...
        def communicate(self, input=None, timeout=None):
            captured["communicate_input"] = input
            return b"pcdump fixture", _remote_staging_ack(retained_bytes)

    def fake_run(cmd, **kwargs):
        captured["scp_cmd"] = cmd
        captured["scp_kwargs"] = kwargs
        return subprocess.CompletedProcess(cmd, 0, "", "")

    monkeypatch.setattr(debug_cli.uuid, "uuid4", lambda: FakeUuid())
    monkeypatch.setattr(debug_cli.subprocess, "Popen", FakePopen)
    monkeypatch.setattr(debug_cli.subprocess, "run", fake_run)

    result = debug_cli._run_remote_pcdump(
        source_rel="build/diagnostics/case/candidate.c",
        compile_source_rel="src/melee/mn/sample.c",
        host="win-host",
        remote_script=r"C:\Users\mikes\code\mwcc_debug\run_pcdump.ps1",
        timeout=120,
        branch="master",
        no_pull=True,
        stage_source_path=retained,
        stage_source_label="build/diagnostics/case/candidate.c",
    )

    assert result.returncode == 0
    assert result.stdout == "pcdump fixture"
    assert result.staging_ack_confirmed is True
    assert result.staging_transport == "scp"
    assert result.remote_stage_source == "mwcc_debug_stage_abc123.c"
    assert captured["scp_cmd"] == [
        "scp",
        "-q",
        str(retained),
        "win-host:mwcc_debug_stage_abc123.c",
    ]
    remote_cmd = captured["cmd"][2]  # type: ignore[index]
    assert 'set "MWCC_DEBUG_STAGE_SOURCE_FILE=mwcc_debug_stage_abc123.c"' in remote_cmd
    assert 'set "MWCC_DEBUG_STAGE_SOURCE_DELETE=1"' in remote_cmd
    assert "MWCC_DEBUG_STAGE_SOURCE_STDIN" not in remote_cmd
    assert "stdin" not in captured["popen_kwargs"]  # type: ignore[operator]
    assert captured["communicate_input"] is None


def test_dump_remote_retained_source_infers_unit_from_candidate_function(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    melee_root = tmp_path / "melee"
    ...
    def fake_find_unit(function: str, _root: Path) -> str | None:
        if function == "fn_80000000":
    ...
    assert captured["stdin"].getvalue() == retained.read_bytes()  # type: ignore[union-attr]


def test_dump_remote_retained_source_infers_shared_unit_from_multiple_functions(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    melee_root = tmp_path / "melee"
    ...
    def fake_find_unit(function: str, _root: Path) -> str | None:
        if function in {"fn_80000000", "fn_80000004"}:
            return "melee/mn/sample"
        return None
...
def test_dump_remote_retained_source_function_hint_disambiguates_multiple_units(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    melee_root = tmp_path / "melee"
    ...
    def fake_find_unit(function: str, _root: Path) -> str | None:
        return {
            "fn_80000000": "melee/mn/sample",
            "fn_80000004": "melee/gr/other",
        }.get(function)
    ...
    remote_cmd = captured["cmd"][2]  # type: ignore[index]
    assert remote_cmd.endswith(
        r"C:\Users\mikes\code\mwcc_debug\run_pcdump.ps1 src/melee/gr/other.c"
    )
    assert captured["stdin"].getvalue() == retained.read_bytes()  # type: ignore[union-attr]


def test_dump_remote_retained_source_rejects_missing_staging_ack(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    melee_root = tmp_path / "melee"
    ...
    retained = melee_root / "build" / "diagnostics" / "case" / "candidate.c"
    retained.parent.mkdir(parents=True)
    retained.write_text("void fn_80000000(void) { retained(); }\n", encoding="utf-8")
    output = tmp_path / "pcdump.txt"
    ...
    class FakePopen:
        ...
        def communicate(self, input=None):
            return b"pcdump from stale helper", b"[mwcc_debug] emitted pcdump\n"
    ...
    assert "did not confirm staged source" in result.stderr
    assert output.read_text(encoding="utf-8") == ""


def test_dump_remote_retained_source_times_out_hung_staging_helper(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    melee_root = tmp_path / "melee"
    ...
    class FakePopen:
        ...
        def communicate(self, input=None, timeout=None):
            if captured["killed"]:
                self.returncode = -9
                return b"", b"remote helper was still waiting\n"
            raise subprocess.TimeoutExpired(
                cmd=captured["cmd"],
                timeout=timeout,
                output=b"",
                stderr=b"remote helper was still waiting",
            )
    ...
    assert "timed out after 1s running ssh" in result.stderr
    assert output.read_text(encoding="utf-8") == ""


def test_dump_remote_retained_source_dependency_context_writes_blocker_sidecar(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    melee_root = tmp_path / "melee"
    source = melee_root / "src" / "melee" / "mn" / "sample.c"
    source.parent.mkdir(parents=True)
    source.write_text("float mnDiagram_803EE774[4];\n", encoding="utf-8")
    retained = melee_root / "build" / "diagnostics" / "case" / "candidate.c"
    retained.parent.mkdir(parents=True)
    retained_text = "struct mnDiagram_803EE774 { int x; };\n"
    retained.write_text(retained_text, encoding="utf-8")
    ...
    class FakePopen:
        ...
        def communicate(self, input=None, timeout=None):
            return (
                ...
                ack
                + (
                    "src/melee/mn/sample.c:12: identifier "
                    "mnDiagram_803EE774 redeclared; was float[], now struct\n"
                ).encode("utf-8"),
            )
    ...
    monkeypatch.setattr(
        debug_cli,
        "_remote_pcdump_local_head",
        lambda _root: "a9124f4df864c22aec58bdf6a36f033450c9711e",
    )
    ...
    assert payload["function"] == "mnDiagram_SortNamesByKOs"
    assert payload["dependency_context"]["required_files"] == [
        source_rel,
        retained_rel,
    ]
    assert (
        payload["dependency_context"]["local_head"]
        == "a9124f4df864c22aec58bdf6a36f033450c9711e"
    )
    assert payload["stage_source_sha256"] == hashlib.sha256(
        retained_text.encode("utf-8")
    ).hexdigest()
    assert "redeclared" in payload["stderr_tail"]
    assert f"{output}.blocker.json" in result.stderr
    stderr_json = next(
        json.loads(line)
        for line in result.stderr.splitlines()
        if line.startswith("{") and "terminal_blocker" in line
    )
...
def test_dump_remote_retained_source_inference_requires_unit_hint(
    ...
    result = runner.invoke(app, ["debug", "dump", "remote", str(retained)])

    assert result.exit_code != 0
    assert "--unit-source" in result.stderr


def test_dump_remote_staged_source_skips_default_cache(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    melee_root = tmp_path / "melee"
    ...
    retained.parent.mkdir(parents=True)
    retained.write_text("void fn_80000000(void) { retained(); }\n", encoding="utf-8")
    scratch_output = tmp_path / "scratch.pcdump.txt"
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: scratch_output,
    )
    ...
    assert result.exit_code == 0, result.stdout + result.stderr
    assert scratch_output.read_text(encoding="utf-8") == "pcdump fixture"
    cache_path = (
        melee_root
        / "build"
        / "mwcc_debug_cache"
    ...
    assert not debug_cli.pcdump_cache.hash_path(cache_path).exists()


def test_dump_remote_failure_preserves_existing_cache(monkeypatch, tmp_path: Path) -> None:
    melee_root = tmp_path / "melee"
    ...
    cache_path.write_text("existing pcdump\n", encoding="utf-8")

    class FakePopen:
        def __init__(self, _cmd, *, stdout, stderr):
            self.stdout = io.BytesIO(b"")
            self.returncode = 70

        def communicate(self, input=None, timeout=None):
            return b"", b"remote failed"

    monkeypatch.setattr(debug_cli, "DEFAULT_MELEE_ROOT", melee_root)
    monkeypatch.setattr(
        debug_cli,
        "_resolve_src_relative",
        lambda _path: "src/melee/mn/sample.c",
    )
    monkeypatch.setattr(debug_cli.subprocess, "Popen", FakePopen)

    result = runner.invoke(app, ["debug", "dump", "remote", str(src_path)])

    assert result.exit_code == 70
    assert cache_path.read_text(encoding="utf-8") == "existing pcdump\n"


def test_dump_remote_failure_does_not_create_empty_cache(monkeypatch, tmp_path: Path) -> None:
    melee_root = tmp_path / "melee"
    src_path = melee_root / "src" / "melee" / "mn" / "sample.c"
    src_path.parent.mkdir(parents=True)
    src_path.write_text("void fn_80000000(void) {}\n", encoding="utf-8")
...
def test_run_pcdump_script_supports_stdin_staging_and_restore() -> None:
    script = (
        Path(__file__).resolve().parents[3]
        / "tools"
        / "mwcc_debug"
    ...
    assert "MWCC_DEBUG_STAGE_SOURCE_STDIN" in script
    assert "MWCC_DEBUG_STAGE_SOURCE_FILE" in script
    assert "MWCC_DEBUG_STAGE_SOURCE_DELETE" in script
    assert "MWCC_DEBUG_STAGE_SOURCE_LABEL" in script
    assert "[Console]::OpenStandardInput()" in script
    assert "[System.IO.File]::ReadAllBytes($stageSourceFile)" in script
    assert "Remove-Item -Force $stageSourceFile" in script
    assert "CopyToAsync($stageBuffer)" in script
    assert "stdin source staging timed out after ${timeoutSecs}s before compile" in script
    assert "[System.Security.Cryptography.SHA256]::Create()" in script
    assert "sha256=$stageHash" in script
    assert "$stagedSourceOriginalBytes = [System.IO.File]::ReadAllBytes($srcAbs)" in script
    assert "[System.IO.File]::WriteAllBytes($srcAbs, $stageBytes)" in script
    assert (
        "[System.IO.File]::WriteAllBytes($srcAbs, $stagedSourceOriginalBytes)"
        in script
    )
    stage_index = script.index("[System.IO.File]::WriteAllBytes($srcAbs, $stageBytes)")
    compile_index = script.index("$job = Start-Job -ScriptBlock")
    restore_index = script.index(
        "[System.IO.File]::WriteAllBytes($srcAbs, $stagedSourceOriginalBytes)"
    )
    lock_release_index = script.rindex("Remove-Item -Force $lockFile")
    assert stage_index < compile_index < restore_index < lock_release_index
...
def _install_score_source_parse_fakes(
    monkeypatch: pytest.MonkeyPatch,
    *,
    function: str = "fn_80000000",
    total: float = 0.0,
) -> None:
    from src.mwcc_debug import scoring as scoring_mod
    import src.mwcc_debug as mwcc_debug_mod

    monkeypatch.setattr(
        mwcc_debug_mod,
        "parse_pcdump",
        lambda _text: [SimpleNamespace(name=function)],
    )
    monkeypatch.setattr(mwcc_debug_mod, "parse_hook_events", lambda _text: [])
    monkeypatch.setattr(mwcc_debug_mod, "find_function", lambda _events, _name: None)
    monkeypatch.setattr(
        mwcc_debug_mod,
        "score_function",
        lambda _fn, _target, events=None: scoring_mod.ScoreBreakdown(
            total=total,
            virtual_distance=0,
            virtual_penalty=0.0,
            spill_unexpected=[],
            ...
            interferer_penalty=0.0,
            wrong=[],
            matched=1,
            targeted=1,
            frame_targeted=False,
...
def _score_source_force_phys_pcdump_text(
    function: str,
    *,
    ...
    for i, ig in enumerate(ig_idxs):
        lines.append(f"  {i}  {ig}  r{assigned_by_ig[ig]}  0  0  0x0")
...
def test_target_score_source_scores_force_phys_target_yaml(
    ...
    compiler_dir = tmp_path / "compiler"
    compiler_dir.mkdir()
    compiler = compiler_dir / "mwcceppc_debug.exe"
    compiler.write_text("")
    wibo = tmp_path / "wibo"
    wibo.write_text("")
    baseline = tmp_path / "baseline.pcdump.txt"
    ...
    def fake_process_tree_runner(cmd, *, cwd, timeout, env=None):
        local_calls.append([str(part) for part in cmd])
    ...
    assert payload["target_score"]["targeted"] == 1
    assert payload["target_score"]["virtuals"]["53"]["actual"] == 4
    assert payload["force_phys_score"]["matched_igs"] == [53]
    assert Path(payload["pcdump_path"]).is_file()
    assert local_calls


def test_target_score_source_timeout_uses_process_tree_runner(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    melee_root = tmp_path / "melee"
    ...
    wibo.write_text("")
    target = tmp_path / "target.json"
    target.write_text('{"virtuals": {"34": 28}}\n')
    calls: list[tuple[list[str], Path, float, dict[str, str] | None]] = []

    def fake_process_tree_runner(cmd, *, cwd, timeout, env=None):
        calls.append(([str(part) for part in cmd], cwd, timeout, env))
        raise subprocess.TimeoutExpired(cmd, timeout, output="out", stderr="stuck")

    def fail_subprocess_run(*args, **kwargs):
        raise AssertionError("score-source timeout must use process-tree runner")
    ...
    assert result.exit_code == 0, result.stdout + result.stderr
    payload = json.loads(result.stdout)
    assert payload["score"] == 2**30
    assert payload["returncode"] == 124
    assert payload["timeout_seconds"] == 0.25
    assert "timed out after 0.25s" in payload["error"]
    assert calls
    assert calls[0][1] == melee_root
    assert calls[0][2] == 0.25
    assert calls[0][3] is not None
    assert calls[0][3]["MWCC_DEBUG_PCDUMP_PATH"].startswith("pcdump_score_")
    assert float(calls[0][3]["MWCC_DEBUG_HANG_TIMEOUT"]) < 0.25


def test_target_score_source_refuses_unsafe_local_lane_before_launch(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    melee_root = tmp_path / "melee"
    ...
    target = tmp_path / "target.json"
    target.write_text('{"virtuals": {"34": 28}}\n')
    unsafe_proc = debug_cli.local_safety.LocalWiboProcess(
        pid=4242,
        ppid=1,
        stat="UE",
        elapsed="00:31",
        command=(
            f"{wibo} {compiler} -proc gekko -c "
            "src/melee/mn/sample.c -o /tmp/out.o"
        ),
        source_rel="src/melee/mn/sample.c",
    )

    def fake_guard(*, source_rel, function, allow_unsafe, processes=None):
        assert source_rel == "src/melee/mn/sample.c"
        assert function == "fn_80000000"
        assert allow_unsafe is False
        assert processes is None
        return debug_cli.local_safety.LocalLaneGuardResult(
            unsafe=True,
            processes=[unsafe_proc],
        )

    def fail_process_tree_runner(*args, **kwargs):
        raise AssertionError("unsafe score-source lane must not launch wibo")
    ...
    assert payload["score"] == 2**30
    assert payload["returncode"] == 124
    unsafe_lane = payload["unsafe_local_pcdump_lane"]
    assert unsafe_lane["source"] == "src/melee/mn/sample.c"
    assert unsafe_lane["function"] == "fn_80000000"
    assert unsafe_lane["cflags_unit"] == "src/melee/mn/sample.c"
    assert unsafe_lane["processes"][0]["pid"] == 4242
    assert unsafe_lane["processes"][0]["stat"] == "UE"
    assert "refusing to launch another local dump" in unsafe_lane["message"]


def test_target_score_source_remote_fallback_scores_retained_candidate_when_local_lane_unsafe(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    melee_root = tmp_path / "melee"
    ...
    unsafe_payload = {
        "source": retained_rel,
        "function": "fn_80000000",
        "cflags_unit": source_rel,
        "message": "unsafe lane",
        "processes": [{"pid": 4242, "stat": "UE"}],
        "related_source_prefixes": ["build/diagnostics/case", source_rel],
    }
    ...
    def fake_remote_pcdump(**kwargs):
        remote_calls.append(kwargs)
        assert kwargs["source_rel"] == retained_rel
        assert kwargs["compile_source_rel"] == source_rel
        assert kwargs["stage_source_path"] == retained
        return debug_cli._RemotePcdumpResult(
            returncode=0,
            stdout="pcdump text",
            stderr="",
            cmd=["ssh", "remote"],
            host=kwargs["host"],
            source_rel=retained_rel,
            compile_source_rel=source_rel,
            staged_source=retained_rel,
        )
    ...
    monkeypatch.setattr(
        debug_cli,
        "_score_source_unsafe_lane_payload",
        lambda **_kwargs: unsafe_payload,
    )
    monkeypatch.setattr(debug_cli, "_run_remote_pcdump", fake_remote_pcdump)
    monkeypatch.setattr(
        debug_cli,
        "_find_wibo",
        lambda: (_ for _ in ()).throw(AssertionError("local wibo must not run")),
    )
    _install_score_source_parse_fakes(monkeypatch, total=7.0)
    ...
    assert payload["unsafe_local_pcdump_lane"] == unsafe_payload


def test_target_score_source_remote_fallback_runs_checkdiff_guard(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    melee_root = tmp_path / "melee"
    ...
    monkeypatch.setattr(
        debug_cli,
        "_score_source_unsafe_lane_payload",
        lambda **_kwargs: {"source": retained_rel, "function": "fn_80000000"},
    )
    monkeypatch.setattr(
        debug_cli,
        "_run_remote_pcdump",
        lambda **kwargs: debug_cli._RemotePcdumpResult(
            0,
            "pcdump text",
            "",
            ["ssh"],
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: debug_cli._SourceCandidateRealScore(
            0.0,
            None,
            structural_guard={"ok": True},
            structural_guard_error=None,
        ),
    )
    _install_score_source_parse_fakes(monkeypatch, total=3.0)
    ...
    assert payload["structural_guard"] == {
        "ok": True,
        "target_score_matched": 1,
        "target_score_targeted": 1,
        "target_score_accepted": True,
    }
    assert payload["structural_guard_error"] is None


def test_target_score_source_remote_fallback_reports_terminal_blocker(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    melee_root = tmp_path / "melee"
    ...
    payload = json.loads(result.stdout)
    assert payload["score"] == 2**30
    assert payload["returncode"] == 66
    assert payload["terminal_blocker"] == "remote-retained-source-staging-failed"
    assert "source file not found" in payload["stderr_tail"]
    assert payload["remote_fallback"]["returncode"] == 66
    assert payload["remote_fallback"]["staging_ack_confirmed"] is None
    assert payload["unsafe_local_pcdump_lane"] == unsafe_payload


def test_target_score_source_remote_dependency_context_mismatch_blocker(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    melee_root = tmp_path / "melee"
    ...
    target = tmp_path / "target.json"
    target.write_text('{"virtuals": {"34": 28}}\n')
    ack = _remote_staging_ack(retained_text.encode("utf-8")).decode("utf-8")
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: debug_cli._RemotePcdumpResult(
            ...
            stderr=(
                ack
                + "src/melee/mn/sample.c:12: identifier mnDiagram_803EE774 "
                "redeclared; previously declared as float[]\n"
            ),
            cmd=["ssh"],
            host=kwargs["host"],
            source_rel=retained_rel,
            compile_source_rel=source_rel,
            staged_source=retained_rel,
            stage_source_sha256=hashlib.sha256(
                retained_text.encode("utf-8")
            ).hexdigest(),
            staging_ack_confirmed=True,
        ),
    )
    ...
    assert payload["staging_ack_confirmed"] is True
    assert payload["remote_fallback"]["staging_ack_confirmed"] is True
    assert "redeclared" in payload["stderr_tail"]


def test_target_score_source_remote_staging_timeout_text_blocker(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    melee_root = tmp_path / "melee"
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: debug_cli._RemotePcdumpResult(
            returncode=66,
            stdout="",
            stderr="stdin source staging timed out after 90s before compile\n",
            cmd=["ssh"],
            host=kwargs["host"],
            source_rel=retained_rel,
            compile_source_rel=source_rel,
    ...
    assert payload["remote_fallback"]["staging_ack_confirmed"] is False


def test_target_score_source_remote_acknowledged_compile_timeout_blocker(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    melee_root = tmp_path / "melee"
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: debug_cli._RemotePcdumpResult(
            returncode=124,
            stdout="",
            stderr=ack + "mwcc compile timed out after source was staged\n",
            cmd=["ssh"],
            host=kwargs["host"],
            source_rel=retained_rel,
            compile_source_rel=source_rel,
    ...
    assert result.exit_code == 0, result.stdout + result.stderr
    payload = json.loads(result.stdout)
    assert payload["terminal_blocker"] == "remote-retained-source-compile-timeout"
    assert payload["timeout_seconds"] == 3.0
    assert payload["remote_fallback"]["staging_ack_confirmed"] is True


def test_target_score_source_remote_timeout_reports_json_blocker(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    melee_root = tmp_path / "melee"
    ...
    assert "timed out after 1s running ssh" in payload["stderr_tail"]
    assert payload["remote_fallback"]["returncode"] == 124
    assert payload["remote_fallback"]["staging_ack_confirmed"] is False


def test_target_score_source_nonstaged_remote_failure_keeps_pcdump_blocker(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    melee_root = tmp_path / "melee"
    ...
    payload = json.loads(result.stdout)
    assert payload["score"] == 2**30
    assert payload["terminal_blocker"] == "remote-pcdump-failed"
    assert payload["remote_fallback"]["staged_source"] is None


def test_target_score_source_remote_mode_skips_local_tool_discovery(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    melee_root = tmp_path / "melee"
    ...
    monkeypatch.setattr(
        debug_cli,
        "_find_wibo",
        lambda: (_ for _ in ()).throw(AssertionError("_find_wibo called")),
    )
    monkeypatch.setattr(
        debug_cli,
        "_find_compiler_dir",
        lambda: (_ for _ in ()).throw(AssertionError("_find_compiler_dir called")),
    )
    monkeypatch.setattr(
        debug_cli,
        "_score_source_unsafe_lane_payload",
        lambda **_kwargs: (_ for _ in ()).throw(AssertionError("local guard called")),
    )
    monkeypatch.setattr(
        debug_cli,
        "_run_remote_pcdump",
    ...
    _install_score_source_parse_fakes(monkeypatch, total=5.0)
    ...
    assert payload["remote_fallback"]["used"] is True
    assert payload["remote_fallback"]["reason"] == "--remote"


def test_target_score_source_remote_fallback_uses_local_when_lane_safe(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    melee_root = tmp_path / "melee"
    ...
    def fake_process_tree_runner(cmd, *, cwd, timeout, env=None):
        local_calls.append([str(part) for part in cmd])
        (cwd / env["MWCC_DEBUG_PCDUMP_PATH"]).write_text("pcdump text")
        return subprocess.CompletedProcess(cmd, 0, "", "")
    ...
    monkeypatch.setattr(
        debug_cli,
        "_run_remote_pcdump",
        lambda **_kwargs: (_ for _ in ()).throw(AssertionError("remote used")),
    )
    monkeypatch.setattr(
        debug_cli,
        "_run_with_process_group_timeout",
    ...
    _install_score_source_parse_fakes(monkeypatch, total=0.0)
    ...
    payload = json.loads(result.stdout)
    assert payload["score"] == 0
    assert "remote_fallback" not in payload
    assert local_calls


def test_target_score_source_timeout_reports_unsafe_local_lane(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    melee_root = tmp_path / "melee"
    ...
    def fake_guard(*, source_rel, function, allow_unsafe, processes=None):
        assert function == "fn_80000000"
        assert processes is None
        guard_calls.append((source_rel, allow_unsafe))
    ...
    assert payload["returncode"] == 124
    assert payload["timeout_seconds"] == 0.25
    assert payload["unsafe_local_pcdump_lane"]["source"] == "src/melee/mn/sample.c"
    assert payload["unsafe_local_pcdump_lane"]["processes"][0]["pid"] == 5151
    assert guard_calls == [
        ("src/melee/mn/sample.c", False),
        ("src/melee/mn/sample.c", False),
    ]


def test_target_score_source_refuses_sibling_retained_candidate_lane(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    melee_root = tmp_path / "melee"
    ...
    current_candidate = retained_dir / "candidate-c19.c"
    current_candidate.write_text("void fn_80000000(void) {}\n")
    sibling_candidate = retained_dir / "candidate-c17.c"
    sibling_candidate.write_text("void fn_80000000(void) {}\n")
    current_rel = (
        "build/mwcc_debug_cache/probes/node_set_split/"
        "wrong_register/candidate-c19.c"
    )
    sibling_rel = (
        "build/mwcc_debug_cache/probes/node_set_split/"
        "wrong_register/candidate-c17.c"
    )
    cflags_rel = "src/melee/mn/sample.c"
    compiler_dir = tmp_path / "compiler"
    compiler_dir.mkdir()
    compiler = compiler_dir / "mwcceppc_debug.exe"
    compiler.write_text("")
    ...
    target = tmp_path / "target.json"
    target.write_text('{"virtuals": {"34": 28}}\n')
    unsafe_sibling_proc = debug_cli.local_safety.LocalWiboProcess(
        pid=6161,
        ppid=1,
        stat="UE",
        elapsed="00:22",
        command=f"{wibo} {compiler} -proc gekko -c {sibling_rel} -o /tmp/out.o",
        source_rel=sibling_rel,
    )
    unsafe_unit_proc = debug_cli.local_safety.LocalWiboProcess(
        pid=6162,
        ppid=1,
        stat="UE",
        elapsed="00:23",
        command=f"{wibo} {compiler} -proc gekko -c {cflags_rel} -o /tmp/out.o",
        source_rel=cflags_rel,
    )

    def fake_resolve(path):
        if str(path) == str(current_candidate):
            return current_rel
        if str(path) == cflags_rel:
            return cflags_rel
        return str(path)

    def fail_process_tree_runner(*args, **kwargs):
        raise AssertionError(
            "retained sibling lane with existing UE wibo must not launch"
        )
    ...
    monkeypatch.setattr(
        ...
        lambda: [unsafe_sibling_proc, unsafe_unit_proc],
    )
    monkeypatch.setattr(
        debug_cli.local_safety,
        "allow_unsafe_local_pcdump",
    ...
    assert payload["returncode"] == 124
    unsafe_lane = payload["unsafe_local_pcdump_lane"]
    assert unsafe_lane["source"] == current_rel
    assert unsafe_lane["related_source_prefixes"] == [
        "build/mwcc_debug_cache/probes/node_set_split/wrong_register",
        cflags_rel,
    ]
    assert {process["pid"] for process in unsafe_lane["processes"]} == {6161, 6162}
    assert {process["source_rel"] for process in unsafe_lane["processes"]} == {
        sibling_rel,
        cflags_rel,
    }


def test_target_score_source_refuses_diagnostics_combine_related_lane(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    melee_root = tmp_path / "melee"
    ...
    retained_dir = (
        ...
        / "build"
        / "diagnostics"
        / "sample_caseA_recombine"
    )
    retained_dir.mkdir(parents=True)
    current_candidate = retained_dir / "combine-1-3.c"
    current_candidate.write_text("void fn_80000000(void) {}\n")
    sibling_candidate = retained_dir / "combine-1-4.c"
    sibling_candidate.write_text("void fn_80000000(void) {}\n")
    current_rel = "build/diagnostics/sample_caseA_recombine/combine-1-3.c"
    sibling_rel = "build/diagnostics/sample_caseA_recombine/combine-1-4.c"
    cflags_rel = "src/melee/mn/sample.c"
    compiler_dir = tmp_path / "compiler"
    compiler_dir.mkdir()
    compiler = compiler_dir / "mwcceppc_debug.exe"
    ...
    def fail_process_tree_runner(*args, **kwargs):
        raise AssertionError(
            "diagnostics sibling lane with existing UE wibo must not launch"
        )
    ...
    assert unsafe_lane["source"] == current_rel
    assert unsafe_lane["related_source_prefixes"] == [
        "build/diagnostics/sample_caseA_recombine",
        cflags_rel,
    ]
    assert {process["pid"] for process in unsafe_lane["processes"]} == {7171, 7172}
    assert {process["source_rel"] for process in unsafe_lane["processes"]} == {
        sibling_rel,
        cflags_rel,
    }
...
def test_target_score_source_retained_node_set_candidate_uses_real_tu(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    from src.mwcc_debug import scoring as scoring_mod
    ...
    source = melee_root / "src" / "melee" / "mn" / "sample.c"
    source.parent.mkdir(parents=True)
    original_source = "void fn_80000000(void) { live_call(); }\n"
    retained_source = "void fn_80000000(void) { retained_call(); }\n"
    source.write_text(original_source)
    retained = (
        melee_root
        / "build"
    ...
    retained.parent.mkdir(parents=True)
    retained.write_text(retained_source)
    retained_rel = (
        "build/mwcc_debug_cache/probes/node_set_split/realized/candidate.c"
    )
    source_rel = "src/melee/mn/sample.c"
    compiler_dir = tmp_path / "compiler"
    compiler_dir.mkdir()
    ...
    def fake_resolve(path):
        if str(path) == str(retained):
            return retained_rel
        if str(path) == source_rel:
            return source_rel
        return str(path)

    def fake_process_tree_runner(cmd, *, cwd, timeout, env=None):
        compile_cmds.append([str(part) for part in cmd])
        assert cwd == melee_root
        assert source.read_text() == retained_source
        (cwd / env["MWCC_DEBUG_PCDUMP_PATH"]).write_text("pcdump text")
        return subprocess.CompletedProcess(cmd, 0, "", "")
    ...
    monkeypatch.setattr(
        mwcc_debug_mod,
        "parse_pcdump",
        lambda _text: [SimpleNamespace(name="fn_80000000")],
    )
    monkeypatch.setattr(mwcc_debug_mod, "parse_hook_events", lambda _text: [])
    monkeypatch.setattr(mwcc_debug_mod, "find_function", lambda _events, _name: None)
    monkeypatch.setattr(
...
def test_target_score_source_diagnostics_combine_candidate_uses_real_tu(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    from src.mwcc_debug import scoring as scoring_mod
    ...
    retained.parent.mkdir(parents=True)
    retained.write_text(retained_source)
    retained_rel = "build/diagnostics/sample_caseA_recombine/combine-1-3.c"
    source_rel = "src/melee/mn/sample.c"
    compiler_dir = tmp_path / "compiler"
    compiler_dir.mkdir()
    compiler = compiler_dir / "mwcceppc_debug.exe"
...
def test_target_score_source_json_reports_virtual_assignments(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    from src.mwcc_debug import scoring as scoring_mod
    ...
    wibo.write_text("")
    target = tmp_path / "target.json"
    target.write_text('{"virtuals": {"34": 28, "44": 30}}\n')
    baseline = tmp_path / "baseline.pcdump.txt"
    baseline.write_text("baseline pcdump text")
    ...
    def fake_expression_score(**kwargs):
        expression_calls.append(kwargs)
        assert kwargs["pcdump_text"] == "pcdump text"
        assert kwargs["baseline_pcdump_text"] == "baseline pcdump text"
        assert kwargs["candidate_source_text"] == source.read_text()
        return {
            "matched": 0,
            "targeted": 2,
            "virtual_distance": 2,
    ...
    details = payload["target_score"]
    assert details["matched"] == 1
    assert details["targeted"] == 2
    assert details["virtual_distance"] == 1
    assert details["virtuals"] == {
        "34": {"expected": 28, "actual": 29, "matched": False},
        "44": {"expected": 30, "actual": 30, "matched": True},
    }
    assert payload["expression_score"]["false_positive_virtual_id_hit_count"] == 1
...
def test_permuter_bootstrap_injects_enum_and_bool_latedefines_for_randomize_helpers() -> None:
    source_text = textwrap.dedent(
        """\
        ...
        void fn_80000000(u8 stat_type)
        {
            mnDiagram2_IsTimeStat(stat_type);
        }
        """
    )
    dependency_text = textwrap.dedent(
    ...
    assert injected == ["mnDiagram2_IsTimeStat"]
    assert "#pragma _permuter define VSSTAT_PLAY_TIME 0x0B" in patched
    assert "#pragma _permuter define true 1" in patched
    assert "#pragma _permuter define false 0" in patched
    assert patched.index("#pragma _permuter define VSSTAT_PLAY_TIME 0x0B") < patched.index(
        "inline bool mnDiagram2_IsTimeStat"
    )
...
def test_dump_local_aborts_on_wibo_missing_import_before_watchdog(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    melee_root = tmp_path / "melee"
    ...
    wibo.write_text(
        ...
        "import sys\n"
        "import time\n"
        "print('wibo: call reached missing import FormatMessageA from kernel32', "
    ...
    monkeypatch.setattr(debug_cli, "_cache_settle_seconds", lambda env=None: 0.0)
    monkeypatch.setattr(debug_cli, "_kill_debug_dump_local_process_tree", fake_kill_tree)
    monkeypatch.setenv("MWCC_DEBUG_HANG_TIMEOUT", "30")
    ...
    assert killed
    assert "FormatMessageA" in result.stderr
    assert "debug dump doctor --repair" in result.stderr
...
def test_debug_dump_local_probe_uses_same_tu_build_settings(
    ...
    wibo.write_text(textwrap.dedent("""\
        ...
        pathlib.Path(os.environ["MELEE_TEST_WIBO_ARGS"]).write_text(
            "pcdump_path=" + os.environ["MWCC_DEBUG_PCDUMP_PATH"] + "\\n"
            + "\\n".join(sys.argv[1:]),
            encoding="utf-8",
        )
    ...
    pcdump_line = next(
        ...
        if line.startswith("pcdump_path=")
    )
    temp_pcdump_path = Path(pcdump_line.split("=", 1)[1])
    assert temp_pcdump_path.is_absolute()
    assert tmp_path not in (temp_pcdump_path, *temp_pcdump_path.parents)
    assert not list(tmp_path.glob("pcdump_*.txt"))
...
def test_force_vector_auto_verify_child_uses_target_root_and_current_package(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    src_path = tmp_path / "src" / "melee" / "mn" / "sample.c"
    src_path.parent.mkdir(parents=True)
    src_path.write_text("void fn_80000000(void) {}\n")
    captured: list[dict] = []
    ...
    debug_cli._run_force_vector_auto_verify(
        src_path=src_path,
        function="fn_80000000",
        entries=[
            debug_cli._ForceVectorEntry(
                raw="class1:ig42:phys=f26",
                kind="phys",
                ig_idx=42,
                phys=26,
                class_id=1,
        ...
        per_probe_timeout_s=1.0,
    )

    assert len(captured) == 1
    assert captured[0]["cwd"] == tmp_path
    package_path = debug_cli._package_melee_root() / "tools" / "melee-agent"
    assert captured[0]["env"]["PYTHONPATH"].split(os.pathsep)[0] == str(package_path)
    assert "CHECKDIFF_NO_LOCK" not in captured[0]["env"]


def test_force_vector_auto_verify_preserves_parent_no_lock_env(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    src_path = tmp_path / "src" / "melee" / "mn" / "sample.c"
    ...
    parent_env = debug_cli._checkdiff_env_for_locked_child(disable_fingerprint=False)
    debug_cli._run_force_vector_auto_verify(
        src_path=src_path,
        function="fn_80000000",
        entries=[
            debug_cli._ForceVectorEntry(
                raw="class0:ig42:phys=r26",
                kind="phys",
                ig_idx=42,
                phys=26,
                class_id=0,
    ...
    assert len(captured) == 1
    package_path = debug_cli._package_melee_root() / "tools" / "melee-agent"
    assert captured[0]["env"]["CHECKDIFF_NO_LOCK"] == "1"
    assert captured[0]["env"]["PYTHONPATH"].split(os.pathsep)[0] == str(package_path)
...
def test_debug_dump_local_missing_unit_source_reports_resolution_context(
    ...
    assert "tools/melee-agent" in unwrapped
    assert "src/melee/ft/ftdynamics.c" in unwrapped
...
def test_dump_local_force_coalesce_accepts_fpr_pair_and_sets_class_env(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    melee_root = tmp_path / "melee"
    src_path = melee_root / "src" / "melee" / "mn" / "sample.c"
    src_path.parent.mkdir(parents=True)
    src_path.write_text("void fn_fpr(void)\n{\n}\n")
    compiler_dir = melee_root / "build" / "compilers" / "GC" / "1.2.5n"
    compiler_dir.mkdir(parents=True)
    (compiler_dir / "mwcceppc_debug.exe").write_text("")
    env_capture = tmp_path / "force-env.json"
    wibo = tmp_path / "wibo"
    wibo.write_text(
        "#!/usr/bin/env python3\n"
        "import json\n"
        ...
        "import sys\n"
        "from pathlib import Path\n"
        "pcdump = Path(os.environ['MWCC_DEBUG_PCDUMP_PATH'])\n"
        "pcdump.write_text('Starting function fn_fpr\\n')\n"
        "obj = Path(sys.argv[sys.argv.index('-o') + 1])\n"
        "obj.write_bytes(b'forced-object')\n"
        "Path(os.environ['FORCE_ENV_CAPTURE']).write_text(json.dumps({\n"
        "    'coalesce': os.environ.get('MWCC_DEBUG_FORCE_COALESCE'),\n"
        "    'class': os.environ.get('MWCC_DEBUG_FORCE_COALESCE_CLASS'),\n"
        "}))\n"
    )
    wibo.chmod(0o755)
    ...
    monkeypatch.setenv("FORCE_ENV_CAPTURE", str(env_capture))
    monkeypatch.setattr(debug_cli, "DEFAULT_MELEE_ROOT", melee_root)
    monkeypatch.setattr(debug_cli, "_find_wibo", lambda: wibo)
    monkeypatch.setattr(debug_cli, "_find_compiler_dir", lambda: compiler_dir)
    monkeypatch.setattr(debug_cli, "_ninja_cflags_for_unit", lambda src_rel: ("", "mwcc"))
    monkeypatch.setattr(debug_cli, "_cache_settle_seconds", lambda env=None: 0.0)
    monkeypatch.setattr(debug_cli, "_reject_unsafe_force_coalesce", fake_reject)
    ...
    assert result.exit_code == 0, result.stdout + result.stderr
    env_payload = json.loads(env_capture.read_text())
    assert env_payload == {"coalesce": "46=56", "class": "1"}
    assert captured_preflight["force_coalesce"] == "46=56"
    assert captured_preflight["register_class"] == "fpr"
...
def test_decl_orders_json_keep_best_applies_winner_and_refreshes_baseline(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(debug_cli, "DEFAULT_MELEE_ROOT", melee_root)
    monkeypatch.setattr(debug_cli, "_compute_melee_root", lambda: melee_root)
    monkeypatch.setattr(
        debug_cli,
        "_build_and_match_with_diagnostic",
        fake_build_and_match,
    )
...
def test_decl_orders_json_reports_build_failure_diagnostic_from_cwd_worktree(
    monkeypatch,
    tmp_path,
) -> None:
    melee_root = tmp_path / "claude-worktree"
    fallback_root = tmp_path / "default-root"
    melee_root.mkdir()
    (melee_root / "configure.py").write_text("# fixture\n")
    report_dir = melee_root / "build" / "GALE01"
    report_dir.mkdir(parents=True)
    (report_dir / "report.json").write_text(
        json.dumps(
    ...
    def fake_compute_melee_root() -> Path:
        nonlocal compute_calls
        compute_calls += 1
        return melee_root

    def fake_build_and_match_with_diagnostic(
        unit,
        function,
        root,
        *,
        ...
        return None, "mwcc fixture diagnostic"

    monkeypatch.setattr(debug_cli, "DEFAULT_MELEE_ROOT", fallback_root)
    monkeypatch.setattr(debug_cli, "_compute_melee_root", fake_compute_melee_root)
    monkeypatch.setattr(
        debug_cli,
        "_build_and_match_with_diagnostic",
    ...
    assert result.exit_code == 0, result.stdout + result.stderr
    payload = json.loads(result.stdout)
    candidate = payload["rounds"][0]["results"][0]
    assert payload["baseline_pct"] == 10.0
    assert payload["baseline_diagnostic"] == "mwcc fixture diagnostic"
    assert candidate["build_failed"] is True
    assert candidate["diagnostic"] == "mwcc fixture diagnostic"
    assert compute_calls == 1
    assert seen_roots == [melee_root, melee_root]
    assert src_path.read_text() == original


def test_decl_orders_promote_strategy_finds_same_type_group_win(
    monkeypatch,
    tmp_path,
) -> None:
    melee_root = tmp_path
    ...
    assert result.exit_code == 0, result.stdout + result.stderr
    payload = json.loads(result.stdout)
    assert payload["best_pct"] == 20.0
    assert payload["best_label"] == "promote-group u8 result+result2"
    assert (
        "    u8 result;\n"
        "    u8 result2;\n"
        "    int i;\n"
        "    int guard;\n"
    ) in src_path.read_text()
...
def _setup_diagnose_tiebreak_env(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    """Wire up a minimal diagnose environment for register-tiebreak tests."""
    melee_root = tmp_path / "melee"
    src_path = melee_root / "src" / "melee" / "ft" / "ftcommon.c"
    src_path.parent.mkdir(parents=True)
    src_path.write_text("void ft_800852B0(void) {}\n")
    pcdump = tmp_path / "ft_800852B0.pcdump.txt"
    pcdump.write_text("Starting function ft_800852B0\n")
    ...
    monkeypatch.setattr(
        debug_cli,
        "_find_unit_for_function",
        lambda function, root: "melee/ft/ftcommon",
    )
    monkeypatch.setattr(debug_cli, "_get_match_pct", lambda function, root: 99.1)
    monkeypatch.setattr(
        debug_cli, "audit_function_casts", lambda source, function: []
    )
    monkeypatch.setattr(
        debug_cli,
        "_resolve_pcdump_path",
...
def test_diagnose_force_phys_emits_compiler_temp_tiebreak_levers(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    # The register-tiebreak heuristic (formerly reached via the removed
    # `suggest register-tiebreak --force-phys`) is now exercised through the
    # live `diagnose --force-phys` caller. Assert the heuristic output here.
    _setup_diagnose_tiebreak_env(monkeypatch, tmp_path)
...
def test_diagnose_force_phys_tiebreak_json_is_structured(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _setup_diagnose_tiebreak_env(monkeypatch, tmp_path)
    ...
    assert result.exit_code == 0, result.stdout + result.stderr
    payload = json.loads(result.stdout)
    tiebreak = payload["register_tiebreak"]
    assert tiebreak["function"] == "ft_800852B0"
    assert tiebreak["normalized_force_phys"] == "0:53:4"
    assert tiebreak["targets"][0]["ig_idx"] == 53
    assert tiebreak["targets"][0]["target_phys"] == 4
    assert any(
        lever["kind"] == "interference-insertion"
        for lever in tiebreak["levers"]
    )
...
def test_mutate_simplify_order_emits_candidate_progress_to_stderr(
    ...
    baseline_sig = SimpleNamespace(simplify_order=(41, 40), assigned_regs=())
...
def test_mutate_simplify_order_flushes_final_stdout_report(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    import src.mwcc_debug.diff_capture as diff_capture_mod
    ...
    monkeypatch.setattr(
        ...
        lambda: flush_calls.__setitem__("n", flush_calls["n"] + 1),
    )
...
def test_inspect_stuck_routes_frame_size_rows_to_frame_tools(
    ...
    assert payload["frame_residual"]["evidence"] == "checkdiff-only"
    assert payload["frame_residual"]["probe_status"] == "needs-attribution"
    assert "closability_tier" not in payload["frame_residual"]
    assert payload["frame_residual"]["subcategory"] == "frame-too-small"
    assert payload["next_steps"][0] == (
        "melee-agent debug dump local src/melee/mn/sample.c "
        "--function fn_80000000 && melee-agent debug inspect "
        "frame-reservations -f fn_80000000 --json"
    )
    assert "debug inspect frame-reservations -f fn_80000000" in payload["next_steps"][0]
    joined = "\n".join(payload["next_steps"][:3])
    assert "debug mutate frame-transform-search" not in joined
...
def test_inspect_stuck_routes_reserved_low_spill_to_attribution(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    melee_root = tmp_path
    ...
    assert payload["next_steps"][0].startswith("melee-agent debug dump local")


def test_frame_residual_hint_from_report_includes_evidence_fields() -> None:
    report = {
        "function": "fn_80000000",
    ...
    assert hint["evidence"] == "pcdump-attributed"
    assert hint["probe_status"] == "needs-attribution"
    assert "closability_tier" not in hint
    assert hint["source_object_symbol"] == "local_temp"
    assert hint["next_steps"][0].startswith("melee-agent debug dump local")
...
def test_debug_guide_warns_when_no_target_is_loaded(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "src.mwcc_debug.suggest",
        lambda parsed_fn, result, events=None: [],
    )
