...
def test_write_hash_sidecar_digest_uses_captured_source_snapshot(
    melee_root: Path,
) -> None:
    src = melee_root / "src" / "melee" / "mn" / "mnvibration.c"
    src.write_text("// compiled source")
    compiled_digest = source_digest(src)

    cache = cache_path(melee_root, "melee/mn/mnvibration")
    cache.parent.mkdir(parents=True, exist_ok=True)
    cache.write_text("Starting function ...")

    src.write_text("// restored source")
    write_hash_sidecar_digest(cache, compiled_digest)

    entry = lookup(melee_root, "melee/mn/mnvibration")
    assert entry is not None
    assert entry.fresh is False
    assert hash_path(cache).read_text(encoding="ascii").strip() == compiled_digest


def test_resolve_pcdump_path_stale_warning_names_exact_local_refresh(
    melee_root: Path,
    capsys,
) -> None:
    src = melee_root / "src" / "melee" / "mn" / "mnvibration.c"
    ...
    report.write_text(
        '{"units":[{"name":"main/melee/mn/mnvibration",'
        '"functions":[{"name":"fn_80000000"}]}]}'
    )
    cache = cache_path(melee_root, "melee/mn/mnvibration")
    cache.parent.mkdir(parents=True, exist_ok=True)
    cache.write_text("Starting function fn_80000000\n")
    write_hash_sidecar(cache, src)
    src.write_text("// edited")

    resolved = debug_cli._resolve_pcdump_path(
        None,
        "fn_80000000",
        melee_root,
        require_fresh=False,
    ...
    assert (
        "melee-agent debug dump local src/melee/mn/mnvibration.c "
        "--function fn_80000000"
    ) in capsys.readouterr().err


def test_pcdump_local_accepts_function_only_refresh_command(
    ...
    result = runner.invoke(
        debug_cli.debug_app,
        ["dump", "local", "-f", "fn_80000000"],
    )

    combined = result.stdout + result.stderr
    assert result.exit_code == 2
    assert "Missing argument" not in combined
    assert "wibo binary not found" in combined


def test_compiled_source_snapshot_rejects_restore_during_settle(
    melee_root: Path,
) -> None:
    src = melee_root / "src" / "melee" / "mn" / "mnvibration.c"
    src.write_text("// transient source")
    compiled_digest = source_digest(src)

    def restore_source(_seconds: float) -> None:
        src.write_text("// restored source")

    ok, current_digest = debug_cli._compiled_source_snapshot_still_current(
        src,
        compiled_digest,
        settle_seconds=0.01,
        sleep_fn=restore_source,
    )

    assert ok is False
    assert current_digest != compiled_digest
...
def test_lookup_fresh_when_hash_matches(melee_root: Path) -> None:
    ...
    This is the scenario that broke after debug mutate decl-orders,
    debug mutate search, or debug permute verify restored the original
    source (updating mtime even though the file bytes are identical).
    """
    src = melee_root / "src" / "melee" / "mn" / "mnvibration.c"
...
def test_lookup_fresh_after_rename_with_utime(melee_root: Path) -> None:
    """Simulate the debug dump local rename path: a temp file created before a
