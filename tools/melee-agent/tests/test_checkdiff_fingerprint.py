...
def test_build_reports_unconfigured_source_target_before_ninja(
    checkdiff, tmp_path, monkeypatch, capsys,
):
    _make_stub_repo(tmp_path, "fn_alpha", match_pct=27.27)
    _patch_paths(checkdiff, monkeypatch, tmp_path)
    (tmp_path / "build.ninja").write_text(
        "build build/GALE01/obj/melee/mn/sample.o: phony\n",
        encoding="utf-8",
    )

    def fail_run_build_command(*args, **kwargs):
        pytest.fail("_run_build_command should not be called")

    monkeypatch.setattr(checkdiff, "_run_build_command", fail_run_build_command)
    monkeypatch.setattr(
        sys,
        "argv",
    ...
    err = capsys.readouterr().err
    assert rc == 1
    assert "no configured source build target" in err
    assert "build/GALE01/src/melee/mn/sample.o" in err
    assert 'Object(NonMatching, "melee/mn/sample.c")' in err
