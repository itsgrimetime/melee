...
_REPORT_GUARD = pytest.mark.skipif(
    not (REPO / "build/GALE01/report.json").exists(),
    reason="GALE01 report has not been generated",
)
...
def test_struct_verify_thpdec_reports_known_discrepancies(tmp_path):
    ...
    """
    pytest.skip("historical pre-fix THP header no longer compiles with current upstream THPDec.c")
