...
def _force_phys_pcdump(*, ig34_reg: int, ig44_reg: int) -> str:
    return (
        "Starting function fn_test\n"
        "COLORGRAPH DECISIONS (class=0, result=1, n_nodes=2)\n"
        "iter ig_idx assigned degree n_interferers flags\n"
        f"  0  34  r{ig34_reg}  0  0  0x0\n"
        f"  1  44  r{ig44_reg}  0  0  0x0\n"
        "[COALESCE] enter class=0 n_virtuals=45\n"
        "[COALESCE] exit class=0 n_virtuals=45 distinct_roots=45 forced=0\n"
        "SIMPLIFY GRAPH (class=0, n_colors=29, n_class_regs=40)\n"
        "iter ig_idx degree array_size flags\n"
        "  0  34  1  1  0x0\n"
        "  1  44  1  1  0x0\n"
...
def _write_retained_fixture(tmp_path: Path) -> tuple[Path, Path]:
    retained = tmp_path / "retained.c"
    retained.write_text(
        "void fn_test(void) {\n"
        "    int retained_a;\n"
        "    retained_a = 1;\n"
    ...
    retained_pcdump.write_text(_stub_pcdump(), encoding="utf-8")
    return retained, retained_pcdump


def test_retained_source_requires_pcdump(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    melee_root = _stage_fake_melee_root(tmp_path, monkeypatch)
    retained = tmp_path / "retained.c"
    retained.write_text("void fn_test(void) { int a; a = 1; }\n", encoding="utf-8")
    ...
    result = CliRunner().invoke(
        ...
        [
            ...
            "--source-file", str(retained),
            "--max-candidates", "1",
        ],
    )

    assert result.exit_code == 2
    assert "--pcdump is required with --source-file" in (
        result.stdout + result.stderr
    )
    assert melee_root.exists()


def test_retained_source_json_retains_force_phys_ranked_probe(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    melee_root = tmp_path / "melee"
    ...
    retained.write_text(
        ...
        "    int retained_a;\n"
        "    int retained_b;\n"
        "    retained_a = retained_b + 1;\n"
        "}\n",
        encoding="utf-8",
    )
    retained_pcdump = tmp_path / "retained.pcdump.txt"
    retained_pcdump.write_text(
        _force_phys_pcdump(ig34_reg=29, ig44_reg=25),
        encoding="utf-8",
    )
    ...
    def fake_compile(
        ...
        *,
        ...
        compile_calls.append({
            "path": Path(diff_input.path),
            "unit_source": unit_source,
            "function_aliases": function_aliases,
        })
        text = Path(diff_input.path).read_text(encoding="utf-8")
        if "retained_b;\n    int retained_a;" in text:
            return _force_phys_pcdump(ig34_reg=27, ig44_reg=25)
        return _force_phys_pcdump(ig34_reg=29, ig44_reg=25)
    ...
    result = CliRunner().invoke(
        ...
        [
            ...
            "--force-phys", "34:27,44:25",
            "--source-file", str(retained),
            "--pcdump", str(retained_pcdump),
            "--json",
            "--retain-probes", str(probes_dir),
            "--max-candidates", "1",
        ],
    )
    ...
    assert payload["retained_mode"] is True
    assert payload["source_file"] == str(retained)
    assert payload["pcdump"] == str(retained_pcdump)
    assert payload["protected_force_phys"] == {"44": 25}
    assert payload["residual_force_phys"] == {"34": 27}
    assert payload["baseline"]["target_score"]["virtuals"]["34"]["actual"] == 29
    assert payload["baseline"]["target_score"]["virtuals"]["44"]["hit"] is True

    probes = payload["ranked_probes"]
    assert len(probes) == 1
    probe = probes[0]
    assert probe["source_retained"].startswith(str(probes_dir))
    assert Path(probe["source_retained"]).is_file()
    assert "void fn_test" in probe["source_hunk"]
    assert probe["target_score"]["virtuals"]["34"]["actual"] == 27
    assert probe["target_score"]["virtuals"]["34"]["hit"] is True
    assert probe["target_score"]["virtuals"]["34"]["matched"] is True
    assert probe["target_score"]["virtuals"]["34"]["distance"] == 0
    assert probe["target_score"]["virtuals"]["44"]["hit"] is True
    assert probe["force_phys_hits"] == 2
    assert probe["force_phys_distance_total"] == 0
    assert probe["baseline_force_phys_distance_total"] == 2
    assert probe["structural_guard"] is None
    assert probe["terminal_blocker"] is None

    assert compile_calls
    assert compile_calls[0]["unit_source"] == repo_source


def test_retained_source_json_clears_terminal_blocker_on_residual_progress(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    melee_root = tmp_path / "melee"
    ...
    def fake_compile(
        ...
        *,
        ...
        text = Path(diff_input.path).read_text(encoding="utf-8")
        if "retained_b;\n    int retained_a;" in text:
            return _force_phys_pcdump(ig34_reg=28, ig44_reg=25)
        return _force_phys_pcdump(ig34_reg=29, ig44_reg=25)
    ...
    payload = json.loads(result.stdout)
    assert payload["terminal_blocker"] is None
    probe = payload["ranked_probes"][0]
    assert probe["target_score"]["virtuals"]["34"]["actual"] == 28
    assert probe["target_score"]["virtuals"]["34"]["hit"] is False
    assert probe["target_score"]["virtuals"]["34"]["matched"] is False
    assert probe["target_score"]["virtuals"]["34"]["distance"] == 1
    assert probe["force_phys_distance_total"] == 1
    assert probe["baseline_force_phys_distance_total"] == 2
    assert probe["force_phys_assignment_improved_count"] == 1


def test_retained_source_json_outputs_partial_payload_on_interrupt(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    melee_root = tmp_path / "melee"
    ...
    def fake_search(*_args, **_kwargs):
        signature = _ss.BaselineSignature(
            interference_edges=frozenset(),
            coalesce_mappings=frozenset(),
            spill_set=frozenset(),
            simplify_order=(-1, -1),
            assigned_regs=frozenset({(34, 28), (44, 25)}),
        )
        scored = _ss.ScoredVariant(
            variant=_ss.SourceVariant(
                text=retained.read_text(encoding="utf-8"),
                provenance="partial-before-interrupt",
                parent_baseline=retained,
            ),
            signature=signature,
            score=_ss.SimplifyScore(
                target_prefix=(34, 44),
                observed_prefix=(44,),
                common_prefix_length=1,
                is_exact_match=False,
                baseline_common_prefix_length=1,
                assignment_distance_total=1,
                baseline_assignment_distance_total=2,
                assignment_improved_count=1,
            ),
            precolor_distance=_ss.PrecolorDistance(0, 0, 0, 0, 0, 0),
        )
        return _ss.SearchResult(
            ...
            last_provenance="interrupting-probe",
        )
    ...
    assert payload["abort_reason"] == "keyboard-interrupt"
    assert payload["last_candidate"] == "interrupting-probe"
    assert payload["terminal_blocker"] == "search-aborted-keyboard-interrupt"
    assert payload["summary"]["compiled"] == 2
    assert payload["ranked_probes"][0]["provenance"] == "partial-before-interrupt"
    assert payload["ranked_probes"][0]["force_phys_distance_total"] == 1


def test_mutate_simplify_order_passes_skip_controls_to_search(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    result = CliRunner().invoke(
        ...
        [
            ...
            "--fn", "fn_test",
            "--want-first", "42,32",
            "--skip-first-candidates", "7",
            "--skip-provenance", "decl-orders sticky",
            "--skip-family", "type-change ",
            "--max-candidates", "1",
        ],
    )
    ...
    assert captured["skip_first_candidates"] == 7
    assert captured["skip_provenances"] == ["decl-orders sticky"]
    assert captured["skip_families"] == ["type-change "]


def test_retained_source_json_includes_resume_skip_summary(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _stage_fake_melee_root(tmp_path, monkeypatch)
    retained, retained_pcdump = _write_retained_fixture(tmp_path)
    ...
    def fake_search(*_args, **_kwargs):
        return _ss.SearchResult(
            ...
            skipped_count=2,
            skip_reasons=[
                {"provenance": "decl-orders sticky", "reason": "skip-provenance"},
                {
                    "provenance": "type-change local: int->unsigned int",
                    "reason": "skip-family:type-change ",
                },
            ],
            candidate_stream_position=3,
        )
    ...
    assert payload["summary"]["candidate_stream_position"] == 3
    assert payload["resume"] == {
        "skip_first_candidates": 2,
        "skip_provenances": ["decl-orders sticky"],
        "skip_families": ["type-change "],
        "skipped_count": 2,
        "skip_reasons": [
            {"provenance": "decl-orders sticky", "reason": "skip-provenance"},
            {
        ...
        "candidate_stream_position": 3,
        "next_skip_first_candidates": 3,
    }


def test_retained_source_json_all_skipped_sets_terminal_blocker(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _stage_fake_melee_root(tmp_path, monkeypatch)
    ...
    payload = json.loads(result.stdout)
    assert payload["summary"]["compiled"] == 0
    assert payload["summary"]["skipped"] == 2
    assert payload["terminal_blocker"] == (
...
def test_render_combined_score_help_text_mentions_flag() -> None:
    ...
    assert "--combined-alp" in proc.stdout or "combined-alp" in proc.stdout
