...
def test_decl_order_candidates_dedup_duplicate_orders() -> None:
    source = """
void fn_test(void)
{
    int a;
...
"""
    ...
    orders = [tuple(candidate.order) for candidate in candidates]
...
# ---------------------------------------------------------------------------
# Fix A: debug permute verify permuter placeholder detection
# ---------------------------------------------------------------------------
...
"""
    ...
    assert "inline_fn" in hits, (
        "debug permute verify should detect 'inline_fn' as a permuter placeholder"
    )
...
def test_reorder_decls_in_function_scope_handles_utf8_prefix_offsets() -> None:
    source = textwrap.dedent("""\
    ...
    assert result is not None
    assert result.count("int first;") == 1
    assert result.count("int holder;") == 1
    assert result.count("int out;") == 1
    assert result.index("int holder;") < result.index("int first;")
    assert "PAD_STACK(4);" in result
    assert "use(first, holder, out);" in result
