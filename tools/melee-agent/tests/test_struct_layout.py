...
_BUILD_NINJA_GUARD = pytest.mark.skipif(
    not (REPO / "build.ninja").exists(),
    reason="build.ninja has not been generated",
)
...
def test_resolve_layout_thpfileinfo():
    ...
    # field-path -> offset
    assert layout["RST"] == 0x740
    assert layout["nMCU"] == 0x742
    assert layout["components[1].predDC"] == 0x864
    assert layout["components[0].DCTableSelector"] == 0x83c
    # reverse map available
    assert struct_layout.offset_to_field(layout, 0x740) == "RST"
...
def test_verify_offsets():
    ok = struct_layout.verify_offsets(REPO, "THPFileInfo",
        "extern/dolphin/src/dolphin/thp/THPDec.c",
        {"RST": 0x740, "components[0].predDC": 0x838})
    assert ok is True
