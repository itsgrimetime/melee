...
def test_build_descriptors_refuses_non_gpr_class():
    # reg_info/bindings are GPR-keyed (parser.analyze_function tracks only GPR
    # virtuals), so building descriptors for a non-GPR class would silently
    # pollute lookups with GPR data sharing the same ig number. Guard it until
    # class-aware extraction exists (carry-forward #2). fn_80247510 has a real
    # class-1 (FPR) section, so the guard must fire there.
    if not _has("fn_80247510"):
        pytest.skip("fixture missing")
    c = rd.Compile.from_text((FIX / "fn_80247510_pcdump.txt").read_text(),
                             "fn_80247510", source="")
    with pytest.raises(NotImplementedError):
        rd.build_descriptors(c, class_id=1)
