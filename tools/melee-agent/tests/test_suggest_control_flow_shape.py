...
""",
    ...
    assert suggestion["source_materialization"]["family_id"] == (
        "loop-init/loop-peel-unroll"
    )
    assert suggestion["source_materialization"]["terminal_proof"][
        "terminal_blocker"
    ] == "simple-counted-loop-not-found"
    assert "no safe loop-init" in suggestion["non_materializable_reason"]


def test_annotate_source_materialization_attaches_terminal_proof_per_family() -> None:
    report = {
        "function": "fn_80000000",
        "suggestions": [
            {
                ...
                "follow_up_commands": ["melee-agent debug mutate control-flow-shape-search"],
            },
            {
...
""",
    ...
    for suggestion in report["suggestions"]:
        materialization = suggestion["source_materialization"]
        assert materialization["status"] == "terminal"
        assert materialization["terminal_proof"]["terminal_blocker"]
        assert suggestion["follow_up_commands"]


def test_annotate_source_materialization_keeps_materializable_followup() -> None:
    report = {
        "function": "fn_80000000",
        "suggestions": [
            {
    ...
    materialization = report["suggestions"][0]["source_materialization"]
    assert materialization["status"] == "materializable"
    assert materialization["example_probe_label"]
    assert materialization["family_id"] == (
        "pointer-walk-loop/pointer-walk-indexed-shape"
    )
    assert report["suggestions"][0]["follow_up_commands"]


def test_annotate_source_materialization_materializes_helper_u8_index_table() -> None:
    report = {
        "function": "fn_80000000",
        "suggestions": [
            {
    ...
    materialization = suggestion["source_materialization"]
    assert materialization["status"] == "materializable"
    assert materialization["probe_count"] > 0
    assert materialization["example_probe_label"].startswith(
        "pointer-walk-index-table-"
    )
    assert "non_materializable_reason" not in suggestion
    assert materialization.get("blocker") != "member-array-anchor-not-found"
...
def test_analyze_without_top_preserves_all_ranked_hints() -> None:
    report = analyze_control_flow_shape(
        function="fn_80000000",
        target_asm=[
            "/* 0000 */ cmpwi r3, 0",
    ...
    assert kinds == sorted(kinds, key=CONTROL_FLOW_SHAPE_HINT_KINDS.index)
    assert [item["rank"] for item in suggestions] == list(
        range(1, len(suggestions) + 1)
    )
    assert all(kind in CONTROL_FLOW_SHAPE_HINT_KINDS for kind in kinds)
