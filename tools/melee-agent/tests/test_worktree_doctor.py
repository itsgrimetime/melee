...
def load_worktree_doctor(script: Path | None = None):
    ...
    # The wrapper imports from the worktree_doctor package. Ensure the
    # real tools/ directory is on sys.path so the package is findable
    # even when the wrapper is symlinked from a test temp directory.
    _real_tools = Path(__file__).resolve().parents[2]
    if str(_real_tools) not in sys.path:
        sys.path.insert(0, str(_real_tools))
...
def test_fix_mode_does_not_reinstall_stale_melee_agent_entrypoint(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(doctor_mod.utils, "run_cmd", fake_run_cmd)
    monkeypatch.setattr(
        doctor_mod,
    ...
    monkeypatch.setattr(doctor_mod.utils, "detect_ghidra_install", lambda: None)
    monkeypatch.setattr(doctor_mod.utils, "collect_melee_agent_entrypoint_warnings", lambda root, executable: [
        doctor_mod.CheckResult(
            "warn",
            "melee-agent imports src.cli from /other/checkout/src/cli/__init__.py, expected local",
            "reinstall with: cd tools/melee-agent && python -m pip install -e .",
...
def test_resolve_melee_agent_module_path_uses_launcher_probe(monkeypatch, tmp_path: Path) -> None:
    ...
    assert doctor_mod.utils.resolve_melee_agent_module_path(fake_agent) == local_cli
    assert len(calls) == 1
...
def test_fix_refreshes_stale_mwcc_debug_overlay_from_master(
    monkeypatch,
    tmp_path: Path,
) -> None:
    doctor_mod = load_worktree_doctor()
    ...
    source = tmp_path / "tools" / "mwcc_debug" / "mwcc_debug.c"
    source.parent.mkdir(parents=True)
    source.write_text("void *memcpy(void *dst, const void *src, __SIZE_TYPE__ n) {}\n", encoding="utf-8")
    subprocess.run(["git", "add", "tools/mwcc_debug/mwcc_debug.c"], cwd=tmp_path, check=True)
    subprocess.run(["git", "commit", "-m", "master mwcc debug"], cwd=tmp_path, check=True, capture_output=True)
    subprocess.run(["git", "switch", "-c", "matcher"], cwd=tmp_path, check=True, capture_output=True)
    source.write_text(
        "void *memcpy(void *dst, const void *src, unsigned long n) {}\n"
        "static int mwcc_debug_output_active(void) { return 1; }\n"
        "static int mwcc_debug_output_active(void) { return 0; }\n",
        encoding="utf-8",
    )
    subprocess.run(["git", "commit", "-am", "stale mwcc debug"], cwd=tmp_path, check=True, capture_output=True)

    monkeypatch.setattr(doctor_mod.utils, "ROOT", tmp_path)
    monkeypatch.setattr(doctor_mod.doctor, "TOOLING_FILES", ["tools/mwcc_debug/mwcc_debug.c"])
...
def test_fix_builds_table_typer_when_missing(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(doctor_mod.utils, "build_table_typer", fake_build)
...
def test_fix_replaces_macos_arm64_dtk_with_rosetta_binary(monkeypatch, tmp_path: Path) -> None:
    ...
    def fake_download(path: Path, **kwargs) -> bool:
        calls.append(path)
        path.write_bytes(b"x86_64 dtk")
        return True

    monkeypatch.setattr(doctor_mod.utils, "redownload_dtk", fake_download)
...
def test_refresh_report_timeout_kills_child_processes(monkeypatch, tmp_path: Path) -> None:
    ...
    try:
        result = doctor_mod.utils.refresh_report_json(tmp_path)
...
def test_table_typer_missing_without_fix_labels_optional(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(doctor_mod.utils, "build_table_typer", must_not_build)
