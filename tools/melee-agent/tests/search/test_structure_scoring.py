...
def test_score_structure_variants_accepts_retained_full_tu_source_path(
    monkeypatch,
    tmp_path: Path,
) -> None:
    melee_root = tmp_path
    ...
    report_path = melee_root / "build" / "GALE01" / "report.json"
    objdiff_path = melee_root / "build" / "tools" / "objdiff-cli"
    retained_path = tmp_path / "diagnostics" / "candidate.c"
    ...
    _write_report(report_path, 12.0)
    objdiff_path.write_text("# fake objdiff\n")
    retained_path.write_text("int fn_80000000(void) { return 7; }\n")

    report_percents = iter([90.0, 93.0])
    compiled_sources: list[bytes] = []

    def fake_run(cmd, **kwargs):
        argv = [str(part) for part in cmd]
        if argv[:2] == ["ninja", "build/GALE01/src/melee/demo.o"]:
            compiled_sources.append(source_path.read_bytes())
            obj_path.write_bytes(b"candidate object")
            return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
        if "objdiff-cli" in argv[0]:
            _write_report(report_path, next(report_percents))
    ...
    assert results[0].compile_status == "ok"
    assert results[0].candidate_percent == 93.0
    assert compiled_sources[0] == b"int fn_80000000(void) { return 0; }\n"
    assert compiled_sources[1] == b"int fn_80000000(void) { return 7; }\n"
    assert source_path.read_text() == "int fn_80000000(void) { return 0; }\n"


def test_score_structure_variants_accepts_retained_seed_distinct_from_candidate(
    monkeypatch,
    tmp_path: Path,
) -> None:
    melee_root = tmp_path
    ...
    report_path = melee_root / "build" / "GALE01" / "report.json"
    objdiff_path = melee_root / "build" / "tools" / "objdiff-cli"
    seed_path = tmp_path / "diagnostics" / "seed.c"
    generated_path = tmp_path / "diagnostics" / "generated.c"
    ...
    _write_report(report_path, 12.0)
    objdiff_path.write_text("# fake objdiff\n")
    seed_path.write_text("int fn_80000000(void) { return 11; }\n")
    generated_path.write_text("int fn_80000000(void) { return 7; }\n")
    ...
    results = score_structure_variants(
        ...
        variants=[
            StructureVariant(
                ...
                operator="call-temp",
                status="candidate",
                source_retained=str(generated_path),
            )
        ],
        timeout=3,
    )
...
def test_score_structure_variants_rejects_wrong_in_tree_source_path(
    tmp_path: Path,
) -> None:
    melee_root = tmp_path
    source_path = melee_root / "src" / "melee" / "demo.c"
    report_path = melee_root / "build" / "GALE01" / "report.json"
    unrelated_path = melee_root / "src" / "melee" / "unrelated.c"
    retained_path = tmp_path / "diagnostics" / "candidate.c"
    ...
    retained_path.parent.mkdir(parents=True)
    source_path.write_text("int fn_80000000(void) { return 0; }\n")
    unrelated_path.write_text("int fn_80000000(void) { return 1; }\n")
    retained_path.write_text("int fn_80000000(void) { return 2; }\n")
    _write_report(report_path, 12.0)
    ...
    try:
        ...
        raise AssertionError("wrong in-tree source path should be rejected")


def test_structure_context_rejects_retained_prototype_without_definition(
    tmp_path: Path,
) -> None:
    melee_root = tmp_path
    source_path = melee_root / "src" / "melee" / "demo.c"
    report_path = melee_root / "build" / "GALE01" / "report.json"
    retained_path = tmp_path / "diagnostics" / "prototype.c"

    source_path.parent.mkdir(parents=True)
    report_path.parent.mkdir(parents=True, exist_ok=True)
    retained_path.parent.mkdir(parents=True)
    source_path.write_text("int fn_80000000(void) { return 0; }\n")
    retained_path.write_text("int fn_80000000(void);\n")
    _write_report(report_path, 12.0)

    try:
        scoring_mod.resolve_structure_score_context(
            melee_root,
            "fn_80000000",
            retained_path,
            retained_source_paths={retained_path.resolve()},
        )
    except ValueError as exc:
        assert "source mismatch" in str(exc)
    else:
        raise AssertionError("prototype-only retained source should be rejected")


def test_score_structure_variants_rejects_retained_source_without_function(
    tmp_path: Path,
) -> None:
    melee_root = tmp_path
    source_path = melee_root / "src" / "melee" / "demo.c"
    ...
    retained_path.parent.mkdir(parents=True)
    source_path.write_text("int fn_80000000(void) { return 0; }\n")
    retained_path.write_text("int other_function(void) { return 7; }\n")
    _write_report(report_path, 12.0)
    ...
    try:
        ...
        raise AssertionError("retained source missing function should be rejected")
