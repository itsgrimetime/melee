...
def test_source_lifetime_axis_accepts_pure_helper_overrides(
    tmp_path: Path,
) -> None:
    source_path = tmp_path / "demo.c"
    source_path.write_text(
    ...
    retained = Path(variant["source_retained"]).read_text()
    assert "u8 ll_probe_helper_result_0 = ExternalPure(slot);" in retained
    families = payload["axes"][0]["metadata"]["families"]
    reuse = next(
        row for row in families
        if row["operator"] == "repeated-helper-result-reuse"
    )
    assert reuse["status"] == "generated"
...
def test_loop_shape_expanded_covers_actual_mndiagram_visible_scans(
    ...
    for function in ("mnDiagram_DrawNameHeaders", "mnDiagram_DrawGridValues"):
        axis, variants = generate_loop_shape_expanded_variants(
            source,
            function,
            tmp_path / function,
    ...
    variants_417d0 = generate_loop_shape_expanded_variants(
        source,
        "mnDiagram_UpdateScrollArrows",
        tmp_path / "mnDiagram_UpdateScrollArrows-snippets",
        baseline_percent=None,
        max_candidates=32,
    )[1]
    assert any(
