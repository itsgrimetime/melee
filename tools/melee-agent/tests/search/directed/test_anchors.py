...
def test_source_shape_anchors_discover_explicit_zero_return_wrapper():
    src = "{\n    un_802FFD94(arg0, &un_803FA8E8, fn_802FFE6C);\n}\n"

    anchors = [
        anchor
        for anchor in iter_source_shape_anchors(src)
        if anchor.mutator_key == "add_explicit_zero_return"
    ]

    assert len(anchors) == 1
    assert anchors[0].payload == {
        "call_line": "    un_802FFD94(arg0, &un_803FA8E8, fn_802FFE6C);",
        "return_line": "    return 0;",
    }


def test_explicit_zero_return_anchor_requires_single_call_without_return():
    already_returns = "{\n    un_802FFD94(arg0);\n    return 0;\n}\n"
    multiple_statements = "{\n    un_802FFD94(arg0);\n    sink(arg0);\n}\n"
    assignment_wrapper = "{\n    result = un_802FFD94(arg0);\n}\n"

    for src in (already_returns, multiple_statements, assignment_wrapper):
        assert not any(
            anchor.mutator_key == "add_explicit_zero_return"
            for anchor in iter_source_shape_anchors(src)
        )
