...
def test_steer_pcode_only_fpr_callarg_temp_validates_exact_span() -> None:
    src = (
        "void f(void) {\n"
        "    base = (f32) digit;\n"
    ...
    anchor = Anchor(
        ...
        {
            "span_text": span_text,
            "replacement_text": "    HSD_JObjReqAnimAll(jobj, (f32) digit);",
        },
    )

    assert apply_mutator("steer_pcode_only_fpr_callarg_temp", anchor, src) == (
        "void f(void) {\n"
    ...
    stale = Anchor(
        ...
        (start, start + len(span_text)),
        {
            "span_text": "    missing = (f32) digit;",
            "replacement_text": "    HSD_JObjReqAnimAll(jobj, (f32) digit);",
        },
    )
    assert apply_mutator("steer_pcode_only_fpr_callarg_temp", stale, src) is None


def test_steer_callarg_local_preserving_structural_repair_validates_exact_span() -> None:
    src = (
        "void f(void) {\n"
        "    rowf = (f32) digit;\n"
        "    HSD_JObjReqAnimAll(jobj, (f32) digit);\n"
    ...
    span_text = "    HSD_JObjReqAnimAll(jobj, (f32) digit);"
    replacement = "    HSD_JObjReqAnimAll(jobj, rowf);"
    start = src.index(span_text)
    anchor = Anchor(
    ...
    assert apply_mutator(
        ...
        anchor,
        src,
    ) == src.replace(span_text, replacement, 1)

    stale = Anchor(
        "steer_callarg_local_preserving_structural_repair",
        (start, start + len(span_text)),
        {
            "span_text": "    HSD_JObjReqAnimAll(jobj, missing);",
            "replacement_text": replacement,
        },
    )
    assert (
