...
def test_directed_empty_source_does_not_advance_parent_state(tmp_path):
    store = ArtifactStore(tmp_path / "store")
    empty_source = FakeSource([])
    active_source = FakeSource([SourceVariant("int f(){return 1;}", None)])
    art = _make_art("active", byte_score=5, tmp=tmp_path)
    backend = FakePcdumpBackend([art])
    pipeline = FakeScorePipeline(
        always_escalate=True,
        metas=[_make_meta(True, 4.0, "root")],
    )
    ...
    sched.run(
        sources=[empty_source, active_source],
        backends=[backend],
        producers=[],
        pipeline=pipeline,
        target=TargetSpec("f", "u", tmp_path / "e.o"),
    ...
    assert empty_source.observed_batches == [0]
    assert active_source.observed_batches == [1]
    assert pipeline.score_directed_calls[0].parent_state.state_id == "root"
