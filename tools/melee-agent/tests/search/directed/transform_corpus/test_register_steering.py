...
def test_generate_transform_probes_preserves_one_shot_alias_iterables() -> None:
    source = (
    ...
    probes = generate_transform_probes(
        ...
        force_phys={32: 28},
        function_aliases=(name for name in ("mnDiagram_80241E78",)),
        max_per_family=2,
    )
    ...
    assert report.source_resolution["source_function"] == "mnDiagram_80241E78"
...
def test_global_load_lifetime_repair_materializes_pointer_global_probes() -> None:
    source = (
        "typedef struct HSD_GObj HSD_GObj;\n"
        "typedef struct Data Data;\n"
        "struct HSD_GObj { void* user_data; };\n"
        "extern HSD_GObj* gDiagram;\n"
        "void fn(HSD_GObj* gobj) {\n"
        "    Data* data;\n"
        "    int buttons;\n"
        "    HSD_GObj* retained;\n"
        "    PAD_STACK(8);\n"
        "    data = gDiagram->user_data;\n"
        "    buttons = poll_buttons(4);\n"
        "    if (buttons != 0) {\n"
        "        update(gDiagram, data);\n"
        "    }\n"
        "    retained = gDiagram;\n"
    ...
    probes = generate_transform_probes(
        ...
        families=("pcode_only_gpr_global_load_lifetime_repair",),
        max_per_family=6,
    )
    ...
    global_load = [
        probe
        for probe in probes
        if probe.family_id == "pcode_only_gpr_global_load_lifetime_repair"
    ]
    assert global_load
    assert all(probe.target_assignments == ("ig96->r6", "ig97->r4") for probe in global_load)
    assert any(
        "HSD_GObj* global_load_gDiagram_0;" in probe.candidate_text
        for probe in global_load
    )
    assert any(
        "    HSD_GObj* retained;\n"
        "    HSD_GObj* global_load_gDiagram_0;\n"
        "    PAD_STACK(8);\n"
        "    global_load_gDiagram_0 = gDiagram;\n"
        "    data = global_load_gDiagram_0->user_data;\n" in probe.candidate_text
        for probe in global_load
    )
    assert any(
        "data = global_load_gDiagram_0->user_data;" in probe.candidate_text
        for probe in global_load
    )
    assert any(
        "retained = global_load_gDiagram_0;" in probe.candidate_text
        for probe in global_load
    )
    assert all(probe.payload["source_hunks"] for probe in global_load)
    assert {
        "alias-first-member-use",
        "alias-first-and-call-uses",
        "hoist-existing-alias-init",
    } <= {probe.payload["strategy"] for probe in global_load}
    ...
    diag = next(
        row
        for row in report.family_diagnostics
        if row.family_id == "pcode_only_gpr_global_load_lifetime_repair"
    )
    assert diag.materialized_count == len(global_load)
    assert diag.matcher_diagnostics["accepted_anchor_count"] == len(global_load)
    assert diag.no_probe_reason is None


def test_global_load_lifetime_repair_rejects_unsafe_bare_global_uses() -> None:
    source = (
        "typedef struct HSD_GObj HSD_GObj;\n"
        "typedef struct Data Data;\n"
        "struct HSD_GObj { void* user_data; };\n"
        "extern HSD_GObj* gDiagram;\n"
        "void fn(HSD_GObj* next) {\n"
        "    Data* data;\n"
        "    HSD_GObj* retained;\n"
        "    data = gDiagram->user_data;\n"
        "    gDiagram = next;\n"
    ...
    assert global_load
    assert all("global_load_gDiagram_0 = next;" not in probe.candidate_text for probe in global_load)
    assert all("&global_load_gDiagram_0" not in probe.candidate_text for probe in global_load)
    assert any("retained = global_load_gDiagram_0;" in probe.candidate_text for probe in global_load)
    assert any("update(global_load_gDiagram_0, data);" in probe.candidate_text for probe in global_load)
...
def test_pcode_only_fpr_callarg_temp_repair_materializes_existing_local_shapes() -> None:
    source = (
        "typedef unsigned char u8;\n"
        "typedef float f32;\n"
        "void mnDiagram_DrawCellNumber(HSD_JObj* jobj, u8 digit) {\n"
        "    f32 y_spacing;\n"
        "    f32 base;\n"
        "    f32 rowf;\n"
        "    f32 col_cast_owner_fpr;\n"
        "    col_cast_owner_fpr = (f32) digit;\n"
        "    use(y_spacing, rowf, col_cast_owner_fpr);\n"
        "    base = (f32) digit;\n"
    ...
    probes = generate_transform_probes(
        ...
        function="mnDiagram_DrawCellNumber",
        unit="melee/mn/mndiagram",
        force_phys={33: 28, 34: 26},
        families=("pcode_only_fpr_callarg_temp_repair",),
        max_per_family=16,
    )

    callarg = [
        probe for probe in probes
        if probe.family_id == "pcode_only_fpr_callarg_temp_repair"
    ]
    strategies = {probe.payload["strategy"] for probe in callarg}

    assert strategies >= {
        "fpr-callarg-dematerialize-existing-local",
        "fpr-callarg-assignment-expression",
        "fpr-callarg-duplicate-direct-cast-preserve-local",
        "fpr-callarg-reuse-dead-fpr-local",
        "fpr-callarg-block-local-assign-temp",
        "fpr-callarg-block-local-init-temp",
    }
    assert all(
        probe.mutator_key == "steer_pcode_only_fpr_callarg_temp"
        for probe in callarg
    )
    assert all("target_virtuals_hint" not in probe.payload for probe in callarg)
    assert any(
        "HSD_JObjReqAnimAll(jobj, (f32) digit);" in probe.candidate_text
        for probe in callarg
    )
    assert any(
        "HSD_JObjReqAnimAll(jobj, (base = (f32) digit));" in probe.candidate_text
        for probe in callarg
    )
    assert any(
        "col_cast_owner_fpr = (f32) digit;\n"
        "    HSD_JObjReqAnimAll(jobj, col_cast_owner_fpr);"
        in probe.candidate_text
        for probe in callarg
    )


def test_pcode_only_fpr_callarg_temp_repair_recovers_nonadjacent_assignment() -> None:
    source = (
        "typedef unsigned char u8;\n"
        "typedef float f32;\n"
    ...
    assert {probe.payload.get("call_arg_local") for probe in callarg} == {"base"}
    assert {probe.payload["call_arg_operand"] for probe in callarg} == {"digit"}
    assert any(
        "    jobj = HSD_JObjLoadJoint(joint);\n"
        "    HSD_JObjAddAnimAll(jobj, a, b, c);\n"
...
def test_pcode_only_fpr_callarg_temp_repair_rejects_unsafe_intervening_regions(
    intervening: str,
) -> None:
    source = (
        "typedef unsigned char u8;\n"
    ...
    assert not [
        probe for probe in probes
        if probe.mutator_key == "steer_pcode_only_fpr_callarg_temp"
    ]


def test_pcode_only_fpr_fsubs_cast_owner_repair_materializes_source_spans() -> None:
    source = (
        "typedef float f32;\n"
    ...
    probes = generate_transform_probes(
        ...
        function="mnDiagram_DrawCellNumber",
        unit="melee/mn/mndiagram",
        force_phys={33: 26, 34: 26},
        families=("pcode_only_fpr_fsubs_cast_owner_repair",),
        max_per_family=8,
    )

    repair = [
        probe for probe in probes
        if probe.family_id == "pcode_only_fpr_fsubs_cast_owner_repair"
    ]

    assert {probe.mutator_key for probe in repair} == {
        "steer_pcode_only_fpr_fsubs_cast_owner"
    }
    assert {probe.payload["owner_local"] for probe in repair} >= {
        "col_cast_owner_fpr",
        "row_offset_adj",
    }
    assert any(
        "f32 col_cast_owner_fpr_owner_fpr;\n"
        "    col_cast_owner_fpr_owner_fpr = (f32) col;\n"
        "    col_cast_owner_fpr = col_cast_owner_fpr_owner_fpr;"
        in probe.candidate_text
        for probe in repair
    )
    assert any(
        "f32 row_offset_adj_owner_fpr;" in probe.candidate_text
        and "    col_cast_owner_fpr = (f32) col;\n"
...
def test_pcode_only_gpr_address_temp_repair_materializes_source_spans() -> None:
    source = (
        "typedef unsigned char u8;\n"
    ...
    probes = generate_transform_probes(
        ...
        families=("pcode_only_gpr_address_temp_repair",),
        max_per_family=16,
    )

    repair = [
        probe for probe in probes
        if probe.family_id == "pcode_only_gpr_address_temp_repair"
    ]
    strategies = {probe.payload["strategy"] for probe in repair}

    assert strategies >= {
        "gpr-indexed-address-temp",
        "gpr-indexed-base-alias-address-temp",
        "gpr-pointer-copy-owner-split",
        "gpr-pointer-copy-owner-address-temp",
    }
    assert {probe.mutator_key for probe in repair} == {
        "steer_pcode_only_gpr_address_temp"
    }
    assert all(
        probe.target_assignments == ("ig34->r27", "ig44->r25")
        for probe in repair
    )
    assert any(
        "u8* sorted_names_totals_idx_probe_addr_gpr;\n" in probe.candidate_text
        and "sorted_names_totals_idx_probe_addr_gpr = "
        "&mnDiagram_804A076C.sorted_names[(max_idx)];\n"
        "    sorted_names_totals_idx_probe = *sorted_names_totals_idx_probe_addr_gpr;"
        in probe.candidate_text
        for probe in repair
    )
    assert any(
        "u8* sorted_names_base_probe_addr_gpr;\n" in probe.candidate_text
        and "sorted_names_base_probe_addr_gpr = "
        "&sorted_names_base_probe[window_order_mnDiagram_804A076C_sorted_names_index_probe];\n"
        "    sorted_names_base_probe_probe = *sorted_names_base_probe_addr_gpr;"
        in probe.candidate_text
        for probe in repair
    )
    assert any(
        "u8* sorted_names_base_probe_copy_gpr;\n" in probe.candidate_text
        and "sorted_names_base_probe_copy_gpr = mnDiagram_804A076C.sorted_names;\n"
        "    sorted_names_base_probe = sorted_names_base_probe_copy_gpr;"
        in probe.candidate_text
        for probe in repair
    )


def test_pcode_only_gpr_bool_mask_temp_repair_materializes_predicates_and_wrappers(
) -> None:
    source = (
        "typedef unsigned int u32;\n"
        "typedef unsigned long long u64;\n"
        "typedef float f32;\n"
        "typedef struct Vec { f32 x; } Vec;\n"
        "typedef struct HSD_JObj { u32 flags; Vec translate; } HSD_JObj;\n"
        "#define JOBJ_MTX_INDEP_SRT 0x2000\n"
        "void HSD_JObjSetMtxDirty(HSD_JObj* jobj);\n"
        "void HSD_JObjSetTranslateX(HSD_JObj* jobj, f32 x);\n"
        "static inline void Demo_JObjSetTranslateX(HSD_JObj* jobj, f32 x) {\n"
        "    jobj->translate.x = x;\n"
        "    if (!(jobj->flags & JOBJ_MTX_INDEP_SRT)) {\n"
        "        HSD_JObjSetMtxDirty(jobj);\n"
        "    }\n"
        "}\n"
        "void Demo_Update(HSD_JObj* cursor_jobj, u64 inputs_repeat, f32 temp_x) {\n"
        "    if ((inputs_repeat & 0x10) && ready()) {\n"
        "        use_mask(inputs_repeat);\n"
        "    }\n"
        "    if (!(cursor_jobj->flags & JOBJ_MTX_INDEP_SRT)) {\n"
        "        HSD_JObjSetMtxDirty(cursor_jobj);\n"
        "    }\n"
    ...
    probes = generate_transform_probes(
        ...
        force_phys={112: 5, 174: 4},
        families=("pcode_only_gpr_bool_mask_temp_repair",),
        max_per_family=8,
    )

    repair = [
        probe for probe in probes
        if probe.family_id == "pcode_only_gpr_bool_mask_temp_repair"
    ]
    strategies = {probe.payload["strategy"] for probe in repair}
    ...
    assert {probe.mutator_key for probe in repair} == {
        "steer_pcode_only_gpr_bool_mask_temp"
    }
    assert all(probe.payload.get("source_hunks") for probe in repair)
    assert any(
        "u64 inputs_repeat_0x10_mask_gpr;\n"
        "    inputs_repeat_0x10_mask_gpr = inputs_repeat & 0x10;\n"
        "    if (inputs_repeat_0x10_mask_gpr && ready()) {"
        in probe.candidate_text
        for probe in repair
    )
    assert any(
        "u32 cursor_jobj_flags_JOBJ_MTX_INDEP_SRT_mask_gpr;\n"
        in probe.candidate_text
        and "    cursor_jobj_flags_JOBJ_MTX_INDEP_SRT_mask_gpr = "
        "cursor_jobj->flags & JOBJ_MTX_INDEP_SRT;\n"
        "    if (!cursor_jobj_flags_JOBJ_MTX_INDEP_SRT_mask_gpr) {"
        in probe.candidate_text
        for probe in repair
    )
    assert any(
        "Demo_JObjSetTranslateX(cursor_jobj, temp_x);" in probe.candidate_text
        for probe in repair
    )


def test_pcode_only_gpr_bool_mask_temp_repair_requires_dirty_wrapper_body(
) -> None:
    source = (
        "typedef unsigned int u32;\n"
        "typedef float f32;\n"
        ...
        "    jobj->translate.x = x;\n"
        "}\n"
        "void Demo_Update(HSD_JObj* cursor_jobj, f32 temp_x) {\n"
        "    HSD_JObjSetTranslateX(cursor_jobj, temp_x);\n"
        "}\n"
    )
    ...
    diagnostic = next(
        ...
        if item.family_id == "pcode_only_gpr_bool_mask_temp_repair"
    )

    assert repair == []
    assert diagnostic.no_probe_reason == "dirty-wrapper-unavailable"
    assert "dirty-wrapper-unavailable" in diagnostic.matcher_diagnostics[
        "zero_probe_reasons"
    ]


def test_pcode_only_gpr_bool_mask_temp_repair_rejects_ambiguous_duplicate_masks(
) -> None:
    source = (
        "typedef unsigned long long u64;\n"
        "void Demo_Update(u64 inputs) {\n"
        "    if (inputs & 1) {\n"
        "        use_first(inputs);\n"
        "    }\n"
        "    if (inputs & 1) {\n"
    ...
    assert repair == []
    assert diagnostic.no_probe_reason == "ambiguous-source-region"
    assert "ambiguous-source-region" in diagnostic.matcher_diagnostics[
        "rejection_reasons"
    ]
    assert diagnostic.matcher_diagnostics["duplicate_mask_expressions"] == [
        "inputs&1"
    ]


def test_pcode_only_gpr_bool_mask_temp_repair_materializes_else_if_predicate(
) -> None:
    source = (
        "typedef unsigned long long u64;\n"
        "void Demo_Update(u64 inputs) {\n"
    ...
    assert any(
        "    } else {\n"
        "        u64 inputs_0x80_mask_gpr;\n"
        "        inputs_0x80_mask_gpr = inputs & 0x80;\n"
        "        if (inputs_0x80_mask_gpr) {\n"
        "            use_mask(inputs);\n"
        "        }\n"
        "    }"
        in probe.candidate_text
...
def test_pcode_only_gpr_bool_mask_temp_repair_materializes_shifted_ll_masks(
) -> None:
    source = (
        "typedef unsigned long long u64;\n"
        "void Demo_Update(u64 inputs) {\n"
    ...
    assert len(repair) == 2
    assert any(
        "    u64 inputs_1LL_32_mask_gpr;\n"
        "    inputs_1LL_32_mask_gpr = inputs & (1LL << 32);\n"
        "    if (inputs_1LL_32_mask_gpr) {"
        in probe.candidate_text
        for probe in repair
    )
    assert any(
        "    u64 inputs_0x40LL_32_mask_gpr;\n"
        in probe.candidate_text
        and "    inputs_0x40LL_32_mask_gpr = inputs & (0x40LL << 32);\n"
        "    if (inputs_0x40LL_32_mask_gpr && ready()) {"
        in probe.candidate_text
        for probe in repair
    )


def test_pcode_only_gpr_copy_product_case_c_materializes_sort_owner_spans() -> None:
    source = (
        "typedef unsigned char u8;\n"
        "typedef unsigned int u32;\n"
    ...
    probes = generate_transform_probes(
        ...
        unit="melee/mn/mndiagram",
        force_phys={34: 27, 44: 25},
        families=("pcode_only_gpr_copy_product_case_c_repair",),
        max_per_family=16,
    )

    repair = [
        probe for probe in probes
        if probe.family_id == "pcode_only_gpr_copy_product_case_c_repair"
    ]
    strategies = {probe.payload["strategy"] for probe in repair}
    ...
    assert any(
        "u8* ll_probe_iter_0_case_c_owner_gpr;\n" in probe.candidate_text
        and "ll_probe_iter_0_case_c_owner_gpr = ll_probe_iter_0;\n"
        "        for (i = 0; i < 0x78; i++, ll_probe_iter_0_case_c_owner_gpr++)"
        in probe.candidate_text
        and "*ll_probe_iter_0_case_c_owner_gpr = temp;" in probe.candidate_text
        for probe in repair
    )
    assert any(
        "u8* ll_probe_iter_0_store_owner_gpr;\n" in probe.candidate_text
        and "ll_probe_iter_0_store_owner_gpr = ll_probe_iter_0;\n"
        "                *ll_probe_iter_0_store_owner_gpr = temp;"
        in probe.candidate_text
        for probe in repair
    )
    assert all(
        "&mnDiagram_804A076C.sorted_names[j]" not in probe.candidate_text
        for probe in repair
    )
    assert all(probe.payload.get("source_hunks") for probe in repair)
    assert {
        probe.payload.get("case_c_target_local") for probe in repair
    } == {"dst_iter"}


def test_retained_gpr_case_c_sensitivity_materializes_sort_cursor_spans() -> None:
    source = (
        "typedef unsigned char u8;\n"
        "typedef unsigned int u32;\n"
        "struct Assets { u8 sorted_names[0x78]; };\n"
    ...
    probes = generate_transform_probes(
        ...
        unit="melee/mn/mndiagram",
        force_phys={34: 27, 44: 25},
        families=("retained_gpr_case_c_sensitivity_search",),
        max_per_family=16,
    )

    sensitivity = [
        probe for probe in probes
        if probe.family_id == "retained_gpr_case_c_sensitivity_search"
    ]
    strategies = {probe.payload["strategy"] for probe in sensitivity}
    ...
    assert {probe.mutator_key for probe in sensitivity} == {
        "steer_retained_gpr_case_c_sensitivity"
    }
    assert all(
        probe.target_assignments == ("ig34->r27", "ig44->r25")
        for probe in sensitivity
    )
    assert all(
        probe.payload.get("case_c_target_local") == "dst_iter"
        for probe in sensitivity
    )
    assert all(
    ...
    assert all(probe.payload.get("source_hunks") for probe in sensitivity)
    assert any(
        "dst_iter = ll_probe_iter_0;\n"
        "                *dst_iter = temp;" in probe.candidate_text
        for probe in sensitivity
    )
    assert any(
        "dst_iter = ll_probe_iter_0_source_34_0;\n"
        "        for (i = 0; i < 0x78; i++, dst_iter++)" in probe.candidate_text
        and "*dst_iter = temp;" in probe.candidate_text
        for probe in sensitivity
    )
    assert any(
        "ll_probe_iter_0 = dst_iter;" in probe.candidate_text
        for probe in sensitivity
    )


def _mixed_pcode_fpr_lifetime_source(
    *,
    between_owner_and_loop: str = "",
    preprocessor: str = "",
    address_taken: str = "",
    paired_translate: bool = True,
) -> str:
    adjusted_call = (
        "            HSD_JObjSetTranslateY(jobj, row_offset_adj);\n"
        if paired_translate
        else "            sink(row_offset_adj);\n"
    )
    return (
        "typedef unsigned char u8;\n"
        "typedef float f32;\n"
...
def test_mixed_pcode_fpr_lifetime_pressure_materializes_row_adj_and_callarg_variants() -> None:
    probes = generate_transform_probes(
    ...
    strategies = {probe.payload["strategy"] for probe in mixed}

    assert strategies >= {
        "mixed-row-adj-direct-translate",
        "mixed-row-adj-call-temp-split",
        "mixed-digit-callarg-fresh-local",
        "mixed-composed-row-adj-plus-digit-callarg",
    }
    assert {probe.mutator_key for probe in mixed} == {
        "steer_mixed_pcode_fpr_lifetime_pressure"
    }
    source = _mixed_pcode_fpr_lifetime_source()
    assert apply_mutator(
        mixed[0].mutator_key,
        Anchor(
            mutator_key=mixed[0].mutator_key,
            span=mixed[0].span,
            payload=mixed[0].payload,
        ),
        source,
    ) == mixed[0].candidate_text
    for probe in mixed:
        assert probe.payload["row_local"] == "row_offset"
        assert probe.payload["row_adj_local"] == "row_offset_adj"
        assert probe.payload["row_adj_owner_local"] == "row_offset_adj_owner_fpr"
        assert probe.payload["call_arg_local"] == "rowf"
        assert probe.payload["source_regions"]
    assert any(
        "HSD_JObjSetTranslateY(jobj, row_offset_adj_owner_fpr);" in probe.candidate_text
        for probe in mixed
    )
    assert any(
        "f32 row_offset_adj_call_fpr;" in probe.candidate_text
        and "HSD_JObjSetTranslateY(jobj, row_offset_adj_call_fpr);" in probe.candidate_text
        for probe in mixed
    )
    assert any(
        "f32 digit_call_fpr;" in probe.candidate_text
        and "HSD_JObjReqAnimAll(jobj, digit_call_fpr);" in probe.candidate_text
        for probe in mixed
    )
    assert any(
        "f32 row_offset_adj_call_fpr;" in probe.candidate_text
        and "f32 digit_call_fpr;" in probe.candidate_text
        for probe in mixed
    )
...
@pytest.mark.parametrize(
    "source",
    (
        _mixed_pcode_fpr_lifetime_source(preprocessor="#if 1\n#endif\n"),
        _mixed_pcode_fpr_lifetime_source(
            between_owner_and_loop="    row_offset = y_offset;\n"
        ),
        _mixed_pcode_fpr_lifetime_source(address_taken="    sink(&rowf);\n"),
        _mixed_pcode_fpr_lifetime_source(paired_translate=False),
    ),
)
def test_mixed_pcode_fpr_lifetime_pressure_rejects_unsafe_shapes(source: str) -> None:
    probes = generate_transform_probes(
        source,
        function="mnDiagram_DrawCellNumber",
        unit="melee/mn/mndiagram",
...
def _callarg_local_structural_source(
    *,
    call_line: str = "        HSD_JObjReqAnimAll(jobj, (f32) digit);\n",
    preprocessor: str = "",
    address_taken: str = "",
    extra_call: str = "",
    extra_decl: str = "",
...
def _callarg_fresh_existing_structural_source(
    *,
    address_taken: str = "",
    duplicate_assignment: str = "",
    between_assignment_and_call: str = "",
    extra_call: str = "",
    extra_decl: str = "",
    preprocessor: str = "",
    after_loop: str = "",
    ...
    return (
        ...
        "    for (i = 0; i < digit_count; i++) {\n"
        "        digit = mn_GetDigitAt(value, i);\n"
        f"{duplicate_assignment}"
        "        digitf = (f32) digit;\n"
        f"{between_assignment_and_call}"
...
def test_callarg_local_structural_repair_materializes_required_source_shapes() -> None:
    source = _callarg_local_structural_source()
    ...
    assert {probe.mutator_key for probe in callarg} == {
        "steer_callarg_local_preserving_structural_repair"
    }
    assert all("score_source" not in probe.payload for probe in callarg)
    assert any(
        "rowf = (f32) digit;\n"
        "        HSD_JObjReqAnimAll(jobj, rowf);"
        in probe.candidate_text
        ...
        for probe in callarg
        if probe.payload["strategy"] == "retain-existing-rowf-callarg-h004-order"
    )
    assert any(
        "f32 digit_frame_fpr;" in probe.candidate_text
        and "digit_frame_fpr = (f32) digit;" in probe.candidate_text
        and "HSD_JObjReqAnimAll(jobj, digit_frame_fpr);" in probe.candidate_text
        for probe in callarg
        if probe.payload["strategy"] == "fresh-loop-callarg-local"
    )
    assert any(
        "{\n"
        "            f32 digit_frame_fpr;\n"
        "            digit_frame_fpr = (f32) digit;\n"
        "            HSD_JObjReqAnimAll(jobj, digit_frame_fpr);\n"
        "        }" in probe.candidate_text
        and "HSD_JObjSetTranslateY(jobj, row_offset_adj);" in probe.candidate_text
        for probe in callarg
        if probe.payload["strategy"] == "block-scoped-callarg-local"
    )
    assert all(probe.candidate_text.count("row_offset *= rowf;") == 1 for probe in callarg)


def test_callarg_local_structural_repair_continues_from_existing_fresh_local() -> None:
    source = _callarg_fresh_existing_structural_source()
    ...
    assert callarg
    assert len(callarg) > 4
    default = next(
        probe for probe in callarg
        if probe.payload["strategy"] == "continue-existing-fresh-callarg-local"
    )
    assert (
        "digitf = (f32) digit;\n"
        "        HSD_JObjReqAnimAll(jobj, digitf);"
    ) in default.candidate_text
    assert "digit_frame_fpr" not in default.candidate_text
    assert default.payload["preserves_existing_callarg_local"] is True
    assert default.payload["uses_fresh_local"] is True


def test_callarg_local_structural_repair_materializes_fresh_existing_call_schedules() -> None:
    source = _callarg_fresh_existing_structural_source(
        between_assignment_and_call=(
            "        jobj = HSD_JObjLoadJoint(joint_data->joint);\n"
            "        HSD_JObjAddAnimAll(jobj, joint_data->anim_joint,\n"
            "                           joint_data->mat_anim);\n"
        )
    )
    ...
    by_strategy = {probe.payload["strategy"]: probe for probe in callarg}
    ...
    assert "fresh-local-call-schedule-before-load" not in by_strategy
    assert len(callarg) > 4

    before_load = by_strategy["fresh-local-decl-demote-to-loop"]
    assert before_load.payload["digit_assignment_schedule"] == "before-load"
    assert (
        "for (i = 0; i < digit_count; i++) {\n"
        "        f32 digitf;\n"
        "        digit = mn_GetDigitAt(value, i);\n"
        "        digitf = (f32) digit;\n"
        "        jobj = HSD_JObjLoadJoint(joint_data->joint);"
    ) in before_load.candidate_text

    after_load = by_strategy["fresh-local-call-schedule-after-load"]
    assert after_load.payload["digit_assignment_schedule"] == "after-load"
    assert (
        "for (i = 0; i < digit_count; i++) {\n"
        "        f32 digitf;\n"
        "        digit = mn_GetDigitAt(value, i);\n"
        ...
        "        digitf = (f32) digit;\n"
        "        HSD_JObjAddAnimAll(jobj, joint_data->anim_joint,"
    ) in after_load.candidate_text

    after_add = by_strategy["fresh-local-call-schedule-after-add"]
    assert after_add.payload["digit_assignment_schedule"] == "after-add"
    assert (
        "HSD_JObjAddAnimAll(jobj, joint_data->anim_joint,\n"
        "                           joint_data->mat_anim);\n"
        "        digitf = (f32) digit;\n"
        "        HSD_JObjReqAnimAll(jobj, digitf);"
    ) in after_add.candidate_text

    for scheduled in (before_load, after_load, after_add):
        assert scheduled.candidate_text.count("HSD_JObjLoadJoint(") == 1
        assert scheduled.candidate_text.count("HSD_JObjAddAnimAll(") == 1
        assert scheduled.candidate_text.count("HSD_JObjReqAnimAll(") == 1


def test_callarg_local_structural_repair_materializes_fresh_existing_handoffs() -> None:
    source = _callarg_fresh_existing_structural_source(
        between_assignment_and_call=(
            "        jobj = HSD_JObjLoadJoint(joint_data->joint);\n"
            "        HSD_JObjAddAnimAll(jobj, joint_data->anim_joint,\n"
    ...
    top = by_strategy["fresh-local-callarg-handoff-top"]
    assert top.payload["handoff_local"] == "digit_call_fpr"
    assert (
        "    f32 digitf;\n"
        "    f32 col_offset;\n"
        "    f32 col_offset_product_fpr;\n"
        ...
        "    f32 digit_call_fpr;\n"
        "    col_offset_product_fpr = y_spacing * col_cast_owner_fpr;"
    ) in top.candidate_text
    assert "        digitf = (f32) digit;\n" in top.candidate_text
    assert (
        "        digit_call_fpr = digitf;\n"
        "        HSD_JObjReqAnimAll(jobj, digit_call_fpr);"
    ) in top.candidate_text

    block = by_strategy["fresh-local-callarg-handoff-block"]
    assert block.payload["handoff_local"] == "digit_call_fpr"
    assert (
        "        digitf = (f32) digit;\n"
        "        jobj = HSD_JObjLoadJoint(joint_data->joint);"
    ) in block.candidate_text
    assert (
        "        {\n"
        "            f32 digit_call_fpr;\n"
        "            digit_call_fpr = digitf;\n"
    ...
    for handoff in (top, block):
        assert handoff.candidate_text.count("HSD_JObjLoadJoint(") == 1
        assert handoff.candidate_text.count("HSD_JObjAddAnimAll(") == 1
        assert handoff.candidate_text.count("HSD_JObjReqAnimAll(") == 1
        assert handoff.payload["call_arg_local"] == "digitf"
        assert handoff.payload["call_arg_operand"] == "digit"
        assert handoff.payload["preserves_existing_callarg_local"] is True


def test_callarg_local_structural_repair_separates_rowf_and_callarg_local() -> None:
    source = _callarg_fresh_existing_structural_source()
    ...
    assert {
        probe.payload["strategy"] for probe in callarg
    } >= {
        "continue-existing-fresh-callarg-local",
        "fresh-local-product-count-order-swap",
        "fresh-local-decl-demote-to-loop",
    ...
    assert {
        probe.payload["rowf_local"] for probe in callarg
    } == {"rowf"}
    assert {
        probe.payload["call_arg_local"] for probe in callarg
    } == {"digitf"}
    assert {
        probe.payload["call_arg_local_kind"] for probe in callarg
    } == {"fresh-existing"}
    assert all(
        probe.payload["call_arg_operand"] == "digit"
        and probe.payload["preserves_existing_callarg_local"] is True
        for probe in callarg
    )
    demote = next(
        probe for probe in callarg
        if probe.payload["strategy"] == "fresh-local-decl-demote-to-loop"
    )
    assert (
        "for (i = 0; i < digit_count; i++) {\n"
        "        f32 digitf;\n"
        "        digit = mn_GetDigitAt(value, i);"
    ) in demote.candidate_text
    assert (
        "digit = mn_GetDigitAt(value, i);\n"
        "        f32 digitf;"
    ) not in demote.candidate_text


@pytest.mark.parametrize(
    "source",
    (
        _callarg_fresh_existing_structural_source(address_taken="    sink(&digitf);\n"),
        _callarg_fresh_existing_structural_source(
            duplicate_assignment="        digitf = (f32) digit;\n"
        ),
        _callarg_fresh_existing_structural_source(
            extra_call="        HSD_JObjReqAnimAll(jobj, digitf);\n"
        ),
        _callarg_fresh_existing_structural_source(
            preprocessor="#if 1\n    sink(jobj);\n#endif\n",
        ),
...
def test_callarg_local_structural_repair_rejects_unsafe_fresh_existing_spans(
    source: str,
) -> None:
    probes = generate_transform_probes(
        source,
...
def test_callarg_local_structural_repair_rejects_demote_when_fresh_local_used_after_loop() -> None:
    source = _callarg_fresh_existing_structural_source(
        between_assignment_and_call=(
            "        jobj = HSD_JObjLoadJoint(joint_data->joint);\n"
            "        HSD_JObjAddAnimAll(jobj, joint_data->anim_joint,\n"
    ...
    strategies = {
        probe.payload["strategy"] for probe in probes
        if probe.family_id == "callarg_local_structural_repair"
    }

    assert "continue-existing-fresh-callarg-local" in strategies
    assert "fresh-local-decl-demote-to-loop" not in strategies
    assert "fresh-local-block-scope-equivalent" not in strategies
    assert "fresh-local-call-schedule-before-load" not in strategies
    assert "fresh-local-call-schedule-after-load" not in strategies
    assert "fresh-local-call-schedule-after-add" not in strategies


def test_callarg_local_structural_repair_block_scope_preserves_between_assignment_and_call() -> None:
    source = _callarg_fresh_existing_structural_source(
        between_assignment_and_call=(
            "        jobj = HSD_JObjLoadJoint(joint_data->joint);\n"
            "        HSD_JObjAddAnimAll(jobj, joint_data->anim_joint,\n"
    ...
    block = next(
        probe for probe in probes
        if probe.payload["strategy"] == "fresh-local-block-scope-equivalent"
    )

    assert "HSD_JObjLoadJoint(joint_data->joint);" in block.candidate_text
    assert "HSD_JObjAddAnimAll(jobj, joint_data->anim_joint," in block.candidate_text
    assert "HSD_JObjReqAnimAll(jobj, digitf);" in block.candidate_text


def test_callarg_local_structural_repair_schedule_does_not_cross_callarg_read() -> None:
    source = _callarg_fresh_existing_structural_source(
        between_assignment_and_call=(
            "        sink(digitf);\n"
            "        jobj = HSD_JObjLoadJoint(joint_data->joint);\n"
    ...
    assert "fresh-local-decl-demote-to-loop" in strategies
    assert "fresh-local-block-scope-equivalent" in strategies
    assert "fresh-local-call-schedule-after-load" not in strategies
    assert "fresh-local-call-schedule-after-add" not in strategies


def test_callarg_local_structural_repair_schedule_can_stop_before_callarg_read() -> None:
    source = _callarg_fresh_existing_structural_source(
        between_assignment_and_call=(
            "        jobj = HSD_JObjLoadJoint(joint_data->joint);\n"
            "        sink(digitf);\n"
    ...
    assert "fresh-local-call-schedule-after-load" in strategies
    assert "fresh-local-call-schedule-after-add" not in strategies


def test_callarg_local_structural_repair_handoff_collision_suppresses_handoff_only() -> None:
    source = _callarg_fresh_existing_structural_source(
        extra_decl="    f32 digit_call_fpr;\n",
        between_assignment_and_call=(
            "        jobj = HSD_JObjLoadJoint(joint_data->joint);\n"
            "        HSD_JObjAddAnimAll(jobj, joint_data->anim_joint,\n"
            "                           joint_data->mat_anim);\n"
    ...
    assert "continue-existing-fresh-callarg-local" in strategies
    assert "fresh-local-call-schedule-after-add" in strategies
    assert "fresh-local-callarg-handoff-top" not in strategies
    assert "fresh-local-callarg-handoff-block" not in strategies


@pytest.mark.parametrize(
    "source",
    (
        _callarg_local_structural_source(address_taken="    sink(&rowf);\n"),
        _callarg_local_structural_source(preprocessor="#if 1\n    sink(jobj);\n#endif\n"),
        _callarg_local_structural_source(
            extra_call="        HSD_JObjReqAnimAll(jobj, (f32) digit);\n"
        ),
    ),
)
def test_callarg_local_structural_repair_rejects_unsafe_spans(source: str) -> None:
    probes = generate_transform_probes(
        source,
        function="mnDiagram_DrawCellNumber",
        unit="melee/mn/mndiagram",
...
def test_callarg_local_structural_repair_digit_frame_collision_suppresses_fresh_variants_only() -> None:
    source = _callarg_local_structural_source(extra_decl="    f32 digit_frame_fpr;\n")
    ...
    assert "retain-existing-rowf-callarg-h004-order" in strategies
    assert "fresh-loop-callarg-local" not in strategies
    assert "block-scoped-callarg-local" not in strategies


def test_pcode_only_fpr_callarg_temp_repair_materializes_inline_cast_call() -> None:
    source = (
        "typedef unsigned char u8;\n"
        "typedef float f32;\n"
        "void mnDiagram_DrawCellNumber(HSD_JObj* jobj, u8 digit) {\n"
    ...
    assert {probe.payload["strategy"] for probe in callarg} == {
        "fpr-callarg-block-local-assign-temp",
        "fpr-callarg-block-local-init-temp",
    }
    assert any(
...
def _stack_array_node_set_delta_source() -> str:
    return (
...
def _stack_array_node_set_delta_payload() -> dict:
    return {
        "kind": "node-set-delta",
...
def test_node_set_delta_steering_generates_stack_array_base_source_probe() -> None:
    probes = generate_transform_probes(
        _stack_array_node_set_delta_source(),
        function="mnDiagram2_GetRankedFighter",
        unit="melee/mn/mndiagram2",
        force_phys={40: 25, 45: 12, 46: 11},
        node_set_delta=_stack_array_node_set_delta_payload(),
        max_per_family=3,
    )
    ...
    assert [request["target_ig"] for request in requests] == [40, 45, 46]
    assert requests[0]["source_kind"] == "stack-array-base"
    assert requests[0]["source_expression"] == "entries"
    assert "source-pattern-not-found" not in {
        item.get("blocked_reason")
        for item in payload["skipped_missing_virtuals"]
    }
    assert "entries_base_bind_40_0" in node_set.candidate_text
    assert node_set.payload["source_hunks"]
    assert payload["hunk"]
    assert payload["touched_ranges"]
...
def _coupled_fpr_draw_cell_number_source(body: str) -> str:
    return (
        "typedef unsigned char u8;\n"
        "typedef float f32;\n"
        "typedef struct HSD_JObj HSD_JObj;\n"
        "void mnDiagram_DrawCellNumber(HSD_JObj* jobj, f32 y_spacing, u8 col, u8 digit) {\n"
        "    f32 col_offset;\n"
        "    f32 base;\n"
        "    int i;\n"
        f"{body}"
...
def _coupled_fpr_probes(source: str):
    return generate_transform_probes(
        source,
        function="mnDiagram_DrawCellNumber",
        unit="melee/mn/mndiagram",
        force_phys={32: 28, 33: 26, 46: 26},
        families=("coupled_fpr_coalesce_product_repair",),
        max_per_family=8,
    )


def test_coupled_fpr_product_callarg_repair_materializes_joint_probe() -> None:
    source = _coupled_fpr_draw_cell_number_source(
        "    col_offset = y_spacing * (f32) col;\n"
        "    for (i = 0; i < 2; i++) {\n"
        "        base = (f32) digit;\n"
        "        HSD_JObjReqAnimAll(jobj, base);\n"
        "        HSD_JObjSetTranslateX(jobj, 4.0f + col_offset);\n"
        "    }\n"
    )

    probes = _coupled_fpr_probes(source)

    coupled = [
        probe for probe in probes
        if probe.mutator_key == "steer_coupled_fpr_product_callarg_repair"
    ]
    assert coupled
    probe = coupled[0]
    assert probe.family_id == "coupled_fpr_coalesce_product_repair"
    assert "col_offset-product-conversion" in probe.payload["strategy"]
    assert "HSD_JObjReqAnimAll-digit-conversion" in probe.payload["strategy"]
    assert probe.payload["product_local"] == "col_offset"
    assert probe.payload["product_expr"] == "y_spacing * (f32) col"
    assert probe.payload["product_temp_local"] == "col_offset_product_fpr"
    assert probe.payload["cast_expr"] == "(f32) col"
    assert probe.payload["cast_temp_local"] == "col_fpr"
    assert probe.payload["callee"] == "HSD_JObjReqAnimAll"
    assert probe.payload["call_arg_local"] == "base"
    assert probe.payload["call_arg_expr"] == "(f32) digit"
    assert probe.payload["call_temp_local"] == "base_call_fpr"
    assert probe.payload["target_virtuals_hint"] == (32, 33, 46)
    assert any("column product/conversion" in item for item in probe.payload["source_regions"])
    assert any("digit call conversion" in item for item in probe.payload["source_regions"])
    assert "    f32 col_fpr;\n" in probe.candidate_text
    assert "    f32 col_offset_product_fpr;\n" in probe.candidate_text
    assert "    f32 base_call_fpr;\n" in probe.candidate_text
    assert "    col_fpr = (f32) col;\n" in probe.candidate_text
    assert "    col_offset_product_fpr = y_spacing * col_fpr;\n" in probe.candidate_text
    assert "    col_offset = col_offset_product_fpr;\n" in probe.candidate_text
    assert "        base_call_fpr = base;\n" in probe.candidate_text
    assert "        HSD_JObjReqAnimAll(jobj, base_call_fpr);" in probe.candidate_text
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (
            ...
            "    col_offset = y_spacing * (f32) col;\n"
            "    HSD_JObjReqAnimAll(jobj, data->frame);\n",
        ),
        (
            "duplicate product and call text",
            "    col_offset = y_spacing * (f32) col;\n"
            "    base = (f32) digit;\n"
            "    HSD_JObjReqAnimAll(jobj, base);\n"
            "    col_offset = y_spacing * (f32) col;\n"
...
def test_coupled_fpr_product_callarg_repair_rejects_unsafe_shapes(
    case: str,
    body: str,
) -> None:
    probes = _coupled_fpr_probes(_coupled_fpr_draw_cell_number_source(body))

    assert "steer_coupled_fpr_product_callarg_repair" not in {
        probe.mutator_key for probe in probes
    }, case


def test_mndiagram_coloring_plan_includes_coupled_fpr_family() -> None:
    ...
    assert coloring.family_ids[:2] == (
        "pcode_only_fpr_callarg_temp_repair",
        "coupled_fpr_coalesce_product_repair",
    )
    assert "coupled_fpr_coalesce_product_repair" in {
