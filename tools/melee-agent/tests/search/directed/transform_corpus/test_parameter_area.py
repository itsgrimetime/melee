...
def test_outgoing_parameter_area_shape_uses_known_hsd_jobj_add_anim_types() -> None:
    source = (
        ...
        "void target(HSD_JObj* jobj, mnDiagram_ArchiveData* joint_data) {\n"
    ...
    probe = next(
        probe for probe in probes
        if probe.payload["mode"] == "call-site"
        and probe.payload["callee"] == "HSD_JObjAddAnimAll"
    )
    assert "HSD_AnimJoint* param_area_0_1 = joint_data->anim_joint;" in (
        probe.candidate_text
    )
    assert "HSD_MatAnimJoint* param_area_0_2 = joint_data->mat_anim;" in (
        probe.candidate_text
    )
    assert "HSD_ShapeAnimJoint* param_area_0_3 = joint_data->shape_anim;" in (
        probe.candidate_text
    )
    assert "mnDiagram_ArchiveData* param_area_0_1" not in probe.candidate_text
    assert probe.payload["updated_call_sites"][0]["tempized_arguments"] == [
        {
            "index": 1,
            "type": "HSD_AnimJoint*",
            "expression": "joint_data->anim_joint",
            "temp_name": "param_area_0_1",
            "span": [
                source.index("joint_data->anim_joint"),
                source.index("joint_data->anim_joint")
                + len("joint_data->anim_joint"),
            ],
        },
        {
            "index": 2,
            "type": "HSD_MatAnimJoint*",
            "expression": "joint_data->mat_anim",
            "temp_name": "param_area_0_2",
            "span": [
                source.index("joint_data->mat_anim"),
                source.index("joint_data->mat_anim") + len("joint_data->mat_anim"),
            ],
        },
        {
            "index": 3,
            "type": "HSD_ShapeAnimJoint*",
            "expression": "joint_data->shape_anim",
            "temp_name": "param_area_0_3",
            "span": [
                source.index("joint_data->shape_anim"),
                source.index("joint_data->shape_anim")
                + len("joint_data->shape_anim"),
            ],
        },
    ]


def test_outgoing_parameter_area_shape_wraps_temps_after_statements() -> None:
    source = (
        "typedef struct HSD_JObj HSD_JObj;\n"
        "typedef struct HSD_Joint HSD_Joint;\n"
        "typedef struct HSD_AnimJoint HSD_AnimJoint;\n"
        ...
        "void target(mnDiagram_ArchiveData* joint_data) {\n"
        "    HSD_JObj* jobj;\n"
        "    int i;\n"
        "    for (i = 0; i < 2; i++) {\n"
    ...
    expected_block = (
        "        {\n"
        "            HSD_AnimJoint* param_area_0_1 = joint_data->anim_joint;\n"
        "            HSD_MatAnimJoint* param_area_0_2 = joint_data->mat_anim;\n"
        "            HSD_ShapeAnimJoint* param_area_0_3 = joint_data->shape_anim;\n"
        "            HSD_JObjAddAnimAll(jobj, param_area_0_1,\n"
        "                               param_area_0_2,\n"
        "                               param_area_0_3);\n"
        "        }\n"
    )
    assert expected_block in probe.candidate_text
    assert (
        "        jobj = HSD_JObjLoadJoint(joint_data->joint);\n"
        "        HSD_AnimJoint* param_area_0_1 = joint_data->anim_joint;\n"
        not in probe.candidate_text
    )
