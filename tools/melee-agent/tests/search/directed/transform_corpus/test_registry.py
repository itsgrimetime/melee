...
def test_default_corpus_names_required_transform_families() -> None:
    ...
    assert "pcode_only_gpr_address_temp_repair" in family_ids
    assert "pcode_only_gpr_copy_product_case_c_repair" in family_ids
    assert "pcode_only_gpr_bool_mask_temp_repair" in family_ids
    assert "retained_gpr_case_c_sensitivity_search" in family_ids
    assert "retained_gpr_case_c_window_order_continuation" in family_ids
    assert "retained_gpr_case_c_post_source_owner_backtrack" in family_ids
    assert "retained_gpr_case_c_target_live_range_repair" in family_ids
    assert "retained_case_c_alternate_source_owner_discovery" in family_ids
    assert "retained_gpr_common_subexpr_coalesce_source" in family_ids
    assert "pcode_only_fpr_callarg_temp_repair" in family_ids
    assert "pcode_only_fpr_fsubs_cast_owner_repair" in family_ids
    assert "mixed_pcode_fpr_lifetime_pressure_repair" in family_ids
...
def test_pcode_only_fpr_callarg_temp_metadata_is_executable() -> None:
    family = next(
        family
        for family in DEFAULT_TRANSFORM_FAMILIES
        if family.family_id == "pcode_only_fpr_callarg_temp_repair"
    )

    assert family.mutator_keys == ("steer_pcode_only_fpr_callarg_temp",)
    assert family.semantic_risk == "medium"
    assert "HSD_JObjReqAnimAll" in family.source_region_selector
    assert "pcode-only" in family.expected_compiler_effect
    assert "record-only" not in family.generated_probe_form
    assert {"pcode-only", "fpr", "call-argument", "fmr"} <= set(family.keywords)


def test_pcode_only_fpr_fsubs_cast_owner_metadata_is_executable() -> None:
    family = next(
        family
        for family in DEFAULT_TRANSFORM_FAMILIES
        if family.family_id == "pcode_only_fpr_fsubs_cast_owner_repair"
    )

    assert family.mutator_keys == ("steer_pcode_only_fpr_fsubs_cast_owner",)
    assert family.semantic_risk == "medium"
    assert "FPR cast" in family.source_region_selector
    assert "pcode-only" in family.expected_compiler_effect
    assert "record-only" not in family.generated_probe_form
    assert {"pcode-only", "fpr", "fsubs", "cast-owner"} <= set(family.keywords)


def test_pcode_only_gpr_address_temp_metadata_is_executable() -> None:
    family = next(
        family
        for family in DEFAULT_TRANSFORM_FAMILIES
        if family.family_id == "pcode_only_gpr_address_temp_repair"
    )

    assert family.mutator_keys == ("steer_pcode_only_gpr_address_temp",)
    assert family.semantic_risk == "medium"
    assert "GPR address-temp" in family.source_region_selector
    assert "pcode-only" in family.expected_compiler_effect
    assert "record-only" not in family.generated_probe_form
    assert {"pcode-only", "gpr", "address-temp", "copy-product"} <= set(
        family.keywords
    )


def test_pcode_only_gpr_copy_product_case_c_metadata_is_executable() -> None:
    family = next(
        family
        for family in DEFAULT_TRANSFORM_FAMILIES
        if family.family_id == "pcode_only_gpr_copy_product_case_c_repair"
    )

    assert family.mutator_keys == ("steer_pcode_only_gpr_copy_product_case_c",)
    assert family.semantic_risk == "medium"
    assert "copy-product" in family.source_region_selector
    assert "Case-C" in family.expected_compiler_effect
    assert "record-only" not in family.generated_probe_form
    assert {"pcode-only", "gpr", "copy-product", "case-c"} <= set(
        family.keywords
    )


def test_pcode_only_gpr_bool_mask_temp_metadata_is_executable() -> None:
    family = next(
        family
        for family in DEFAULT_TRANSFORM_FAMILIES
        if family.family_id == "pcode_only_gpr_bool_mask_temp_repair"
    )

    assert family.mutator_keys == ("steer_pcode_only_gpr_bool_mask_temp",)
    assert family.semantic_risk == "medium"
    assert "boolean/mask predicates" in family.source_region_selector
    assert "pcode-only GPR bool/mask" in family.expected_compiler_effect
    assert "record-only" not in family.generated_probe_form
    assert {"pcode-only", "gpr", "bool-mask", "dirty-wrapper"} <= set(
        family.keywords
    )


def test_retained_gpr_case_c_sensitivity_metadata_is_executable() -> None:
    family = next(
        family
        for family in DEFAULT_TRANSFORM_FAMILIES
        if family.family_id == "retained_gpr_case_c_sensitivity_search"
    )

    assert family.mutator_keys == ("steer_retained_gpr_case_c_sensitivity",)
    assert family.semantic_risk == "medium"
    assert "retained-source" in family.source_region_selector
    assert "first-divergence" in family.expected_compiler_effect
    assert "record-only" not in family.generated_probe_form
    assert {"retained-source", "gpr", "case-c", "sensitivity"} <= set(
        family.keywords
    )


def test_retained_gpr_case_c_window_order_continuation_metadata_is_executable(
) -> None:
    family = next(
        family
        for family in DEFAULT_TRANSFORM_FAMILIES
        if family.family_id == "retained_gpr_case_c_window_order_continuation"
    )
    ...
    assert family.semantic_risk == "medium"
    assert "select-order JSON" in family.source_region_selector
    assert "IG44" in family.expected_compiler_effect
    assert "record-only" not in family.generated_probe_form
    assert {"retained-source", "gpr", "case-c", "ig44", "window-order"} <= set(
        family.keywords
    )


def test_retained_gpr_case_c_post_source_owner_backtrack_metadata_is_executable(
) -> None:
    family = next(
        family
        for family in DEFAULT_TRANSFORM_FAMILIES
        if family.family_id == "retained_gpr_case_c_post_source_owner_backtrack"
    )
    ...
    assert family.semantic_risk == "medium"
    assert "post-source-owner" in family.source_region_selector
    assert "later ranked indexed-byte" in family.expected_compiler_effect
    assert "record-only" not in family.generated_probe_form
    assert {
        "retained-source",
        "gpr",
...
def test_retained_gpr_common_subexpr_coalesce_source_metadata_is_executable(
) -> None:
    family = next(
        family
        for family in DEFAULT_TRANSFORM_FAMILIES
        if family.family_id == "retained_gpr_common_subexpr_coalesce_source"
    )
    ...
    assert family.semantic_risk == "medium"
    assert "coalesce suggestions" in family.source_region_selector
    assert "shared C temporary" in family.expected_compiler_effect
    assert "record-only" not in family.generated_probe_form
    assert {"retained-source", "gpr", "coalesce", "common-subexpr"} <= set(
        family.keywords
    )


def test_retained_case_c_alternate_source_owner_metadata_is_executable() -> None:
    family = next(
        family
        for family in DEFAULT_TRANSFORM_FAMILIES
        if family.family_id == "retained_case_c_alternate_source_owner_discovery"
    )
    ...
    assert family.semantic_risk == "medium"
    assert "current-owner terminal spans" in family.source_region_selector
    assert "alternate source-owner" in family.expected_compiler_effect
    assert "record-only" not in family.generated_probe_form
    assert {
        "retained-source",
        "case-c",
...
def test_retained_gpr_case_c_target_live_range_repair_metadata_is_executable(
) -> None:
    family = next(
        family
        for family in DEFAULT_TRANSFORM_FAMILIES
        if family.family_id == "retained_gpr_case_c_target_live_range_repair"
    )
    ...
    assert family.semantic_risk == "medium"
    assert "blocker-color-chain" in family.source_region_selector
    assert "current blocker" in family.expected_compiler_effect
    assert "record-only" not in family.generated_probe_form
    assert {
        "retained-source",
        "case-c",
...
def test_retained_fpr_case_c_target_live_range_repair_metadata_is_executable(
) -> None:
    family = next(
        family
        for family in DEFAULT_TRANSFORM_FAMILIES
        if family.family_id == "retained_fpr_case_c_target_live_range_repair"
    )
    ...
    assert family.semantic_risk == "medium"
    assert "DrawCellNumber" in family.source_region_selector
    assert "scalar FPR interference shape" in family.expected_compiler_effect
    assert "paired row/column overlap temps" in family.generated_probe_form
    assert {
        "retained-source",
        "fpr",
        "case-c",
...
def test_retained_gpr_case_c_simplify_order_continuation_metadata_is_executable(
) -> None:
    family = next(
        family
        for family in DEFAULT_TRANSFORM_FAMILIES
        if family.family_id == "retained_gpr_case_c_simplify_order_continuation"
    )
    ...
    assert family.semantic_risk == "medium"
    assert "simplify-order" in family.source_region_selector
    assert "IG44" in family.expected_compiler_effect
    assert "record-only" not in family.generated_probe_form
    assert {
        "retained-source",
...
def test_mixed_pcode_fpr_lifetime_pressure_metadata_is_executable() -> None:
    family = next(
        family
        for family in DEFAULT_TRANSFORM_FAMILIES
        if family.family_id == "mixed_pcode_fpr_lifetime_pressure_repair"
    )

    assert family.mutator_keys == ("steer_mixed_pcode_fpr_lifetime_pressure",)
    assert family.semantic_risk == "medium"
    assert "row_offset_adj" in family.source_region_selector
    assert "FPR lifetime pressure" in family.expected_compiler_effect
    assert "record-only" not in family.generated_probe_form
    assert {"mixed-pcode", "fpr", "row_offset", "callarg", "f25"} <= set(
        family.keywords
    )


def test_callarg_local_structural_repair_metadata_is_executable() -> None:
    family = next(
        family
        for family in DEFAULT_TRANSFORM_FAMILIES
        if family.family_id == "callarg_local_structural_repair"
    )
    ...
    assert family.semantic_risk == "medium"
    assert "protected" in family.source_region_selector
    assert "FPR" in family.expected_compiler_effect
    assert "record-only" not in family.generated_probe_form
    assert {"callarg", "protected-expression", "fpr", "structural"} <= set(
        family.keywords
    )
...
def test_plan_transform_experiments_adds_window_order_continuation_for_sort_names(
) -> None:
    plan = plan_transform_experiments(
    ...
    assert "retained_gpr_case_c_window_order_continuation" in (
        plan.clusters[0].family_ids
    )
    assert "retained_gpr_case_c_target_live_range_repair" in (
        plan.clusters[0].family_ids
    )
    assert "retained_gpr_case_c_simplify_order_continuation" in (
        plan.clusters[0].family_ids
    )
    assert "retained_gpr_case_c_post_source_owner_backtrack" in (
        plan.clusters[0].family_ids
    )
    assert "retained_gpr_common_subexpr_coalesce_source" in (
        plan.clusters[0].family_ids
    )
    assert "retained_gpr_case_c_window_order_continuation" in [
...
def test_plan_transform_experiments_keeps_generic_fallback_for_other_force_phys() -> None:
    ...
    assert "pcode_only_gpr_bool_mask_temp_repair" in {
        family.family_id for family in plan.families
    }
