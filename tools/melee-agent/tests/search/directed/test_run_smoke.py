...
def test_run_directed_dry_returns_telemetry(tmp_path):
    ...
    assert res["accounting"]["source_drained"] is True
...
def test_run_directed_live_preflight_failure_has_source_shape_accounting(
    tmp_path,
    monkeypatch,
):
    ...
    def fail_preflight(_objective):
        raise PreflightError("wrong_register_class")
    ...
    assert res["gate"]["reason"] == "not_preflight"
    assert res["accounting"]["preflight_failed"] is True
    assert res["accounting"]["source_shape_drained"] is False
