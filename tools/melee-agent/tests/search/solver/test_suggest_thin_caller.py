...
def test_suggest_register_tiebreak_json_emits_terminal_payload_without_worksheet(
    monkeypatch,
):
    def fake_solve(**_kw):
        return SolveResult(
            ...
            solver_diagnostics={"classification": "normalized-structural-near-match"},
        )
    ...
    assert payload["function"] == "mnDiagram2_HandleInput"
    assert payload["status"] == "solver-abstain"
    assert payload["exit_code"] == 3
    assert payload["attempted_class_ids"] == [0]
    assert "empty force-phys target" in payload["reason"]
    assert payload["terminal_proof"]["kind"] == "register-tiebreak-solver-result"
