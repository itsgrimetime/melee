...
def test_abstain_exit3_on_force_phys_collision():
    ...
    diagnostics = {
        "force_vector": "class0:ig42:phys=r27",
        "force_vector_verify": {"ran": True, "union": {"status": "match"}},
    }
    res = solve_coloring(function="f", class_id=0,
    ...
    # Reframed message (2026-06-15): scoped to current structure + points to a
    # structural lever instead of declaring the function "unreachable".
    assert "structural" in res.reason.lower()
    assert res.node_set_delta == delta
    assert res.solver_diagnostics == diagnostics
