...
def _multi_sig(assignments: dict[int, int]):
    return BaselineSignature(
        interference_edges=frozenset(),
        coalesce_mappings=frozenset(),
        spill_set=frozenset(),
        simplify_order=tuple(sorted(assignments)),
        assigned_regs=frozenset(assignments.items()),
    )
...
def test_solve_node_set_split_reports_source_rejected_candidate_without_compile(
    monkeypatch,
    tmp_path,
):
    source = _node_split_repo(tmp_path, monkeypatch)

    def fake_generate(src, function, request, **_kwargs):
        return [
            CandidatePatch(
                "node-split-decl-order-holder-ig40-s0-c0-promote-holder",
                src,
                "unsafe decl-order",
                (),
                "",
    ...
    def fail_if_candidate_compiled(*_args, **kwargs):
        if kwargs.get("label") == "baseline":
            return _sig(reg=31)
        raise AssertionError("source-rejected patch should not be compiled")
    ...
    payload = json.loads(result.output)
    row = payload["candidates"][0]
    assert row["objective_status"] == "source-rejected"
    assert row["source_rejection_reason"] == (
        "duplicate local declaration(s) after decl-order rewrite: holder"
    )
    assert row["diagnostics_path"] is None
    assert payload["objective_counts"] == {"source-rejected": 1}
    probe_root = (
        tmp_path
        / "build"
        / "mwcc_debug_cache"
    ...
    assert not list((probe_root / "compile_failed").glob("*.c"))
...
def test_solve_node_set_split_budget_exhausted_writes_pending_manifest(
    monkeypatch,
    tmp_path,
):
    _node_split_repo(tmp_path, monkeypatch)
    summary_path = tmp_path / "node-set-summary.json"
    patches = [
        CandidatePatch(
            f"cand-{idx}",
    ...
    def fail_compile(*_args, **_kwargs):
        raise AssertionError("budget exhaustion should skip baseline compile")
    ...
    assert payload["scored_count"] == 0
    assert payload["pending_count"] == 3
    assert payload["output"] == str(summary_path)
    assert summary_path.exists()
    manifest_path = Path(payload["manifest_path"])
    assert manifest_path.exists()
    assert payload["generated_candidate_manifest"] == str(manifest_path)
    assert len(payload["pending_candidates"]) == 3
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    assert manifest["candidate_order"] == ["cand-0", "cand-1", "cand-2"]
    assert len(manifest["candidates"]) == 3
    for idx, record in enumerate(manifest["candidates"]):
        source_path = Path(record["source_path"])
        assert source_path.exists()
        assert f"use({idx})" in source_path.read_text(encoding="utf-8")
    resume_command = payload["stop_condition"]["resume_command"]
    assert "--resume-summary" in resume_command
    assert "node-set-summary.json" in resume_command


def test_solve_node_set_split_retain_generated_budget_exhaustion_uses_default_summary_path(
    monkeypatch,
    tmp_path,
):
    _node_split_repo(tmp_path, monkeypatch)
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("compile must not run")
        ),
        raising=False,
    )
    ...
    assert "ig40_holder_r30.json" in payload["stop_condition"]["resume_command"]
...
def test_solve_node_set_split_coupled_generation_honors_budget(
    ...
    output_path = Path(payload["output"])
    assert output_path.exists()
    resume_summary_arg = str(output_path.relative_to(tmp_path))
    assert (
        f"--resume-summary {resume_summary_arg}"
        in payload["stop_condition"]["resume_command"]
    )
    assert compile_labels == []


def test_solve_node_set_split_coupled_budget_partial_generation_writes_output(
    monkeypatch,
    tmp_path,
):
    source = _node_split_repo(
    ...
    output_path = tmp_path / "node-set-summary.json"
    ...
    def fake_compile_signature(path, **kwargs):
        compile_labels.append(kwargs["label"])
        raise AssertionError("compile must not run after generation budget")
    ...
    assert payload["pending_candidates"][0]["candidate_id"] == patch.candidate_id
    assert Path(payload["generated_candidate_manifest"]).exists()
    assert written["generated_count"] == 1
    assert compile_labels == []


def test_node_set_generation_watchdog_raises_typed_timeout():
    with pytest.raises(debugcli._NodeSetSplitGenerationTimeout):
        debugcli._run_node_set_split_generation_with_watchdog(
            lambda: [],
            timeout_s=0,
        )


def test_generate_coupled_stops_next_frontier_at_candidate_cap(monkeypatch):
    import src.mwcc_debug.node_set_split as node_set_split
    from src.mwcc_debug.node_set_split import NodeSetSplitRequest
    ...
    def fake_request_patches(cur_source, function, request, **_kwargs):
        ...
        return [
            CandidatePatch(
                f"single-{request.target_ig}-{index}",
                cur_source + f"\n/* {request.target_ig} {index} */",
                "single",
                (),
                "",
            )
    ...
    result = node_set_split.generate_coupled_node_set_split_patches(
        _node_split_source(),
        "fn_test",
        [
            NodeSetSplitRequest("fn_test", 0, 34, target_reg="r27", var_name="a"),
            NodeSetSplitRequest("fn_test", 0, 44, target_reg="r25", var_name="b"),
        ],
        max_candidates=2,
        return_result=True,
    )

    assert result.stop_reason == "candidate-limit"
    assert len(result.patches) == 2
    assert calls == [34, 44]


def test_generate_coupled_preserves_final_request_patch_after_deadline(monkeypatch):
    import src.mwcc_debug.node_set_split as node_set_split
    from src.mwcc_debug.node_set_split import NodeSetSplitRequest
    ...
    assert result.patches[0].candidate_id == "node-split-coupled-ig34+ig44-c0"
...
def test_solve_node_set_split_coupled_stack_array_base_retains_source_pcdump_and_target_score(
    monkeypatch,
    tmp_path,
):
    source = _node_split_repo(
    ...
    def fake_compile(path, **kwargs):
        if kwargs["label"] == "baseline":
            return _multi_sig({40: 40, 45: 45, 46: 46}), "baseline-pcdump"
        return _multi_sig({40: 25, 45: 12, 46: 11}), "candidate-pcdump"

    def fake_score(patch, **_kwargs):
        return CandidateScore(
            patch.candidate_id,
            compile_ok=True,
            checkdiff_pct=51.0,
    ...
    assert Path(row["pcdump_path"]).read_text(encoding="utf-8") == "candidate-pcdump"
    assert row["target_score"]["hits"] == 3
    assert row["target_score"]["virtuals"]["40"]["hit"] is True
    assert row["target_score"]["virtuals"]["45"]["hit"] is True
    assert row["target_score"]["virtuals"]["46"]["hit"] is True
...
def test_solve_node_set_split_max_candidates_stops_after_cap(
    ...
    assert payload["exhaustive"] is False
    resume_command = payload["stop_condition"]["resume_command"]
    assert "--remote" in resume_command
    assert "--max-candidates 0" in resume_command
    assert "--resume-summary" in resume_command
    assert resume_command in payload["next_steps"]
    assert Path(payload["output"]).exists()
    assert compile_labels == ["baseline", "cand-0"]


def test_solve_node_set_split_single_generation_passes_candidate_limit(
    monkeypatch,
    tmp_path,
):
    _node_split_repo(tmp_path, monkeypatch)
    ...
    def fake_generate(*_args, max_candidates=None, **_kwargs):
    ...
    assert result.exit_code == 4, result.output
    payload = json.loads(result.output)
    assert received["max_candidates"] == 2
    assert payload["generated_count"] == 2
    assert payload["stop_reason"] == "candidate-limit"
    assert payload["stop_condition"]["kind"] == "candidate-limit"
    assert payload["exhaustive"] is False
    ...
    assert compile_labels == ["baseline", "cand-0", "cand-1"]


def test_solve_node_set_split_resume_summary_regenerates_without_manifest(
    monkeypatch,
    tmp_path,
):
    _node_split_repo(tmp_path, monkeypatch)
    resume_summary = tmp_path / "prior-node-set.json"
    resume_summary.write_text(json.dumps({"function": "fn_test"}), encoding="utf-8")
    patches = [
        CandidatePatch(
            f"cand-{idx}",
            f"void fn_test(void) {{ use({idx}); }}\n",
    ...
    result = runner.invoke(debugcli.debug_app, [
        ...
        "--var", "holder",
        "--max-candidates", "1",
        "--resume-summary", str(resume_summary),
        "--json",
    ])

    assert result.exit_code == 4, result.output
    payload = json.loads(result.output)
    assert payload["resume_mode"] == "regenerated-no-manifest"
    assert payload["stop_condition"] is None
    assert payload["scored_count"] == 3
    assert compile_labels == ["baseline", "cand-0", "cand-1", "cand-2"]


def test_solve_node_set_split_resume_manifest_scores_retained_candidates_without_regeneration(
    monkeypatch,
    tmp_path,
):
    _node_split_repo(tmp_path, monkeypatch)
    summary_path = tmp_path / "prior-node-set.json"
    patches = [
        CandidatePatch(
            f"cand-{idx}",
            f"void fn_test(void) {{ use({idx}); }}\n",
    ...
    setup = runner.invoke(debugcli.debug_app, [
        "solve", "node-set-split",
        "-f", "fn_test",
        "--class", "gpr",
        "--ig", "40",
    ...
    assert setup.exit_code == 4, setup.output

    def fail_generate(*_args, **_kwargs):
        raise AssertionError("manifest resume must not regenerate candidates")

    compile_labels = []
    scored_touched_ranges = []

    def fake_compile_signature(path, **kwargs):
        label = kwargs["label"]
        compile_labels.append(label)
        return _sig(reg=30 if label == "cand-1" else 31)

    def fake_score(patch, **_kwargs):
        scored_touched_ranges.append(patch.touched_ranges)
        return CandidateScore(
            patch.candidate_id,
            compile_ok=True,
            checkdiff_pct=50.3,
    ...
    monkeypatch.setattr(
        "src.mwcc_debug.node_set_split.generate_node_set_introduce_binding_patches",
        fail_generate,
    )
    monkeypatch.setattr(
        "src.mwcc_debug.node_set_split.generate_coupled_node_set_split_patches",
    ...
    result = runner.invoke(debugcli.debug_app, [
        ...
        "--target-reg", "r30",
        "--var", "holder",
        "--resume-summary", str(summary_path),
        "--json",
    ])

    assert result.exit_code == 0, result.output
    payload = json.loads(result.output)
    assert payload["resume_mode"] == "manifest"
    assert payload["generated_count"] == 3
    assert [row["candidate_id"] for row in payload["candidates"]] == [
        "cand-0",
        "cand-1",
    ...
    assert compile_labels == ["baseline", "cand-0", "cand-1", "cand-2"]
    assert scored_touched_ranges == [((1, 3),)]


def test_solve_node_set_split_resume_manifest_infers_fpr_class(
    monkeypatch,
    tmp_path,
):
    _node_split_repo(tmp_path, monkeypatch)
    summary_path = tmp_path / "fpr-node-set.json"
    patch = CandidatePatch(
        "fpr-cand",
        "void fn_test(void) { use(1); }\n",
        "candidate",
    ...
    def fake_compile_signature(path, **kwargs):
        label = kwargs["label"]
        compile_labels.append(label)
        compile_classes.append(kwargs["class_id"])
        return _sig(reg=30 if label == "fpr-cand" else 31)
    ...
    assert payload["resume_mode"] == "manifest"
    assert payload["request"]["class_id"] == 1
    assert compile_classes == [1, 1]
    assert compile_labels == ["baseline", "fpr-cand"]


def test_solve_node_set_split_coupled_resume_manifest_preserves_requests_without_regeneration(
    monkeypatch,
    tmp_path,
):
    source = _node_split_repo(
    ...
    summary_path = tmp_path / "coupled-node-set.json"
    patch = CandidatePatch(
        "coupled-cand",
    ...
    def fake_compile_signature(path, **kwargs):
        ...
        compile_labels.append(label)
        if label == "baseline":
            return _multi_sig({34: 24, 44: 27})
        return _multi_sig({34: 27, 44: 25})
    ...
    payload = json.loads(result.output)
    assert payload["resume_mode"] == "manifest"
    assert [req["target_ig"] for req in payload["coupled_requests"]] == [34, 44]
    assert [row["candidate_id"] for row in payload["candidates"]] == [
        "coupled-cand"
    ]
    assert compile_labels == ["baseline", "coupled-cand"]
    assert scored_touched_ranges == [((4, 12),)]


def test_solve_node_set_split_budget_exhausted_after_objective_keeps_manifest_resume(
    monkeypatch,
    tmp_path,
):
    _node_split_repo(tmp_path, monkeypatch)
    summary_path = tmp_path / "objective-budget.json"
    patch = CandidatePatch(
        "cand-realized",
        "void fn_test(void) { use(1); }\n",
        "candidate",
    ...
    def fake_compile_signature(path, **kwargs):
        label = kwargs["label"]
        compile_labels.append(label)
        if label == "cand-realized":
            clock["now"] = 102.0
            return _sig(reg=30)
        return _sig(reg=31)

    def fail_score(*_args, **_kwargs):
        raise AssertionError("first run should exhaust before checkdiff scoring")
    ...
    first = runner.invoke(debugcli.debug_app, [
        "solve", "node-set-split",
        "-f", "fn_test",
        "--class", "gpr",
        "--ig", "40",
    ...
    assert first_payload["stop_condition"]["kind"] == "budget-exhausted"
    assert first_payload["pending_count"] == 1
    assert first_payload["candidates"][0]["objective_status"] == "realized"
    assert first_payload["candidates"][0]["score_status"] == "budget-exhausted"
    assert Path(first_payload["manifest_path"]).exists()
    assert "--resume-summary" in first_payload["stop_condition"]["resume_command"]
    assert compile_labels == ["baseline", "cand-realized"]

    def fail_generate(*_args, **_kwargs):
        raise AssertionError("manifest resume must not regenerate candidates")

    compile_labels.clear()

    def resume_compile_signature(path, **kwargs):
        label = kwargs["label"]
        compile_labels.append(label)
        return _sig(reg=30 if label == "cand-realized" else 31)
    ...
    resumed = runner.invoke(debugcli.debug_app, [
        "solve", "node-set-split",
        "-f", "fn_test",
        "--class", "gpr",
        "--ig", "40",
    ...
    assert resumed_payload["resume_mode"] == "manifest"
    assert resumed_payload["pending_count"] == 0
    assert compile_labels == ["baseline", "cand-realized"]


def test_solve_node_set_split_resume_manifest_rejects_modified_candidate_source(
    monkeypatch,
    tmp_path,
):
    _node_split_repo(tmp_path, monkeypatch)
    summary_path = tmp_path / "stale-node-set.json"
    patch = CandidatePatch(
        "cand-stale",
        "void fn_test(void) { use(1); }\n",
        "candidate",
    ...
    assert setup.exit_code == 4, setup.output
    manifest_path = Path(json.loads(setup.output)["manifest_path"])
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    source_path = Path(manifest["candidates"][0]["source_path"])
    source_path.write_text("void fn_test(void) { use(2); }\n", encoding="utf-8")
    compile_called = False

    def fail_generate(*_args, **_kwargs):
        raise AssertionError("stale manifest must fail before regeneration")

    def fail_compile(*_args, **_kwargs):
        nonlocal compile_called
        compile_called = True
        raise AssertionError("stale manifest must fail before compile")
...
def test_solve_node_set_split_remote_backend_scores_generated_candidate(
    monkeypatch,
    tmp_path,
):
    _node_split_repo(tmp_path, monkeypatch)
    ...
    def fail_local_compile(*_args, **_kwargs):
        raise AssertionError("local pcdump compile should not run in --remote mode")

    def fake_remote(**kwargs):
        remote_calls.append({
            "source_rel": kwargs["source_rel"],
            "compile_source_rel": kwargs["compile_source_rel"],
            "stage_source_path": kwargs["stage_source_path"],
            "stage_source_label": kwargs["stage_source_label"],
        })
        staged = kwargs["stage_source_path"] is not None
        return debugcli._RemotePcdumpResult(
            returncode=0,
            stdout="candidate-pcdump" if staged else "baseline-pcdump",
            stderr="",
            cmd=["ssh"],
            host=kwargs["host"],
            source_rel=kwargs["source_rel"],
            compile_source_rel=kwargs["compile_source_rel"],
            staged_source=kwargs["stage_source_label"] if staged else None,
            staging_ack_confirmed=True if staged else None,
        )

    def fake_signature(pcdump_text, **_kwargs):
        return _sig(reg=30 if pcdump_text == "candidate-pcdump" else 31)
    ...
    monkeypatch.setattr(debugcli, "_run_remote_pcdump", fake_remote)
    monkeypatch.setattr(
        debugcli,
    ...
    assert remote_calls[0]["stage_source_path"] is None
    assert remote_calls[0]["compile_source_rel"] == "src/melee/mn/demo.c"
    assert remote_calls[1]["stage_source_path"] is not None
    assert remote_calls[1]["stage_source_label"].endswith("cand-remote.c")
    assert remote_calls[1]["compile_source_rel"] == "src/melee/mn/demo.c"


def test_solve_node_set_split_remote_candidate_failure_is_blocked(
    monkeypatch,
    tmp_path,
):
    _node_split_repo(tmp_path, monkeypatch)
    ...
    output = tmp_path / "node-set-summary.json"
    ...
    def fake_remote(**kwargs):
        ...
        return debugcli._RemotePcdumpResult(
            ...
            source_rel=kwargs["source_rel"],
            compile_source_rel=kwargs["compile_source_rel"],
            staged_source=kwargs["stage_source_label"],
            stage_source_sha256="abc123",
    ...
    monkeypatch.setattr(
        debugcli,
        "_node_set_split_signature_from_pcdump_text",
        lambda *_args, **_kwargs: _sig(reg=31),
    )
    monkeypatch.setattr(
        debugcli,
        "_score_node_set_split_candidate",
        lambda *_args, **_kwargs: pytest.fail("candidate should not be scored"),
        raising=False,
    )
    ...
    payload = json.loads(result.output)
    assert payload["status"] == "blocked"
    assert payload["stop_reason"] == "remote-retained-source-staging-failed"
    assert payload["terminal_blocker"] == "remote-retained-source-staging-failed"
    assert payload["candidate_id"] == "cand-remote-fails"
    assert payload["remote_fallback"]["returncode"] == 1
    assert payload["candidates"] == []
    assert "--remote --max-candidates 0" in (
        payload["stop_condition"]["resume_command"]
    )
    assert "node-set-summary.json" in payload["stop_condition"]["resume_command"]
...
def test_solve_coloring_abstain_json_includes_force_vector_verify(monkeypatch):
    diagnostics = {
        "force_phys": {"32": 28, "37": 26},
        "force_vector": "class1:ig32:phys=f28,class1:ig37:phys=f26",
        "force_vector_verify": {
            "ran": True,
            "union": {"status": "match", "pcdump": "forced.pcdump.txt"},
        },
        "natural_pcdump": "natural.pcdump.txt",
    }
    monkeypatch.setattr(
        debugcli,
        "_run_solve_coloring",
        lambda **kw: SolveResult(
            exit_code=3,
            reason="force-vector diagnostic abstain",
    ...
    assert result.exit_code == 3, result.output
    payload = json.loads(result.output)
    assert payload["force_vector"] == diagnostics["force_vector"]
    assert payload["force_phys"] == diagnostics["force_phys"]
    assert payload["force_vector_verify"]["union"]["status"] == "match"
    assert payload["natural_pcdump"] == "natural.pcdump.txt"


def test_force_vector_auto_verify_can_retain_union_pcdump(monkeypatch, tmp_path):
    src_path = tmp_path / "sample.c"
    src_path.write_text("void fn_test(void) {}\n", encoding="utf-8")
    entries = debugcli._parse_force_vector("ig40:phys=r30")

    def fake_run(cmd, **_kwargs):
        output_path = Path(cmd[cmd.index("-o") + 1])
        output_path.write_text("retained forced pcdump\n", encoding="utf-8")
    ...
    result = debugcli._run_force_vector_auto_verify(
        src_path=src_path,
        function="fn_test",
        entries=entries,
        melee_root=tmp_path,
    ...
    retained = Path(result["union"]["pcdump"])
    assert retained.exists()
    assert retained.parent == tmp_path / "retained"
    assert result["union"]["retained_pcdump"] is True


def test_force_vector_auto_verify_default_still_cleans_temp_pcdump(
    monkeypatch,
    tmp_path,
):
    src_path = tmp_path / "sample.c"
    ...
    def fake_run(cmd, **_kwargs):
        ...
        output_path.write_text("temporary forced pcdump\n", encoding="utf-8")
        return SimpleNamespace(
            returncode=0,
            stdout="[diff] MATCH - function bytes are identical.\n",
            stderr="",
    ...
    assert result["union"]["status"] == "match"
    assert result["union"]["retained_pcdump"] is False
    assert "pcdump" not in result["union"]
    assert "forced_pcdump" not in result["union"]
    assert outputs and all(not path.exists() for path in outputs)


def test_solve_force_vector_diagnostics_drops_unretained_temp_pcdumps(tmp_path):
    natural = tmp_path / "natural.pcdump.txt"
    retained = tmp_path / "retained.pcdump.txt"
    payload = debugcli._solve_force_vector_diagnostics(
        class_id=0,
        phys_target={40: 30},
        force_vector_probe={
            "union": {
                "status": "match",
                "pcdump": str(tmp_path / "deleted-union.pcdump.txt"),
                "forced_pcdump": str(tmp_path / "deleted-union.pcdump.txt"),
            },
            "probes": [
                {
                    "status": "match",
                    "pcdump": str(tmp_path / "deleted-single.pcdump.txt"),
                },
                {
                    "status": "match",
                    "retained_pcdump": True,
                    "pcdump": str(retained),
                    "forced_pcdump": str(retained),
                },
            ],
        },
        natural_pcdump=natural,
    )

    assert payload is not None
    verify = payload["force_vector_verify"]
    assert "pcdump" not in verify["union"]
    assert "forced_pcdump" not in verify["union"]
    assert "pcdump" not in verify["probes"][0]
    assert verify["probes"][1]["pcdump"] == str(retained)
    assert payload["natural_pcdump"] == str(natural)


def test_force_vector_auto_verify_retain_request_omits_nonmatch_temp_pcdump(
    monkeypatch,
    tmp_path,
):
    src_path = tmp_path / "sample.c"
    ...
    def fake_run(cmd, **_kwargs):
        output_path = Path(cmd[cmd.index("-o") + 1])
        output_path.write_text("temporary no-match pcdump\n", encoding="utf-8")
        return SimpleNamespace(
            returncode=0,
            stdout="[diff] MISMATCH - function bytes differ.\n",
            stderr="",
        )
    ...
    assert result["union"]["status"] == "no_match"
    assert result["union"]["retained_pcdump"] is False
    assert "pcdump" not in result["union"]
    assert not list((tmp_path / "retained").glob("*.pcdump.txt"))
...
def test_run_solve_coloring_prefers_retained_fresh_natural_pcdump(
    monkeypatch,
    tmp_path,
):
    ...
    source.parent.mkdir(parents=True)
    source.write_text("void f(void) {}\n", encoding="utf-8")
    stale_pcdump = tmp_path / "stale-cache.pcdump.txt"
    stale_pcdump.write_text("Starting function f\n", encoding="utf-8")
    fresh_natural = tmp_path / "fresh-natural.pcdump.txt"
    fresh_natural.write_text("Starting function f\n", encoding="utf-8")
    ...
    monkeypatch.setattr(
        debugcli,
        "_resolve_pcdump_path",
        lambda pcdump_arg, function, melee_root: stale_pcdump,
    )
    monkeypatch.setattr(
        debugcli,
        "_load_checkdiff_normalized_structural_lines",
    ...
    monkeypatch.setattr(
        ...
        lambda **_kwargs: DeriveInputs(
            function="f",
            unit="melee/mn/demo",
            class_id=0,
            checkdiff_primary="backend-ceiling",
            phys_target={40: 30},
            phys_conflicts=[],
            force_iter_first=[40],
            applied_positions={},
            ...
            baseline_ig_set=set(),
            forced_ig_set=set(),
            self_reanchored_roles=set(),
            unscored_roles=[],
            forced_decisions_sha256=[],
            baseline_source_sha256="source",
            baseline_pcdump_sha256="natural",
            direct_evidence_register_only=True,
            force_vector_probe={
                "union": {
                    "status": "no_match",
                    "entries": [{"raw": "class0:ig40:iter-first"}],
                },
                "probe_count": 1,
            },
            natural_pcdump=str(fresh_natural),
        ),
    )

    result = debugcli._run_solve_coloring(
        function="f",
        class_id=0,
        pcdump=None,
        max_perturb=2,
    ...
    assert result.solver_diagnostics["natural_pcdump"] == str(fresh_natural)
    assert result.solver_diagnostics["natural_pcdump"] != str(stale_pcdump)
    assert result.solver_diagnostics["force_vector_verify"]["union"]["status"] == (
        "no_match"
    )
