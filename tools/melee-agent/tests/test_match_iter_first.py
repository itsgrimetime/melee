...
def test_force_phys_from_diff_prefers_near_position_over_distant_current_reg() -> None:
    target_asm = [
        "<fn_test>:",
        "+000: 7c 08 02 a6 \tmflr    r0",
        "+004: 94 21 ff e0 \tstwu    r1,-32(r1)",
        "+008: 3b 20 00 00 \tli      r25,0",
    ]
    current_asm = [
        "<fn_test>:",
        "+000: 7c 08 02 a6 \tmflr    r0",
        "+004: 94 21 ff e0 \tstwu    r1,-32(r1)",
        "+008: 3a e0 00 00 \tli      r23,0",
    ]
    far_filler = [
        Instruction(
            opcode="mr",
            operands=f"r{200 + index},r3",
            annotations=[],
            regs=[("r", 200 + index), ("r", 3)],
        )
    ...
    assert vector["force_phys_csv"] == "0:36:25"
    assert vector["targets"][0]["ig_idx"] == 36
...
def test_force_vector_auto_verify_threads_per_probe_timeout(
    monkeypatch,
    tmp_path: pathlib.Path,
) -> None:
    # #620: each force-vector probe build must be bounded by a per-probe
    # wall-clock timeout so a hung mwcc/wibo child cannot stall the union build
    # indefinitely. The runner already supports timeout_s; the auto-verify
    # wrapper must PASS it for every probe (union + diagnostics).
    src_path = tmp_path / "sample.c"
    src_path.write_text("void fn_test(void) {}\n", encoding="utf-8")
    entries = debug_cli._parse_force_vector("ig40:phys=r30,ig50:iter-first")
    seen_timeouts: list = []

    def fake_run(cmd, *, cwd, status_label, phase="testing",
                 status_interval_s=10.0, timeout_s=None, env=None):
        seen_timeouts.append(timeout_s)
        return subprocess.CompletedProcess(cmd, 0, "diff remained\n", "")

    monkeypatch.setattr(
        debug_cli, "_run_auto_verify_command_with_status", fake_run)
    ...
    debug_cli._run_force_vector_auto_verify(
        ...
        checkdiff_timeout=30.0,
        run_diagnostic_probes=True,
        per_probe_timeout_s=42.0,
    )

    # union + single[1] + single[2] = 3 probes, each bounded.
    assert seen_timeouts == [42.0, 42.0, 42.0]


def test_force_vector_probe_payload_records_timeout_as_inconclusive(
    tmp_path: pathlib.Path,
) -> None:
    # #620: a probe that timed out (the runner returns rc=124) must be recorded
    # as INCONCLUSIVE (timed_out=True), NOT a real `failed` / `no_match`, so the
    # collector can continue/abstain cleanly instead of trusting a stalled build.
    entries = debug_cli._parse_force_vector("ig40:phys=r30")
    proc = subprocess.CompletedProcess(
        ["x"], 124, "", "[auto-verify] testing timed out after 42s\n")
    payload = debug_cli._force_vector_probe_payload(
        label="union", entries=entries, proc=proc,
        output_path=tmp_path / "x.pcdump.txt", ordinal=None)
    assert payload["status"] == "inconclusive"
    assert payload["timed_out"] is True
    assert payload["match"] is False


def test_force_vector_auto_verify_union_timeout_is_not_a_match(
    monkeypatch,
    tmp_path: pathlib.Path,
) -> None:
    # End-to-end over the wrapper: a union build that times out yields a union
    # probe with status inconclusive and match False (the solve path then sees
    # forced_class_clean=False and abstains cleanly rather than hanging).
    src_path = tmp_path / "sample.c"
    src_path.write_text("void fn_test(void) {}\n", encoding="utf-8")
    entries = debug_cli._parse_force_vector("ig40:phys=r30")
    ...
    result = debug_cli._run_force_vector_auto_verify(
        src_path=src_path, function="fn_test", entries=entries,
        melee_root=tmp_path, checkdiff_timeout=30.0,
        run_diagnostic_probes=False, per_probe_timeout_s=1.0)

    assert result["union"]["status"] == "inconclusive"
    assert result["union"]["timed_out"] is True
    assert result["union"]["match"] is False
