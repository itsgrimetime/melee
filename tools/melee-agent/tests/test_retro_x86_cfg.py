...
def nested_argument_readable_global_effect_image(
    *,
    ...
    """Publish one object reached through a caller argument's pointer field."""
    from tools.mwcc_retro import pe as pe_mod
    ...
    emit(cursor, "a3 00 30 40 00 c3")

    cursor = emit(0xE0, "b8 ef be ad de 50")
    cursor = emit_call(cursor, 0x40)
    emit(cursor, "59 c3")
...
def paired_global_fresh_publication_image(
    *,
    ...
    checked_allocation=True,
    bypass_dereference=False,
):
    ...
    cursor = emit(cursor, "59")
    failure_branch = None
    dereference_bypass = None
    if checked_allocation:
        cursor = emit(cursor, "85 c0 74 00")
        failure_branch = cursor - 2
    elif bypass_dereference:
        cursor = emit(cursor, "83 3d 08 30 40 00 00 74 00")
        dereference_bypass = cursor - 2
    ...
    if dereference_bypass is not None:
        text[dereference_bypass + 1] = (
            cursor - (dereference_bypass + 2)
        ) & 0xFF
...
def global_append_tail_pointee_image(*, mutation=None):
    ...
    assert mutation in {
        ...
        "unselected-clone",
        "selected-clone",
        "selected-clone-finite-payload",
        "selected-clone-finite-payload-rewrite",
        "selected-clone-unknown-payload-rewrite",
        ...
        "distinct-association-field",
        "fault-barrier-bypass",
        "fault-barrier-only",
        "missing-link-zero",
        "postpublication-global-reload",
        "postpublication-global-reload-after-call",
        "postpublication-global-reload-after-replacement",
        "relocated-reader-finite-association",
    }
    text_va = 0x00401000
    rdata_va = 0x00402000
    data_va = 0x00403000
    ...
    def emit_producer(offset, tag):
        ...
        cursor = emit(cursor, "8d 04 24")
        if mutation == "argument-discriminator":
            cursor = emit(cursor, f"6a {tag:02x}")
        cursor = emit(cursor, "50")
        append_target = (
            0x260
            if (
        ...
        if mutation == "foreign-global-write" and tag == 0:
            cursor = emit_call(cursor, 0x3E0)
        if (
            mutation == "relocated-reader-finite-association"
            and tag == 0
        ):
            cursor = emit_call(cursor, 0x3A0)
        cursor = emit(cursor, "8d 44 24 08 50")
    ...
    cursor = emit_call(cursor, 0x300)
    cursor = emit(cursor, "59 89 c3")
    failure_branch = None
    bypass_dereference_branch = None
    if mutation == "fault-barrier-bypass":
        cursor = emit(cursor, "80 3d 00 30 40 00 00")
        bypass_dereference_branch = cursor
        cursor = emit(cursor, "74 00")
    elif mutation != "fault-barrier-only":
        cursor = emit(cursor, "85 db")
        failure_branch = cursor
        cursor = emit(cursor, "74 00")
    initial_tag = (
        6
    ...
    if mutation != "missing-link-zero":
        cursor = emit(cursor, "c7 03 00 00 00 00")
    if mutation == "argument-discriminator":
        cursor = emit(cursor, "8b 4c 24 0c 88 4b 04")
    elif mutation == "unknown-discriminator":
        cursor = emit(cursor, "88 53 04")
    else:
        cursor = emit(cursor, f"c6 43 04 {initial_tag:02x}")
    if bypass_dereference_branch is not None:
        text[bypass_dereference_branch + 1] = (
            cursor - (bypass_dereference_branch + 2)
        )
    cursor = emit(cursor, "a1 00 30 40 00")
    if mutation != "missing-link":
    ...
    if mutation in {
        ...
        if mutation == "postpublication-global-reload-after-call":
            cursor = emit_call(cursor, 0x3C0)
        elif (
            mutation
            == "postpublication-global-reload-after-replacement"
        ):
            cursor = emit(cursor, "89 15 00 30 40 00")
        cursor = emit(
            cursor,
            "a1 00 30 40 00 8b 54 24 08 89 50 0a",
        )
    elif mutation in {
        "unknown-payload",
        "unselected-clone",
    ...
    if mutation in {
        ...
        cursor = emit(cursor, "8b 44 24 08 89 43 12")
    ...
    if mutation in {
        ...
        if mutation == "selected-clone-finite-payload-rewrite":
            cursor = emit(cursor, "8b 44 24 08 89 42 0a")
        elif mutation == "selected-clone-unknown-payload-rewrite":
            cursor = emit(cursor, "89 4a 0a")
    cursor = emit(cursor, "5b c3")
    failure = cursor
    emit(cursor, "eb fe")
    if failure_branch is not None:
    ...
    elif mutation == "relocated-reader-finite-association":
        cursor = emit(0x3A0, "a1 00 30 40 00 50")
        cursor = emit_call(cursor, 0x3E0)
        emit(cursor, "59 c3")
    emit(0x3C0, "c3")
    if mutation == "relocated-reader-finite-association":
        cursor = emit(0x3E0, "56 8b 74 24 08 6a 04")
        cursor = emit_call(cursor, 0x300)
        cursor = emit(cursor, "59 85 c0")
        allocation_failure_branch = cursor
        cursor = emit(cursor, "74 00 c6 00 09 89 46 12 5e c3")
        allocation_failure = cursor
        emit(cursor, "eb fe")
        text[allocation_failure_branch + 1] = (
            allocation_failure - (allocation_failure_branch + 2)
        )
    else:
    ...
    image = pe_mod.Image(
        ...
        + (
            (
                pe_mod.Relocation(text_va + 0x3A1, 3),
            )
            if mutation == "relocated-reader-finite-association"
            else ()
        ),
...
def test_global_append_tail_collects_relocated_reader_callee_effect():
    ...
    assert effect is not None
    assert effect.values == frozenset({9})
    assert effect.requires_input
...
def test_global_append_tail_rejects_unknown_register_discriminator():
    image, _movzx_address, _transfer_address = (
        global_append_tail_pointee_image(
            mutation="unknown-discriminator",
        )
    )
    recovery = _DirectCfgRecovery(
        image,
...
def test_nonzero_validator_closes_same_slot_object_reload(mode):
    ...
    assert result is not None
    assert result[0] == frozenset(range(0x100))
    assert "nonzero-validated-object-reload" in result[1]
...
def test_guarded_exact_call_closes_local_global_object_publication(mode):
    ...
    assert result is not None
    assert result[0] == frozenset(range(0x100))
    assert "exact-call-global-object" in result[1]
...
def test_readable_global_call_infers_nested_argument_requirement():
    ...
    assert result is not None
    assert result[0] == frozenset(range(0x100))
    assert "argument=0;field-path=(8, 0)" in result[1]
...
def test_readable_global_call_accepts_guard_disjoint_nested_requirement():
    ...
    assert result is not None
    assert result[0] == frozenset(range(0x100))
    assert "guard-disjoint-argument-requirement" in result[1]
...
def test_readable_global_call_ignores_guard_disjoint_argument_mutation():
    ...
    assert result is not None
    assert result[0] == frozenset(range(0x100))
    assert "guarded-argument-requirement" in result[1]
...
def multi_alias_callee_argument_effect_image(
    *,
    ...
    return pe_mod.Image(
        ...
        sections=(
            pe_mod.Section(".text", text_va, 0, 0x100, 0x100, 0x60000020),
            pe_mod.Section(".data", data_va, 0x100, 0x100, 0x100, 0xC0000040),
        ),
        imports=(),
        exports=(),
        relocations=(),
...
def scoped_fresh_global_wrapper_publication_image(
    *,
    ...
    """Publish a fresh wrapper only across one closed transformation call."""
    from tools.mwcc_retro import pe as pe_mod
    ...
    cursor = emit(0, "56 8b 74 24 08 6a 1a")
    cursor = emit_call(cursor, 0x100)
    cursor = emit(
        cursor,
        "59"
...
def indexed_object_store_forward_image(mode):
    """Load an object pointer from one exactly matching indexed local store."""
    from tools.mwcc_retro import pe as pe_mod
...
def test_object_byte_follows_dominating_indexed_local_store():
    ...
    assert "indexed-local-field-store" in result[1]
...
def branched_fresh_allocation_image(*, unknown_second_write=False):
    """Merge two guarded fixed-bump allocations before one object read."""
    from tools.mwcc_retro import pe as pe_mod
...
def test_object_byte_merges_branched_fresh_allocations():
    ...
    assert result is not None
    assert result[0] == frozenset({0x2A, 0x2B})
...
@pytest.mark.parametrize(
    ...
    (
        "missing-replacement",
        "mutate-before-replacement",
        "mutate-after-replacement",
    ),
)
...
def test_callee_argument_effect_bounds_recursive_formal_global_alias():
    ...
    assert result is not None
    assert result[0] == frozenset(range(0x100))
    assert "recursive-formal-global-alias-effect" in result[1]
...
def conditional_fresh_wrapper_object_image(*, unknown_wrapper_byte=False):
    """Merge an older fresh object with a wrapper that contains it."""
    from tools.mwcc_retro import pe as pe_mod
...
def test_object_domain_scopes_fresh_origins_to_reaching_definitions():
    ...
    assert result is not None
    assert result[0] == frozenset({0x04, 0x3C})
...
def forwarded_success_object_outparam_image():
    """Forward a guarded success result from a nested outparam writer."""
    from tools.mwcc_retro import pe as pe_mod
    ...
    cursor = emit(
        0,
        "83 ec 20 c6 44 24 08 29 "
        "8d 4c 24 08 8d 44 24 04 51 50",
    )
    cursor = emit_call(cursor, 0x80)
    cursor = emit(cursor, "83 c4 08 84 c0")
    failure_branch = cursor
    cursor = emit(cursor, "74 00")
    observation = text_va + cursor
    cursor = emit(cursor, "8b 44 24 04 83 c4 20 c3")
    failure = cursor
    emit(cursor, "31 c0 83 c4 20 c3")
    text[failure_branch + 1] = failure - (failure_branch + 2)

    cursor = emit(0x80, "8b 44 24 04 8b 4c 24 08 51 50")
    cursor = emit_call(cursor, 0xC0)
    emit(cursor, "83 c4 08 c3")

    cursor = emit(0xC0, "8b 44 24 04 8b 54 24 08 85 d2")
    inner_failure_branch = cursor
...
@pytest.mark.parametrize(
    ...
    (
        (None, True),
        ("payload-clobber", False),
        ("node-call-escape", False),
        ("reader-clobber", False),
        ("returned-context-helper", True),
        ("missing-tail", False),
        ("multiple-node-arms", True),
        ("multiple-restore-loads", True),
        ("nullable-node-arm", True),
        ("optional-restore", True),
        ("wrong-restore-field", False),
    ),
)
...
def test_exact_caller_narrows_open_global_stack_callback_domain(tmp_path):
    ...
    recovery.recover()
    publisher = 0x004010A0
    consumer = 0x00401100
    consumer_call = next(
        call.address
        for call in recovery.direct_calls
        if call.target == consumer
        and recovery._registrar_function_entry(call.address) == publisher
    )
    contexts = (
        (publisher, 0x00401085, 0x00401080),
        (consumer, consumer_call, publisher),
    )
    recovery.producer_exact_call_contexts.extend(contexts)
    try:
        decoded = recovery._owned_decoded(indirect_call)
        result = recovery._finite_operand_values_before(
            indirect_call,
            decoded.operands[0],
            consumer,
            frozenset(),
        )
        effect = recovery._callee_argument_byte_effect(
            consumer,
            x86_cfg_module._GLOBAL_ALIAS_ARGUMENT_INDEX,
            (0,),
            frozenset(),
            global_alias_slots=frozenset({0x00402304}),
        )
    finally:
        for context in reversed(contexts):
            assert recovery.producer_exact_call_contexts.pop() == context
    ...
    assert result is not None
    assert result[0] == frozenset({0x00401150})
    assert "exact-global-stack-context=0x402300" in result[1]
    assert effect is not None
    assert effect[0] == frozenset()


def test_sibling_guard_closes_exact_global_stack_callback_effect(tmp_path):
    image, indirect_call = global_stack_callback_image(
        tmp_path,
        separate_guard=True,
    )
    ...
    recovery.recover()
    recovery.call_targets_by_source.pop(indirect_call, None)
...
def test_zero_global_stack_callback_arm_is_unreachable(tmp_path):
    image, indirect_call = global_stack_callback_image(
        tmp_path,
        missing_guarded_callback=True,
    )
...
def recursive_field_preservation_image(*, hostile=False, aliasing_base=False):
    ...
    return pe.Image(
        ...
        entrypoint=text_va,
        directories=(),
        sections=(pe.Section(".text", text_va, 0, 0x40, 0x40, 0x60000020),),
        imports=(),
        exports=(),
        relocations=(),
...
def test_callee_argument_effect_accepts_closed_post_publication_initializer():
    ...
    assert result is not None
    assert result[0] == frozenset({0x35})
    assert "closed-post-publication-initializer" in result[1]
...
def test_callee_argument_effect_bounds_fresh_pointer_publication_byte():
    ...
    assert result is not None
    assert result[0] == frozenset(range(0x100))
    assert "fresh-allocation-pointer-byte" in result[1]
...
def nested_exact_post_publication_image():
    ...
    cursor = emit(0, "53 8b 5c 24 08 53 6a 35")
    exact_call = text_va + cursor
    cursor = emit_call(cursor, 0x40)
    cursor = emit(cursor, "59 59")
    cursor = emit_call(cursor, 0xC0)
    ...
    cursor = emit(0x40, "53 8b 5c 24 0c 6a 1a")
    cursor = emit_call(cursor, 0x100)
    cursor = emit(cursor, "59 85 c0")
    failure_branch = cursor
    cursor = emit(cursor, "74 00 89 03 8a 54 24 08 88 50 04 5b c3")
    failure = cursor
    emit(cursor, "eb fe")
    patch_short_branch(failure_branch, failure)
...
def test_object_callback_hypothesis_records_receiver_argument_binding(tmp_path, monkeypatch):
    image = load_dispatch_image(tmp_path, mode="object-callback-table")
    data = bytearray(image.data)
    consumer_offset = image.va_to_offset(0x00401060)
    assert consumer_offset is not None
    consumer = bytes.fromhex("8b 44 24 048b 88 04 01 00 0050ff 51 0459c3")
    ...
    assert tuple(consumer.receiver_argument_index for consumer in hypothesis.consumers) == (0,)
...
def test_negative_copy_argument_origin_is_memoized_per_context(
    tmp_path,
    monkeypatch,
):
    image = load_dispatch_image(tmp_path, mode="object-callback-table")
    ...
    recovery.recover()
    original_incoming_call_sites = recovery._incoming_call_sites
    calls = 0

    def counted_incoming_call_sites(function_entry):
        nonlocal calls
        calls += 1
        return original_incoming_call_sites(function_entry)

    monkeypatch.setattr(
        recovery,
        "_incoming_call_sites",
        counted_incoming_call_sites,
    )

    assert not recovery._argument_has_copy_constructor_origin(
        0x004010A0,
        7,
        0x00401090,
        frozenset(),
...
def recursive_copy_argument_origin_image(*, unknown_entry=False):
    """Forward one copy-origin argument around a closed mutual recursion."""
    from tools.mwcc_retro import pe as pe_mod
    ...
    cursor = 0
    if unknown_entry:
        cursor = emit(cursor, "68 34 12 00 00")
    else:
        cursor = emit_call(cursor, 0xC0)
        cursor = emit(cursor, "50")
    cursor = emit_call(cursor, 0x40)
...
def test_copy_argument_origin_closes_recursive_forwarding_cycle():
    image = recursive_copy_argument_origin_image()
    recovery = _DirectCfgRecovery(
        image,
        build_seed_inventory(image, ()),
        generous_limits(image),
...
def test_copy_argument_origin_rejects_recursive_cycle_with_unknown_entry():
    image = recursive_copy_argument_origin_image(unknown_entry=True)
    recovery = _DirectCfgRecovery(
        image,
        build_seed_inventory(image, ()),
        generous_limits(image),
...
def test_copy_origin_definition_order_checks_formal_load_last(tmp_path):
    image = load_cfg_program(
        tmp_path,
        "8b 44 24 04 8b 40 06 c3",
    )
    recovery = _DirectCfgRecovery(
        image,
        inventory(image),
    ...
    definitions = tuple(
        ...
        if recovery.instructions[address].operands
        in {"eax, dword ptr [esp + 4]", "eax, dword ptr [eax + 6]"}
    )

    assert recovery._copy_origin_definition_order(
...
def test_registered_copy_definition_detects_structural_unknown(
    tmp_path,
    field,
    expected,
):
    ...
    definition = next(
        address
        for address, instruction in recovery.instructions.items()
        if instruction.operands
        in {
            f"eax, dword ptr [eax + {field}]",
            f"eax, dword ptr [eax + {field:#x}]",
        }
    )

    assert (
        recovery._registered_copy_definition_is_structurally_unknown(
            definition,
            "eax",
            image.entrypoint,
            ((0x00402000, 16, image.entrypoint),),
        )
        is expected
    )
