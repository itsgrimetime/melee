...
def test_search_skip_first_candidates_after_dedupe_before_compile_count(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path,
) -> None:
    ctx = _ctx(tmp_path)
    baseline = baseline_signature(
    ...
    calls, _alias_calls = _stub_compiles(
        monkeypatch,
        pcdumps_by_text={
            "C": _pcdump_for([32, 42]),
            "D": _pcdump_for([32, 42]),
    ...
    def src(_ctx):
        yield SourceVariant(text="A", provenance="skip-a",
                            parent_baseline=ctx.source_path)
        yield SourceVariant(text="A", provenance="duplicate-a",
                            parent_baseline=ctx.source_path)
        yield SourceVariant(text="B", provenance="skip-b",
                            parent_baseline=ctx.source_path)
        yield SourceVariant(text="C", provenance="compile-c",
                            parent_baseline=ctx.source_path)
    ...
    assert result.candidate_stream_position == 3
...
def test_search_skips_exact_provenance_and_family_without_consuming_limit(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path,
) -> None:
    ctx = _ctx(tmp_path)
    baseline = baseline_signature(
    ...
    calls, _alias_calls = _stub_compiles(
        monkeypatch,
        pcdumps_by_text={
            "KEEP_A": _pcdump_for([32, 42]),
            "KEEP_B": _pcdump_for([32, 42]),
        },
    )

    def src(_ctx):
        yield SourceVariant(text="SKIP_EXACT", provenance="decl-orders sticky",
                            parent_baseline=ctx.source_path)
        yield SourceVariant(text="SKIP_FAMILY",
                            provenance="type-change local: int->unsigned int",
                            parent_baseline=ctx.source_path)
        yield SourceVariant(text="KEEP_A", provenance="holder-lifetime a",
                            parent_baseline=ctx.source_path)
        yield SourceVariant(text="KEEP_B", provenance="insert-alias b",
                            parent_baseline=ctx.source_path)
    ...
    assert calls == ["KEEP_A", "KEEP_B"]
    assert result.total_compiles == 2
    assert result.skipped_count == 2
    assert result.candidate_stream_position == 4
    assert result.skip_reasons == [
...
def test_search_skip_first_lazy_candidate_without_materializing(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path,
) -> None:
    ctx = _ctx(tmp_path)
    baseline = baseline_signature(
    ...
    calls, _alias_calls = _stub_compiles(
        monkeypatch,
        pcdumps_by_text={
            "KEEP": _pcdump_for([32, 42]),
        },
    )
    materialized: list[str] = []

    def sticky_text() -> str:
        materialized.append("sticky")
        raise AssertionError("skipped lazy candidate should not materialize")

    def src(_ctx):
        yield LazySourceVariant(
            provenance="type-change sticky: int->unsigned int",
            parent_baseline=ctx.source_path,
            materialize=sticky_text,
        )
        yield SourceVariant(
            text="KEEP",
            provenance="holder-lifetime keep@0",
            parent_baseline=ctx.source_path,
        )
    ...
    assert result.candidate_stream_position == 2
    assert result.skip_reasons == [
        {
            "provenance": "type-change sticky: int->unsigned int",
            "reason": "skip-first-candidates",
        },
    ]


def test_search_skip_family_lazy_candidates_without_materializing(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path,
) -> None:
    ctx = _ctx(tmp_path)
    baseline = baseline_signature(
    ...
    calls, _alias_calls = _stub_compiles(monkeypatch, pcdumps_by_text={})
    materialized: list[str] = []
    ...
    def src(_ctx):
        yield LazySourceVariant(
            provenance="type-change a: int->unsigned int",
            parent_baseline=ctx.source_path,
            materialize=sticky_text,
        )
        yield LazySourceVariant(
            provenance="type-change b: int->unsigned int",
            parent_baseline=ctx.source_path,
            materialize=sticky_text,
        )
    ...
    assert materialized == []
    assert calls == []
    assert result.total_compiles == 0
    assert result.skipped_count == 2
    assert result.candidate_stream_position == 2
    assert result.skip_reasons == [
        {
...
def test_search_skip_exact_lazy_candidate_without_materializing(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path,
) -> None:
    ctx = _ctx(tmp_path)
    baseline = baseline_signature(
    ...
    materialized: list[str] = []
    sticky_provenance = "type-change sticky: int->unsigned int"
    ...
    result = search(
        ...
        max_candidates=1,
        timeout=10,
        skip_provenances=(sticky_provenance,),
    )
    ...
    assert result.skip_reasons == [
        {
            "provenance": sticky_provenance,
            "reason": "skip-provenance",
        },
    ]


def test_search_interrupt_after_skips_reports_active_candidate(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path,
) -> None:
    ctx = _ctx(tmp_path)
    baseline = baseline_signature(
    ...
    def src(_ctx):
        yield SourceVariant(text="SKIPPED", provenance="skipped-probe",
                            parent_baseline=ctx.source_path)
        yield SourceVariant(text="INTERRUPT", provenance="interrupting-probe",
                            parent_baseline=ctx.source_path)
    ...
    assert result.abort_reason == "keyboard-interrupt"
    assert result.last_provenance == "interrupting-probe"
    assert result.total_compiles == 1
    assert result.skipped_count == 1
    assert result.candidate_stream_position == 2


def test_search_interrupt_during_next_variant_reports_previous_candidate(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path,
) -> None:
    ctx = _ctx(tmp_path)
    baseline = baseline_signature(
    ...
    assert result.aborted is True
    assert result.abort_reason == "keyboard-interrupt"
    assert result.last_provenance == "compiled-probe"
    assert result.total_compiles == 1
    assert result.candidate_stream_position == 1
...
def test_search_rejects_force_phys_candidate_that_loses_protected_assignment(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path,
) -> None:
    ctx = _ctx(tmp_path)
    baseline = baseline_signature(
    ...
    _stub_compiles(
        monkeypatch,
        pcdumps_by_text={
            "LOSES_PROTECTED": _pcdump_for(
                [-1, -1],
                assigned_by_ig={34: 27, 44: 24},
            ),
            "KEEPS_PROTECTED": _pcdump_for(
                [-1, -1],
                assigned_by_ig={34: 27, 44: 25},
            ),
        },
    )
    ...
    def src(_ctx):
        yield SourceVariant(
            text="LOSES_PROTECTED",
            provenance="loses-ig44",
            parent_baseline=ctx.source_path,
        )
        yield SourceVariant(
            text="KEEPS_PROTECTED",
            provenance="keeps-ig44",
            parent_baseline=ctx.source_path,
        )
    ...
    assert "protected force-phys lost" in result.gate_rejection_reasons[0]
    assert result.exact_match is not None
    assert result.exact_match.provenance == "keeps-ig44"
    assert [scored.variant.provenance for scored in result.scored_candidates] == [
        "keeps-ig44"
    ]


def test_search_treats_residual_force_phys_distance_as_progress(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path,
) -> None:
    ctx = _ctx(tmp_path)
    baseline = baseline_signature(
    ...
    _stub_compiles(
        monkeypatch,
        pcdumps_by_text={
            "UNCHANGED": _pcdump_for(
                [-1, -1],
                assigned_by_ig={34: 29, 44: 25},
            ),
            "CLOSER": _pcdump_for(
                [-1, -1],
                assigned_by_ig={34: 28, 44: 25},
            ),
            "EXACT": _pcdump_for(
                [-1, -1],
                assigned_by_ig={34: 27, 44: 25},
    ...
    scored = {entry.variant.provenance: entry for entry in result.scored_candidates}
    assert scored["ig34-r29"].score.assignment_distance_total == 2
    assert scored["ig34-r28"].score.assignment_distance_total == 1
    assert scored["ig34-r27"].score.assignment_distance_total == 0
    assert scored["ig34-r28"].score.assignment_improved_count == 1
    assert result.progress[0].variant.provenance == "ig34-r28"
    assert result.exact_match is not None
    assert result.exact_match.provenance == "ig34-r27"


def test_search_passes_compile_unit_source_for_retained_candidates(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path,
) -> None:
    unit_source = tmp_path / "src" / "melee" / "mn" / "fn_test.c"
    unit_source.parent.mkdir(parents=True)
    unit_source.write_text("void fn_test(void) {}\n", encoding="utf-8")
    retained = tmp_path / "retained.c"
    retained.write_text("void fn_test(void) {}\n", encoding="utf-8")
    ctx = FunctionContext(
        function="fn_test",
        unit="melee/mn/fn_test",
        source_path=retained,
        melee_root=tmp_path,
        compile_unit_source=unit_source,
    )
    baseline = baseline_signature(
        _events_for_class(
            simplify_order=[-1, -1],
    ...
    seen_unit_sources: list[Path | None] = []
    ...
    def fake_compile(
        ...
        *,
        ...
        seen_unit_sources.append(unit_source)
        seen_paths.append(Path(diff_input.path))
        return _pcdump_for([-1, -1], assigned_by_ig={34: 27, 44: 25})
    ...
    assert result.exact_match is not None
    assert seen_unit_sources == [unit_source]
    assert seen_paths[0].is_relative_to(
        tmp_path / "build" / "mwcc_debug_cache" / "probes" / "simplify-order"
    )


def test_search_returns_partial_result_on_keyboard_interrupt(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path,
) -> None:
    ctx = _ctx(tmp_path)
    baseline = baseline_signature(
    ...
    def fake_compile(
        ...
        *,
        ...
        if text == "INTERRUPT":
            raise KeyboardInterrupt
        return _pcdump_for([32, 42], assigned_by_ig={34: 28, 44: 25})
    ...
    def src(_ctx):
        yield SourceVariant(
            text="SCORED",
            provenance="scored-before-interrupt",
            parent_baseline=ctx.source_path,
        )
        yield SourceVariant(
            text="INTERRUPT",
            provenance="interrupting-probe",
            parent_baseline=ctx.source_path,
        )
    ...
    assert result.last_provenance == "interrupting-probe"
    assert result.total_compiles == 2
    assert [entry.variant.provenance for entry in result.scored_candidates] == [
        "scored-before-interrupt"
    ]
