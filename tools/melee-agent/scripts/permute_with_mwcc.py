...
the standard objdiff byte-distance score with `melee-agent debug target score-dump`
...
2. Monkey-patch `Scorer.score` to:
   ...
   - If MWCC_DEBUG_TARGET + MWCC_DEBUG_FN + MWCC_DEBUG_UNIT are set:
     ...
     `melee-agent debug target score-source --cflags-from <unit>`, blend.
   - Otherwise pass through objdiff score.

Env vars:
    MELEE_PERMUTER_ROOT   where decomp-permuter is checked out
                          (default: ~/code/decomp-permuter)
    MWCC_DEBUG_TARGET     path to target spec (from debug target derive)
...
Usage: invoked by `melee-agent debug permute run -f <fn>`. Can also be run
directly:
...
def _looks_like_permuter_root(path: Path) -> bool:
...
def _score_candidate(c_file: str) -> int:
    """Run `melee-agent debug target score-source` on a candidate.
