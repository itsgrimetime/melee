...
def _count_matching_only_file_exclusions(
    functions,
    *,
    min_match: float,
    max_match: float,
    ...
    """Count visible report-backed functions hidden only by --matching-only."""
    count = 0
    for f in functions:
        ...
        if module and f"/{module}/" not in f.file_path.lower():
            continue
        if is_excluded_subdir(f.file_path):
            continue
        if not matches_file_filter(f.file_path):
            continue
        if f.object_status != "Matching":
            count += 1
    return count
...
def extract_list(
    ...
    json_out: Annotated[
        bool, typer.Option("--json", help="Emit machine-readable JSON instead of a Rich table")
    ] = False,
):
    ...
    if not json_out and not (melee_root / "build" / "GALE01" / "report.json").exists():
        console.print(
            "[yellow]Warning: report.json not found. Run 'ninja build/GALE01/report.json' to generate it.[/yellow]"
        )
    elif not json_out and report_parser.is_report_stale(max_age_hours=168):  # 1 week
    ...
    if not json_out:
        newer_sources = report_parser.newer_sources_than_report(limit=3)
        if newer_sources:
            first = newer_sources[0].relative_to(melee_root)
            extra = (
                f" (+{len(newer_sources) - 1} more)"
                if len(newer_sources) > 1
                else ""
            )
            console.print(
                f"[yellow]Warning: report.json is older than {first}{extra}; "
                "match percentages may be stale. Run "
                "'ninja build/GALE01/report.json' to refresh.[/yellow]"
            )
    ...
    if json_out:
        payload = {
            ...
            "filters": {
                ...
                "exclude_subdir": exclude_subdir or [],
                "file": file_filter,
            },
            "functions": [
                {
                    ...
                    "match": func.current_match,
                    "match_percent": func.current_match * 100.0,
                    "size_bytes": func.size_bytes,
                    "address": func.address,
                    "object_status": func.object_status,
    ...
    if len(functions) == 0 and result.total_functions > 0:
        if matching_only and file_filter:
            hidden_by_matching_only = _count_matching_only_file_exclusions(
                result.functions,
                min_match=min_match,
                max_match=max_match,
                min_size=min_size,
                ...
                merged=merged,
                module=module,
                is_excluded_subdir=_is_excluded_subdir,
                matches_file_filter=_matches_file_filter,
            )
            if hidden_by_matching_only:
                console.print(
...
def _run_local_m2c(function_name: str, melee_root: Path, timeout: int = 240) -> tuple[str | None, str]:
    """Run the local m2c first-pass decompiler (tools/decomp.py) for a function.

    Returns (c_code, "") on success or (None, reason) on failure. Never raises:
    a missing m2c install or a stale build/ctx.c is reported as a message so
    callers can degrade gracefully instead of aborting the whole command.
    """
    decomp_py = Path(melee_root) / "tools" / "decomp.py"
    if not decomp_py.exists():
        return None, f"tools/decomp.py not found under {melee_root}"
    try:
        # --no-copy MUST come before the function name: decomp.py collects
        # trailing args into an argparse.REMAINDER and forwards them to m2c,
        # which would reject --no-copy.
        proc = subprocess.run(
            [sys.executable, str(decomp_py), "--no-copy", function_name],
            cwd=str(melee_root),
            capture_output=True,
            text=True,
            timeout=timeout,
        ...
        return None, f"local m2c timed out after {timeout}s"
    except OSError as exc:
        return None, f"could not launch local m2c: {exc}"
    if proc.returncode != 0:
        detail = (proc.stderr or proc.stdout or "").strip().splitlines()
        tail = detail[-1] if detail else f"exit code {proc.returncode}"
        return None, f"local m2c failed ({tail}); ensure m2c is installed and build/ctx.c is current"
    return proc.stdout, ""
...
def _print_no_server_guidance(func, *, already_ran_local_c: bool) -> None:
    ...
    console.print("")
    console.print(f"[yellow]No decomp.me server found (tried: {tried}).[/yellow]")
    console.print("[bold]The supported local workflow does not need a server:[/bold]")
    console.print(f"  1. Edit the source directly:  src/{func.file_path}")
    console.print(f"  2. Check your diff:           python tools/checkdiff.py {func.name}")
    console.print("  3. Build to verify:           python configure.py && ninja")
    if not already_ran_local_c:
        console.print(
            f"  - For first-pass C (no server): re-run with --decompile-c "
            f"(or run: python tools/decomp.py {func.name})"
        )
    console.print("[dim]If a decomp.me server IS available, set DECOMP_API_BASE=http://<host> and retry.[/dim]")
...
def extract_get(
    ...
    decompile_c: Annotated[
        bool,
        typer.Option(
            "--decompile-c",
            help="Also run local m2c (tools/decomp.py) and print first-pass C. Works without a decomp.me server.",
        ),
    ] = False,
):
    ...
    Use --decompile-c to print a local m2c first-pass decompilation (no server needed).
    """
    # First pass: use default melee root to look up function info
    initial_root = melee_root or DEFAULT_MELEE_ROOT

    # Auto-detect API URL if creating a scratch. A missing server is NOT fatal:
    # the supported local /decomp workflow does not need decomp.me, so we still
    # extract and print the ASM/context below, then show actionable guidance
    # rather than dead-ending the agent (issues #21/#23/#85).
    ...
    # First-pass C via local m2c (no decomp.me server required).
    if decompile_c:
        console.print("\n[bold]First-pass C (local m2c):[/bold]")
        c_code, err = _run_local_m2c(func.name, melee_root)
        if c_code:
            console.print(c_code, markup=False, highlight=False)
        else:
            console.print(f"[yellow]First-pass C unavailable: {err}[/yellow]")

    # Create scratch if requested
    if create_scratch:
        if not api_url:
            # No server reachable: the local /decomp workflow is the supported
            # path, so guide the agent there instead of failing (issues #21/#23/#85).
            _print_no_server_guidance(func, already_ran_local_c=decompile_c)
            return
