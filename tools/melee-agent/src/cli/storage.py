...
def get_context_file(source_file: str | None = None, melee_root: Path | None = None) -> Path:
    ...
    If source_file is provided, we return the corresponding .ctx file path (it may need to be built).
    ...
    # If source_file provided, return per-file .ctx path (may need to be built via ninja)
    if source_file:
        ...
        # Return the expected path even if it doesn't exist - caller will build it
        return ctx_path

    # Legacy fallback (only when source_file not provided): consolidated ctx.c
