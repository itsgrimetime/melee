...
def _build_stripped_context(function_name, func, melee_root, context_file):
    ...
    # Strip the target function's definition (keeping its declaration and any
    # call sites) to avoid redefinition errors. Delegate to the shared,
    # tree-sitter-backed stripper that the extract path uses: the previous
    # inline line-based heuristic mis-classified multi-line declarations and
    # multi-line call sites as definition openers, which mangled the prototype
    # and corrupted the calling functions' bodies (leaving dangling fragments)
    # so the context no longer compiled.
    if function_name in melee_context:
        from src.cli.extract import _strip_target_function

        melee_context = _strip_target_function(melee_context, function_name)
...
@scratch_app.command("recover-best")
def scratch_recover_best(
    function_name: Annotated[str, typer.Argument(help="Function name to recover")],
    output_file: Annotated[
        Path | None,
        typer.Option("--output", "-o", help="Write recovered source to this path"),
    ] = None,
    output_json: Annotated[bool, typer.Option("--json", help="Output source, diff, and metadata as JSON")] = False,
    ...
    if attempt is None:
        message = f"No retained source snapshot for {function_name}"
        if output_json:
    ...
    source_code = attempt.get("source_code", "")
    diff = attempt.get("diff", "")
    payload = {
        "success": True,
        "function": function_name,
        "attempt_index": attempt.get("index"),
        "match_percent": attempt.get("match_percent"),
        "timestamp_utc": attempt.get("timestamp_utc"),
        "fingerprint": attempt.get("fingerprint", ""),
        "fingerprint_norm": attempt.get("fingerprint_norm", ""),
        "source_file": attempt.get("source_file", ""),
    ...
    console.print(
        f"[green]Recovered[/green] {function_name} attempt "
    ...
    if show_diff and diff:
        console.print("\n[bold]Retained compact diff:[/bold]")
...
def scratch_sync_from_repo(
    ...
    async def sync():
        ...
        if ctx_path.exists():
            ...
            # Strip the target function's definition (keeping its declaration
            # and call sites) via the shared tree-sitter-backed stripper. The
            # previous inline line-based heuristic mangled multi-line
            # declarations and multi-line call sites.
            if function_name in fresh_context:
                from src.cli.extract import _strip_target_function

                fresh_context = _strip_target_function(fresh_context, function_name)
