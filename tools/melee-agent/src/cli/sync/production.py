...
def production_command(
    ...
    local_url: Annotated[
        str | None, typer.Option("--local-url", help="Local decomp.me instance URL (auto-detected if not specified)")
    ] = None,
    min_match: Annotated[float, typer.Option("--min-match", help="Minimum match percentage to sync")] = 0.0,
    limit: Annotated[int, typer.Option("--limit", "-n", help="Maximum scratches to sync")] = 10,
    dry_run: Annotated[bool, typer.Option("--dry-run", help="Show what would be synced without syncing")] = False,
    force: Annotated[bool, typer.Option("--force", "-f", help="Re-sync even if already exists on production")] = False,
    function: Annotated[str | None, typer.Option("--function", help="Only sync this specific function")] = None,
    slug: Annotated[str | None, typer.Option("--slug", "-s", help="Use this specific local scratch slug (requires --function)")] = None,
):
    ...
    """
    # Validate --slug requires --function
    if slug and not function:
        console.print("[red]--slug requires --function to be specified[/red]")
        raise typer.Exit(1)
    ...
    # If --slug is provided, use it directly instead of querying the database
    if slug and function:
        to_sync = [{
            'name': function,
            'slug': slug,
            'match_pct': 100.0,  # Assume 100% when manually specifying
        }]
    else:
        ...
        async with DecompMeAPIClient(base_url=local_url) as local_client:
            ...
            async with httpx.AsyncClient(
                ...
                for entry in to_sync:
                    ...
                    skip_creation = False
                    try:
                        console.print("[dim]  Searching production for existing scratch...[/dim]")
                        ...
                        if search_resp.status_code == 200:
                            ...
                            if exact_match and not force:
                                ...
                                console.print(
                                    f"[yellow]  Found existing 100% match on production: {existing_slug}[/yellow]"
                                )
                                console.print("[dim]  Linking instead of creating (use --force to create anyway)[/dim]")
                                # Record the mapping
                                ...
                                best = existing[0]
                                console.print(
                                    f"[dim]  Found {len(existing)} existing scratch(es), best: {best.get('name')} (score={best.get('score', '?')})[/dim]"
                                )
                    ...
                    try:
                        console.print("[dim]  Fetching local scratch data...[/dim]")
                        local_scratch = await local_client.get_scratch(local_slug)
                        console.print("[dim]  Local scratch fetched[/dim]")
                        ...
                        is_placeholder = (
                            not source_code
                            or len(source_code.strip()) < 50
                            or "TODO" in source_code
                            or source_code.strip().startswith("//")
                        )
                        ...
                        if needs_fresh_context:
                            console.print(
                                "[yellow]  Local scratch has placeholder code, refreshing from repo...[/yellow]"
                            )
                            try:
                                ...
                                if file_path:
                                    ...
                                    if full_path.exists():
                                        ...
                                        if extracted_code:
                                            ...
                                            console.print(
                                                f"[green]  Extracted {len(source_code)} bytes from repo[/green]"
                                            )
                                        else:
                                            console.print("[yellow]  Could not extract function from repo[/yellow]")

                                        # Get fresh context and strip the function
                                        ctx_path = (
                                            melee_root / "build" / "GALE01" / "src" / file_path.replace(".c", ".ctx")
                                        )
                                        if ctx_path.exists():
                                            context = ctx_path.read_text(encoding="utf-8")
                                            original_len = len(context)

                                            # Strip the target function's definition (keeping its
                                            # declaration and call sites) via the shared
                                            # tree-sitter-backed stripper. The previous inline
                                            # line-based heuristic mangled multi-line declarations
                                            # and multi-line call sites.
                                            ...
                                            console.print(
                                                f"[green]  Loaded fresh context ({len(context):,} bytes, stripped {original_len - len(context):,})[/green]"
                                            )
                                        else:
                                            console.print(f"[yellow]  Context file not found: {ctx_path}[/yellow]")
                                            console.print(
                                                f"[dim]  Run 'ninja {ctx_path.relative_to(melee_root)}' to generate[/dim]"
                                            )
                                    else:
                        ...
                        try:
                            ...
                            console.print("[dim]  Exporting target ASM...[/dim]")
                            export_data = await local_client.export_scratch(local_slug, target_only=True)
                            console.print("[dim]  Export complete[/dim]")
