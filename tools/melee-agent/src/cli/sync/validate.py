...
def validate_command(
    local_url: Annotated[
        str | None,
        typer.Option("--local-url", "-u", help="Local decomp.me instance URL (auto-detected if not specified)"),
    ] = None,
    min_match: Annotated[float, typer.Option("--min-match", help="Minimum match percentage to validate")] = 95.0,
    limit: Annotated[int, typer.Option("--limit", "-n", help="Maximum scratches to validate")] = 50,
    fix: Annotated[bool, typer.Option("--fix", help="Automatically fix issues where possible")] = False,
    ...
    async def do_validate():
        ...
        async with DecompMeAPIClient(base_url=local_url) as client:
            for i, candidate in enumerate(candidates):
                ...
                try:
                    scratch = await asyncio.wait_for(client.get_scratch(slug), timeout=10.0)
                    ...
                    if func_name not in scratch.source_code:
                        ...
                        if scratch.name not in scratch.source_code:
                            issues.append(
                                {
                                    "type": "wrong_code",
                                    "message": "Function name not found in scratch code",
                                }
                            )
                        else:
                            issues.append(
        ...
        for entry in results["issues"]:
            ...
            for issue in entry["issues"]:
                ...
                if issue_type == "name_mismatch":
                    console.print(
                        f"  [yellow]• Name mismatch:[/yellow] scratch='{issue['actual']}', expected='{issue['expected']}'"
                    )
                elif issue_type == "wrong_code":
                    console.print("  [red]• Wrong code:[/red] function name not in scratch code")
                    ...
                    console.print(
                        f"  [yellow]• Match differs:[/yellow] recorded {issue['expected']:.1f}% vs actual {issue['actual']:.1f}%"
                    )
    ...
    if results["issues"] and not fix:
        ...
        console.print("[dim]Or manually inspect and fix issues before syncing[/dim]")
...
def dedup_command(
    ...
    min_match: Annotated[float, typer.Option("--min-match", help="Minimum match percentage")] = 95.0,
            ...
            for i, dup in enumerate(duplicates):
                ...
                for slug in slugs:
                    try:
                        ...
                        scratch = await asyncio.wait_for(client.get_scratch(slug), timeout=10.0)
                        match_pct = (
                            (scratch.max_score - scratch.score) / scratch.max_score * 100
                            if scratch.max_score > 0
                            else 0
                        )
                        scratch_info.append(
...
def find_duplicates_command(
    ...
    min_match: Annotated[float, typer.Option("--min-match", help="Minimum match percentage to check")] = 95.0,
    limit: Annotated[int, typer.Option("--limit", "-n", help="Maximum functions to check")] = 100,
    update_db: Annotated[
    ...
    async def search_duplicates():
        ...
        async with httpx.AsyncClient(base_url=local_url, timeout=15.0) as client:
            for i, candidate in enumerate(candidates):
                ...
                if tracked_slug:
                    try:
                        tracked_resp = await asyncio.wait_for(client.get(f"/api/scratch/{tracked_slug}"), timeout=10.0)
                ...
                try:
                    resp = await asyncio.wait_for(
                        client.get("/api/scratch", params={"search": func_name}), timeout=10.0
                    )
                    ...
                    if len(exact_matches) > 1:
                        ...
                        console.print(" [green]1 scratch[/green]")
                    else:
                        console.print(" [dim]0 exact matches[/dim]")
