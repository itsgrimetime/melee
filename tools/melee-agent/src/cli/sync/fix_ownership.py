...
def fix_ownership_command(
    limit: Annotated[int, typer.Option("--limit", "-n", help="Maximum scratches to fix")] = 10,
    dry_run: Annotated[bool, typer.Option("--dry-run", help="Show what would be fixed without making changes")] = False,
    function: Annotated[str | None, typer.Option("--function", help="Only fix this specific function")] = None,
):
            ...
            for candidate in candidates:
                ...
                try:
                    ...
                    if owner is not None:
                        console.print(" [green]already owned[/green]")
                        results["already_owned"] += 1
                        continue

                    console.print(" [yellow]no owner[/yellow]")

                    if dry_run:
                        console.print("  [dim]Would fork and claim[/dim]")
                        results["fixed"] += 1
                        continue

                    # Fork the scratch (creates new scratch with claim token)
                    console.print("  [dim]Forking...[/dim]", end="")
                    fork_resp = await rate_limited_request(client, "post", f"/api/scratch/{prod_slug}/fork", json={})
                    ...
                    if not new_slug or not claim_token:
                        console.print(" [red]missing slug/token in response[/red]")
                        results["failed"] += 1
                        continue
                    ...
                    # Claim the fork
                    console.print("  [dim]Claiming...[/dim]", end="")
                    claim_resp = await rate_limited_request(
                        client, "post", f"/api/scratch/{new_slug}/claim", json={"token": claim_token}
                    )
                    ...
                    if not claim_result.get("success"):
                        console.print(" [red]claim returned success=false[/red]")
                        results["failed"] += 1
                        continue
