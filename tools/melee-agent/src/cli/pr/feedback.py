...
def feedback_command(
    pr_refs: Annotated[list[str], typer.Argument(help="PR number(s) or URL(s) to analyze")],
    include_logs: Annotated[bool, typer.Option("--logs", "-l", help="Include failed check logs")] = False,
...
def _feedback_single_pr(repo: str, pr_number: int, include_logs: bool, output_json: bool) -> dict | None:
    ...
    if decomp_report:
        ...
        if decomp_report.get("delta_bytes") is not None:
            ...
            if delta_bytes > 0:
                console.print(
                    f"  [green]Matched code: {completion:.2f}% (+{delta_pct:.2f}%, +{delta_bytes:,} bytes)[/green]"
                )
            elif delta_bytes < 0:
            ...
            if improvement_count > 0:
                console.print(
                    f"  [cyan]📈 {improvement_count} improvement{'s' if improvement_count > 1 else ''}[/cyan]"
                )
            ...
            console.print(
                f"  Matching bytes: {decomp_report.get('matching_bytes', 0):,} / {decomp_report.get('total_bytes', 0):,}"
            )
    ...
    if merge_status and merge_status.get("state") == "MERGED":
        ...
        if blockers:
            console.print("  [bold red]🚫 NOT ready to merge:[/bold red]")
            for blocker in blockers:
                console.print(f"    - {blocker}")
        elif warnings_list:
            console.print("  [yellow]⚠ Mergeable with warnings:[/yellow]")
