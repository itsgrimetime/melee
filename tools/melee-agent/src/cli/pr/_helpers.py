    ...
    if functions and body:
        ...
        if missing_funcs:
            if len(missing_funcs) == len(functions[:10]):
                warnings.append("Description doesn't mention any matched functions")
            else:
                warnings.append(
                    f"Description missing {len(missing_funcs)} function(s): {', '.join(missing_funcs[:3])}..."
                )
...
def get_failed_check_logs(repo: str, run_id: str, max_lines: int = 100) -> str:
    ...
    try:
        ...
        result = subprocess.run(
            ["gh", "run", "view", run_id, "--repo", repo, "--log-failed"], capture_output=True, text=True, timeout=30
        )
    ...
    try:
        result = subprocess.run(
            [
                ...
                "--jq",
                "[.[] | {body, path, line, user: .user.login, created_at}]",
            ],
            capture_output=True,
            text=True,
            check=True,
...
def parse_decomp_dev_report(body: str) -> dict | None:
    ...
    bytes_match = re.search(
        r"Matching bytes?:\s*([\d,]+)\s*/\s*([\d,]+)\s*\(?([\d.]+)%?\)?", report_text, re.IGNORECASE
    )
    if bytes_match:
    ...
    try:
        result = subprocess.run(
            [
                ...
                "--jq",
                "[.[] | {body, user: .user.login, created_at, updated_at}]",
            ],
            capture_output=True,
            text=True,
            check=True,
