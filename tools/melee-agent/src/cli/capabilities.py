...
def skill_capabilities(
    ...
    if tracked_only:
        tracked = subprocess.run(
            ["git", "-C", str(repo_root), "ls-files", "--", ".claude/skills"],
            capture_output=True,
            text=True,
            check=False,
        )
        if tracked.returncode == 0:
            ...
            skill_paths = [path for path in skill_paths if path in tracked_paths]
...
TASK_ALIASES: dict[str, list[str]] = {
    "dolphin": ["dolphin"],
    "runtime": ["dolphin"],
    "runtime validation": ["dolphin"],
    "controller": ["dolphin"],
    "controller capture": ["dolphin"],
    "capture": ["dolphin"],
    "screenshot": ["dolphin"],
    "game input": ["dolphin"],
...
def all_capabilities(
    repo_root: Path | None = None, *, tracked_skills_only: bool = False
) -> list[Capability]:
    repo_root = repo_root or DEFAULT_MELEE_ROOT
    return command_capabilities() + skill_capabilities(
        repo_root, tracked_only=tracked_skills_only
    )
...
def generate() -> None:
    ...
    caps = all_capabilities(_repo_root(), tracked_skills_only=True)
