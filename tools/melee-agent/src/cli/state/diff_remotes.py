...
def _terminate_process_group(pgid: int) -> None:
    """Terminate a child process group before returning to worktree cleanup."""
    try:
...
def _run_owned_process_group(
    ...
    *,
    ...
    """Run a command in its own process group and kill the whole group on abort."""
    ...
    def abort_handler(signum, frame) -> None:
        terminate_once()
