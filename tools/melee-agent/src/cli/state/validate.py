...
def validate_command(
    ...
    melee_root: Annotated[
        Path | None, typer.Option("--melee-root", "-r", help="Path to melee repo for --verify-git")
    ] = None,
    limit: Annotated[int, typer.Option("--limit", "-n", help="Limit entries to verify")] = 100,
    ...
    for func in all_functions:
        if (
            func.get("local_scratch_slug")
            and func.get("match_percent", 0) > 0
            and not func.get("production_scratch_slug")
        ):
            issues.append(
                {
            ...
            issues.append(
                {
                    ...
                    "severity": "warning",
                    "function": func["function_name"],
                    "message": "PR linked but state unknown",
                    "pr_url": func["pr_url"],
                }
            )
                ...
                async with httpx.AsyncClient(base_url=api_base, timeout=10.0) as client:
                    for i, func in enumerate(funcs_with_scratch):
                        ...
                        if verbose or (i + 1) % 10 == 0 or i == 0:
                            console.print(
                                f"[dim]  Checking ({i + 1}/{len(funcs_with_scratch)}) {name}...[/dim]",
                                end="" if verbose else "\n",
                            )

                        try:
                            resp = await asyncio.wait_for(client.get(f"/api/scratch/{slug}"), timeout=5.0)
                            checked += 1
    ...
    if verify_git:
        ...
        if not report_path.exists():
            ...
            console.print("[dim]Run 'ninja' in melee repo to generate report.json[/dim]")
        else:
            console.print("[dim]Verifying against build report...[/dim]")
            ...
            try:
                ...
                from ..pr import _check_upstream_status, _get_cached_baseline_path

                commit_hash, _, _ = _check_upstream_status(repo_path)
                if commit_hash:
                    baseline_path = _get_cached_baseline_path(commit_hash)
                    if baseline_path.exists():
                        console.print("[dim]Comparing against baseline to find untracked improvements...[/dim]")
                        try:
                        ...
                        console.print("[dim]No baseline report cached - run 'pr describe' first to generate[/dim]")
    ...
    if "missing_local_scratch" in by_type:
        suggestions.append(
            "missing_local_scratch: run with --fix to search, or 'melee-agent extract get <func> --create-scratch'"
        )
