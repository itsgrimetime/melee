...
from ._helpers import find_best_local_scratch, format_age, format_datetime


def status_command(
    function_name: Annotated[str | None, typer.Argument(help="Function name (optional, shows all if omitted)")] = None,
    category: Annotated[str, typer.Option("--category", "-c", help="Filter by category")] = "all",
    ...
    if function_name:
        ...
        if func.get("build_status") == "broken":
            console.print("[bold]Build:[/bold] [yellow]broken[/yellow]")
            if func.get("build_diagnosis"):
                console.print(f"[bold]Diagnosis:[/bold] {func['build_diagnosis']}")
            console.print("[dim]Use /decomp-fixup to resolve build issues[/dim]")
        ...
        if doc_status == "complete":
            console.print("[bold]Docs:[/bold] [green]complete[/green]")
        elif doc_status == "partial":
            console.print("[bold]Docs:[/bold] [yellow]partial[/yellow]")
        elif func.get("is_documented"):
            console.print("[bold]Docs:[/bold] [green]yes[/green]")
...
def history_command(
    function_name: Annotated[str, typer.Argument(help="Function name")],
    limit: Annotated[int, typer.Option("--limit", "-n", help="Maximum entries")] = 20,
    output_json: Annotated[bool, typer.Option("--json", help="Output as JSON")] = False,
):
