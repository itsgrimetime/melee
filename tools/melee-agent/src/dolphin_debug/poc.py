...
def main():
    ...
    try:
        if not args.connect_only:
            ...
            # The readiness connection is Dolphin's only accepted client.
            print("Waiting for GDB readiness and confirmed halt...")
            debugger = launcher.wait_for_gdb_ready(timeout=30.0)
            client = debugger._gdb
        else:
            print(f"\nConnecting to explicit GDB port {args.port}...")
            if not client.connect():
                print("Failed to connect to GDB stub")
                print(f"Set GDBPort = {args.port} in [General] of an isolated Dolphin.ini and launch a game")
                return 1
            if not client.halt():
                print(f"Initial halt not confirmed: {client.last_error}")
                return 1
        ...
        # Run tests
        passed = [test_memory_read(client), test_registers(client), test_breakpoints(client)]
        ...
        print("\n=== POC Complete ===")
        print("For persistent commands, use melee-agent dolphin launch --session /tmp/<new-session>")
        if not all(passed):
            return 1
        ...
        client.disconnect()
        if launcher:
            # A dropped one-shot connection is not a reusable running session.
            launcher.cleanup()
