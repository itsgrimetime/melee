...
class DolphinDebugger:
    ...
    def __init__(
        ...
        # Connection state
        self._gdb = GDBClient(gdb_host, gdb_port)
    ...
    @property
    def _gdb_sock(self):
        return self._gdb.sock

    @property
    def last_error(self):
        return self._gdb.last_error

    @property
    def execution_state(self):
        return self._gdb.state

    @property
    def last_execution(self):
        return dict(self._gdb.last_execution)

    def _connect_gdb(self, timeout: float = 10.0) -> bool:
        return self._gdb.connect(timeout)
    ...
    def _gdb_send(self, cmd: str, timeout: float = 5.0) -> str | None:
        """Compatibility entry point using the single connection-owned reader."""
        return self._gdb._command(cmd, timeout)
        ...
        if self.has_gdb:
            ...
            if resp and not resp.startswith("E"):
                try:
                    ...
                    if len(resp) == length * 2 and len(data) == length:
                        return data
                    self._gdb.last_error = "Invalid memory reply length"
                except ValueError:
                    pass
        return None
    ...
    def continue_execution(self, timeout: float = 60.0) -> str | None:
        return self._gdb.continue_execution(timeout)

    def step(self, timeout: float = 5.0) -> str | None:
        return self._gdb.step(timeout)

    def halt(self, timeout: float = 5.0) -> bool:
        return self._gdb.halt(timeout)
    ...
    def read_registers(self) -> dict | None:
        ...
        if len(data) < 128:
            self._gdb.last_error = "Incomplete GPR reply"
            return None
    ...
    def read_pc(self) -> int | None:
        ...
        if not self.has_gdb:
            return None
        # Dolphin's bulk `g` reply contains GPRs only. PC is register 64.
        response = self._gdb_send("p40")
        if response is None or len(response) != 8:
            return None
        try:
            data = bytes.fromhex(response)
            return int.from_bytes(data, "big") if len(data) == 4 else None
        except ValueError:
            return None
