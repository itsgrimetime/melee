    ...
    *,
    ...
    if not report_path.is_file():
        raise typer.BadParameter(
            "source file was not provided and build/GALE01/report.json "
            "was not found"
        )
    try:
    ...
    raise typer.BadParameter(
        f"could not resolve source for {function!r} from build/GALE01/report.json; "
        "pass --source-file"
    )
...
def structure_cmd(
    ...
    source_file: Annotated[
        ...
        typer.Option(
            ...
            "--source",
            help=(
                "Source file to use for source-based axes. When omitted, "
                "resolved from build/GALE01/report.json."
            ),
        ),
    ] = None,
    axes: Annotated[
    ...
    output_dir: Annotated[
        ...
        typer.Option(
            "--output-dir",
            help=(
                "Directory for generated candidates. Defaults to "
                "build/structure-search/<function>."
            ),
        ),
    ] = None,
...
def plan_transforms_cmd(
    ...
    if scheduler_order_target_path is not None:
        try:
            ...
            typer.echo(f"error: could not read --scheduler-order-target: {exc}", err=True)
    ...
    try:
        ...
        elif scheduler_order_target is not None or function == "mnDiagram2_GetRankedFighter":
            force_phys_map = {}
        else:
            raise ValueError("--directed-force-phys did not contain any entries")
    except ValueError as exc:
...
def run_cmd(
    ...
    if directed_force_phys_map is not None:
        ...
        directed_source_text = (
            base_seed_text
            if base_seed_text is not None else baseline_source_text
        )
    ...
    *,
    ...
    accounting = {
        **accounting_totals,
        ...
        "source_shape_drained": bool(source_shape_values)
        and all(source_shape_values),
...
def directed_cmd(
    ...
    if proof_force_phys_groups is not None and len(proof_force_phys_groups) > 1:
        ...
        typer.echo(_json.dumps(_aggregate_directed_class_results(
            function=function,
            unit=unit,
