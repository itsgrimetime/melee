...
def _edit_was_order_change(last_lever: str | None) -> bool:
    if last_lever is None:
        return False
    return (
        last_lever in ORDER_CHANGE_MUTATORS
        or last_lever.startswith("transform-corpus:independent_statement_order:")
    )
...
class DirectedScorePipeline:
    ...
    def score_directed(self, art: Any, call: DirectedScoringCall) -> Any:
        ...
        # --- classify progress ---
        eoc = _edit_was_order_change(parent_state.last_lever)
