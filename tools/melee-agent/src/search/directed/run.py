...
def _source_shape_proposal(
    ...
    *,
    ...
    """Return the next untried transform-corpus source-shape proposal."""
...
def _apply_candidate_text_or_mutator(
    key: str,
    anchor,
    source_text: str,
    candidate_text_by_key: dict[str, str],
):
    if key in candidate_text_by_key:
        return candidate_text_by_key[key]
...
def _build_directed_source(
    *,
    ...
    function: str,
    unit: str,
    proof_force_phys: dict[int, int] | None,
):
    """Build the shared diagnosis-first directed source for directed CLI paths."""
    from src.search.directed.source import DirectedSource
    from src.search.types import SourceSpec, SourceVariant

    candidate_text_by_key: dict[str, str] = {}
    diagnosis_by_source_hash: dict[str, Any] = {}
    source_shape_state = {"drained": False}

    def diagnose(current_source_text: str):
        source_hash = hashlib.sha256(current_source_text.encode()).hexdigest()
        if source_hash in diagnosis_by_source_hash:
            return diagnosis_by_source_hash[source_hash]
        try:
            variant = SourceVariant(current_source_text, None)
            art = pcdump_backend.compile(variant, want_pcdump=True)
            diag = None
            if art.status == "ok" and art.pcdump_path is not None:
                pcdump_text = Path(art.pcdump_path).read_text(encoding="utf-8")
                from src.mwcc_debug.role_descriptor import Compile

                compile_obj = Compile.from_text(pcdump_text, function, current_source_text)
                from src.mwcc_debug.convergence import analyze_iteration_full

                state, report, reanchor = analyze_iteration_full(
                    objective.role_target,
                    compile_obj,
            ...
            diag = None
        diagnosis_by_source_hash[source_hash] = diag
        return diag

    def propose(current_source_text: str, tried: frozenset):
        """Try diagnosed levers, then transform-corpus, then blind decl pairs."""
        import re as _re

        from src.search.directed.anchors import resolve_anchor, Anchor

        diag = diagnose(current_source_text)
        ...
        if diag is not None and diag.source_idea is not None:
            ...
            if si.var_name is not None:
                for lever in _DIAGNOSIS_LEVER_ORDER:
                    if lever in tried:
                        continue
                    anchor = resolve_anchor(si, current_source_text)
        ...
        if source_shape is not None:
            key, _anchor, meta = source_shape
            candidate_text = meta.get("candidate_text")
            if isinstance(candidate_text, str):
                candidate_text_by_key[key] = candidate_text
            return source_shape
        source_shape_state["drained"] = True

        # Fallback path (NON-ACTIONABLE / attribution-integrity): when no
        # causal source idea resolves, blind adjacent declaration enumeration
        ...
        matches = list(_DECL_RE.finditer(current_source_text))
        for i in range(len(matches) - 1):
            pair_key = f"reorder_local_decls@{i}"
            if pair_key in tried:
                continue
            m1, m2 = matches[i], matches[i + 1]
            between = current_source_text[m1.end():m2.start()]
            if between.strip():
                continue
            line_start1 = current_source_text.rfind("\n", 0, m1.start()) + 1
            line_end1 = current_source_text.find("\n", m1.end())
            if line_end1 == -1:
                line_end1 = len(current_source_text)
            first_line = current_source_text[line_start1:line_end1]

            line_start2 = current_source_text.rfind("\n", 0, m2.start()) + 1
            line_end2 = current_source_text.find("\n", m2.end())
            if line_end2 == -1:
                line_end2 = len(current_source_text)
            second_line = current_source_text[line_start2:line_end2]

            if first_line + "\n" + second_line not in current_source_text:
                continue

            start = line_start1
            end = line_end2 + 1 if line_end2 < len(current_source_text) else line_end2
            anchor = Anchor(
    ...
    def _apply_fn(key: str, anchor, current_source_text: str):
        return _apply_candidate_text_or_mutator(
            key,
            anchor,
            current_source_text,
    ...
    directed_source.source_shape_state = source_shape_state
    directed_source.seed(SourceSpec(source_text, target))
    return directed_source
...
def run_directed(
    *,
    ...
    Returns
    -------
    dict with keys ``"function"``, ``"unit"``, ``"gate"``,
    ``"directed_telemetry"``, and ``"accounting"``.
    """
...
def _run_live(
    *,
    ...
    directed_source = _build_directed_source(
        source_text=tu_source,
        target=target,
    ...
    source_shape_state = getattr(directed_source, "source_shape_state", {})
    accounting["source_shape_drained"] = bool(
        isinstance(source_shape_state, dict) and source_shape_state.get("drained")
    )
    accounting["context"] = {
