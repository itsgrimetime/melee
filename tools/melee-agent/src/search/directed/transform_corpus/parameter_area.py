...
_OUTGOING_PARAM_KNOWN_ARG_TYPES = {
    "HSD_JObjAddAnim": {
        1: "HSD_AnimJoint*",
        2: "HSD_MatAnimJoint*",
        3: "HSD_ShapeAnimJoint*",
    },
    "HSD_JObjAddAnimAll": {
        1: "HSD_AnimJoint*",
        2: "HSD_MatAnimJoint*",
...
def _known_call_shape_arg_type(callee: str, arg_index: int) -> str | None:
    signature = _OUTGOING_PARAM_KNOWN_ARG_TYPES.get(callee)
    if signature is None:
        return None
    return signature.get(arg_index)
    ...
    *,
    ...
    for arg_index, arg in enumerate(args):
        if arg_index < first_index:
            continue
        type_name = _known_call_shape_arg_type(callee, arg_index)
        if type_name is None:
    ...
    return _CallShapeSite(
        ...
        arg_count=len(args),
        args=tuple(selected),
        wrap_temp_declarations=not _call_shape_can_insert_c89_declaration(
            searchable,
            span,
            statement_start=statement_start,
        ),
...
def _call_shape_statement_with_temps(source_text: str, site: _CallShapeSite) -> str:
    statement = source_text[site.statement_start:site.statement_end]
    statement_start = site.statement_start
    for arg in reversed(site.args):
        start = arg.start - statement_start
        end = arg.end - statement_start
        statement = statement[:start] + arg.temp_name + statement[end:]
    return statement


def _call_shape_indent_statement(statement: str) -> str:
    return "".join(
...
def _call_shape_wrapped_statement(source_text: str, site: _CallShapeSite) -> str:
    inner_indent = f"{site.indent}    "
    declarations = "".join(
        f"{inner_indent}{arg.type_name} {arg.temp_name} = {arg.text};\n"
        for arg in site.args
    )
    statement = _call_shape_indent_statement(
        _call_shape_statement_with_temps(source_text, site)
    )
    return f"{site.indent}{{\n{declarations}{statement}\n{site.indent}}}"
...
def _call_shape_edits(
    ...
    for site in sites:
        if site.wrap_temp_declarations:
            edits.append(
                _contract_make_edit(
                    source_text,
                    start=site.statement_start,
                    end=site.statement_end,
                    replacement_text=_call_shape_wrapped_statement(source_text, site),
                    kind="call-argument-temp-block",
                )
            )
        else:
            declarations = "".join(
...
def _call_shape_enclosing_scope_bounds(
    ...
    return _call_shape_enclosing_scope_bounds_for_offset(
        searchable,
        span,
        before_offset=site.statement_start,
    )


def _call_shape_enclosing_scope_bounds_for_offset(
    searchable: str,
    span,
    *,
    before_offset: int,
    ...
    stack: list[int] = []
    for idx in range(span.body_open, before_offset):
        ch = searchable[idx]
        if ch == "{":
            stack.append(idx)
        elif ch == "}" and stack:
...
def _call_shape_line_is_c89_declaration_prefix(line: str) -> bool:
    stripped = line.strip()
    if not stripped:
        return True
    first_word = re.match(r"[A-Za-z_]\w*", stripped)
    if first_word is not None and first_word.group(0) in {
        "case",
        "default",
        "do",
        "else",
        ...
        return False
    return re.match(
        r"^(?:static\s+|register\s+)?(?:const\s+|volatile\s+)*"
...
def _call_shape_can_insert_c89_declaration(
    searchable: str,
    span,
    *,
    statement_start: int,
) -> bool:
    scope_start, _scope_end = _call_shape_enclosing_scope_bounds_for_offset(
        searchable,
        span,
        before_offset=statement_start,
    )
    prefix = searchable[scope_start:statement_start]
    return all(
        _call_shape_line_is_c89_declaration_prefix(line)
