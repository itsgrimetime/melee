...
DEFAULT_TRANSFORM_FAMILIES: tuple[TransformFamily, ...] = (
    ...
    TransformFamily(
        family_id="pcode_only_gpr_address_temp_repair",
        label="pcode-only GPR address-temp repair",
        mutator_keys=("steer_pcode_only_gpr_address_temp",),
        semantic_risk="medium",
        source_region_selector=(
            "standalone GPR address-temp indexed loads and pointer-copy "
            "owners whose base+index temp only appears in pcode"
        ),
        expected_compiler_effect=(
            "steer pcode-only GPR address-temp and copy-product chains by "
            "splitting indexed loads through explicit source-visible pointer "
            "temps"
        ),
        generated_probe_form=(
            "rewrite one exact indexed load or pointer-copy/indexed-load span "
            "with base-alias, address-temp, or coupled copy/address temps"
        ),
        keywords=(
            "pcode-only",
            "gpr",
    ...
    TransformFamily(
        family_id="pcode_only_gpr_copy_product_case_c_repair",
        label="pcode-only GPR copy-product Case-C repair",
        mutator_keys=("steer_pcode_only_gpr_copy_product_case_c",),
        semantic_risk="medium",
        source_region_selector=(
            "low-confidence pointer locals and pcode-only GPR copy-product "
            "owners whose Case-C simplify/source order is target-only"
        ),
        expected_compiler_effect=(
            "steer pcode-only GPR copy-product Case-C order by splitting or "
            "retargeting source-visible pointer owners without rewrapping "
            "indexed loads"
        ),
        generated_probe_form=(
            "rewrite one exact pointer-owner initialization, loop cursor, or "
            "store-owner span with a source-visible copy-product temp"
        ),
        keywords=(
            "pcode-only",
            "gpr",
    ...
    TransformFamily(
        ...
        label="pcode-only GPR bool/mask temp repair",
        mutator_keys=("steer_pcode_only_gpr_bool_mask_temp",),
        semantic_risk="medium",
        source_region_selector=(
            "unique scalar boolean/mask predicates, negated JObj dirty-flag "
            "mask predicates, and JObj translate setter calls with available "
            "dirty-wrapper spellings"
        ),
        expected_compiler_effect=(
            "repair pcode-only GPR bool/mask source temps by materializing "
            "single-evaluation scalar mask locals or substituting equivalent "
            "dirty-wrapper setter calls"
        ),
        generated_probe_form=(
            "rewrite one exact predicate or translate-call span with a bounded "
            "source-visible mask temp or dirty-wrapper call substitution"
        ),
        keywords=(
            "pcode-only",
            "gpr",
    ...
    TransformFamily(
        family_id="pcode_only_gpr_global_load_lifetime_repair",
        label="pcode-only GPR global-load lifetime repair",
        mutator_keys=("steer_pcode_only_gpr_global_load_lifetime",),
        semantic_risk="medium",
        source_region_selector=(
            "source-visible pointer globals whose loads are pcode-only "
            "COLORGRAPH nodes and whose member/bare uses can be routed "
            "through bounded local aliases"
        ),
        expected_compiler_effect=(
            "materialize a source-visible global pointer alias to change "
            "global-load lifetime, interference, and first-use tie-breaks"
        ),
        generated_probe_form=(
            "insert one typed local alias for a pointer global and rewrite "
            "bounded member/call/alias initialization uses through it"
        ),
        keywords=(
            "pcode-only",
            "gpr",
    ...
    TransformFamily(
        family_id="retained_gpr_case_c_sensitivity_search",
        label="retained-source GPR Case-C sensitivity search",
        mutator_keys=("steer_retained_gpr_case_c_sensitivity",),
        semantic_risk="medium",
        source_region_selector=(
            "retained-source low-confidence GPR pointer locals around Case-C "
            "copy-product loop cursors and stores"
        ),
        expected_compiler_effect=(
            "probe retained-source first-divergence movement by routing the "
            "Case-C loop/store owner through the advisory low-confidence local"
        ),
        generated_probe_form=(
            "rewrite one exact retained Case-C pointer-owner declaration, loop "
            "cursor, or store through the low-confidence pointer local"
        ),
        keywords=(
            "retained-source",
            "gpr",
    ...
    TransformFamily(
        ...
        label="retained-source GPR Case-C window-order continuation",
        mutator_keys=("steer_retained_gpr_case_c_window_order_continuation",),
        semantic_risk="medium",
        source_region_selector=(
            "retained-source GPR Case-C implicit-add window-order leads "
            "materialized from select-order JSON"
        ),
        expected_compiler_effect=(
            "continue retained Case-C IG44/source steering by materializing "
            "ranked indexed-byte source probes for selected force-phys targets "
            "while preserving the remaining protected targets"
        ),
        generated_probe_form=(
            "convert window-order-source-steering LifetimeLayoutProbe sources "
            "from select-order evidence into transform-corpus probe records"
        ),
        keywords=(
            "retained-source",
            "gpr",
    ...
    TransformFamily(
        family_id="retained_gpr_case_c_post_source_owner_backtrack",
        label="retained-source GPR Case-C post-source-owner backtrack",
        mutator_keys=("steer_retained_gpr_case_c_post_source_owner_backtrack",),
        semantic_risk="medium",
        source_region_selector=(
            "retained-source GPR Case-C post-source-owner indexed-byte "
            "source-owner/frontier candidates after the first select-order "
            "owner has been exhausted"
        ),
        expected_compiler_effect=(
            "backtrack from the already-scored current source owner by "
            "materializing later ranked indexed-byte candidates while "
            "preserving protected force-phys targets"
        ),
        generated_probe_form=(
            "convert later window-order-source-steering ranked indexed-byte "
            "LifetimeLayoutProbe sources from select-order evidence into "
            "alternate transform-corpus probe records"
        ),
        keywords=(
            "retained-source",
            "gpr",
    ...
    TransformFamily(
        family_id="retained_gpr_common_subexpr_coalesce_source",
        label="retained-source GPR common-subexpr coalesce source",
        mutator_keys=("steer_retained_gpr_common_subexpr_coalesce_source",),
        semantic_risk="medium",
        source_region_selector=(
            "retained-source GPR coalesce suggestions whose pair members are "
            "computed from the same pre-coloring source virtual"
        ),
        expected_compiler_effect=(
            "materialize a shared C temporary for common-subexpr coalesce "
            "leads so MWCC can reuse one source expression instead of "
            "coloring duplicate virtuals independently"
        ),
        generated_probe_form=(
            "rewrite compatible bridge-variable assignments with a shared "
            "common_subexpr_rA_rB_probe local, or terminalize when no safe "
            "source span exists"
        ),
        keywords=(
            "retained-source",
            ...
            "register-reuse",
            "source-shape",
            "suggest-coalesce-json",
        ),
    ),
    TransformFamily(
        family_id="retained_gpr_case_c_target_live_range_repair",
        label="retained-source GPR Case-C target live-range repair",
        mutator_keys=("steer_retained_gpr_case_c_target_live_range_repair",),
        semantic_risk="medium",
        source_region_selector=(
            "retained-source SortNames blocker-color-chain live-range and interference "
            "spans around sorted_names[j], sorted_names[max_idx], and "
            "GetNameText"
        ),
        expected_compiler_effect=(
            "preserve protected IGs while steering the retained Case-C target "
            "before the current blocker by shrinking blocker live range, shaping target "
            "interference, preserving implicit indexed expressions, or "
            "materializing coupled address/value temps"
        ),
        generated_probe_form=(
            "materialize bounded target-aware source temps for interferer "
            "live-range anchors, target indexed-expression interference "
            "shapes, same-line indexed-expression spellings, index/base "
            "aliases, address-side pointers, duplicated value temps, and "
            "coupled address/value rewrites"
        ),
        keywords=(
            "retained-source",
            "gpr",
    ...
    TransformFamily(
        ...
        label="retained Case-C alternate source-owner discovery",
        mutator_keys=("steer_retained_case_c_alternate_source_owner",),
        semantic_risk="medium",
        source_region_selector=(
            "retained-source Case-C current-owner terminal spans from target "
            "live-range repair artifacts"
        ),
        expected_compiler_effect=(
            "continue after current source-owner exhaustion by materializing "
            "ranked adjacent alternate source-owner C probes while preserving "
            "protected force-phys targets"
        ),
        generated_probe_form=(
            "convert alternate owner graph candidates into target-aware "
            "LifetimeLayoutProbe source edits"
        ),
        keywords=(
            "retained-source",
            "case-c",
            ...
            "pcode-only",
            "stack-reload",
            "current-owner-exhaustion",
        ),
    ),
    TransformFamily(
        family_id="retained_fpr_case_c_target_live_range_repair",
        label="retained-source FPR Case-C target live-range repair",
        mutator_keys=("steer_retained_fpr_case_c_target_live_range_repair",),
        semantic_risk="medium",
        source_region_selector=(
            "retained-source DrawCellNumber FPR live-range and interference "
            "spans around row_offset_adj, row_offset - 0.4f, and col_offset"
        ),
        expected_compiler_effect=(
            "preserve protected FPR IGs while steering retained Case-C target "
            "IG37 toward f26 by anchoring row_offset_adj lifetime, materializing "
            "its fsubs owner expression, reshaping the IG32 col_offset "
            "interferer live range, or changing non-indexed scalar FPR "
            "interference shape"
        ),
        generated_probe_form=(
            "materialize bounded target-aware f32 source temps around retained "
            "row_offset_adj uses, the row_offset_adj owner expression, and "
            "col_offset source uses, plus duplicate scalar and paired "
            "row/column overlap temps with target-score provenance"
        ),
        keywords=(
            "retained-source",
            "fpr",
    ...
    TransformFamily(
        ...
        label="retained-source GPR Case-C simplify-order continuation",
        mutator_keys=("steer_retained_gpr_case_c_simplify_order_continuation",),
        semantic_risk="medium",
        source_region_selector=(
            "retained-source SortNames Case-C simplify-order region "
            "around max_idx, sorted_names[max_idx], sorted_names[j], and "
            "GetNameText"
        ),
        expected_compiler_effect=(
            "preserve protected retained virtuals while steering the active "
            "Case-C IG44 simplify-order/select-order target through local "
            "declaration, reload, scope, and same-line expression variants"
        ),
        generated_probe_form=(
            "materialize bounded retained source probes for max_idx aliases, "
            "retained max-index probe residuals, max/j name reloads, "
            "comparison block scopes, setup-order movement, and same-line "
            "max-index spellings without pointer walks"
        ),
        keywords=(
            "retained-source",
            "gpr",
    ...
    TransformFamily(
        family_id="pcode_only_fpr_fsubs_cast_owner_repair",
        label="pcode-only FPR fsubs/cast-owner repair",
        mutator_keys=("steer_pcode_only_fpr_fsubs_cast_owner",),
        semantic_risk="medium",
        source_region_selector=(
            "standalone FPR cast-owner and subtraction assignments whose "
            "source-visible owner temp needs a pcode-only first-def repair"
        ),
        expected_compiler_effect=(
            "steer pcode-only FPR fsubs/cast-owner temps by splitting a "
            "standalone cast or subtraction assignment through a narrow "
            "source-visible local"
        ),
        generated_probe_form=(
            "rewrite one exact FPR cast/subtraction owner assignment with a "
            "local temp split while preserving surrounding statements"
        ),
        keywords=(
            "pcode-only",
            "fpr",
    ...
    TransformFamily(
        ...
        label="pcode-only FPR call-arg temp repair",
        mutator_keys=("steer_pcode_only_fpr_callarg_temp",),
        semantic_risk="medium",
        source_region_selector=(
            "HSD_JObjReqAnimAll animation-frame conversion whose FPR temp "
            "appears only as pcode copied into physical f1"
        ),
        expected_compiler_effect=(
            "steer pcode-only FPR call-argument conversion temps by "
            "materializing, dematerializing, or reusing narrow source call "
            "argument shapes without touching the surrounding product"
        ),
        generated_probe_form=(
            "rewrite one HSD_JObjReqAnimAll digit-conversion call site with "
            "direct casts, assignment expressions, dead-FPR-local reuse, or "
            "block-scoped call temps"
        ),
        keywords=(
            "pcode-only",
            "fpr",
    ...
    TransformFamily(
        ...
        label="coupled FPR coalesce/product repair",
        mutator_keys=("steer_coupled_fpr_product_callarg_repair",),
        semantic_risk="medium",
        source_region_selector=(
            "mndiagram column FPR product/conversion plus "
            "HSD_JObjReqAnimAll animation-frame conversion"
        ),
        expected_compiler_effect=(
            "jointly steer FPR product/conversion coloring and call-argument "
            "coalescing by materializing coupled source-visible temps"
        ),
        generated_probe_form=(
            "replace one declaration-to-HSD_JObjReqAnimAll span with coupled "
            "column product/cast temps and animation-frame call-arg temps"
        ),
        keywords=(
            "coupled",
            "fpr",
    ...
    TransformFamily(
        family_id="mixed_pcode_fpr_lifetime_pressure_repair",
        label="mixed pcode/FPR lifetime pressure repair",
        mutator_keys=("steer_mixed_pcode_fpr_lifetime_pressure",),
        semantic_risk="medium",
        source_region_selector=(
            "mnDiagram row_offset/row_offset_adj_owner_fpr subtraction plus "
            "HSD_JObjReqAnimAll digit callarg conversion lifetime region"
        ),
        expected_compiler_effect=(
            "repair mixed pcode and FPR lifetime pressure by shortening "
            "row_offset_adj and digit call-argument live ranges that can keep "
            "f25 saved"
        ),
        generated_probe_form=(
            "rewrite one retained row_offset_adj translate/callarg span with "
            "direct adjusted use, branch-local adjusted temps, fresh digit "
            "callarg temps, or a bounded composed candidate"
        ),
        keywords=(
            "mixed-pcode",
            "fpr",
    ...
    TransformFamily(
        ...
        label="callarg-local structural repair",
        mutator_keys=("steer_callarg_local_preserving_structural_repair",),
        semantic_risk="medium",
        source_region_selector=(
            "protected-expression HSD_JObjReqAnimAll digit callarg local "
            "and adjacent mnDiagram row/column FPR structural region"
        ),
        expected_compiler_effect=(
            "preserve or reintroduce source-visible digit FPR call-argument "
            "locals while retaining the structurally better product/count order"
        ),
        generated_probe_form=(
            "replace one exact digit-count/product/row-offset/callarg span "
            "with retained rowf, fresh loop-local, or block-scoped callarg "
            "local variants"
        ),
        keywords=(
            "callarg",
        ...
        if force_phys:
            coloring_cluster = TransformCluster(
                ...
                source_regions=(
                    ...
                    "column product/conversion plus HSD_JObjReqAnimAll digit conversion",
            ...
            TransformCluster(
                ...
                family_ids=(
                    *(
                        ...
                        if function == "mnDiagram_DrawCellNumber"
                        else (
                            "pcode_only_gpr_address_temp_repair",
                            "pcode_only_gpr_copy_product_case_c_repair",
                            "pcode_only_gpr_bool_mask_temp_repair",
                            ...
                            *(
                                ...
                                if function == "mnDiagram_SortNamesByKOs"
                                else ()
                            ),
                            "pcode_only_fpr_fsubs_cast_owner_repair",
                            "pcode_only_fpr_callarg_temp_repair",
