...
def _steer_coupled_fpr_product_callarg_repair(
    anchor: Anchor,
    source_text: str,
) -> Optional[str]:
    """Couple one FPR product split with a HSD_JObjReqAnimAll call-arg temp."""
    return _replace_validated_span(anchor, source_text)


def _steer_pcode_only_fpr_callarg_temp(
    anchor: Anchor,
    source_text: str,
) -> Optional[str]:
    """Materialize or dematerialize one pcode-only FPR call-argument temp."""
    return _replace_validated_span(anchor, source_text)


def _steer_pcode_only_fpr_fsubs_cast_owner(
    anchor: Anchor,
    source_text: str,
) -> Optional[str]:
    """Rewrite one exact FPR fsubs/cast-owner assignment with a local temp."""
    return _replace_validated_span(anchor, source_text)


def _steer_pcode_only_gpr_address_temp(
    anchor: Anchor,
    source_text: str,
) -> Optional[str]:
    """Rewrite one GPR indexed address/copy-product span with explicit temps."""
    return _replace_validated_span(anchor, source_text)


def _steer_pcode_only_gpr_copy_product_case_c(
    anchor: Anchor,
    source_text: str,
) -> Optional[str]:
    """Rewrite one GPR copy-product Case-C owner span with explicit temps."""
    return _replace_validated_span(anchor, source_text)


def _steer_pcode_only_gpr_bool_mask_temp(
    anchor: Anchor,
    source_text: str,
) -> Optional[str]:
    """Rewrite one GPR bool/mask predicate or dirty-wrapper call span."""
    return _replace_validated_span(anchor, source_text)


def _steer_retained_gpr_case_c_sensitivity(
    anchor: Anchor,
    source_text: str,
) -> Optional[str]:
    """Rewrite one retained-source GPR Case-C cursor sensitivity span."""
    return _replace_validated_span(anchor, source_text)


def _steer_mixed_pcode_fpr_lifetime_pressure(
    anchor: Anchor,
    source_text: str,
) -> Optional[str]:
    """Rewrite one mixed row-offset/callarg FPR pressure span."""
    return _replace_validated_span(anchor, source_text)


def _steer_callarg_local_preserving_structural_repair(
    anchor: Anchor,
    source_text: str,
) -> Optional[str]:
    """Rewrite one exact structural span while preserving a callarg local."""
    return _replace_validated_span(anchor, source_text)
...
_DISPATCH = {
    ...
    "steer_coupled_fpr_product_callarg_repair": (
        _steer_coupled_fpr_product_callarg_repair
    ),
    "steer_pcode_only_fpr_callarg_temp": _steer_pcode_only_fpr_callarg_temp,
    "steer_pcode_only_fpr_fsubs_cast_owner": (
        _steer_pcode_only_fpr_fsubs_cast_owner
    ),
    "steer_pcode_only_gpr_address_temp": _steer_pcode_only_gpr_address_temp,
    "steer_pcode_only_gpr_copy_product_case_c": (
        _steer_pcode_only_gpr_copy_product_case_c
    ),
    "steer_pcode_only_gpr_bool_mask_temp": (
        _steer_pcode_only_gpr_bool_mask_temp
    ),
    "steer_retained_gpr_case_c_sensitivity": (
        _steer_retained_gpr_case_c_sensitivity
    ),
    "steer_retained_fpr_case_c_target_live_range_repair": (
        _replace_validated_span
    ),
    "steer_retained_case_c_alternate_source_owner": _replace_validated_span,
    "steer_mixed_pcode_fpr_lifetime_pressure": (
        _steer_mixed_pcode_fpr_lifetime_pressure
    ),
    "steer_callarg_local_preserving_structural_repair": (
        _steer_callarg_local_preserving_structural_repair
    ),
