...
class Preconditions:
    ...
    recoverable_order_collision: bool = False
    node_set_delta: dict | None = None
...
def solve_coloring(*, function: str, class_id: int,
    ...
    if not pre.register_only:
        return _abstain("checkdiff not register-only (admit set: "
                        ...
                        node_set_delta=pre.node_set_delta,
                        solver_diagnostics=pre.solver_diagnostics)
    ...
    if ((pre.force_phys_collision or not pre.reachable)
            and not pre.recoverable_order_collision):
        return _abstain("no register relabel reaches the target for the CURRENT "
                        "source structure (the escape is a structurally different "
                        "virtual). This is NOT a verdict on the function — source "
                        "provably exists. The fix is a STRUCTURAL source change: "
                        "run the cheap-structural-lever checklist + a permuter "
                        "sweep before concluding anything is exhausted "
                        "(docs/matching-tooling-postmortem-2026-06-15.md)",
                        node_set_delta=pre.node_set_delta,
                        solver_diagnostics=pre.solver_diagnostics)
    if pre.g1_truncated:
        return _abstain(
            "target IG truncated/incomplete; dispense untrustworthy",
            solver_diagnostics=pre.solver_diagnostics,
        )
    if pre.g1_rate < 1.0:
    ...
    # tooling_leads (no-source-object FULL hits) are surviving hits too, so their
    # presence means NOT all survivors are window-flagged -> fall through to the
    # generic budgeted string.
    if (worksheet.window_order and not worksheet.candidates
            and not worksheet.tooling_leads):
        return SolveResult(
            exit_code=4,
            reason="window-order",
            worksheet=worksheet,
