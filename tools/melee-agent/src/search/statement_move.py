...
def _compound_siblings(compound_node, source_bytes) -> list[SiblingStmt]:
    sibs: list[SiblingStmt] = []
    for child in compound_node.named_children:
...
def toplevel_siblings(source: str, function: str) -> Optional[list[SiblingStmt]]:
    ...
    if body is None:
        return None
    return _compound_siblings(body, source_bytes)
...
def _direct_declared_names(compound_node, source_bytes) -> set[str]:
    names: set[str] = set()
    for child in compound_node.named_children:
...
def _function_parameter_names(function_node, source_bytes) -> set[str]:
    names: set[str] = set()
    stack = [function_node] if function_node is not None else []
    param_list = None
    while stack:
        n = stack.pop()
...
def _header_declared_names(node, source_bytes) -> set[str]:
    names: set[str] = set()
    ...
    while stack:
        ...
        if child.type == "declaration":
            decl_text = _ts.node_text(source_bytes, child)
            if not re.search(r"\bvolatile\b", _mask(decl_text)):
                names |= _declared_names(child, source_bytes)
            continue
...
def _direct_nested_compounds(node) -> list:
    compounds = []
    stack = list(node.named_children)
    while stack:
        child = stack.pop(0)
        if child.type == "compound_statement":
            ...
            continue
        stack[0:0] = list(child.named_children)
    return compounds


def sibling_groups(source: str, function: str) -> Optional[list[SiblingGroup]]:
    body, source_bytes = _body_node(source, function)
    if body is None:
        return None

    groups: list[SiblingGroup] = []
    function_node = body.parent
    parameter_names = _function_parameter_names(function_node, source_bytes)

    def visit(compound_node, inherited: set[str], depth: int) -> None:
        current = set(inherited)
        current |= _direct_declared_names(compound_node, source_bytes)
        groups.append(
            SiblingGroup(
                siblings=_compound_siblings(compound_node, source_bytes),
                locals_=frozenset(current),
                scope_depth=depth,
                block_start_line=compound_node.start_point[0] + 1,
            )
        )
        for child in compound_node.named_children:
            child_locals = current | _header_declared_names(child, source_bytes)
            for nested in _direct_nested_compounds(child):
                visit(nested, child_locals, depth + 1)

    visit(body, set(parameter_names), 0)
    return groups
...
def local_names(source: str, function: str) -> set[str]:
    ...
    if body is None:
        return names
    names |= _direct_declared_names(body, source_bytes)
    names |= _function_parameter_names(body.parent, source_bytes)
    return names
...
def generate_statement_hoist_sink_variants(
    ...
    ast-unavailable."""
    groups = sibling_groups(source, function)
    if groups is None:
        return []
    source_bytes = source.encode("utf-8")
