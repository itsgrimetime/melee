...
from src.mwcc_debug.source_patch import find_function as find_source_function
...
def resolve_structure_score_context(
    ...
    function: str,
    source_path: Path,
    retained_source_paths: set[Path] | None = None,
) -> StructureScoreContext:
    melee_root = Path(melee_root).resolve()
    source_path = Path(source_path).resolve()
    retained_source_paths = retained_source_paths or set()
    ...
    for unit in report.get("units", []) or []:
        ...
        if (
            resolved_source.resolve() != source_path
            and not _is_retained_source_for_function(
                source_path,
...
def _is_retained_source_for_function(
    source_path: Path,
    *,
    melee_root: Path,
    function: str,
    retained_source_paths: set[Path],
) -> bool:
    if not source_path.is_file() or source_path.suffix != ".c":
        return False
    resolved_source = source_path.resolve()
    if _is_relative_to(resolved_source, (melee_root / "src").resolve()):
        return False
    try:
        source_text = source_path.read_text(encoding="utf-8", errors="replace")
    except OSError:
        return False
    return find_source_function(source_text, function) is not None
...
def score_structure_variants(
    *,
    ...
    retained_source_paths = {
        Path(variant.source_retained).resolve()
        for variant in scoreable
        if variant.source_retained
    }
    context = resolve_structure_score_context(
        melee_root,
        function,
        source_path,
        retained_source_paths=retained_source_paths,
