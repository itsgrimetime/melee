...
class HarvestRequest:
    ...
    frame_evidence: str = ""
    frame_probe_status: str = ""
    frame_match_relevance: str = ""
    # Deprecated input-only alias for schema-v1 callers.
...
def _harvest_tool_fingerprint(work_bucket: str) -> str | None:
    ...
    tool_paths = [src_root / "cli" / "debug" / "__init__.py"]
    if work_bucket == "stack-local-layout":
        tool_paths.extend(
            [
                src_root / "mwcc_debug" / "pressure_explorer" / "__init__.py",
                src_root / "mwcc_debug" / "frame_reservations" / "__init__.py",
            ]
        )
...
def _request_from_queue_row(
    ...
    *,
    ...
    raw = _normalize_queue_frame_fields(raw, work_bucket=work_bucket)
    ...
    return HarvestRequest(
        ...
        frame_evidence=(raw.get("frame_evidence") or "").strip(),
        frame_probe_status=(raw.get("frame_probe_status") or "").strip(),
        frame_match_relevance=(raw.get("frame_match_relevance") or "").strip(),
...
def _normalize_queue_frame_fields(
    raw: Mapping[str, str],
    *,
    work_bucket: str,
) -> dict[str, str]:
    row = dict(raw)
    if work_bucket != "stack-local-layout" and not any(
        row.get(key)
        for key in (
            "frame_cause",
            "frame_verdict",
        ...
        row.pop("frame_closability_tier", None)
        return row

    legacy = bool(row.get("frame_closability_tier")) or not all(
        row.get(key)
        for key in (
            "frame_evidence",
            "frame_probe_status",
    ...
    cause = (row.get("frame_cause") or "").strip()
    verdict = (
        row.get("frame_raw_verdict")
        or row.get("frame_verdict")
        or ""
    ).strip()
    attribution = (row.get("frame_attribution_status") or "").strip()
    evidence = (row.get("frame_evidence") or "").strip()
    status = (row.get("frame_probe_status") or "").strip()
    relevance = (row.get("frame_match_relevance") or "").strip()
    ...
    if not status:
        if evidence == "checkdiff-only":
            ...
            status = "ceiling"
        elif verdict == "source-reachable-candidate" and _has_bounded_frame_probe(row):
    ...
    if legacy:
        ...
        row["headline_tool"] = headline
        row["actionability_reason"] = reason
    return row
...
def _has_bounded_frame_probe(row: Mapping[str, Any]) -> bool:
    ...
    for key in ("frame_probe_plan", "frame_transform_probe_plan", "probe_plan"):
        ...
        if isinstance(commands, (list, tuple)) and any(
...
def _preview_sample_entry(request: HarvestRequest) -> dict[str, Any]:
    return {
        ...
        "frame_evidence": request.frame_evidence,
        "frame_probe_status": request.frame_probe_status,
        "frame_match_relevance": request.frame_match_relevance,
...
def select_harness(request: HarvestRequest) -> str | None:
    if request.terminal_attempt_status == "active":
        return None
    if request.frame_probe_status in {
        "needs-attribution",
        "probe-inconclusive",
    ...
    if request.frame_probe_status == "materializable":
...
def _needs_pcdump_preflight(
    ...
    *,
    ...
    frame_transform_current_tools = (
        ...
        and request.frame_probe_status == "materializable"
    )
...
def summarize_ledger(results: list[HarvestResult | dict[str, Any]]) -> dict[str, Any]:
    ...
    by_frame_probe_status: Counter[str] = Counter()
    ...
    for result in normalized:
        ...
        frame_probe_status = result.get("frame_probe_status")
        if frame_probe_status:
            by_frame_probe_status[str(frame_probe_status)] += 1
    ...
    return {
        ...
        "by_frame_probe_status": dict(by_frame_probe_status),
