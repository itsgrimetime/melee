...
class StateDB:
    ...
    # =========================================================================
    # Tool Issue Operations
    # =========================================================================

    _TOOL_ISSUE_STATUSES = {"open", "resolved"}
    _TOOL_ISSUE_KINDS = {"bug", "feature", "papercut", "blocker", "note"}

    def _decode_tool_issue_row(self, row: sqlite3.Row | None) -> dict | None:
        """Convert a tool_issues row into a plain dict with parsed functions."""
        if row is None:
            return None

        issue = dict(row)
        raw_functions = issue.get("functions")
        if raw_functions:
            try:
                issue["functions"] = json.loads(raw_functions)
    ...
    def report_tool_issue(
        ...
        """Record a tooling issue, feature request, papercut, or blocker.

        Args:
            summary: Short human-readable issue summary.
            kind: One of bug, feature, papercut, blocker, note.
            tool: Tool or subsystem involved, such as mwcc-debug.
            body: Optional detailed context.
            functions: Optional functions this issue blocked or affected.
            agent_id: Reporting agent ID.
            session_id: Claude/Codex session or thread ID, if available.
            worktree_path: Worktree where the problem occurred.
            branch: Branch where the problem occurred.

        Returns:
            The created issue row.
        """
        summary = summary.strip()
        if not summary:
            raise ValueError("summary is required")
        if kind not in self._TOOL_ISSUE_KINDS:
            valid = ", ".join(sorted(self._TOOL_ISSUE_KINDS))
            raise ValueError(f"invalid issue kind: {kind} (valid: {valid})")

        cleaned_functions = [fn.strip() for fn in (functions or []) if fn and fn.strip()]
        functions_json = json.dumps(cleaned_functions) if cleaned_functions else None
        now = time.time()
        ...
        with self.transaction() as conn:
            cursor = conn.execute(
                ...
                INSERT INTO tool_issues (
                    status, kind, tool, summary, body, functions, agent_id,
                    session_id, worktree_path, branch, created_at, updated_at
                )
                VALUES ('open', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
            ...
            row = conn.execute("SELECT * FROM tool_issues WHERE id = ?", (issue_id,)).fetchone()
            issue = self._decode_tool_issue_row(row)
    ...
    def get_tool_issue(self, issue_id: int) -> dict | None:
        """Get a reported tool issue by ID."""
        with self.connection() as conn:
            row = conn.execute("SELECT * FROM tool_issues WHERE id = ?", (issue_id,)).fetchone()
            return self._decode_tool_issue_row(row)
    ...
    def list_tool_issues(
        ...
        if status in ("all", ""):
            status = None
        if status is not None and status not in self._TOOL_ISSUE_STATUSES:
            valid = ", ".join(sorted(self._TOOL_ISSUE_STATUSES | {"all"}))
            raise ValueError(f"invalid issue status: {status} (valid: {valid})")
        if kind is not None and kind not in self._TOOL_ISSUE_KINDS:
            valid = ", ".join(sorted(self._TOOL_ISSUE_KINDS))
            raise ValueError(f"invalid issue kind: {kind} (valid: {valid})")

        query = "SELECT * FROM tool_issues WHERE 1=1"
        params: list[Any] = []
        if status:
            query += " AND status = ?"
        ...
        with self.connection() as conn:
            ...
            return [issue for row in cursor.fetchall() if (issue := self._decode_tool_issue_row(row)) is not None]

    def resolve_tool_issue(
        self,
        issue_id: int,
        agent_id: str | None = None,
