...
-- Agent-reported issues and feature requests for local tooling
CREATE TABLE IF NOT EXISTS tool_issues (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    status TEXT CHECK(status IN ('open', 'resolved')) NOT NULL DEFAULT 'open',
    kind TEXT CHECK(kind IN ('bug', 'feature', 'papercut', 'blocker', 'note')) NOT NULL DEFAULT 'bug',
    tool TEXT,
...
def get_migrations() -> dict[int, str]:
    ...
    return {
        ...
        7: """
            ...
            CREATE INDEX IF NOT EXISTS idx_match_history_branch ON match_history(branch);
        """,
        # Version 8 -> 9: Add agent-reported tooling issue queue
        8: """
            CREATE TABLE IF NOT EXISTS tool_issues (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                status TEXT CHECK(status IN ('open', 'resolved')) NOT NULL DEFAULT 'open',
