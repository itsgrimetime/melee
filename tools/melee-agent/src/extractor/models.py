...
class FunctionSymbol(BaseModel):
    ...
    scope: str | None = Field(default=None, description="Scope: global, local, weak")
...
class FunctionMatch(BaseModel):
    ...
    name: str = Field(description="Function name")
    fuzzy_match_percent: float = Field(description="Fuzzy match percentage (0.0 to 100.0)", ge=0.0, le=100.0)
    address: str | None = Field(default=None, description="Hex address like 0x80003100 (from virtual_address metadata)")
...
class FunctionInfo(BaseModel):
    ...
    lib: str | None = Field(default=None, description="Library this function belongs to")
...
class ExtractionResult(BaseModel):
    ...
    functions: list[FunctionInfo] = Field(default_factory=list, description="List of extracted functions")
    total_functions: int = Field(default=0, description="Total number of functions extracted")
    matched_functions: int = Field(default=0, description="Number of fully matched functions")
    unmatched_functions: int = Field(default=0, description="Number of unmatched functions")
