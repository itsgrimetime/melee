...
def _valid_category_message() -> str:
    return "Valid categories: " + ", ".join(sorted(CATEGORIES))


def _validate_category(category: str) -> None:
    if category not in CATEGORIES:
        _exit_with_error(
            f"Unsupported category: {category}\n{_valid_category_message()}"
        )
...
@mismatch_app.command("show")
def show(
    pattern_id: str,
    db: Annotated[Path | None, typer.Option(help="Database path")] = None,
    as_json: Annotated[bool, typer.Option("--json", help="Output as JSON")] = False,
):
    """Alias for `get`; show details for a specific pattern."""
    get(pattern_id=pattern_id, db=db, as_json=as_json)


@mismatch_app.command("rm")
def remove(
    pattern_id: Annotated[str, typer.Argument(help="Pattern ID slug to delete")],
    db: Annotated[Path | None, typer.Option(help="Database path")] = None,
):
    """Delete a mismatch pattern by ID."""
    db_path = get_db_path(db)
    pattern_db = PatternDB(init_db(db_path))

    if not pattern_db.get(pattern_id):
        _exit_with_error(f"Pattern not found: {pattern_id}")
    if not pattern_db.delete(pattern_id):
        _exit_with_error(f"Pattern not deleted: {pattern_id}")

    rprint(f"Deleted pattern: {pattern_id}")
