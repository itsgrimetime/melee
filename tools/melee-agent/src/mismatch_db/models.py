...
def user_query_to_fts_query(query: str) -> str:
    """Convert a plain user query into safe FTS syntax.

    The CLI accepts ordinary search text, not raw SQLite FTS syntax. Hyphenated
    terms such as ``stack-layout`` otherwise parse as operators/columns.
    """
...
class PatternDB:
    ...
    def search_fulltext(self, query: str) -> list[Pattern]:
        ...
        fts_query = user_query_to_fts_query(query)
        rows = self.conn.execute(
            """
