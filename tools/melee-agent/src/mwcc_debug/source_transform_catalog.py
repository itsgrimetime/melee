...
SOURCE_TRANSFORM_CATALOG: tuple[SourceTransformCatalogEntry, ...] = (
    ...
    SourceTransformCatalogEntry(
        ...
        notes=(
            ...
            "pcode_only_fpr_callarg_temp_repair is backed by HSD_JObjReqAnimAll digit-conversion call-argument dematerialization, assignment-expression, dead-FPR-local reuse, and block-local temp probes for pcode-only fmr-to-f1 residuals.",
            "pcode_only_fpr_fsubs_cast_owner_repair is backed by exact FPR cast-owner and fsubs assignment temp-split probes for pcode-only first-def residuals.",
            "retained_gpr_case_c_sensitivity_search is backed by retained-source low-confidence pointer local rewrites that probe Case-C first-divergence movement after pcode-only GPR copy-product repairs go flat.",
            "retained_gpr_case_c_window_order_continuation is fed by debug search plan-transforms --select-order-json and converts select-order window-order-source-steering probes into retained IG44 transform-corpus candidates.",
            "retained_gpr_case_c_post_source_owner_backtrack is fed by debug search plan-transforms --select-order-json and skips the first/current ranked indexed-byte source-owner candidate so later retained source-owner/frontier alternates can be scored or terminally exhausted.",
            "retained_gpr_case_c_target_live_range_repair is fed by retained select-order/frontier evidence and materializes bounded IG44/r39/r41 live-range, implicit indexed-expression spelling, index/base alias, address-side, value-side, and coupled address/value probes while preserving protected IG34.",
            "retained_gpr_case_c_simplify_order_continuation is fed by retained select-order/frontier evidence and materializes bounded declaration, reload, scope, setup-order, and same-line expression probes for the IG44 Case-C simplify-order frontier while preserving protected IG34.",
            "coupled_fpr_coalesce_product_repair is backed by coupled column product/cast temp plus HSD_JObjReqAnimAll call-argument temp probes for mndiagram FPR coalesce/product residuals.",
            "coloring_register_steering aliases guarded declaration/lifetime edits plus FPR product recompute/order/cast/argument/temp-split probes for mndiagram force-phys register-coloring residuals.",
            "indexed_byte_address_temp_steering is backed by same-line indexed byte-array spellings plus byte value-temp, index-temp, and base-alias probes that avoid source-visible element pointers.",
