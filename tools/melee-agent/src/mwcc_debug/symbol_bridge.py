...
class Binding:
    ...
    confidence: str        # "best-guess" | "verified" | "low-confidence"
                           ...
                           # may be off. `debug mutate search` skips these by
...
def _extract_function_text(
    ...
    if find_function is not None:
        ...
        if span is not None:
            header_source = source[span.sig_start:span.body_open]
            header_cleaned = _strip_strings_and_comments(header_source)
            name_m = re.search(r"\b" + re.escape(fn_name) + r"\s*\(", header_cleaned)
            if name_m is not None:
                paren_open = name_m.end() - 1
                depth = 1
                idx = paren_open + 1
                while idx < len(header_cleaned) and depth > 0:
                    c = header_cleaned[idx]
                    if c == "(":
                        depth += 1
                    elif c == ")":
                        depth -= 1
                ...
                if depth == 0:
                    paren_close = idx - 1
                    params_text = header_source[paren_open + 1:paren_close].strip()
...
def _locals_in_binding_order(
    locals_: list[LocalDecl],
    decls_by_scope: dict[tuple[str, ...], list[LocalDecl]],
    fn_name: str,
) -> list[LocalDecl]:
    """Return locals in the Phase-1 cursor-assignment order.

    `parsed_locals` stays lexical/source-order for diagnostics, but virtual
    assignment claims all function-top locals first, then nested scopes by the
    first declaration they contain. This preserves the old top-of-function
    cursor model while making nested decls visible after that top-level claim.
    """
    if not decls_by_scope:
        return list(locals_)

    top_scope = (fn_name,) if fn_name else ()
    ordered: list[LocalDecl] = []

    top_decls = decls_by_scope.get(top_scope)
    if top_decls is not None:
        ordered.extend(sorted(top_decls, key=lambda d: d.decl_index))

    nested_scopes = [
        scope for scope in decls_by_scope
        if scope != top_scope
    ]
    nested_scopes.sort(
        key=lambda scope: min(d.decl_index for d in decls_by_scope[scope])
    )
    for scope in nested_scopes:
        ordered.extend(sorted(decls_by_scope[scope], key=lambda d: d.decl_index))
...
def list_bindings_with_basis(
    ...
    For LOCALS:
      ...
      - If the predicted virtual IS observed but red flags ARE set →
        ...
        match could be coincidental, and `debug mutate search` will skip it
    ...
    for p in params:
        ...
        cursor += 1
    for ld in _locals_in_binding_order(locals_, decls_by_scope, fn_name):
        expected = 32 + cursor
        if expected in virtuals_set:
            ...
            out.append(Binding(
                var_name=ld.name,
                virtual=expected,
                decl_line=ld.line_no or start_line,
                kind="local",
...
def find_var_for_virtual(
    ...
    such binding exists. Nested-block cursor assignments are intentionally
    excluded from this inverse lookup: they are visible through
    list_bindings/var-to-virtual, but virtual-to-var should not present an
    unvalidated nested lexical scope as a fact.
    """
    for b in list_bindings(source, fn_name, pre_pass):
        if b.virtual == virtual and b.confidence != "ambiguous-nested":
            return b
    return None
