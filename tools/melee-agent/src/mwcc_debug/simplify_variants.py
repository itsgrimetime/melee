...
from .simplify_search import FunctionContext, LazySourceVariant, SourceVariant
...
def decl_orders_source(ctx: FunctionContext) -> Iterator[SourceVariant]:
    ...
    candidates = [
        (f"decl-orders {candidate.label}", candidate.order)
...
def type_change_source(
    ctx: FunctionContext,
) -> Iterator[SourceVariant | LazySourceVariant]:
        ...
        for from_type, to_type in _TYPE_FLIPS:
            ...
            def materialize(
                *,
                ...
                if patched == source_text or patched in seen_texts:
                    raise MutationUnsupported("duplicate or no-op type change")
                seen_texts.add(patched)
                return patched
