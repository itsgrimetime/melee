...
Why content hashing instead of pure mtime:
  Several commands (``debug mutate decl-orders``, ``debug mutate search``,
  ``debug permute verify``) temporarily patch and restore the source file.  A
...
def write_hash_sidecar(cache_p: Path, src: Path) -> None:
    ...
    rather than mtime.
    """
    write_hash_sidecar_digest(cache_p, source_digest(src))


def write_hash_sidecar_digest(cache_p: Path, digest: str) -> None:
    """Write a precomputed source digest to ``cache_p``'s hash sidecar."""
...
def lookup(melee_root: Path, unit: str) -> Optional[CacheEntry]:
    ...
    pcdump or trigger a fresh ``debug dump remote`` run.
    """
