...
source variants) keeps the driver agnostic to where candidates come from.
...
class SimplifyScore:
    ...
    assignment_distance_total: int | None = None
    baseline_assignment_distance_total: int | None = None
    assignment_improved_count: int = 0
...
class FunctionContext:
    ...
    compile_unit_source: Path | None = None
...
@dataclass
class LazySourceVariant:
    """One source variant whose full text is built only if needed.

    Skip controls can inspect provenance before invoking expensive source
    mutation work. The `text` property preserves the same read surface as
    `SourceVariant` for tests and exploratory callers.
    """
    ...
    def text(self) -> str:
        if self._text_cache is None:
            self._text_cache = self.materialize()
...
class SearchResult:
    ...
    Fields:
      ...
      aborted: True when search stopped early due to an interrupt that should
        still return partial diagnostics to the CLI.
      abort_reason: Stable reason string for the early abort.
      last_provenance: Variant provenance active when the abort happened.
      skipped_count: How many unique variants were skipped by explicit
        skip controls before compile.
      skip_reasons: Sample of skipped variant provenance/reason records.
      candidate_stream_position: Count of unique, non-duplicate variants seen
        in the stream, including skipped and compiled candidates.
    """

    exact_match: SourceVariant | None
    progress: list[ScoredVariant]
    scored_candidates: list[ScoredVariant] = field(default_factory=list)
    ...
    last_provenance: str | None = None
    skipped_count: int = 0
    skip_reasons: list[dict[str, str]] = field(default_factory=list)
    candidate_stream_position: int = 0
...
# iterable.
VariantItem = SourceVariant | LazySourceVariant
VariantSource = Callable[[FunctionContext], Iterable[VariantItem]]
...
def score_force_phys_assignment(
    ...
    candidate_distances = [
        abs(candidate_assigned[ig] - target_phys[ig])
        for ig in target_igs
        if ig in candidate_assigned
    ]
    baseline_distances = [
        abs(baseline_assigned[ig] - target_phys[ig])
        for ig in target_igs
        if ig in baseline_assigned
    ]
    candidate_distance_total = (
        sum(candidate_distances)
        if len(candidate_distances) == len(target_igs)
        else None
    )
    baseline_distance_total = (
        sum(baseline_distances)
        if len(baseline_distances) == len(target_igs)
        else None
    )
    improved_count = sum(
        1
        for ig in target_igs
        if (
            ig in candidate_assigned
            and ig in baseline_assigned
            and abs(candidate_assigned[ig] - target_phys[ig])
            < abs(baseline_assigned[ig] - target_phys[ig])
        )
    )
    return SimplifyScore(
        ...
        assignment_distance_total=candidate_distance_total,
        baseline_assignment_distance_total=baseline_distance_total,
        assignment_improved_count=improved_count,
    )


def protected_force_phys_loss(
    candidate: BaselineSignature,
    protected_phys: dict[int, int],
) -> str | None:
    """Return a rejection reason when a protected assignment is lost."""
    if not protected_phys:
        return None
    assigned = dict(candidate.assigned_regs)
    lost: list[str] = []
    for ig_idx, expected in sorted(protected_phys.items()):
        actual = assigned.get(ig_idx)
        if actual != expected:
            actual_text = "unassigned" if actual is None else f"r{actual}"
            lost.append(f"ig{ig_idx} expected r{expected} actual {actual_text}")
    if not lost:
        return None
    return "protected force-phys lost (" + "; ".join(lost[:4]) + ")"
...
def search(
    ...
    *,
    ...
    protected_force_phys: dict[int, int] | None = None,
    class_id: int = 0,
    ...
    Args:
      ...
      protected_force_phys: optional physical-register mappings that must stay
        satisfied for force-phys searches. Candidates that lose any protected
        mapping are gate-rejected before ranking.
      ...
      skip_families: Provenance prefixes to skip after deduplication.
    """
    if skip_first_candidates < 0:
        raise ValueError("skip_first_candidates must be non-negative")
    melee_root = melee_root or ctx.melee_root
    start = time.time()
    progress: list[ScoredVariant] = []
    scored_candidates: list[ScoredVariant] = []
    ...
    compiled = 0
    skipped = 0
    skip_reason_details: list[dict[str, str]] = []
    candidate_stream_position = 0
    skip_provenance_set = set(skip_provenances)
    skip_family_prefixes = tuple(skip_families)
    ...
    def build_result(
        *,
        ...
        progress.sort(
            key=lambda sv: (
                -sv.score.common_prefix_length,
                (
                    sv.score.assignment_distance_total
                    if sv.score.assignment_distance_total is not None
                    else 1_000_000
                ),
                sv.precolor_distance.total,
        ...
        return SearchResult(
            ...
            skip_reasons=skip_reason_details,
            candidate_stream_position=candidate_stream_position,
        )

    def skip_reason_for(
        *, provenance: str, stream_position: int,
    ) -> str | None:
        if stream_position <= skip_first_candidates:
            return "skip-first-candidates"
        if provenance in skip_provenance_set:
            return "skip-provenance"
        for prefix in skip_family_prefixes:
            if provenance.startswith(prefix):
                return f"skip-family:{prefix}"
        return None

    def record_skip(*, provenance: str, reason: str) -> None:
        nonlocal skipped
        skipped += 1
        if len(skip_reason_details) < 20:
            skip_reason_details.append({
    ...
    for source in sources:
        ...
        try:
            ...
            return build_result(
                ...
                last_provenance=active_provenance,
            )
        while exact_match is None and compiled < max_candidates:
            ...
            active_provenance = variant.provenance
            if isinstance(variant, LazySourceVariant):
                next_position = candidate_stream_position + 1
                lazy_skip_reason = skip_reason_for(
                    provenance=variant.provenance,
                    stream_position=next_position,
                )
                if lazy_skip_reason is not None:
                    candidate_stream_position = next_position
                    record_skip(
                        provenance=variant.provenance,
                        reason=lazy_skip_reason,
                    )
                    continue
                try:
                    variant = SourceVariant(
                        text=variant.text,
                        provenance=variant.provenance,
                        parent_baseline=variant.parent_baseline,
                    )
                except KeyboardInterrupt:
                    return build_result(
                        aborted=True,
            ...
            candidate_stream_position += 1
            skip_reason = skip_reason_for(
                provenance=variant.provenance,
                stream_position=candidate_stream_position,
            )
            if skip_reason is not None:
                record_skip(provenance=variant.provenance, reason=skip_reason)
                continue
            compiled += 1
            ...
            protected_loss = (
                protected_force_phys_loss(sig, protected_force_phys or {})
                if force_phys
                else None
            )
            if protected_loss is not None:
                rejected += 1
                if protected_loss not in seen_reasons:
                    seen_reasons.add(protected_loss)
                    if len(rejection_reasons) < 8:
                        rejection_reasons.append(protected_loss)
                rejected_scored.append(RejectedCandidate(
                    provenance=variant.provenance,
                    score=score,
                    rejection_reason=protected_loss,
                    precolor_distance=dist,
                ))
                continue
            ...
            score_has_progress = (
                score.common_prefix_length > score.baseline_common_prefix_length
            )
            if (
                force_phys
                and score.assignment_distance_total is not None
                and score.baseline_assignment_distance_total is not None
                and score.assignment_distance_total
                < score.baseline_assignment_distance_total
            ):
                score_has_progress = True
            if score_has_progress:
...
def _compile_variant(
    ...
    *,
    ...
    temp_kwargs: dict[str, object] = {"prefix": "mwcc_simplify_"}
    if ctx.compile_unit_source is not None:
        probe_root = melee_root / "build" / "mwcc_debug_cache" / "probes" / "simplify-order"
        probe_root.mkdir(parents=True, exist_ok=True)
        temp_kwargs["dir"] = probe_root
    with tempfile.TemporaryDirectory(**temp_kwargs) as td:
        ...
        if ctx.compile_unit_source is not None:
            compile_kwargs["unit_source"] = ctx.compile_unit_source
        try:
            return compile_source_variant(**compile_kwargs)
        except TypeError as exc:
            if (
                "function_aliases" not in str(exc)
                and "unit_source" not in str(exc)
            ):
                raise
            compile_kwargs.pop("function_aliases", None)
            if "unit_source" in str(exc):
                compile_kwargs.pop("unit_source", None)
            return compile_source_variant(**compile_kwargs)
