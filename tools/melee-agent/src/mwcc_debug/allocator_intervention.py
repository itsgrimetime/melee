...
class CoalesceInterventionSpec:
    ...
    def backend_env(self) -> dict[str, str]:
        env = {"MWCC_DEBUG_FORCE_COALESCE": self.backend_value}
        if self.class_id != 0:
            env["MWCC_DEBUG_FORCE_COALESCE_CLASS"] = str(self.class_id)
        return env
...
class CoalesceInterventionReport:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "register_class": _register_class_name(self.spec.class_id),
...
def parse_coalesce_pair(raw: str) -> tuple[int, int]:
    virt, root, _class_id = parse_coalesce_pair_with_class(raw)
    return virt, root


def parse_coalesce_pair_with_class(raw: str) -> tuple[int, int, int]:
    ...
    left_virtual, left_class = _parse_virtual_with_class(left)
    right_virtual, right_class = _parse_virtual_with_class(right)
    if (
        ...
        raise ValueError(
            f"invalid coalesce pair {raw!r}; mixed register classes"
        )
    class_id = left_class if left_class is not None else right_class
    return left_virtual, right_virtual, 0 if class_id is None else class_id


def _parse_virtual(raw: str) -> int:
    value, _class_id = _parse_virtual_with_class(raw)
    return value


def _parse_virtual_with_class(raw: str) -> tuple[int, int | None]:
    stripped = raw.strip()
    class_id = None
...
def render_coalesce_intervention_text(report: CoalesceInterventionReport) -> str:
    spec = report.spec
    prefix = _register_prefix(spec.class_id)
    lines = [
        f"allocator-intervention coalesce - {report.function}",
        f"hook: {spec.action} {prefix}{spec.virt} -> {prefix}{spec.root}",
        "backend env: "
        ...
        (
            "pair state: "
            f"baseline {_fmt_pair_root(spec.virt, report.baseline_pair_root, spec.class_id)}; "
            f"intervention {_fmt_pair_root(spec.virt, report.intervention_pair_root, spec.class_id)}"
        ),
    ]
...
def _fmt_pair_root(virt: int, root: int | None, class_id: int = 0) -> str:
    prefix = _register_prefix(class_id)
    if root is None:
        return f"{prefix}{virt} independent"
    return f"{prefix}{virt}->{prefix}{root}"


def _register_prefix(class_id: int) -> str:
    return "f" if class_id == 1 else "r"


def _register_class_name(class_id: int) -> str:
