"""Normalize stack-frame divergence diagnostics into evidence-bounded states."""
    ...
    *,
    ...
    if reserved is not None:
        ...
        return _build_result(
            ...
            probe_status="needs-attribution",
            attribution_status="checkdiff-only",
            source_object=None,
            source_object_symbol=None,
            next_command=_frame_attribution_command(function, source_path),
    ...
    if isinstance(missing, int) and missing > 0:
        ...
        verdict = "source-reachable-candidate"
        command = _frame_attribution_command(function, source_path)
        reason = (
            f"current frame is {missing} byte(s) smaller; pcdump attribution "
            "must precede source-probe selection"
        )
        ...
        command = _frame_attribution_command(function, source_path)
        reason = (
            f"current frame is {-missing} byte(s) larger; pcdump attribution "
            "must precede source-probe selection"
        )
...
def _classify_frame_report(
    ...
    *,
    ...
    raw_verdict = _raw_frame_report_verdict(first_divergence, frame_report)
    verdict = _normalize_verdict(raw_verdict)
    evidence = _frame_evidence(frame_report, first_divergence)
    probe_status = _frame_probe_status(
        frame_report,
        first_divergence,
        verdict=verdict,
    )
...
def _normalize_frame_report_cause(raw_cause: str, frame_delta: int | None) -> str:
    ...
    if raw_cause == "frame-too-large":
        return "frame-too-large"
    if raw_cause == "local-area-vs-outgoing-floor-divergence":
        return raw_cause
    return "unresolved-attribution"


def _raw_frame_report_verdict(
    first_divergence: Mapping[str, Any],
    frame_report: Mapping[str, Any],
) -> str:
    stack_home = _stack_home_validated_verdict(frame_report)
    if stack_home:
        return stack_home
...
def _probe_evaluations(frame_report: Mapping[str, Any]) -> list[Mapping[str, Any]]:
    return [
        value
        for key in ("frame_transform_probe_evaluation", "stack_home_probe_evaluation")
        if isinstance((value := frame_report.get(key)), Mapping)
    ]


def _stack_home_validated_verdict(frame_report: Mapping[str, Any]) -> str | None:
    ...
    if not isinstance(guidance, Mapping):
        return None
    validated = guidance.get("validated_verdict")
    if not isinstance(validated, Mapping):
        return None
    return _string(validated.get("status"))


def _has_no_safe_marker(value: Any) -> bool:
    if not isinstance(value, Mapping):
        return False
    if any(
        _string(value.get(key)) == "no-safe-semantic-lever"
        for key in ("status", "verdict", "kind", "blocker")
    ):
        return True
    return _has_no_safe_marker(value.get("stop_condition"))


def _no_safe_sources(
    frame_report: Mapping[str, Any],
    first_divergence: Mapping[str, Any],
) -> list[Mapping[str, Any]]:
    sources: list[Mapping[str, Any]] = []
    for value in (
        frame_report.get("semantic_lever_status"),
        first_divergence.get("validated_verdict"),
        first_divergence.get("verdict"),
        first_divergence.get("stop_condition"),
        frame_report.get("stop_condition"),
        frame_report.get("frame_transform_terminal_summary"),
        frame_report.get("stack_home_terminal_summary"),
    ):
        if isinstance(value, Mapping):
            sources.append(value)
    sources.extend(_probe_evaluations(frame_report))
    current = frame_report.get("current")
...
def _has_no_safe_outcome(
    frame_report: Mapping[str, Any],
    first_divergence: Mapping[str, Any],
) -> bool:
    return any(
        _has_no_safe_marker(source)
        for source in _no_safe_sources(frame_report, first_divergence)
    )
...
def _frame_evidence(
    ...
    evaluations = _probe_evaluations(frame_report)
    if _has_no_safe_outcome(frame_report, first_divergence):
        if any(
            _has_no_safe_marker(evaluation)
            and evaluation.get("status") == "evaluated"
            and int(evaluation.get("variant_count") or 0) > 0
            for evaluation in evaluations
        ):
            return "probe-validated"
        return "tool-evaluated"
    if isinstance(first_divergence.get("validated_verdict"), Mapping):
        return "probe-validated"
    if _stack_home_validated_verdict(frame_report):
        return "probe-validated"
    if any(
        ...
        return "probe-validated"
    if evaluations or isinstance(frame_report.get("semantic_lever_status"), Mapping):
        return "tool-evaluated"
    return "pcdump-attributed"


def _frame_probe_status(
    frame_report: Mapping[str, Any],
    first_divergence: Mapping[str, Any],
    *,
    verdict: str,
    ...
    evaluations = _probe_evaluations(frame_report)
    if _has_no_safe_outcome(frame_report, first_divergence):
        return "terminal-no-safe-lever"

    statuses = [
        _string((first_divergence.get("validated_verdict") or {}).get("status"))
        if isinstance(first_divergence.get("validated_verdict"), Mapping)
        else None,
        _stack_home_validated_verdict(frame_report),
    ]
    statuses.extend(_string(evaluation.get("verdict")) for evaluation in evaluations)
    if any(status in {
        "source-reachable-validated",
        "partial-source-reachable-validated",
        "source-reachable-frame-transform",
        ...
        "source-reachable-reorder",
        "partial-source-reachable-reorder",
        "source-reachable-local-area-transform",
        "partial-source-reachable-local-area-transform",
        "source-reachable-parameter-area-transform",
        "partial-source-reachable-parameter-area-transform",
    } for status in statuses):
        return "validated-improving"
    if any(status in {
        "attributed-frame-unchanged",
        "internal-tiebreak-ceiling",
        "frame-transform-ceiling-candidate",
        "internal-tiebreak-ceiling-candidate",
        "local-area-transform-ceiling-candidate",
        "parameter-area-transform-ceiling-candidate",
        "ceiling",
    } for status in statuses):
        return "ceiling"
    if evaluations:
        return "probe-inconclusive"
    if verdict == "source-reachable-candidate" and _has_materializable_probe_plan(
        frame_report
    ):
...
def _has_materializable_probe_plan(frame_report: Mapping[str, Any]) -> bool:
    first_divergence = frame_report.get("frame_first_divergence")
    if not isinstance(first_divergence, Mapping):
        return False
    ...
    if isinstance(plan, Mapping) and plan.get("status") == "ready":
        return any(
            isinstance(item, Mapping) and _string(item.get("command"))
    ...
    return (
        isinstance(probe_plan, Mapping)
        and probe_plan.get("status") == "ready"
        and _planned_stack_home_command(frame_report, "<function>") is not None
    )
...
def _frame_match_relevance(
    ...
    *,
    ...
    if _explicit_match_improvement(frame_report):
        return (
            "match-gating-candidate",
            "a compiled frame probe improved explicit before/after match-score evidence",
        )
    if verdict in {
        "source-reachable-validated",
        "partial-source-reachable-validated",
...
def _explicit_match_improvement(
    frame_report: Mapping[str, Any] | None,
) -> bool:
    if not isinstance(frame_report, Mapping):
        return False
    for evaluation in _probe_evaluations(frame_report):
        baseline = evaluation.get("baseline_match_percent")
        best = evaluation.get("best_variant")
        after = best.get("match_percent") if isinstance(best, Mapping) else None
        if isinstance(baseline, (int, float)) and isinstance(after, (int, float)):
            if after > baseline and _target_attained(best):
                return True
    evidence = frame_report.get("match_score_evidence")
    if isinstance(evidence, Mapping):
        ...
        return (
            ...
            and after > before
            and _target_attained(evidence)
        )
    return False


def _target_attained(value: Mapping[str, Any]) -> bool:
    if any(
        value.get(key) is True
        for key in (
            "target_frame_fixed",
            "target_fixed",
            "target_local_area_fixed",
            "target_parameter_words_fixed",
        )
    ):
        return True
    objective = value.get("objective")
    return isinstance(objective, Mapping) and _target_attained(objective)
...
def _next_command(
    ...
    *,
    ...
    if probe_status == "needs-attribution":
        return _frame_attribution_command(function, source_path)
    ...
    if probe_status in {
        ...
        "validated-improving",
        "probe-inconclusive",
    } and _uses_same_frame_command_family(cause, frame_delta):
        planned = _planned_stack_home_command(frame_report, function)
    ...
    if probe_status in {
        ...
        if planned is not None:
            return planned
        if probe_status == "materializable" and cause == "pure-reservation":
...
def _uses_same_frame_command_family(cause: str, frame_delta: int | None) -> bool:
    return cause == "stack-object-offset-shift" or (
        cause == "lifetime-or-ordering-shift" and frame_delta == 0
    )


def _frame_attribution_command(function: str, source_path: str | None) -> str:
    source = source_path or "<source.c>"
    return (
        f"melee-agent debug dump local {source} --function {function} && "
...
def _planned_stack_home_command(
    frame_report: Mapping[str, Any],
    function: str,
) -> str | None:
    current = frame_report.get("current")
    ...
    for item in plan.get("suggested_commands") or ():
        if not isinstance(item, Mapping):
            continue
        command = _string(item.get("command"))
        if command and "debug mutate lifetime-layout" in command:
            return command.replace("<function>", function)
    return None
