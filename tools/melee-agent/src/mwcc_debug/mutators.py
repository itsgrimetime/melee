...
def mutate_anti_coalesce_copy_before_use(
    source: str,
    fn_name: str,
    var_name: str,
    at_stmt_index: int,
    ...
    """Insert a volatile copy round-trip before a read of `var_name`.

    This is a stronger source-shape lever than a plain alias. The generated
    form creates a named copy, bounces it through a volatile local, then rewrites
    one read to use the copy. That gives coalesce-directed searches a bounded
    way to ask MWCC for two non-coalesced sub-ranges of one C value.
    """
    if new_name is None:
        new_name = f"{var_name}_split"
    ...
    if "[" in var_type or "(" in var_type:
        raise MutationUnsupported(
            f"variable {var_name!r} has unsupported anti-coalesce type {var_type!r}"
        )
    ...
    if field_re.search(rewritten):
        raise MutationUnsupported(
            f"[debug mutate search] skipping anti-coalesce seed for "
            f"'{var_name}': appears to be a struct field access."
        )
    ...
    decl_insert_abs = _find_block_top_pos(source, target_span.scope_byte_range)
    barrier_name = f"{new_name}_barrier"
    decl_lines = (
        f"{indent}{var_type} {new_name};\n"
        f"{indent}volatile {var_type} {barrier_name};\n"
    )
    assign_lines = (
        f"{indent}{new_name} = {var_name};\n"
        f"{indent}{barrier_name} = {new_name};\n"
        f"{indent}{new_name} = {barrier_name};\n"
    )

    edits: list[tuple[int, int, str]] = [
        (target_start_abs, target_end_abs, rewritten),
        (target_line_start_abs, target_line_start_abs, assign_lines),
        (decl_insert_abs, decl_insert_abs, decl_lines),
    ]
    out = source
    for start, end, replacement in sorted(edits, key=lambda e: e[0], reverse=True):
        out = out[:start] + replacement + out[end:]
