...
def compile_source_variant(
    ...
    *,
    ...
    with temporary_directory(prefix="mwcc_diff_") as td:
        out_path = Path(td) / f"{_safe_pcdump_output_label(diff_input.label)}.pcdump.txt"
...
def _safe_pcdump_output_label(label: str, *, max_bytes: int = 160) -> str:
    safe = re.sub(r"[^A-Za-z0-9_.-]+", "-", label.lower()).strip(".-") or "input"
    suffix = ".pcdump.txt"
    max_stem_bytes = max(1, max_bytes - len(suffix.encode("utf-8")))
    if len(safe.encode("utf-8")) <= max_stem_bytes:
        return safe
    digest = hashlib.sha1(label.encode("utf-8")).hexdigest()[:12]
    digest_suffix = f"-{digest}"
    keep_bytes = max(1, max_stem_bytes - len(digest_suffix.encode("utf-8")))
    stem = safe.encode("utf-8")[:keep_bytes].decode("ascii", "ignore")
    stem = stem.rstrip(".-") or "input"
    return f"{stem}{digest_suffix}"
...
def _melee_agent_entrypoint() -> str:
    preferred = Path("/opt/homebrew/bin/melee-agent")
    if preferred.exists():
        return str(preferred)
    return "melee-agent"


def _dump_local_command(
    *,
    compile_path: Path,
    out_path: Path,
    ...
    if unit_source_path is None:
        ...
        cmd = [_melee_agent_entrypoint(), "debug", "dump", "local"]
    cmd.extend([
        str(compile_path),
        "--output",
        str(out_path),
    ...
    if dump_function is not None:
        cmd.extend(["--function", dump_function])
