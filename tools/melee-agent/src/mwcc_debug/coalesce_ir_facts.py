"""IR facts layer for `debug suggest coalesce`.
...
class IrFacts:
    ...
    by_reg: dict[tuple[str, int], VirtualFacts] = field(default_factory=dict)
    register_kind: str = "r"


def collect(fn: Function, source: str, *, register_kind: str = "r") -> IrFacts:
    ...
    if pre_pass is None:
        ...
        return IrFacts(
            ...
            by_reg={}, register_kind=register_kind,
        )

    # Collect all operand mentions. by_reg is class-aware and prevents
    # fNN/rNN pollution; by_virtual is the selected class map for legacy
    # consumers of this suggest-coalesce layer.
    by_virtual: dict[int, VirtualFacts] = {}
    by_reg: dict[tuple[str, int], VirtualFacts] = {}

    # First pass: discover every register mentioned anywhere.
    ...
    for kind, v in seen:
        first_def = find_first_def(v, pre_pass, reg_kind=kind)
        use_sites: list[tuple[int, Instruction]] = []
        truncated = False
        for block in pre_pass.blocks:
            for ist in block.instructions:
                # A "use" is any occurrence of the virtual in the operands
                if any(k == kind and n == v for k, n in ist.regs):
        ...
        is_phys = v < 32
        is_param = (
            _is_param(v, first_def, pre_pass, bindings, basis)
            if kind == "r"
            else False
        )
        facts = VirtualFacts(
    ...
    return IrFacts(
        ...
        cg_section=None, by_reg=by_reg, register_kind=register_kind,
    )
...
_FPR_CALLEE_SAVES = list(range(14, 32))  # f14..f31
...
def analyze_cascade(facts: IrFacts) -> list[CascadeCandidate]:
    ...
    saves = [d for d in cg.decisions if d.assigned_reg in _callee_saves(facts)]
    if len(saves) < 2:
        return []
...
def _callee_saves(facts: IrFacts) -> list[int]:
    if facts.register_kind == "f":
        return _FPR_CALLEE_SAVES
    return _GPR_CALLEE_SAVES


def _virtual_instruction_ranges(
    pre_pass: Pass,
    *,
    register_kind: str = "r",
    ...
    for _block_idx, _instr_idx, inst in pre_pass.all_instructions():
        for kind, num in inst.regs:
            if kind == register_kind and num >= 32:
...
def _find_register_reuse_candidates(
    ...
    *,
    ...
    ranges = _virtual_instruction_ranges(
        facts.pre_pass,
        register_kind=facts.register_kind,
    )
