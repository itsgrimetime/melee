...
class LifetimeLayoutProbe:
    ...
    def to_dict(self, *, include_source: bool = False) -> dict:
        ...
        if self.provenance is not None:
            ...
            if self.provenance.get("kind") == "transform-corpus":
...
_READ_ONLY_SOURCE_LIFETIME_HELPERS = {
    "fn_803AC634": {
        "return_type": "s32",
        "cast_result": True,
        "source": "known-read-only",
    },
    "GetNameText": {
        "return_type": "char*",
        "cast_result": False,
...
def _source_lifetime_read_only_helper_specs(
    overrides: dict[str, str] | None = None,
) -> dict[str, dict[str, object]]:
    specs = {
        name: dict(spec)
        for name, spec in _READ_ONLY_SOURCE_LIFETIME_HELPERS.items()
    }
    for name, return_type in (overrides or {}).items():
        helper_name = str(name).strip()
        if not helper_name:
            continue
        normalized_return = _normalize_type_spelling(
            str(return_type or "s32").strip() or "s32"
        )
        specs[helper_name] = {
...
def _read_only_helper_spec(
    callee: str,
    specs: dict[str, dict[str, object]] | None,
) -> dict[str, object] | None:
    merged = specs if specs is not None else _source_lifetime_read_only_helper_specs()
    return merged.get(callee)


def _read_only_helper_initializer(
    call_text: str,
    spec: dict[str, object],
) -> str:
    return_type = str(spec.get("return_type") or "s32")
    if bool(spec.get("cast_result")):
        return f"({return_type}) {call_text}"
    return call_text
...
def pressure_signature_from_pcdump(
    ...
    *,
    ...
    if events is not None:
        ...
        spill_set = tuple(sorted(_spilled_markers(events, class_id=spill_class_id)))
    ...
    pair_states = _target_pair_states(
        pcdump_text,
        function,
        pairs,
        reg_class=_reg_class_for_allocator_class(class_id),
    )
...
def _spilled_markers(events, *, class_id: int | None = None) -> set[int]:
...
def _reg_class_for_allocator_class(class_id: int) -> str:
    if class_id == 1:
        return "fpr"
    return "gpr"
...
def generate_source_lifetime_probes(
    ...
    *,
    ...
    read_only_helpers: dict[str, str] | None = None,
    ...
    helper_specs = _source_lifetime_read_only_helper_specs(read_only_helpers)
    ...
    for operator, generator in targeted_generators:
        if selected_operators is not None and operator not in selected_operators:
            continue
        if operator == "repeated-helper-result-reuse":
            candidates, summary = generator(
                source_text,
                function,
                read_only_helpers=helper_specs,
            )
        elif operator == "helper-result-dematerialize":
            candidates, summary = generator(
                source_text,
                function,
                read_only_helpers=helper_specs,
...
def _probe_repeated_helper_result_reuse(
    ...
    function: str,
    *,
    read_only_helpers: dict[str, dict[str, object]] | None = None,
) -> tuple[list[LifetimeLayoutProbe], dict]:
    ...
    for occurrences in groups.values():
        ...
        if not _helper_call_args_are_simple(first.args_text):
            continue
        if not _helper_call_is_read_only(
            first.callee,
            source_text,
            function,
            read_only_helpers=read_only_helpers,
        ):
        ...
        if reuse_temp_spec is None:
            ...
            continue
        temp_type, temp_initializer, helper_source = reuse_temp_spec
        occurrence_blocker = _repeated_helper_occurrence_blocker(
            source_text,
            occurrences,
        )
        if occurrence_blocker is not None:
            condition_probe = _repeated_helper_condition_bool_probe(
                source_text,
                function,
                occurrences,
                temp_initializer=temp_initializer,
                helper_source=helper_source,
                spans=spans,
                byte_to_char=byte_to_char,
            )
            if condition_probe is not None:
                return [condition_probe], _source_lifetime_summary(
                    operator,
                    status="generated",
                    candidate_count=1,
                    blocker=None,
                    reason=(
                        "generated safe repeated helper condition bool reuse "
                        "candidate"
                    ),
                )
    ...
    *,
    ...
    for match in re.finditer(
        ...
        if not _helper_call_args_are_simple(call.args_text):
            ...
            continue
        if not _helper_callee_supported_for_dematerialize(
            call.callee,
            source_text,
            read_only_helpers=read_only_helpers,
        ):
...
def _helper_call_is_read_only(
    ...
    *,
    ...
    del function
    if _read_only_helper_spec(callee, read_only_helpers) is not None:
        return True
...
def _helper_reuse_temp_spec(
    ...
    *,
    ...
    helper_spec = _read_only_helper_spec(callee, read_only_helpers)
    if helper_spec is not None:
        return_type = str(helper_spec.get("return_type") or "s32")
        return (
            return_type,
            _read_only_helper_initializer(call_text, helper_spec),
            str(helper_spec.get("source") or "known-read-only"),
        )
    ...
    if _canonical_scalar_return_type(return_type) not in _REUSE_SCALAR_RETURN_TYPES:
        return None
    return return_type, call_text, "same-tu-simple-expression"


def _helper_callee_supported_for_dematerialize(
    callee: str,
    source: str,
    *,
    read_only_helpers: dict[str, dict[str, object]] | None = None,
...
def _next_unique_repeated_helper_exists_temp_name(source: str, function: str) -> str:
    span = _find_function_body_span(source, function)
    if span is None:
        return "ll_probe_helper_exists_0"
    body_start, body_end = span
    function_source = source[_line_start(source, body_start):body_end]
    used = {
        int(match.group(1))
        for match in re.finditer(r"\bll_probe_helper_exists_(\d+)\b", function_source)
    }
    suffix = 0
    while suffix in used:
        suffix += 1
    return f"ll_probe_helper_exists_{suffix}"
...
def _if_condition_context_for_call(
    source: str,
    *,
    call_start: int,
    call_end: int,
    ...
    cursor = call_start
    while cursor >= 0:
        line_start = _line_start(source, cursor)
        line_end = _line_end(source, line_start)
        line_text = source[line_start:line_end]
        if re.match(r"^\s*if\s*\(", line_text) is None:
            cursor = line_start - 1
            continue
        open_paren = source.find("(", line_start, line_end)
        if open_paren < 0:
        ...
        condition_end = close_paren
        if occurrence_start < condition_start or occurrence_end > condition_end:
            cursor = line_start - 1
            continue
        brace_match = re.match(r"\s*\{", source[close_paren + 1:])
        if brace_match is None:
            return None
        brace_start = close_paren + 1 + brace_match.start()
        condition = source[condition_start:condition_end]
        masked = _mask_c_non_code_text(condition)
        if not _balanced_expression_delimiters(masked) or "?" in masked:
            return None
        return line_start, condition_start, condition_end, _line_end(source, brace_start)
    return None


def _condition_bool_call_context(
    source: str,
    call: _SimpleHelperCall,
) -> tuple[int, int, int, int] | None:
    context = _if_condition_context_for_call(
        source,
...
def _condition_prefix_has_short_circuit_guard(text: str) -> bool:
    masked = _mask_c_non_code_text(text)
    return "&&" in masked or "||" in masked


def _condition_prefix_guards_identifier(text: str, name: str) -> bool:
    masked = _mask_c_non_code_text(text)
    token = rf"\b{re.escape(name)}\b"
    nullish = r"(?:NULL|0(?![\w.]))"
    return (
        re.search(rf"{token}\s*(?:==|!=)\s*{nullish}", masked) is not None
        or re.search(rf"{nullish}\s*(?:==|!=)\s*{token}", masked) is not None
        or re.search(rf"(?:^|&&|\|\|)\s*!?\s*{token}\s*(?:&&|\|\|)\s*$", masked)
        is not None
    )


def _helper_args_need_short_circuit_guard(args_text: str) -> bool:
    masked = _mask_c_non_code_text(args_text)
    return bool(re.search(r"(^|[,(])\s*\*", masked)) or "->" in masked or "[" in masked


def _condition_bool_replacement_for_call(
    source: str,
    call: _SimpleHelperCall,
    *,
    temp_name: str,
    condition_start: int,
    condition_end: int,
    guarded_pointer_names: set[str],
    allow_guarded_reuse: bool = False,
    ...
    if occurrence_start < condition_start or occurrence_end > condition_end:
        return None
    prefix = source[condition_start:occurrence_start]
    if (
        not allow_guarded_reuse
        and
        _condition_prefix_has_short_circuit_guard(prefix)
        and (
            _helper_args_need_short_circuit_guard(call.args_text)
            or any(
                _condition_prefix_guards_identifier(prefix, name)
                for name in guarded_pointer_names
            )
        )
    ):
        return None
    after = source[occurrence_end:condition_end]
    after_match = re.match(
        r"\s*(?P<op>==|!=)\s*(?P<null>NULL|0(?![\w.]))",
        after,
    )
    if after_match is not None:
        replacement = temp_name if after_match.group("op") == "!=" else f"!{temp_name}"
        return occurrence_start, occurrence_end + after_match.end(), replacement
    if re.match(r"\s*(?:==|!=)", after) is not None:
        return None
    before_match = re.search(
        r"(?P<null>NULL|0(?![\w.]))\s*(?P<op>==|!=)\s*$",
        prefix,
    )
    if before_match is not None:
        replacement = temp_name if before_match.group("op") == "!=" else f"!{temp_name}"
        return condition_start + before_match.start(), occurrence_end, replacement
    if re.search(r"(?:==|!=)\s*$", prefix) is not None:
        return None
    return occurrence_start, occurrence_end, temp_name
...
def _repeated_helper_condition_bool_probe(
    ...
    *,
    ...
    replacements: list[tuple[int, int, str]] = []
    temp_name = _next_unique_repeated_helper_exists_temp_name(source, function)
    ...
    if body_span is not None:
        ...
        identifier_types = _scoped_identifier_types(
            source,
    ...
    pointer_arg_names = {
        name for name in _helper_arg_identifier_names(first.args_text)
        if "*" in identifier_types.get(name, "")
    }
    first_context = _condition_bool_call_context(source, first)
    first_has_unguarded_condition_eval = False
    if first_context is not None:
        _line_start_abs, first_condition_start, _condition_end, _line_end_abs = (
            first_context
        )
        first_occurrence_start, _first_occurrence_end = _cast_prefixed_call_range(
            source,
            first.start,
            first.end,
        )
        first_has_unguarded_condition_eval = not _condition_prefix_has_short_circuit_guard(
            source[first_condition_start:first_occurrence_start],
        )
    for call in occurrences:
        line_start = _line_start(source, call.start)
        if (
            _line_is_plain_label(source, line_start)
            or _line_follows_plain_label(source, line_start)
            or _line_is_case_or_default_label(source, line_start)
            or _line_follows_case_or_default_label(source, line_start)
        ):
            return None
        context = _condition_bool_call_context(source, call)
        if context is None:
            return None
        line_start, condition_start, condition_end, _line_end_abs = context
        replacement = _condition_bool_replacement_for_call(
            source,
            call,
            temp_name=temp_name,
            condition_start=condition_start,
            condition_end=condition_end,
            guarded_pointer_names=pointer_arg_names,
            allow_guarded_reuse=(
                first_has_unguarded_condition_eval and call != first
            ),
        )
    ...
    if mutation_blocker is not None:
        return None
    first_scope = _brace_scope_stack_for_offset(source, function, first.start)
    if first_scope is None:
        return None
    occurrence_scopes = [
        _brace_scope_stack_for_offset(source, function, call.start)
        for call in occurrences
    ]
    if not _repeated_helper_occurrences_share_safe_region(
        source,
        ...
        return None
    has_prior_executable = _brace_scope_has_prior_executable_statement(
        spans,
        source,
        function,
        first_scope,
    ...
    affected_end = _line_end(source, occurrences[-1].end)
    if _region_has_preprocessor_directive(source[line_start:affected_end]):
        return None
    indent = _line_indent_at(source, line_start)
    reuse_mode = "condition-bool"
    if has_prior_executable:
        if not first_has_unguarded_condition_eval:
            return None
        decl_insert = _brace_scope_declaration_insert_offset(
            spans,
            source,
            function,
            first_scope,
        ...
        if decl_insert is None:
            return None
        replacements.append((decl_insert, decl_insert, f"{indent}int {temp_name};\n"))
        replacements.append((
            line_start,
            line_start,
            f"{indent}{temp_name} = {temp_initializer} != 0;\n",
        ))
        reuse_mode = "condition-bool-split"
    else:
        replacements.append((
            line_start,
            line_start,
            f"{indent}int {temp_name} = {temp_initializer} != 0;\n",
        ))
    return LifetimeLayoutProbe(
        label="repeated-helper-result-reuse-condition-bool-0",
        operator="repeated-helper-result-reuse",
        description=(
            f"Materialize repeated helper call `{first.callee}` into one bool temp."
        ),
        source_text=_replace_absolute_slices(source, replacements),
        provenance={
            "kind": "repeated-helper-result-reuse",
            "callee": first.callee,
            "call": first.call_text,
            "occurrences": len(occurrences),
            "temp_name": temp_name,
...
def _brace_scope_declaration_insert_offset(
    spans: list[StatementSpan],
    source: str,
    function: str,
    scope_stack: tuple[int, ...],
    ...
    if not scope_stack:
        return None
    insert = _line_end(source, scope_stack[-1])
    for span in spans:
        span_start, span_end = _span_char_range(source, span, byte_to_char)
        if span_end > first_offset:
            continue
        ...
        if span.kind != "declaration":
            break
        insert = _line_end(source, span_end)
    return insert
...
def _probe_frame_direct_literal_fp_calls(
    ...
    probes: list[LifetimeLayoutProbe] = []
    for cand in _fp_source_value_candidates(body):
        replacements = [
        ...
        if cand.source_kind == "literal":
            operator = "frame-direct-literal-at-final-fp-call"
        elif cand.source_kind == "global":
            operator = "frame-direct-fp-global-at-final-call"
        else:
            operator = "frame-direct-fp-expression-at-call"
        probes.append(LifetimeLayoutProbe(
            label=f"{operator}-{cand.name}",
            operator=operator,
            description=(
                f"Pass `{cand.value}` directly to {cand.call_name} instead "
...
def _probe_frame_split_fp_const_lifetimes(
    ...
    for cand in _fp_source_value_candidates(body):
        ...
        if cand.source_kind == "literal":
            ...
            operator = "frame-split-fp-global-lifetime"
        else:
            operator = "frame-split-conversion-scratch-at-call"
        probes.append(LifetimeLayoutProbe(
            label=f"{operator}-{cand.name}",
            operator=operator,
            description=(
                f"Move one-use FP value `{cand.name}` assignment next to "
...
def _probe_frame_magic_scratch_relocations(
    ...
    for assign in assign_re.finditer(body):
        ...
        if call is None:
            continue
        if not _call_has_top_level_arg(call.args, name):
            continue
        between = body[assign.end():call.call_start]
        if re.search(rf"\b{re.escape(name)}\b", between):
            continue
        insert = f"{indent}{name} = (f32) {expr.strip()};\n"
        if body[assign.end():call.call_start].strip() == "":
            continue
        mutated = _replace_body_slices(
            source,
            body_start,
            [
                (call.call_start, call.call_start, insert),
...
def _fp_source_value_candidates(body: str) -> list[_FpConstCandidate]:
    candidates: list[_FpConstCandidate] = []
    decl_re = re.compile(
        r"(?m)^([ \t]*)(f32|float|double)\s+([A-Za-z_]\w*)"
        r"\s*(?:=\s*([^;\n]+))?;[ \t]*\n?"
    )
    for decl in decl_re.finditer(body):
        indent, decl_type, name, init_value = decl.groups()
        value = init_value.strip() if init_value is not None else None
        if value is not None and not _safe_fp_source_value(value):
            continue
        source_kind = _fp_source_value_kind(value) if value is not None else None
        ...
        if value is None:
            ...
            if not _safe_fp_source_value(value):
                continue
            source_kind = _fp_source_value_kind(value)
            ...
            assign_text = f"{indent}{name} = {value};\n"
            expected_uses = 2
            search_start = decl.end()
            decl_replacement = f"{indent}{decl_type} {name};\n"
        ...
        if call is None:
            continue
        args = list(_split_top_level_args(call.args))
        replaced = False
        for idx, arg in enumerate(args):
            if arg.strip() == name:
                args[idx] = value
        ...
        if not replaced:
            continue
        call_replacement = (
            f"{call.indent}{call.callee}({', '.join(args)});{call.newline}"
        )
...
def _fp_source_value_kind(value: str | None) -> str | None:
    if value is None:
        return None
    stripped = value.strip()
    if re.fullmatch(_FLOAT_LITERAL_RE, stripped):
        return "literal"
...
def _safe_fp_source_value(value: str) -> bool:
    stripped = value.strip()
    if not stripped:
        return False
    if not _balanced_expression_delimiters(stripped):
        return False
...
def _find_later_call_using_name(
    ...
    for call in call_re.finditer(body, start):
        ...
        call_end = cursor + 1
        newline = ""
        if call_end < len(body) and body[call_end] == "\n":
            newline = "\n"
            call_end += 1
...
@dataclass(frozen=True)
class _PointerWalkIndexedControlFlowUse:
    loop_start: int
    loop_end: int
    loop_indent: str
    counter: str
...
def _probe_pointer_walk_loop(
    ...
    probes = _probe_pointer_walk_indexed_control_flow(
        source,
        body,
        body_start,
    )
...
def _probe_pointer_walk_indexed_control_flow(
    source: str,
    body: str,
    body_start: int,
) -> list[LifetimeLayoutProbe]:
    use = _find_pointer_walk_indexed_control_flow_use(source, body, body_start)
    if use is None:
        return []

    probes: list[LifetimeLayoutProbe] = []
    for probe in (
        _pointer_walk_indexed_data_field_direct_probe(source, use),
        _pointer_walk_indexed_data_byte_direct_probe(source, use),
        _pointer_walk_indexed_array_direct_probe(source, use, "array-direct"),
        _pointer_walk_indexed_array_direct_probe(source, use, "array-base-plus-index"),
        _pointer_walk_indexed_pad_typed_probe(source, use),
        _pointer_walk_indexed_pad_byte_probe(source, use),
        _pointer_walk_indexed_child_hoist_probe(source, use),
    ):
...
def _find_pointer_walk_indexed_control_flow_use(
    source: str,
    body: str,
    body_start: int,
) -> _PointerWalkIndexedControlFlowUse | None:
    code_body = _mask_c_non_code_text(body)
    do_re = re.compile(r"(?m)^(?P<indent>[ \t]*)do\s*\{")
    for match in do_re.finditer(code_body):
        loop_open = body_start + match.end() - 1
        loop_close = _find_matching_brace(source, loop_open)
        if loop_close is None or loop_close > body_start + len(body):
            continue
        tail = source[loop_close + 1 : loop_close + 160]
        tail_match = re.match(
            r"\s*while\s*\(\s*(?:\([^)]*\)\s*)*"
            r"(?P<counter>[A-Za-z_]\w*)\s*<\s*(?P<bound>[^)]+?)\s*\)\s*;",
            tail,
        )
        if tail_match is None:
            continue
        counter = tail_match.group("counter")
        bound = tail_match.group("bound").strip()
        loop_start = body_start + match.start()
        loop_end = loop_close + 1 + tail_match.end()
        loop_text = source[loop_start:loop_end]
        loop_body = source[loop_open + 1 : loop_close]
        if _region_has_preprocessor_directive(loop_text):
            continue
        if _offset_inside_preprocessor_region(source, body_start, loop_start):
            continue
        if re.search(r"\b(?:goto|break|continue)\b", _mask_c_non_code_text(loop_body)):
            continue
        if re.search(r"(?m)^[ \t]*[A-Za-z_]\w*\s*:", loop_body):
            continue
        if not _pointer_walk_indexed_counter_is_safe(loop_body, counter):
            continue

        prefix = source[body_start:loop_start]
        data_name = _pointer_walk_indexed_data_name(prefix)
        cursor_name, backing_array = _pointer_walk_indexed_cursor(prefix, counter)
        if cursor_name is not None and not _pointer_walk_indexed_cursor_is_safe(
            loop_body,
            cursor_name,
        ):
            cursor_name = None
            backing_array = None
        anchors = _pointer_walk_indexed_anchors(
            loop_body,
            counter=counter,
            data_name=data_name,
        ...
        if len(anchors) < 2:
            continue
        line_start = _line_col(source, loop_start)[0]
        line_end = _line_col(source, max(loop_start, loop_end - 1))[0]
        return _PointerWalkIndexedControlFlowUse(
...
def _pointer_walk_indexed_counter_is_safe(loop_body: str, counter: str) -> bool:
    code_body = _mask_c_non_code_text(loop_body)
    token = re.escape(counter)
    increments = list(
        re.finditer(
            rf"(?:\b{token}\b\s*\+\+|\+\+\s*\b{token}\b|"
            rf"\b{token}\b\s*\+=\s*1\b)",
            code_body,
        )
    )
    if len(increments) != 1:
        return False
    if not _pointer_walk_simple_increment_statement(code_body, increments[0], counter):
        return False
    ...
    if re.search(rf"(?<!&)&\s*\(*\s*\b{token}\b", remaining):
        return False
    if re.search(rf"\b{token}\b\s*(?:[+\-*/%&|^]=|<<=|>>=|=(?!=))", remaining):
        return False
    if re.search(rf"(?:\+\+|--)\s*\b{token}\b|\b{token}\b\s*(?:\+\+|--)", remaining):
        return False
    return True


def _pointer_walk_indexed_cursor_is_safe(loop_body: str, cursor: str) -> bool:
    code_body = _mask_c_non_code_text(loop_body)
    token = re.escape(cursor)
    increments = list(
        re.finditer(
            rf"(?:\b{token}\b\s*\+\+|\+\+\s*\b{token}\b|"
            rf"\b{token}\b\s*\+=\s*1\b)",
    ...
    if len(increments) != 1:
        return False
    if not _pointer_walk_simple_increment_statement(code_body, increments[0], cursor):
        return False
    scrubbed = list(code_body)
    for match in increments:
        for idx in range(match.start(), match.end()):
    ...
    if re.search(rf"(?:\+\+|--)\s*\b{token}\b|\b{token}\b\s*(?:\+\+|--)", remaining):
        return False
    if re.search(rf"(?<![A-Za-z0-9_])\*\s*{token}\b\s*=", remaining):
        return False
    return True


def _pointer_walk_simple_increment_statement(
    code_body: str,
    match: re.Match[str],
    name: str,
) -> bool:
    ...
    if line_end == -1:
        line_end = len(code_body)
    if _pointer_walk_brace_depth(code_body[:line_start]) != 0:
        return False
    for previous in reversed(code_body[:line_start].splitlines()):
        ...
        if previous == "else":
            return False
        if (
            re.match(r"(?:if|else\s+if|for|while|switch)\s*\(", previous)
            and "{" not in previous
        ):
            return False
        break
    line = code_body[line_start:line_end].strip()
    token = re.escape(name)
    return re.fullmatch(
        rf"(?:{token}\s*\+\+|\+\+\s*{token}|{token}\s*\+=\s*1)\s*;",
        line,
    ) is not None


def _pointer_walk_brace_depth(text: str) -> int:
    depth = 0
    for char in text:
        if char == "{":
            depth += 1
...
def _pointer_walk_indexed_data_name(prefix: str) -> str | None:
    code_prefix = _mask_c_non_code_text(prefix)
    matches = list(
        re.finditer(
            r"\b(?P<name>[A-Za-z_]\w*)\s*=\s*"
            r"(?:\([^;\n]+?\)\s*)?gobj\s*->\s*user_data\s*;",
            code_prefix,
        )
    )
    if matches:
...
def _pointer_walk_indexed_cursor(
    prefix: str,
    counter: str,
    ...
    indexed_matches = list(
        re.finditer(
            rf"\b(?P<cursor>[A-Za-z_]\w*)\s*=\s*&\s*"
            rf"(?P<array>[A-Za-z_]\w*)\s*\[\s*(?:\(u8\)\s*)?"
            rf"{token}\s*\]\s*;",
            code_prefix,
        )
    )
    if indexed_matches:
        match = indexed_matches[-1]
        return match.group("cursor"), match.group("array")
    base_matches = list(
        re.finditer(
            r"\b(?P<cursor>[A-Za-z_]\w*)\s*=\s*"
            r"(?P<array>[A-Za-z_]\w*)\s*;",
            code_prefix,
        )
    )
    for match in reversed(base_matches):
        if match.group("array").startswith("mnVibration_"):
            return match.group("cursor"), match.group("array")
    return None, None


def _pointer_walk_indexed_anchors(
    loop_body: str,
    *,
    counter: str,
    data_name: str | None,
    ...
    if data_name is not None:
        data = re.escape(data_name)
        if re.search(rf"\b{data}\s*->\s*(?:x6|x0)\s*\[", code_body):
            anchors.append(f"{data_name}->x")
        if re.search(rf"\(\s*\(u8\s*\*\)\s*{data}\s*\)\s*\[", code_body):
            anchors.append(f"(u8*){data_name}")
        if re.search(
            rf"\b{data}\s*\[\s*(?:\(u8\)\s*)?{re.escape(counter)}\s*\+\s*(?:2|6)\s*\]",
            code_body,
        ):
            anchors.append(f"{data_name}[]")
    if cursor_name is not None and re.search(
        rf"(?<![A-Za-z0-9_])\*\s*{re.escape(cursor_name)}\b",
        code_body,
    ):
        anchors.append(cursor_name)
    if "HSD_PadCopyStatus" in code_body:
        anchors.append("HSD_PadCopyStatus")
    if (
        "gobj->user_data" in code_body
        and "jobjs[23]" in code_body
        and "mnVibration_802480B4" in code_body
    ):
        anchors.append("child-walk")
    if re.search(rf"\b{re.escape(counter)}\b", code_body) is None:
        return []
    return anchors


def _pointer_walk_indexed_provenance(
    use: _PointerWalkIndexedControlFlowUse,
    variant: str,
    families: tuple[str, ...],
) -> dict[str, object]:
    data: dict[str, object] = {
        "kind": "pointer-walk-indexed-control-flow",
        "variant": variant,
        "counter": use.counter,
        "bound": use.bound,
        "families": list(families),
        "source_lines": list(use.source_lines),
        "anchors": list(use.anchors),
    }
    if use.data_name is not None:
        data["data"] = use.data_name
    if use.cursor_name is not None:
        data["cursor"] = use.cursor_name
    if use.backing_array is not None:
        data["backing_array"] = use.backing_array
    return data


def _pointer_walk_indexed_make_probe(
    source: str,
    use: _PointerWalkIndexedControlFlowUse,
    *,
    label: str,
    ...
    families: tuple[str, ...],
    description: str,
    loop_replacement: str,
) -> LifetimeLayoutProbe | None:
    if loop_replacement == source[use.loop_start:use.loop_end]:
        return None
    return LifetimeLayoutProbe(
        label=label,
        operator="pointer-walk-loop",
        ...
        source_text=_replace_absolute_slices(
            source,
            [(use.loop_start, use.loop_end, loop_replacement)],
        ),
        provenance=_pointer_walk_indexed_provenance(use, variant, families),
    )


def _pointer_walk_indexed_data_field_direct_probe(
    source: str,
    use: _PointerWalkIndexedControlFlowUse,
) -> LifetimeLayoutProbe | None:
    if use.data_name is None:
        return None
    loop_text = source[use.loop_start:use.loop_end]
    data = re.escape(use.data_name)
    counter = re.escape(use.counter)
    data_struct = _pointer_walk_indexed_data_struct_expr(use.data_name)
    rewritten = re.sub(
        rf"\b{data}\s*->\s*x6\s*\[\s*(?:\(u8\)\s*)?{counter}\s*\]",
        f"{data_struct}->x6[(u8) {use.counter}]",
        loop_text,
    )
    rewritten = re.sub(
        rf"\b{data}\s*->\s*x0\s*\[\s*(?:\(u8\)\s*)?{counter}\s*\+\s*2\s*\]",
        f"{data_struct}->x0[(u8) {use.counter} + 2]",
        rewritten,
    )
    rewritten = re.sub(
        rf"\(\s*\(u8\s*\*\)\s*{data}\s*\)\s*"
        rf"\[\s*(?:\(u8\)\s*)?{counter}\s*\+\s*6\s*\]",
        f"{data_struct}->x6[(u8) {use.counter}]",
        rewritten,
    )
    rewritten = re.sub(
        rf"\(\s*\(u8\s*\*\)\s*{data}\s*\)\s*"
        rf"\[\s*(?:\(u8\)\s*)?{counter}\s*\+\s*2\s*\]",
        f"{data_struct}->x0[(u8) {use.counter} + 2]",
        rewritten,
    )
    rewritten = re.sub(
        rf"\b{data}\s*\[\s*(?:\(u8\)\s*)?{counter}\s*\+\s*6\s*\]",
        f"{data_struct}->x6[(u8) {use.counter}]",
        rewritten,
    )
    rewritten = re.sub(
        rf"\b{data}\s*\[\s*(?:\(u8\)\s*)?{counter}\s*\+\s*2\s*\]",
        f"{data_struct}->x0[(u8) {use.counter} + 2]",
        rewritten,
    )
    return _pointer_walk_indexed_make_probe(
        source,
        use,
        label="pointer-walk-indexed-data-field-direct-0",
        variant="data-field-direct",
        families=("data-index-owner",),
        description=(
            f"Use typed direct-index data fields through `{use.data_name}` "
            f"and `{use.counter}`."
        ),
        loop_replacement=rewritten,
    )


def _pointer_walk_indexed_data_byte_direct_probe(
    source: str,
    use: _PointerWalkIndexedControlFlowUse,
) -> LifetimeLayoutProbe | None:
    if use.data_name is None:
    ...
    rewritten = re.sub(
        rf"\b{data}\s*->\s*x6\s*\[\s*(?:\(u8\)\s*)?{counter}\s*\]",
        f"((u8*) {use.data_name})[(u8) {use.counter} + 6]",
        loop_text,
    )
    rewritten = re.sub(
        rf"\b{data}\s*->\s*x0\s*\[\s*(?:\(u8\)\s*)?{counter}\s*\+\s*2\s*\]",
        f"((u8*) {use.data_name})[(u8) {use.counter} + 2]",
        rewritten,
    )
    return _pointer_walk_indexed_make_probe(
        source,
        use,
        label="pointer-walk-indexed-data-byte-direct-0",
        variant="data-byte-direct",
        families=("data-index-owner",),
        description=(
            f"Use byte-offset data indexing through `{use.data_name}` and "
            f"`{use.counter}`."
        ),
        loop_replacement=rewritten,
    )
...
def _pointer_walk_indexed_data_struct_expr(data_name: str) -> str:
    return f"((MnVibrationData*) {data_name})"


def _pointer_walk_indexed_array_direct_probe(
    source: str,
    use: _PointerWalkIndexedControlFlowUse,
    variant: str,
) -> LifetimeLayoutProbe | None:
    if use.cursor_name is None or use.backing_array is None:
        return None
    loop_text = source[use.loop_start:use.loop_end]
    replacement = (
        f"{use.backing_array}[{use.counter}]"
        if variant == "array-direct"
        else f"*({use.backing_array} + {use.counter})"
    )
    rewritten = re.sub(
        rf"(?<![A-Za-z0-9_])\*\s*{re.escape(use.cursor_name)}\b",
        replacement,
        loop_text,
    )
    label = (
        ...
        if variant == "array-direct"
        else "pointer-walk-indexed-array-base-plus-index-0"
    )
    return _pointer_walk_indexed_make_probe(
        source,
        use,
        label=label,
        variant=variant,
        families=("mnVibration_804D4FE8-index-owner",),
        description=(
            f"Rewrite cursor dereference `{use.cursor_name}` through "
            f"`{replacement}`."
        ),
        loop_replacement=rewritten,
    )
...
def _pointer_walk_indexed_pad_typed_probe(
    source: str,
    use: _PointerWalkIndexedControlFlowUse,
) -> LifetimeLayoutProbe | None:
    loop_text = source[use.loop_start:use.loop_end]
    counter = re.escape(use.counter)
    raw_pad_re = re.compile(
        rf"\*\s*\(\s*\(u8\s*\*\)\s*"
        rf"\(\s*\(u8\s*\*\)\s*HSD_PadCopyStatus\s*\+\s*"
        rf"(?:\(u8\)\s*)?{counter}\s*\*\s*0x44\s*\)\s*"
        rf"\+\s*0x41\s*\)"
    )
    rewritten = raw_pad_re.sub(
        f"HSD_PadCopyStatus[(u8) {use.counter}].err",
        loop_text,
    )
    return _pointer_walk_indexed_make_probe(
        source,
        use,
        label="pointer-walk-indexed-pad-typed-base-0",
        variant="pad-typed-base",
        families=("HSD_PadCopyStatus-base-owner",),
        description="Use typed direct-index HSD pad status error field.",
        loop_replacement=rewritten,
    )


def _pointer_walk_indexed_pad_byte_probe(
    source: str,
    use: _PointerWalkIndexedControlFlowUse,
) -> LifetimeLayoutProbe | None:
    loop_text = source[use.loop_start:use.loop_end]
    counter = re.escape(use.counter)
    typed_pad_re = re.compile(
        rf"\bHSD_PadCopyStatus\s*\[\s*(?:\(u8\)\s*)?"
        rf"{counter}\s*\]\s*\.\s*err\b"
    )
    rewritten = typed_pad_re.sub(
        (
            f"*((u8*) ((u8*) HSD_PadCopyStatus + "
            f"(u8) {use.counter} * 0x44) + 0x41)"
        ),
        loop_text,
    )
    return _pointer_walk_indexed_make_probe(
        source,
        use,
        label="pointer-walk-indexed-pad-byte-base-0",
        variant="pad-byte-base",
        families=("HSD_PadCopyStatus-base-owner",),
        description="Use raw byte-offset HSD pad status error field.",
        loop_replacement=rewritten,
    )


def _pointer_walk_indexed_child_hoist_probe(
    source: str,
    use: _PointerWalkIndexedControlFlowUse,
) -> LifetimeLayoutProbe | None:
    if use.data_name is None:
    ...
    loop_text = source[use.loop_start:use.loop_end]
    rewritten = re.sub(
        r"\(\s*\(\s*MnVibrationData\s*\*\s*\)\s*gobj\s*->\s*user_data\s*\)"
        r"\s*->\s*jobjs\s*\[\s*23\s*\]",
        f"{_pointer_walk_indexed_data_struct_expr(use.data_name)}->jobjs[23]",
        loop_text,
    )
    return _pointer_walk_indexed_make_probe(
        source,
        use,
        label="pointer-walk-indexed-child-hoist-data-0",
        variant="child-hoist-data",
        families=("child-walk-call-hoist",),
        description=(
            f"Use stable `{use.data_name}` local for repeated child-walk "
            "jobjs[23] ownership."
        ),
        loop_replacement=rewritten,
    )
