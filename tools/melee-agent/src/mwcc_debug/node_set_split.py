...
from .source_field_attribution import (
    ...
    parse_pcode_load_expression,
    parse_symbolic_global_load_expression,
    source_for_field_offset,
    source_for_global_symbol,
    source_for_stack_array_base,
    source_for_stack_array_field_offset,
)
...
_GENERATED_POINTER_WALK_LOCAL_RE = re.compile(r"^ll_probe_iter_\d+$")
...
def request_from_node_set_delta(
    ...
    field_context = (
        build_source_field_context(
            source_text,
            function=str(delta.get("function") or "") or None,
...
def generate_node_set_split_patches(
    ...
    *,
    ...
    def done() -> bool:
        return _deadline_expired(deadline) or (
            max_candidates is not None and len(patches) >= max_candidates
        )

    def ordered_result() -> list[CandidatePatch]:
        return _order_node_set_patches_for_search(patches, cap=max_candidates)

    _append_generated_pointer_walk_source_binding_patches(
        patches,
        seen_sources,
        source,
        fn_name,
    ...
    _append_generated_pointer_walk_initializer_patches(
        patches,
        seen_sources,
        source,
        fn_name,
...
def generate_node_set_introduce_binding_patches(
    ...
    *,
    ...
    if (
        not is_node_set_request_introducible(request)
        or _deadline_expired(deadline)
        or max_candidates == 0
    ):
        return []
    ...
    if not _source_expression_is_safe_to_bind(expression):
        return []
    if request.source_kind == "stack-array-base":
        return generate_node_set_stack_array_base_patches(
            source,
            function,
            request,
    ...
    for site_idx, site in enumerate(sites):
        ...
        remaining = None if max_candidates is None else max_candidates - len(patches)
        if remaining == 0:
            break
...
def generate_node_set_stack_array_base_patches(
    source: str,
    function: str,
    request: NodeSetSplitRequest | None,
    *,
    ...
    """Generate typed pointer bindings for a local stack-array base temp."""
    if (
        not is_node_set_request_introducible(request)
        or request is None
        or request.source_kind != "stack-array-base"
        or _deadline_expired(deadline)
        or max_candidates == 0
    ):
        return []
    ...
    if (
        expression is None
        or binding_type is None
        or not _is_simple_identifier(expression)
        or "*" not in binding_type
    ):
        return []
    fn_name = function or request.function
    sites = _introduce_binding_sites(
    ...
    for site_idx, site in enumerate(sites):
        ...
        _append_unique_patch(
            ...
            bound_source,
            candidate_id=(
                f"node-split-stack-array-base-ig{request.target_ig}-"
                f"{_candidate_id_fragment(binding_name)}-bind-site{site_idx}"
            ),
            summary=(
                f"Introduce typed stack-array base binding {binding_name} for "
                f"{expression!r} targeting ig{request.target_ig}."
            ),
        )
    return patches
    ...
    *,
...
) -> list[CandidatePatch] | NodeSetGenerationResult:
    ...
    def _finish(
        ...
        *,
        ...
        result = NodeSetGenerationResult(
            patches=patches,
            stop_reason=stop_reason,
            elapsed_seconds=time.monotonic() - started_at,
            budget_seconds=(
                None if deadline is None else max(0.0, deadline - started_at)
            ),
            candidate_limit=candidate_limit,
            generation_complete=generation_complete,
        )
        return result if return_result else result.patches
    ...
    frontier_budget_exhausted = False
    ...
    for request_index, request in enumerate(requests):
        ...
        for cur_source, ids, summaries in frontier:
            ...
            for patch in ordered_singles:
                if not hit_budget and _deadline_expired(deadline):
                    hit_budget = True
                    break
                ...
                if (
                    ...
                    and len(next_frontier) >= candidate_limit
                ):
        ...
        if not next_frontier:
            ...
            return _finish(
                [],
                stop_reason="budget-exhausted" if hit_budget else None,
                generation_complete=not hit_budget,
            )
    ...
    generation_complete = not (frontier_limited or frontier_budget_exhausted)
    for final_source, _ids, summaries in frontier:
        if not frontier_budget_exhausted and _deadline_expired(deadline):
            stop_reason = "budget-exhausted"
            generation_complete = False
            break
        ...
        if candidate_limit is not None and len(patches) >= candidate_limit:
            if len(frontier) > len(patches):
...
def evaluate_node_set_split_signature(
    ...
    *,
    ...
    target_score = _node_set_force_phys_target_score(
        candidate,
        request,
...
def _node_set_force_phys_target_score(
    candidate: BaselineSignature,
    requests: NodeSetSplitRequest | list[NodeSetSplitRequest],
    *,
    force_phys: Mapping[int, int] | None = None,
) -> dict[str, Any] | None:
    targets: dict[int, int] = {}
    for ig_idx, phys in (force_phys or {}).items():
        try:
            targets[int(ig_idx)] = int(phys)
        except (TypeError, ValueError):
            continue
    request_list = requests if isinstance(requests, list) else [requests]
    for request in request_list:
        target_nums = _target_reg_numbers(request)
        if target_nums and request.target_ig not in targets:
            targets[request.target_ig] = target_nums[0]
    if not targets:
        return None
    ...
    matched = 0
    distance_total = 0
    all_distances_known = True
    for ig_idx, expected in sorted(targets.items()):
    ...
    return {
        ...
        "virtuals": virtuals,
        "distance_total": distance_total if all_distances_known else None,
    }
...
def summarize_node_set_split_scores(
    ...
    *,
    ...
    generated_candidate_cap_reached = (
        candidate_limit is not None and len(patches) >= candidate_limit
    )
    effective_stop_reason = stop_reason
    if effective_stop_reason is None and generated_candidate_cap_reached:
        effective_stop_reason = "candidate-limit"

    if effective_stop_reason == "budget-exhausted":
        status = "exhausted"
    elif not patches:
        status = "blocked"
    elif improved:
    ...
    final_candidate_ids = {
        ...
        and not _node_set_split_row_needs_resume(row)
    }
    pending_count = sum(
        1 for patch in patches
        if str(patch.candidate_id) not in final_candidate_ids
    )
    ...
    wrong_register_exhausted = (
        bool(patches)
        and bool(rows)
        and effective_stop_reason is None
        and pending_count == 0
    ...
    stop_condition = None
    if effective_stop_reason is not None:
        stop_condition = {
            "kind": effective_stop_reason,
            "candidate_limit": candidate_limit,
            "budget_seconds": budget_seconds,
            "elapsed_seconds": elapsed_seconds,
        }
        if resume_command:
            stop_condition["resume_command"] = resume_command
    summary = {
        "status": status,
        "function": function,
    ...
    if case_c_order_repair is not None:
        summary["case_c_order_repair"] = case_c_order_repair
        summary["next_steps"].append(
            "Case-C order repair: use case_c_order_repair.routes against the "
            "retained pcdump/source before rerunning node-set-split"
        )
    ...
    proof_requests = coupled_requests if coupled_requests is not None else [request]
    stack_array_proof = _stack_array_base_terminal_proof(
    ...
    if stack_array_proof is not None:
        summary["terminal_reason"] = "stack-array-base-targets-not-realized"
        summary["terminal_proof"] = stack_array_proof
    return summary


def _stack_array_base_terminal_proof(
    *,
    requests: list[NodeSetSplitRequest],
    rows: list[dict[str, Any]],
    generated_count: int,
    ...
    if (
        stop_reason in {"budget-exhausted", "candidate-limit"}
        or pending_count
        or generated_count <= 0
        or not rows
        or not any(req.source_kind == "stack-array-base" for req in requests)
    ):
        return None
    if not requests or any(not _target_reg_names(req) for req in requests):
        return None
    if not all(
        _stack_array_row_has_target_score(row, requests)
        for row in rows
    ):
        return None
    if any(_stack_array_row_hits_all_targets(row, requests) for row in rows):
        return None
    return {
        "source_family": "stack-array-base",
        "target_registers": {
            str(req.target_ig): _target_reg_names(req)[0]
            for req in requests
        },
        "generated_count": generated_count,
        "evaluated_count": len(rows),
        "candidates": [
            _stack_array_terminal_candidate(row, requests)
            for row in rows
        ],
    }


def _stack_array_row_has_target_score(
    row: Mapping[str, Any],
    requests: list[NodeSetSplitRequest],
) -> bool:
    ...
    if not isinstance(virtuals, Mapping):
        return False
    return all(str(request.target_ig) in virtuals for request in requests)


def _stack_array_row_hits_all_targets(
    row: Mapping[str, Any],
    requests: list[NodeSetSplitRequest],
) -> bool:
    target_score = row.get("target_score")
    if not isinstance(target_score, Mapping):
        return row.get("objective_status") == "realized"
    virtuals = target_score.get("virtuals")
    if not isinstance(virtuals, Mapping):
        return False
    for request in requests:
        info = virtuals.get(str(request.target_ig))
        if not isinstance(info, Mapping) or not info.get("hit"):
            return False
    return True


def _stack_array_terminal_candidate(
    row: Mapping[str, Any],
    requests: list[NodeSetSplitRequest],
) -> dict[str, Any]:
    target_score = row.get("target_score")
    virtuals = (
        target_score.get("virtuals")
    ...
    for request in requests:
        ...
        if isinstance(virtuals, Mapping):
            ...
            if isinstance(info, Mapping):
                actual = _as_int(info.get("actual"))
        assigned[str(request.target_ig)] = (
            _register_name_for_class(request.class_id, actual)
    ...
    candidate = {
        ...
        "source_retained_exists": source_retained is not None,
        "pcdump_path": pcdump_path,
        "pcdump_path_exists": pcdump_path is not None,
    }
    if row.get("source_hunks") is not None:
        candidate["source_hunks"] = row.get("source_hunks")
    return candidate


def _case_c_order_repair_handoff(
    *,
    function: str,
    request: NodeSetSplitRequest,
    rows: list[dict[str, Any]],
    ...
    """Build the next repair lane after FPR node-set candidates all miss.

    An all-wrong-register FPR node-set run proves the source-shape split was
    materialized but selected/dispensed into the wrong physical register. The
    next source-actionable lane is allocator order repair, so carry forward the
    retained source/pcdump and the widest force-phys target we have.
    """
    if not wrong_register_exhausted or request.class_id != 1:
        return None
    wrong_rows = [
        row for row in rows
        if row.get("objective_status") == "wrong-register"
    ]
    if not wrong_rows:
        return None

    target_score = _node_set_first_target_score(wrong_rows)
    force_phys = _node_set_force_phys_csv(target_score, request=request)
    if not force_phys:
        return None
    row = _node_set_best_order_repair_row(wrong_rows)
    source_file = _node_set_row_source_file(row)
    pcdump_path = _node_set_row_pcdump_path(row)
    target_orders = _node_set_target_order_csv(
        row,
        function=function,
        request=request,
    )

    routes = _node_set_case_c_order_routes(
        function=function,
        class_id=request.class_id,
        force_phys=force_phys,
        source_file=source_file,
    ...
    if isinstance(target_score, Mapping):
        handoff["target_score"] = dict(target_score)
    if target_orders is not None:
        handoff["target_order"] = target_orders
    return handoff


def _node_set_first_target_score(rows: list[dict[str, Any]]) -> Mapping[str, Any] | None:
    for row in rows:
        target_score = row.get("target_score")
        if isinstance(target_score, Mapping):
            return target_score
...
def _node_set_force_phys_csv(
    ...
    *,
    ...
    if isinstance(target_score, Mapping):
        virtuals = target_score.get("virtuals")
        if isinstance(virtuals, Mapping):
            for raw_ig, raw_info in virtuals.items():
                ig_idx = _as_int(raw_ig)
                info = raw_info if isinstance(raw_info, Mapping) else {}
                expected = _as_int(info.get("expected"))
                if ig_idx is not None and expected is not None:
                    pairs.append((ig_idx, expected))
    if not pairs:
        target_nums = _target_reg_numbers(request)
        if target_nums:
            pairs.append((request.target_ig, target_nums[0]))
    if not pairs:
        return None
    return ",".join(f"{ig_idx}:{phys}" for ig_idx, phys in sorted(set(pairs)))


def _node_set_best_order_repair_row(rows: list[dict[str, Any]]) -> dict[str, Any]:
    def rank(row: dict[str, Any]) -> tuple[int, int]:
        return (
            0 if _node_set_row_pcdump_path(row) else 1,
            0 if _node_set_row_source_file(row) else 1,
        )

    return sorted(rows, key=rank)[0]


def _node_set_row_source_file(row: Mapping[str, Any]) -> str | None:
    source = row.get("source_retained")
    if source is not None:
        return str(source)
    objective = row.get("objective")
    if isinstance(objective, Mapping) and objective.get("source_path") is not None:
        return str(objective["source_path"])
    return None


def _node_set_row_pcdump_path(row: Mapping[str, Any]) -> str | None:
    pcdump_path = row.get("pcdump_path")
    if pcdump_path is not None:
        return str(pcdump_path)
    objective = row.get("objective")
    if isinstance(objective, Mapping) and objective.get("pcdump_path") is not None:
        return str(objective["pcdump_path"])
    return None


def _node_set_target_order_csv(
    row: Mapping[str, Any],
    *,
    function: str,
    request: NodeSetSplitRequest,
    ...
    if isinstance(objective, Mapping):
        leads = objective.get("target_color_select_order_leads")
        if isinstance(leads, list):
            for lead in leads:
                if not isinstance(lead, Mapping):
                    continue
                order = lead.get("target_order")
                if (
                    isinstance(order, (list, tuple))
                    and len(order) == 2
                    and _as_int(order[0]) is not None
                    and _as_int(order[1]) is not None
                ):
                    parts.append(f"r{int(order[0])}<r{int(order[1])}")
    if not parts:
        parts.extend(
            _node_set_target_orders_from_retained_pcdump(
                row,
                function=function,
                request=request,
            )
...
def _node_set_target_orders_from_retained_pcdump(
    row: Mapping[str, Any],
    *,
    function: str,
    request: NodeSetSplitRequest,
    ...
    if not pcdump_path:
        return []
    try:
        pcdump_text = Path(pcdump_path).read_text(
            encoding="utf-8",
            errors="replace",
        )
    except OSError:
    ...
    try:
        ...
        ig = tb.load_ig(pcdump_text, function, class_id=request.class_id)
        if ig is None:
            return []
        assigned = tb.predict_assignments(ig)
    except Exception:
    ...
    target_ig = int(request.target_ig)
    target_regs = _target_reg_numbers(request)
    nodes = getattr(ig, "nodes", {}) or {}
    target_node = nodes.get(target_ig)
    order = [int(item) for item in getattr(ig, "select_order", ()) or ()]
    order_index = {ig_idx: index for index, ig_idx in enumerate(order)}
    target_pos = order_index.get(target_ig, 0)

    candidate_orders: list[tuple[int, int, str]] = []
    for desired_reg in target_regs:
        for owner_ig, assigned_reg in assigned.items():
            owner = int(owner_ig)
            owner_node = nodes.get(owner)
            observed_reg = getattr(owner_node, "observed_reg", None)
            if owner == target_ig or (
                int(assigned_reg) != desired_reg
                and observed_reg != desired_reg
            ):
                continue
            if (
                target_node is not None
                and owner in getattr(target_node, "neighbors", set())
            ):
                continue
            if (
                owner_node is not None
                and target_ig in getattr(owner_node, "neighbors", set())
            ):
                continue
            owner_pos = order_index.get(owner, target_pos)
            # Prefer owners already after the target; moving them before the
            # target is the minimal Case-C reuse probe.
            before_target = 0 if owner_pos > target_pos else 1
            candidate_orders.append((
                before_target,
                abs(owner_pos - target_pos),
                f"r{owner}<r{target_ig}",
            ))
    out: list[str] = []
    for _rank, _distance, order_text in sorted(candidate_orders):
        if order_text not in out:
            out.append(order_text)
        if len(out) >= max_orders:
            break
    return out


def _node_set_case_c_order_routes(
    *,
    function: str,
    class_id: int,
    force_phys: str,
    ...
    target_orders: str | None,
) -> list[dict[str, Any]]:
    routes: list[dict[str, Any]] = []
    first_divergence = ["melee-agent", "debug", "inspect", "first-divergence"]
    if pcdump_path:
        first_divergence.append(pcdump_path)
    first_divergence.extend([
        "-f", function,
        "--class", str(class_id),
        "--force-phys", force_phys,
        "--source",
    ])
    routes.append({
        "rank": 1,
        "kind": "inspect-retained-first-divergence",
        "command": _node_set_shell_command(first_divergence),
    })
    ...
    if target_orders is not None:
        ...
        if pcdump_path:
            select_order.extend(["--pcdump", pcdump_path])
        if source_file:
            select_order.extend(["--source-file", source_file])
        select_order.extend([
            "--include-transform-corpus",
            "--transform-force-phys", force_phys,
            "--json",
        ])
        routes.append({
            "rank": rank,
            "kind": "retained-source-select-order-repair",
            "command": _node_set_shell_command(select_order),
        })
        rank += 1
    ...
    if pcdump_path:
        permuter.extend(["--baseline-dump", pcdump_path])
    routes.append({
        "rank": rank,
        "kind": "setup-force-phys-simplify-order-permuter",
        "command": _node_set_shell_command(permuter),
    })
    rank += 1
    ...
    if source_file:
        plan.extend(["--source-file", source_file])
    plan.append("--json")
    routes.append({
        "rank": rank,
        "kind": "plan-transform-corpus-from-retained-source",
        "command": _node_set_shell_command(plan),
    })
    return routes


def _node_set_shell_command(parts: list[str]) -> str:
...
def _node_set_split_next_steps(
    *,
    ...
    if (
        resume_command
        and stop_reason not in {"candidate-limit", "budget-exhausted"}
    ):
        steps.append(resume_command)
    return steps
...
def _request_from_missing_virtual(
    ...
    *,
    ...
    source_file: str | Path | None = None,
    missing_virtuals: list[Any] | None = None,
    ...
    source_map = _enriched_node_set_source_map(
        source_map,
        function=function,
        source_text=source_text,
        source_file=source_file,
    ...
    var_name = (
        None
        if _source_requests_introduced_binding(source_map)
        else _bindable_source_name(source_map)
    )
    ...
    if var_name is None:
        if not (
            ...
            and _source_expression_is_safe_to_bind(source_expression)
        ):
            blocked_reason = (
...
def _enriched_node_set_source_map(
    source: Mapping[str, Any],
    *,
    function: str,
    source_text: str | None,
    source_file: str | Path | None,
    missing_virtuals: list[Any] | None,
    field_context: Any | None = None,
) -> Mapping[str, Any]:
    """Upgrade stale pcode-only node-set sources to typed C expressions."""
    if not source_text or not isinstance(source, Mapping):
        return source
    if _optional_str(source.get("type")) and _optional_str(source.get("expression")):
        return source
    expression = _optional_str(source.get("expression"))
    if expression is None:
        return source
    parsed = parse_pcode_load_expression(expression)
    symbolic = parse_symbolic_global_load_expression(expression)

    context = field_context or build_source_field_context(
        source_text,
        function=function or None,
        source_file=source_file,
    )
    stack_base = source_for_stack_array_base(context, expression)
    if stack_base is not None:
        out = dict(source)
        out.update({
            "kind": "stack-array-base",
            "confidence": stack_base.confidence,
            "name": stack_base.base_var,
            "base_var": stack_base.base_var,
            "expression": stack_base.expression,
            "type": stack_base.type,
            "source_line": stack_base.source_line,
            "source_col": stack_base.source_col,
            "introduce_binding": True,
        })
        return out
    if parsed is None and symbolic is None:
        return source

    entries_by_virtual = _node_set_entries_by_pcode_dest(missing_virtuals)
    cache: dict[int, Mapping[str, Any] | None] = {}

    def resolve_virtual(virtual: int) -> Mapping[str, Any] | None:
        if virtual in cache:
            return cache[virtual]
        cache[virtual] = None
        entry = entries_by_virtual.get(virtual)
        if entry is None:
            return None
        raw = entry.get("source")
        if not isinstance(raw, Mapping):
            return None
        cache[virtual] = _resolve_pcode_source_map(
            raw,
            context=context,
    ...
    enriched = _resolve_pcode_source_map(
        source,
        context=context,
        resolve_virtual=resolve_virtual,
    )
    return source if enriched is None else enriched


def _node_set_entries_by_pcode_dest(
    missing_virtuals: list[Any] | None,
) -> dict[int, Mapping[str, Any]]:
    out: dict[int, Mapping[str, Any]] = {}
    if not isinstance(missing_virtuals, list):
        return out
    for item in missing_virtuals:
        if not isinstance(item, Mapping):
            continue
        source = item.get("source")
        source_map = source if isinstance(source, Mapping) else {}
        expression = _optional_str(source_map.get("expression"))
        parsed = parse_pcode_load_expression(expression)
        if parsed is not None:
            dest, _offset, _base = parsed
            if dest is not None:
                out.setdefault(dest, item)
        target_ig = _as_int(item.get("target_ig"))
        if target_ig is not None:
            out.setdefault(target_ig, item)
    return out


def _resolve_pcode_source_map(
    source: Mapping[str, Any],
    *,
    context,
    resolve_virtual,
    ...
    if kind in {"field-load", "global-load", "stack-array-base"}:
        return dict(source)
    stack_base = source_for_stack_array_base(context, expression)
    if stack_base is not None:
        out = dict(source)
    ...
    if symbolic is not None:
        _dest, symbol = symbolic
        resolved = source_for_global_symbol(context, symbol)
        if resolved is None:
            return None
        out = dict(source)
        out.update({
    ...
    if parsed is None:
        return None
    _dest, offset, base_virtual = parsed
    base = resolve_virtual(base_virtual)
    resolved = None
    if base is not None:
        base_expression = (
            _optional_str(base.get("expression"))
            or _optional_str(base.get("name"))
        )
        if _optional_str(base.get("kind")) == "stack-array-base":
            resolved = source_for_stack_array_field_offset(
                context,
                base_expression=base_expression,
                base_type=_optional_str(base.get("type")),
                offset=offset,
            )
        else:
    ...
    confidence = "recursive-source-span" if resolved is not None else None
    if resolved is None:
        resolved = infer_global_field_source(context, offset=offset)
    ...
    out.update({
        "kind": "field-load",
        "confidence": confidence or "source-span",
        "type": resolved.type,
        "expression": resolved.expression,
...
def _source_requests_introduced_binding(source: Mapping[str, Any]) -> bool:
    return (
        source.get("introduce_binding") is True
        or _optional_str(source.get("kind")) == "synthetic-owner-split"
        or _optional_str(source.get("kind")) == "stack-array-base"
    )
...
def _bindable_source_name(source: Mapping[str, Any]) -> str | None:
    ...
    if _is_simple_identifier(expression):
        return expression
    if _is_field_expression(expression):
        return None
...
def _generated_pointer_walk_local_blocker(
    source_text: str,
    function: str,
    request: NodeSetSplitRequest,
) -> dict[str, Any] | None:
    var_name = request.var_name
    if var_name is None or _GENERATED_POINTER_WALK_LOCAL_RE.fullmatch(var_name) is None:
        return None
    if not _source_declares_name(source_text, function, var_name):
        return None
    extracted = _extract_function_text(source_text, function)
    if extracted is None:
        return None
    ...
    decl_match = re.search(
        rf"(?m)^\s*(?P<type>[A-Za-z_][A-Za-z_0-9\s\*]*?)"
        rf"\b{re.escape(var_name)}\s*=\s*"
        rf"(?P<base>[A-Za-z_][A-Za-z_0-9]*)\s*;",
        body_text,
    )
    if decl_match is None:
        return None
    mapped_base = decl_match.group("base")
    if not _is_simple_identifier(mapped_base):
        return None

    header_match = re.search(
        rf"for\s*\((?P<header>[^)]*\b{re.escape(var_name)}\b[^)]*)\)",
        body_text,
        re.DOTALL,
    )
    header = header_match.group("header") if header_match is not None else ""
    counter_match = re.search(
        r"\b(?P<counter>[A-Za-z_][A-Za-z_0-9]*)\s*=\s*[^;]*;",
        header,
    )
    counter = counter_match.group("counter") if counter_match is not None else None
    increment_uses_local = bool(re.search(
        rf"(?:\b{re.escape(var_name)}\s*(?:\+\+|--|\+=\s*1)"
        rf"|(?:\+\+|--)\s*{re.escape(var_name)}\b)",
        header,
    ))
    deref_store_uses_local = bool(re.search(
        rf"\*\s*{re.escape(var_name)}\s*=",
        body_text,
    ))
    if not (increment_uses_local or deref_store_uses_local):
        return None

    fallback_vars: list[str] = []
    for candidate in (mapped_base, counter):
        if (
            candidate is not None
            and _is_simple_identifier(candidate)
            and candidate not in fallback_vars
            and _source_declares_name(source_text, function, candidate)
        ):
            fallback_vars.append(candidate)

    return {
        "kind": "generated-pointer-walk-local-no-read-sites",
        "status": "terminal-blocker",
        "var_name": var_name,
        "declared": True,
        "reason": (
            "generated pointer-walk induction local has no safe read-site "
            "anchors for current node-set mutators; uses are loop-header "
            "increment and dereferenced store"
        ),
        "mapped_base": mapped_base,
        "mapped_counter": counter,
        "candidate_fallback_vars": fallback_vars,
        "evidence": {
            "loop_header_increment": increment_uses_local,
            "dereferenced_store": deref_store_uses_local,
        },
    }


def _append_generated_pointer_walk_initializer_patches(
    patches: list[CandidatePatch],
    seen_sources: set[str],
    source: str,
    function: str,
    ...
    if request.blocked_reason is not None:
        return
    rewrite = _generated_pointer_walk_initializer_rewrite(
        source,
        function,
        var_name,
    )
    ...
    patches.append(CandidatePatch(
        ...
        summary=(
            f"Rewrite generated pointer-walk local {var_name} initializer "
            f"from {base} to {base_expr} targeting ig{request.target_ig}."
        ),
        touched_ranges=((start, end),),
        hunk=_patch_hunk(source, patched_source, candidate_id),
        metadata={
            "kind": "generated-pointer-walk-initializer",
            "var_name": var_name,
            "base": base,
            "base_expr": base_expr,
        },
...
def _append_generated_pointer_walk_source_binding_patches(
    patches: list[CandidatePatch],
    seen_sources: set[str],
    source: str,
    function: str,
    ...
    var_name = request.var_name
    expression = (
        request.source_expression.strip()
        if isinstance(request.source_expression, str)
        else None
    )
    binding_type = _normalize_safe_binding_type(request.source_type)
    if (
        var_name is None
        or _GENERATED_POINTER_WALK_LOCAL_RE.fullmatch(var_name) is None
        or request.blocked_reason is not None
        or not expression
        or binding_type is None
        or not _source_expression_is_safe_to_bind(expression)
    ):
        return
    rewrite = _generated_pointer_walk_source_binding_rewrite(
        source,
        function,
        var_name,
        expression=expression,
    ...
    if rewrite is None:
        return
    start, end, replacement, binding_name = rewrite
    patched_source = source[:start] + replacement + source[end:]
    if patched_source == source or patched_source in seen_sources:
        return
    candidate_id = (
        f"node-split-generated-pointer-walk-source-{var_name}-"
        f"ig{request.target_ig}"
    )
    seen_sources.add(patched_source)
    patches.append(CandidatePatch(
        ...
        patched_source=patched_source,
        summary=(
            f"Introduce source binding {binding_name} for generated "
            f"pointer-walk local {var_name} targeting ig{request.target_ig}."
        ),
        touched_ranges=((start, end),),
        hunk=_patch_hunk(source, patched_source, candidate_id),
        metadata={
            "kind": "generated-pointer-walk-source-binding",
            "var_name": var_name,
            "binding_name": binding_name,
            "source_expression": expression,
            "source_type": binding_type,
        },
    ))


def _generated_pointer_walk_source_binding_rewrite(
    source: str,
    function: str,
    var_name: str,
    *,
    ...
    decl_re = re.compile(
        ...
        r"(?:struct\s+[A-Za-z_][A-Za-z_0-9]*|[A-Za-z_][A-Za-z_0-9]*)"
        r"(?:\s+[A-Za-z_][A-Za-z_0-9]*)*\s*\*+)\s*"
        rf"{re.escape(var_name)}\s*=\s*"
        r"(?P<rhs>[^;\n]+)\s*;\s*$"
    )
    for decl_match in decl_re.finditer(body):
        rhs_start = body_start + decl_match.start("rhs")
        rhs_end = body_start + decl_match.end("rhs")
        rhs = source[rhs_start:rhs_end].strip()
        if not _node_set_same_expression(rhs, expression):
            continue
        decl_start = body_start + decl_match.start()
        decl_end = body_start + decl_match.end()
        line_text = source[decl_start:decl_end]
        line_rhs_start = rhs_start - decl_start
        line_rhs_end = rhs_end - decl_start
        binding_name = _unique_binding_name(
            source,
            f"{var_name}_source_{target_ig}_0",
        )
        indent = decl_match.group("indent")
        binding_line = f"{indent}{binding_type} {binding_name} = {expression};\n"
        replacement = (
            binding_line
            + line_text[:line_rhs_start]
            + binding_name
            + line_text[line_rhs_end:]
        )
        return decl_start, decl_end, replacement, binding_name
    return None


def _node_set_same_expression(left: str, right: str) -> bool:
    return " ".join(left.split()) == " ".join(right.split())


def _generated_pointer_walk_initializer_rewrite(
    source: str,
    function: str,
    var_name: str,
    ...
    decl_re = re.compile(
        ...
        r"(?:\s+[A-Za-z_][A-Za-z_0-9]*)*\s*\*+)\s*"
        rf"{re.escape(var_name)}\s*=\s*"
        r"(?P<base>[A-Za-z_][A-Za-z_0-9]*)\s*;\s*$"
    )
    decl_match = decl_re.search(body)
    if decl_match is None:
        return None
    base = decl_match.group("base")
    if not _is_simple_identifier(base):
        return None
    decl_start = body_start + decl_match.start()
    decl_end = body_start + decl_match.end()
    base_expr = _generated_pointer_walk_base_initializer(
        source,
        stripped,
        function,
        base,
    ...
    if base_expr is None or not _source_expression_is_safe_to_bind(base_expr):
        return None
    line_text = source[decl_start:decl_end]
    init_start = line_text.find("=")
    semi = line_text.rfind(";")
    if init_start < 0 or semi < init_start:
        return None
    prefix = line_text[:init_start + 1]
    suffix = line_text[semi:]
    replacement = f"{prefix} {base_expr}{suffix}"
    return decl_start, decl_end, replacement, base, base_expr


def _generated_pointer_walk_base_initializer(
    source: str,
    stripped: str,
    function: str,
    base: str,
    *,
    ...
    if span is None:
        return None
    body_start = span.body_open + 1
    search_text = stripped[body_start:before]
    base_re = re.compile(
        r"(?m)^[ \t]*"
        r"(?:const\s+|volatile\s+|static\s+|register\s+)*"
        r"(?:struct\s+[A-Za-z_][A-Za-z_0-9]*|[A-Za-z_][A-Za-z_0-9]*)"
        r"(?:\s+[A-Za-z_][A-Za-z_0-9]*)*\s*\*+\s*"
        rf"{re.escape(base)}\s*=\s*(?P<init>[^;\n]+)\s*;\s*$"
    )
    for match in reversed(list(base_re.finditer(search_text))):
        abs_start = body_start + match.start()
        block_open = _innermost_open_brace_before(stripped, abs_start)
        if block_open is not None:
            ...
            if block_close is None or before >= block_close:
                continue
        init_start = body_start + match.start("init")
        init_end = body_start + match.end("init")
        initializer = source[init_start:init_end].strip()
        if initializer and initializer != base:
            return initializer
    return None
    ...
    for span in statements:
        ...
        for rel_start in _iter_expression_offsets(statement_text, expression):
            ...
            if not _expression_occurrence_is_rewritable(
                ...
                continue
            if mode == "call_argument" and not _call_argument_occurrence_is_bindable(
                source,
                function,
                span,
                abs_start,
...
def _binding_context_for_span(
    ...
    *,
    ...
    if match is None:
        if source_kind != "synthetic-owner-split":
            return None
        if expression not in stripped_text:
            return None
        if not _call_argument_context_allows_unconditional_binding(
            stripped_text,
            expression,
        ):
            return None
        binding_type = _normalize_safe_binding_type(fallback_type)
        if binding_type is None:
            return None
        return "call_argument", binding_type
    rhs = match.group("rhs")
    if expression not in rhs:
        return None
    if not _rhs_context_allows_unconditional_binding(rhs, expression):
...
def _statement_span_starts_on_plain_line(source: str, span: StatementSpan) -> bool:
    span_start, _span_end = _byte_range_to_char_range(source, span.byte_range)
    line_start = source.rfind("\n", 0, span_start) + 1
    return not source[line_start:span_start].strip()
...
def _call_argument_context_allows_unconditional_binding(
    statement_text: str,
    expression: str,
) -> bool:
    text = _strip_c_comments(statement_text).strip()
    if expression not in text:
        return False
    if not _source_expression_is_safe_to_bind(expression):
        return False
...
def _call_argument_occurrence_is_bindable(
    source: str,
    function: str,
    span: StatementSpan,
    occurrence_start: int,
    ...
    fn_node = ast_walker._find_function_definition(
        tree.root_node,
        source_bytes,
        function,
    )
    if fn_node is None:
        return False
    occurrence_byte_start = len(source[:occurrence_start].encode("utf-8"))
    occurrence_byte_end = len(source[:occurrence_end].encode("utf-8"))
    statement_start, statement_end = span.byte_range
    calls: list[Any] = []
    stack = [fn_node]
    while stack:
        node = stack.pop()
        if (
            ...
            and statement_start <= node.start_byte
            and node.end_byte <= statement_end
        ):
            calls.append(node)
    ...
    if (
        callee is not None
        and callee.start_byte <= occurrence_byte_start
        and occurrence_byte_end <= callee.end_byte
    ):
        return False
    ...
    for child in args.children:
        if child.type in {"(", ")", ","}:
            continue
        if (
            child.start_byte <= occurrence_byte_start
            and occurrence_byte_end <= child.end_byte
        ):
            return True
    return False
...
def _binding_name_prefix(
    ...
    *,
    ...
    if source_kind == "stack-array-base":
        return f"{base}_base_bind_{target_ig}_{site_idx}"
...
def _block_top_insert_pos(source: str, scope_byte_range: tuple[int, int]) -> int:
    start, end = _byte_range_to_char_range(source, scope_byte_range)
...
def _append_source_rejected_patch(
    patches: list[CandidatePatch],
    *,
    source: str,
    candidate_id: str,
    ...
    patches.append(CandidatePatch(
        ...
        touched_ranges=(),
        hunk="",
        metadata={"source_rejection_reason": reason},
    ))


def _duplicate_local_declaration_reason(source: str, function: str) -> str | None:
    scope_map = get_decl_names_by_scope(source, function)
    duplicate_names: list[str] = []
    for names in scope_map.values():
        counts = Counter(names)
        duplicate_names.extend(name for name, count in counts.items() if count > 1)
    if not duplicate_names:
        return None
    names = ", ".join(sorted(set(duplicate_names)))
    return f"duplicate local declaration(s) after decl-order rewrite: {names}"
    ...
    try:
        ...
        for scope_idx, scope_path in enumerate(matching_scopes):
            ...
            for candidate_idx, candidate in enumerate(candidates):
                ...
                duplicate_reason = _duplicate_local_declaration_reason(
                    patched_source,
                    function,
                )
                if duplicate_reason is not None:
                    _append_source_rejected_patch(
                        patches,
                        source=source,
                        candidate_id=candidate_id,
                        summary=summary,
...
def _source_hunk_for_row(entry: Any, objective: Any) -> str | None:
    if isinstance(entry, Mapping):
        source_hunk = entry.get("source_hunk")
        if source_hunk is not None:
            return str(source_hunk)
    if isinstance(objective, Mapping):
        source_hunk = objective.get("source_hunk")
        if source_hunk is not None:
            return str(source_hunk)
    return None


def _source_hunks_for_row(
    entry: Any,
    objective: Any,
    candidate_id: Any,
) -> list[dict[str, str]] | None:
    for container in (entry, objective):
        if isinstance(container, Mapping):
            hunks = container.get("source_hunks")
            if isinstance(hunks, list):
                ...
                if out:
                    return out
    source_hunk = _source_hunk_for_row(entry, objective)
    if source_hunk is None:
        return None
    return [{
        "hunk_id": str(candidate_id) if candidate_id is not None else "source",
        "unified_diff": source_hunk,
    }]


def _pcdump_path_for_row(entry: Any, objective: Any) -> str | None:
    if isinstance(entry, Mapping):
        pcdump_path = entry.get("pcdump_path")
        if pcdump_path is not None:
            return str(pcdump_path)
    if isinstance(objective, Mapping):
        pcdump_path = objective.get("pcdump_path")
        if pcdump_path is not None:
            return str(pcdump_path)
    return None
...
def _score_row(entry: Any) -> dict[str, Any]:
    ...
    score_status = (
        entry.get("score_status")
        if isinstance(entry, Mapping) and entry.get("score_status") is not None
        else getattr(score, "status", None)
    )
    ...
    source_hunk = _source_hunk_for_row(entry, objective)
    source_hunks = _source_hunks_for_row(entry, objective, candidate_id)
    pcdump_path = _pcdump_path_for_row(entry, objective)
    objective_error = (
        objective.get("error")
    ...
    if isinstance(objective, Mapping) and objective.get("source_rejection_reason"):
        row["source_rejection_reason"] = str(objective["source_rejection_reason"])
    if source_retained is not None:
        row["source_retained"] = source_retained
    if source_hunk is not None:
        row["source_hunk"] = source_hunk
    if source_hunks is not None:
        row["source_hunks"] = source_hunks
    if pcdump_path is not None:
        row["pcdump_path"] = pcdump_path
    target_score = None
    if isinstance(entry, Mapping) and isinstance(entry.get("target_score"), Mapping):
        target_score = dict(entry["target_score"])
    elif isinstance(objective, Mapping) and isinstance(
        objective.get("target_score"), Mapping
    ):
    ...
    if isinstance(entry, Mapping) and isinstance(
        entry.get("expression_score"), Mapping
    ):
        expression_score = dict(entry["expression_score"])
    elif isinstance(objective, Mapping) and isinstance(
        objective.get("expression_score"), Mapping
    ):
        expression_score = dict(objective["expression_score"])
    if expression_score is not None:
        row["expression_score"] = expression_score
    if isinstance(objective, Mapping):
