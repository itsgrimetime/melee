...
def instr_signature(opcode: str, operands: str) -> tuple[str, str]:
    ...
    if opcode == "addi":
        copy_match = re.match(
            r"^\s*r\d+\s*,\s*(r\d+)\s*,\s*0\s*$",
            norm,
        )
        if copy_match is not None and copy_match.group(1) != "r0":
            return ("mr", "R,R")
...
def match_virtual_for_expected_def(
    ...
    for i, ist in enumerate(instructions):
        if instr_signature(ist.opcode, ist.operands) != target_sig:
            continue
        if _destination_virtual(ist, reg_kind=reg_kind) is None:
            continue
    ...
    virt = _destination_virtual(best_ist, reg_kind=reg_kind)
