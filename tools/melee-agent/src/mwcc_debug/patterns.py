...
so `debug inspect guide` can suggest them by name and
`debug util patterns` can dump them for reference.
...
class MutationPattern:
    ...
    # When True, `debug permute config` refuses to write a settings.toml
    # for this pattern unless `--force` is set. Use for cases where no
    # useful decomp-permuter weight profile exists yet and agents should
    # collect force-target evidence before starting a campaign.
...
PATTERNS: dict[str, MutationPattern] = {
    ...
    "decl-order": MutationPattern(
        ...
        when_to_try=(
            ...
            "slot. Run `debug mutate decl-orders <fn>` to brute-force "
    ...
    "param-iter-ceiling": MutationPattern(
        ...
        title="Allocator-order mismatch: parameter loses to local in IG iteration order",
        summary=(
            ...
            "with whatever's left. Treat this as an allocator-order "
            "mismatch unresolved by current heuristics until force-target "
            "evidence or broader source-shape search proves otherwise."
        ),
        when_to_try=(
            "First collect evidence instead of spinning on local edits: "
            "use Tier 6 force-phys / force-iter-first for hypothesis "
            "testing, then decide whether source-shape search or a "
            "compiler-hook path is warranted. "
            ...
            "produces the target .text, (e) debug mutate decl-orders finds "
        ...
        example_before=(
            ...
            "// Target wants gobj=r31 — allocator-order mismatch\n"
        ...
        example_after=(
            "// Evidence-gathering path before another edit campaign:\n"
            ...
            "//   melee-agent debug dump local src/... --force-phys '32:31,...'\n"
            "// If force-phys matches: record Tier 6 allocator-order evidence,\n"
            "// then choose source-shape search or a hook path."
        ),
        mechanism=(
            ...
            "Simple C-source reorder is not expected to shift parameter "
            "ig_idx because the parameter list is processed before locals."
        ),
        addresses=("rank", "param-iter-ceiling"),
        # No useful decomp-permuter weight profile is known for this
        # allocator-order mismatch; config generation refuses unless --force.
        permuter_skip=True,
    ),
    ...
    "anon-magic-constant": MutationPattern(
        ...
        example_after=(
            ...
            "melee-agent debug util verify-name-magic -f my_function \\\n"
    ...
    "register-cascade": MutationPattern(
        ...
        example_after=(
            ...
            "#        (`debug permute run -f fn`) which mutates the source\n"
            "#        to find a move-instruction that triggers coalesce\n"
            "#    (c) Last resort: record the cascade as unresolved by\n"
            "#        current heuristics and preserve the evidence."
        ),
        mechanism=(
