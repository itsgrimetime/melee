...
def smoke_compile(
    ...
    pcdump_path = (
        ...
        / f"tier3_smoke_pcdump_{os.getpid()}_{time.time_ns()}.txt"
    )
    env = os.environ.copy()
...
#
# v2: for each compiling seed, invoke `debug permute run` for a bounded time,
...
# transfer the winner to the real source tree via `debug permute verify`.
#
...
# `melee-agent debug permute run` or equivalent) and to write any candidate
