...
# Hook/debug sections are not pcode passes. They often contain indented table
# rows such as "interferers: 40=r6", which look enough like instructions that
# the loose instruction parser would otherwise attach them to the previous
# basic block.
_HOOK_SECTION_RE = re.compile(
    r"^(?:"
    r"COLORGRAPH DECISIONS\b|"
    r"SIMPLIFY GRAPH\b|"
    r"IG CONSTRUCTED\b|"
    r"CONSTPROP RAN\b|"
    r"COALESCED ALIASES\b|"
    r"\[COALESCE\]\b"
    r")"
)
...
def _is_hook_section_marker(stripped: str) -> bool:
    """Return True for mwcc_debug hook sections that are not pass dumps."""
    return bool(_HOOK_SECTION_RE.match(stripped))
...
def parse_pcdump(text: str, function: Optional[str] = None) -> list[Function]:
    ...
    while i < len(lines):
        ...
        # Hook sections (COLORGRAPH/SIMPLIFY/COALESCE/etc.) are outside the
        # pcode pass stream. Drop the active pass/block so their indented table
        # rows cannot be mistaken for instructions in the preceding block.
        if _is_hook_section_marker(stripped):
            current_pass = None
            current_block = None
            i += 1
            continue
