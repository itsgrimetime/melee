    ...
    def to_dict(self) -> dict:
        return {
            ...
            "from_token": (
                ...
                else _reg_token_for_class(self.register_class, self.from_virtual)
            ),
            "to_token": (
                self.to_token
                if self.to_token is not None
                else _reg_token_for_class(self.register_class, self.to_virtual)
            ),
            "copy_opcode": self.copy_opcode,
        }
...
def _reg_kind_for_class_name(reg_class: Optional[str]) -> str:
    return _reg_kind_for_class_id(_normalize_reg_class(reg_class)) or "r"


def _class_name_for_kind(reg_kind: str) -> str:
    return "fpr" if reg_kind == "f" else "gpr"


def _copy_opcode_for_kind(reg_kind: str) -> str:
    return "fmr" if reg_kind == "f" else "mr"


def _reg_token_for_kind(reg_kind: str, reg: int) -> str:
    prefix = "f" if reg_kind == "f" else "r"
    return f"{prefix}{reg}"


def _reg_token_for_class(reg_class: str | None, reg: int) -> str:
    return _reg_token_for_kind(
        "f" if (reg_class or "").lower() == "fpr" else "r",
        reg,
    )


def _regs_for_kind(instr, reg_kind: str) -> list[tuple[str, int]]:
    return [(kind, reg) for kind, reg in instr.regs if kind == reg_kind]


def _copy_instruction_matches(
    instr,
    from_virtual: int,
    to_virtual: int,
    *,
    ...
    regs = _regs_for_kind(instr, reg_kind)
    return (
        instr.opcode.lower() == _copy_opcode_for_kind(reg_kind)
        and len(regs) >= 2
        and regs[0] == (reg_kind, to_virtual)
        and regs[1] == (reg_kind, from_virtual)
    )
...
def _pass_has_virtual(p, virtual: int, *, reg_kind: str = "r") -> bool:
    for block in p.blocks:
        for instr in block.instructions:
            if any(kind == reg_kind and reg == virtual for kind, reg in instr.regs):
                return True
    return False
...
def trace_copy_lifetime(
    ...
    *,
    ...
    """Trace a same-class copy across pcode and allocator output."""
    reg_kind = _reg_kind_for_class_name(reg_class)
    register_class = _class_name_for_kind(reg_kind)
    copy_opcode = _copy_opcode_for_kind(reg_kind)
    from_token = _reg_token_for_kind(reg_kind, from_virtual)
    to_token = _reg_token_for_kind(reg_kind, to_virtual)
    ...
    if not copies:
        return CopyTraceReport(
            ...
            note=f"no `{copy_opcode} {to_token},{from_token}` was found",
            register_class=register_class,
            from_token=from_token,
            to_token=to_token,
            copy_opcode=copy_opcode,
        )
    ...
    if to_mapping.status == "pcode-only":
        ...
        if fn is not None and first_absent is not None:
            ...
            if absent_pass is not None:
                from_present = _pass_has_virtual(
                    absent_pass,
                    from_virtual,
                    reg_kind=reg_kind,
                )
                to_present = _pass_has_virtual(
                    absent_pass,
                    to_virtual,
                    reg_kind=reg_kind,
                )
        ...
        note = (
            f"{to_token} and {from_token} both colored to "
            f"{_reg_token_for_kind(reg_kind, to_mapping.assigned_reg)}"
        )
    elif (
        to_mapping.assigned_reg is not None
        and from_mapping.assigned_reg is not None
        ...
        transform_category = "copy-survived"
        note = (
            "copy destination colored to "
            f"{_reg_token_for_kind(reg_kind, to_mapping.assigned_reg)}, "
            "source colored to "
            f"{_reg_token_for_kind(reg_kind, from_mapping.assigned_reg)}"
        )
...
def _copy_pairs(fn, *, reg_kind: str = "r") -> list[tuple[int, int, InstructionOccurrence]]:
    out: list[tuple[int, int, InstructionOccurrence]] = []
    for p in fn.passes:
        for block in p.blocks:
            for instr_idx, instr in enumerate(block.instructions):
                regs = _regs_for_kind(instr, reg_kind)
                if instr.opcode.lower() != _copy_opcode_for_kind(reg_kind) or len(regs) < 2:
                    continue
                to_kind, to_virtual = regs[0]
                from_kind, from_virtual = regs[1]
                if to_kind != reg_kind or from_kind != reg_kind:
                    continue
                if reg_kind == "r" and (to_virtual < 32 or from_virtual < 32):
                    continue
                if reg_kind == "f" and to_virtual < 32 and from_virtual < 32:
                    continue
                out.append((
                    from_virtual,
                    to_virtual,
...
def _copy_pair_set(
    ...
    *,
    ...
    return {
        ...
        for from_virtual, to_virtual, _occurrence in _copy_pairs(fn, reg_kind=reg_kind)
    }
...
def list_copy_lifetimes(
    ...
    *,
    ...
    for from_virtual, to_virtual, occurrence in _copy_pairs(fn, reg_kind=reg_kind):
