...
def _byte_offset_to_char_offset(file_text: str, byte_offset: int) -> int:
    if byte_offset <= 0:
        return 0
    encoded = file_text.encode("utf-8")
...
def _byte_range_to_char_range(
    ...
    return (
        _byte_offset_to_char_offset(file_text, byte_range[0]),
        _byte_offset_to_char_offset(file_text, byte_range[1]),
    )


def _line_bounds_for_range(file_text: str, byte_range: tuple[int, int]) -> tuple[int, int]:
    start, end = _byte_range_to_char_range(file_text, byte_range)
...
def _split_decl_items_for_scope(
    ...
    for line_bounds, line_decls in by_line.items():
        ...
        ordered = sorted(
            line_decls,
            key=lambda decl: _byte_range_to_char_range(file_text, decl.byte_range)[0],
        )
        if len(ordered) == 1:
            decl = ordered[0]
        ...
        first_rel = min(
            _byte_range_to_char_range(file_text, decl.byte_range)[0] - line_start
            for decl in ordered
        )
        shared_prefix = line_text[:first_rel]
        newline = "\n" if line_text.endswith("\n") else ""
        for decl in ordered:
            decl_start, decl_end = _byte_range_to_char_range(file_text, decl.byte_range)
