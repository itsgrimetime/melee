...
CONTROL_FLOW_SHAPE_HINT_KINDS: tuple[str, ...] = tuple(_PRIORITY)
...
def analyze_control_flow_shape(
    *,
    ...
    clipped = suggestions if top is None else suggestions[: max(0, top)]
...
def annotate_source_materialization(
    ...
    *,
    ...
    from ..control_flow_shape import materialize_control_flow_suggestions
    ...
    probes, materialization_status = materialize_control_flow_suggestions(
        source_text,
        report_function,
    ...
    if isinstance(family_results, list):
        for result in family_results:
            ...
            if isinstance(family_id, str):
                result_by_family[family_id] = result
    probe_by_family: dict[str, list[Any]] = {}
    for probe in probes:
        provenance = probe.provenance if isinstance(probe.provenance, dict) else {}
        family_id = provenance.get("family_id")
        if isinstance(family_id, str):
            probe_by_family.setdefault(family_id, []).append(probe)
    ...
    for suggestion in suggestions:
        ...
        family_id = f"{operator}/{kind}"
        family_result = result_by_family.get(family_id)
        if family_result is None:
            continue
        family_probes = probe_by_family.get(family_id, [])
        terminal_proof = family_result.get("terminal_proof")
        family_status = str(family_result.get("status") or "terminal")
        status = "materializable" if family_probes else family_status
        ...
        if family_probes:
            materialization["example_probe_label"] = family_probes[0].label
        if isinstance(terminal_proof, dict):
            materialization["terminal_proof"] = terminal_proof
            materialization["blocker"] = terminal_proof.get("terminal_blocker")
            materialization["reason"] = terminal_proof.get("terminal_reason")
        if not family_probes and isinstance(terminal_proof, dict):
            suggestion["non_materializable_reason"] = str(
                terminal_proof.get("terminal_reason")
                or terminal_proof.get("terminal_blocker")
                or "no probes"
            )
