...
def allocator_fact_to_dict(fact: AllocatorFact) -> dict[str, object]:
    """Return a stable JSON-ready view of the gated allocator fact."""
    ...
    return {
        ...
        "baseline_reg_name": _reg_name(fact.class_id, fact.baseline_reg),
        "target_reg": fact.target_reg,
        "target_reg_name": _reg_name(fact.class_id, fact.target_reg),
        "coalesced_nodes": list(fact.coalesced_nodes),
        "coalesced_root": fact.coalesced_root,
        "coalesced_root_phys": fact.coalesced_root_phys,
        "coalesced_root_phys_name": _reg_name(
            fact.class_id, fact.coalesced_root_phys),
        "blocker_ig": fact.blocker_ig,
        "blocker_dependency": fact.blocker_dependency,
        "working_mask": (
            sorted(fact.working_mask) if fact.working_mask is not None else None
        ),
...
def source_idea_to_dict(source: SourceIdea) -> dict[str, object]:
    """Return a stable JSON-ready view of advisory source attribution."""

    return {
        "ig_idx": source.ig_idx,
        "var_name": source.var_name,
        "confidence": source.confidence,
        "alternates": list(source.alternates),
        "ideas": list(source.ideas),
        "rejected": list(source.rejected),
        "first_def": source.first_def,
        "blocker_ig": source.blocker_ig,
        "blocker_var_name": source.blocker_var_name,
        "blocker_confidence": source.blocker_confidence,
        "blocker_alternates": list(source.blocker_alternates),
        "blocker_rejected": list(source.blocker_rejected),
        "blocker_first_def": source.blocker_first_def,
        ...
        "candidate_spans": list(source.candidate_spans),
        "negative_diagnostics": list(source.negative_diagnostics),
    }


def report_to_dict(report: FirstDivergenceReport) -> dict[str, object]:
    """Return the machine-readable allocator first-divergence report."""

    return {
        "kind": "allocator-first-divergence",
        "fact": allocator_fact_to_dict(report.fact),
        "source": (
            source_idea_to_dict(report.source)
