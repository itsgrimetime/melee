...
@dataclass(frozen=True)
class SelectOrderForcePhysAssignment:
    virtual: int
    expected: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "baseline_actual": self.baseline_actual,
            "actual": self.actual,
...
class SelectOrderObjective:
    ...
    force_phys_assignments: tuple[SelectOrderForcePhysAssignment, ...]
    ...
    force_phys_baseline_distance: int | None
    force_phys_distance: int | None
    force_phys_distance_delta: int | None
    force_phys_progress_kind: str
    satisfied_count: int
    improved_count: int
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "force_phys_assignments": {
                str(row.virtual): row.to_dict()
                for row in self.force_phys_assignments
            },
            ...
            "force_phys_baseline_distance": self.force_phys_baseline_distance,
            "force_phys_distance": self.force_phys_distance,
            "force_phys_distance_delta": self.force_phys_distance_delta,
            "force_phys_progress_kind": self.force_phys_progress_kind,
...
def score_select_order_candidate(
    ...
    *,
    ...
    force_phys_assignments = _force_phys_assignments(
        force_phys_targets,
        baseline_assigned=baseline_assigned,
        candidate_assigned=candidate_assigned,
    )
    (
        baseline_force_phys_missing,
        baseline_force_phys_mismatches,
        _baseline_force_phys_satisfied_count,
    ) = _force_phys_state(force_phys_targets, baseline_assigned)
    (
        force_phys_missing,
        force_phys_mismatches,
        force_phys_satisfied_count,
    ) = _force_phys_state(force_phys_targets, candidate_assigned)
    ...
    force_phys_baseline_distance = (
        None
        if not force_phys_targets
        else _force_phys_distance(
            baseline_force_phys_missing,
            baseline_force_phys_mismatches,
        )
    )
    force_phys_distance_delta = (
        None
        if force_phys_baseline_distance is None or force_phys_distance is None
        else force_phys_baseline_distance - force_phys_distance
    )
    spill_removed = delta.spill_removed if delta is not None else ()
    spill_added = delta.spill_added if delta is not None else ()
    force_phys_progress_kind = _force_phys_progress_kind(
        force_phys_assignments,
        distance_delta=force_phys_distance_delta,
    ...
    objective = SelectOrderObjective(
        ...
        force_phys_assignments=force_phys_assignments,
        ...
        force_phys_baseline_distance=force_phys_baseline_distance,
        force_phys_distance=force_phys_distance,
        force_phys_distance_delta=force_phys_distance_delta,
        force_phys_progress_kind=force_phys_progress_kind,
...
def _force_phys_state(
    targets: tuple[tuple[int, int], ...],
    assigned: dict[int, int],
) -> tuple[tuple[int, ...], tuple[tuple[int, int, int], ...], int]:
    missing = tuple(
        virtual for virtual, _phys in targets
        if virtual not in assigned
    )
    mismatches = tuple(
        (virtual, phys, assigned[virtual])
        for virtual, phys in targets
        if virtual in assigned and assigned[virtual] != phys
    )
    satisfied_count = sum(
        1
        for virtual, phys in targets
        if assigned.get(virtual) == phys
    )
    return missing, mismatches, satisfied_count


def _force_phys_status(actual: int | None, expected: int) -> str:
    if actual is None:
        return "missing_or_coalesced"
    if actual == expected:
        return "matched"
    return "mismatched"


def _force_phys_assignments(
    targets: tuple[tuple[int, int], ...],
    *,
    baseline_assigned: dict[int, int],
    candidate_assigned: dict[int, int],
) -> tuple[SelectOrderForcePhysAssignment, ...]:
    assignments: list[SelectOrderForcePhysAssignment] = []
    for virtual, expected in targets:
        baseline_actual = baseline_assigned.get(virtual)
        actual = candidate_assigned.get(virtual)
        assignments.append(
            SelectOrderForcePhysAssignment(
                virtual=virtual,
                expected=expected,
                baseline_actual=baseline_actual,
                actual=actual,
                baseline_status=_force_phys_status(baseline_actual, expected),
                status=_force_phys_status(actual, expected),
                changed=baseline_actual != actual,
            )
        )
    return tuple(assignments)


def _force_phys_progress_kind(
    assignments: tuple[SelectOrderForcePhysAssignment, ...],
    *,
    distance_delta: int | None,
    spill_removed: tuple[int, ...],
    spill_added: tuple[int, ...],
) -> str:
    if not assignments:
        return "no-force-phys-targets"
    if any(row.status == "matched" for row in assignments):
        return "target-hit"
    if any(row.status == "missing_or_coalesced" for row in assignments):
        return "target-missing"
    if distance_delta is not None and distance_delta > 0:
        return "target-progress"
    if len(spill_removed) > len(spill_added):
        return "spill-only"
    return "target-no-progress"
...
def render_select_order_variant(variant: dict[str, Any]) -> str:
    ...
    if force_phys_targets:
        assignments = objective.get("force_phys_assignments") or {}
        lines.append(
            "   force_phys: "
            ...
            f"progress={objective.get('force_phys_progress_kind', 'unknown')} "
            f"assignments={_fmt_force_phys_assignments(assignments)}"
        )
...
def _objective_sort_key(objective: SelectOrderObjective) -> tuple[float, ...]:
    ...
    return (
        ...
        (
            float(_force_phys_progress_rank(objective.force_phys_progress_kind))
...
def _force_phys_progress_rank(kind: str | None) -> int:
    return {
        "target-hit": 5,
        "target-progress": 4,
        "target-no-progress": 3,
        "spill-only": 2,
        "target-missing": 1,
        "no-force-phys-targets": 0,
    }.get(kind or "no-force-phys-targets", 0)


def _variant_sort_key(variant: dict[str, Any]) -> tuple[float, ...]:
    if variant.get("status") != "ok":
        return (-1.0,)
    structural_guard_score = _structural_guard_sort_value(variant)
    objective = variant.get("objective") or {}
    sort_key = objective.get("sort_key")
    if isinstance(sort_key, list):
        return (structural_guard_score, *tuple(float(item) for item in sort_key))
    if isinstance(sort_key, tuple):
        return (structural_guard_score, *tuple(float(item) for item in sort_key))
    return (structural_guard_score, 0.0)


def _structural_guard_sort_value(variant: dict[str, Any]) -> float:
    ...
    if opcode_shape_preserved is True:
        return 0.5
    if opcode_shape_preserved is False:
        return -0.5
    return 0.0
...
def _fmt_force_phys_assignments(assignments: object) -> str:
    ...
    for key, value in assignments.items():
        if not isinstance(value, dict):
            continue
        try:
            virtual = int(value.get("virtual", key))
        except (TypeError, ValueError):
            continue
        rows.append((virtual, value))
    tokens = [
        _fmt_force_phys_assignment(row)
        for _virtual, row in sorted(rows, key=lambda item: item[0])
    ]
    return ",".join(token for token in tokens if token) or "-"


def _fmt_force_phys_assignment(row: dict[str, Any]) -> str:
    virtual = _fmt_force_phys_reg(row.get("virtual"))
    expected = _fmt_force_phys_reg(row.get("expected"))
    baseline = _fmt_force_phys_reg(row.get("baseline_actual"))
    actual = _fmt_force_phys_reg(row.get("actual"))
    comparator = "==" if row.get("status") == "matched" else "!="
    return f"{virtual}:{baseline}->{actual}{comparator}{expected}"


def _fmt_force_phys_reg(value: object) -> str:
    if value is None:
        return "missing/coalesced"
    try:
