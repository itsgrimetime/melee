...
def render_coalesce_variant(variant: dict[str, Any]) -> str:
    ...
    reg_prefix = "f" if variant.get("register_class") == "fpr" else "r"
    lines.append(
        "   target_spill_removed: "
        f"{_fmt_virtuals(target_spill_removed, prefix=reg_prefix)}"
    )
    for pair in target_pairs:
        lines.append(
            "   "
            f"{reg_prefix}{pair['virtual']}/{reg_prefix}{pair['other_virtual']}: "
