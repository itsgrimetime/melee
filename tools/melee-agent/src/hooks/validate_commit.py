...
DESCRIPTIVE_SYMBOL_RE = re.compile(
    r"\b([A-Z][a-zA-Z0-9_]*(?:Table|State|Data|Info|List|Array)[a-zA-Z0-9_]*)\b"
)
ADDRESS_SYMBOL_RE = re.compile(r"\b((?:fn|it|ft|gr|lb|gm|if|mp|vi)_[0-9A-Fa-f]{8})\b")


def _looks_like_c_type_reference(line: str, name: str) -> bool:
    """Return True when a descriptive token is used as a C type, not a symbol."""
    escaped = re.escape(name)
    qualifiers = r"(?:(?:const|volatile|static|extern|register)\s+)*"
    type_prefix = rf"{qualifiers}(?:struct\s+)?{escaped}"

    # Local declarations and function parameters, including `CardState* state`
    # and `CardState state`.
    declaration_start = r"(?:^\s*|[({,;]\s*)"
    declaration_suffix = r"(?:\s*\*+\s*|\s+)\w+\b"
    if re.search(rf"{declaration_start}{type_prefix}{declaration_suffix}", line):
        return True

    # Casts and sizeof references are type uses, even if no variable is declared.
    if re.search(rf"\(\s*{type_prefix}\s*(?:\*+\s*)?\)\s*(?=[A-Za-z_&*(0-9+-])", line):
        return True
    if re.search(rf"\bsizeof\s*\(\s*{type_prefix}\s*(?:\*+\s*)?\)", line):
        return True

    # Type declarations can mention descriptive type names without renaming
    # runtime symbols.
    if re.search(rf"\b(?:typedef\s+)?struct\s+{escaped}\b", line):
        return True

    return False


def _removed_descriptive_symbol_names(line: str) -> set[str]:
    """Extract removed descriptive names that are plausible runtime symbols."""
    return {
        name
        for name in DESCRIPTIVE_SYMBOL_RE.findall(line)
        if not _looks_like_c_type_reference(line, name)
    }
