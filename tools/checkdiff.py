...
PCDUMP_OBJECT_INPUT_HINT = (
    "--pcdump expects mwcc_debug pcdump text, not a compiled object. "
    "To compare a kept object, run `melee-agent debug dump local ... --diff`, "
    "or copy the object over the matching build/GALE01/src/<unit>.o and run "
    "`tools/checkdiff.py <function> --no-build`."
)


class PcdumpInputError(ValueError):
    """Raised when --pcdump is pointed at data that cannot be pcdump text."""
...
def _rel24_call_targets(lines: list[str]) -> list[str]:
    targets = []
    for line in lines:
        relocation = _parse_relocation_reference(line)
        if relocation is None or relocation["kind"] != "R_PPC_REL24":
            continue
        targets.append(relocation["symbol"])
    return targets


def _paired_call_targets_for_shape(
    ref_lines: list[str],
    our_lines: list[str],
) -> tuple[list[str], list[str], str]:
    ref_reloc_calls = _rel24_call_targets(ref_lines)
    our_reloc_calls = _rel24_call_targets(our_lines)
    if ref_reloc_calls and our_reloc_calls:
        return ref_reloc_calls, our_reloc_calls, "rel24"
    return _call_targets(ref_lines), _call_targets(our_lines), "rendered"
...
def _call_target_multiplicity_differs(
    ...
    return _call_target_base_multiplicities(ref_calls) != (
        _call_target_base_multiplicities(our_calls)
    )
...
_GPR_SAVE_WINDOW_RE = re.compile(
    r"\b(?P<op>stmw|lmw)\s+"
    r"r(?P<first>[0-9]|[12][0-9]|3[01])\s*,\s*"
    r"(?P<offset>-?(?:0x[0-9A-Fa-f]+|\d+))\s*"
    r"\(\s*r1\s*\)"
)
...
def _self_relative_call_base(target: str) -> str | None:
    ...
    match = _SELF_RELATIVE_CALL_RE.match(target)
    if match is None or match.group("delta") is None:
        return None
    return match.group("base")


def _call_target_base(target: str) -> str:
    target = target.strip()
    if target.startswith("<") and target.endswith(">"):
        target = target[1:-1].strip()
    match = _SELF_RELATIVE_CALL_RE.match(target)
...
def _call_target_base_multiplicities(calls: list[str]) -> Counter[str]:
    return Counter(_call_target_base(call) for call in calls)
...
def _saved_gpr_window_signature(lines: list[str]) -> tuple[int, int, int] | None:
    save: tuple[int, int] | None = None
    restore: tuple[int, int] | None = None
    for line in lines:
        match = _GPR_SAVE_WINDOW_RE.search(_asm_body(line))
        if match is None:
            continue
        first = int(match.group("first"), 10)
        offset = int(match.group("offset"), 0)
        if match.group("op") == "stmw":
            save = (first, offset)
        else:
    ...
    if save[0] != restore[0]:
        return None
    return save[0], save[1], restore[1]


def _saved_gpr_window_summary(signature: tuple[int, int, int]) -> dict:
    first = signature[0]
    return {
        "first_saved_reg": f"r{first}",
        "last_saved_reg": "r31",
        "count": 32 - first,
    }
...
def _detect_register_window_rotation_ceiling(
    ref_lines: list[str],
    our_lines: list[str],
    ref_calls: list[str],
    our_calls: list[str],
    *,
    ...
    register_allocation_guidance: dict | None,
) -> dict | None:
    if len(ref_lines) != len(our_lines):
        return None
    if normalized_diff_lines <= 0 or normalized_diff_lines > 8:
        return None
    if _call_target_base_multiplicities(ref_calls) != (
        _call_target_base_multiplicities(our_calls)
    ):
        return None

    ref_window = _saved_gpr_window_signature(ref_lines)
    our_window = _saved_gpr_window_signature(our_lines)
    if ref_window is None or ref_window != our_window:
        return None

    frame_sizes = _stack_frame_size_summary(ref_lines, our_lines)
    if not frame_sizes or frame_sizes.get("frame_growth") != 0:
        return None

    guidance = register_allocation_guidance or {}
    if not guidance.get("callee_swap_pairs"):
        return None

    return {
        "subclass": "register-window-rotation",
        "confidence": "medium",
        "reason": (
            "line count, frame size, call multiset, and same callee-save "
            "save/restore window match; tiny normalized structural churn is "
            "dominated by callee-save register-window rotation/coalescing "
            "tiebreak fallout"
        ),
        "saved_gpr_window": _saved_gpr_window_summary(ref_window),
        "normalized_diff_lines": normalized_diff_lines,
        "callee_swap_pairs": guidance.get("callee_swap_pairs") or [],
    }
...
def detect_backend_ceiling(
    ...
    *,
    ...
    register_allocation_guidance: dict | None = None,
) -> dict | None:
    ...
    register_window_rotation = _detect_register_window_rotation_ceiling(
        ref_lines,
        our_lines,
        ref_calls,
        our_calls,
        normalized_diff_lines=normalized_diff_lines,
        register_allocation_guidance=register_allocation_guidance,
    )
    if register_window_rotation:
        return register_window_rotation
...
def _detect_inline_boundary_artifact(
    ...
    if (
        missing_ref_calls
        and _call_target_base_multiplicities(ref_calls)
        == _call_target_base_multiplicities(our_calls)
    ):
        return None
...
_INDEXED_STRUCT_POINTER_SHAPE_WINDOW = 4
...
def detect_indexed_struct_pointer_materialization(
    ...
    def _near_expected_indexed(materialized: dict) -> bool:
        materialized_lines = [materialized["line_index"]]
        materialized_lines.extend(
            access["line_index"]
            for access in materialized.get("field_accesses", [])
        )
        return any(
            abs(expected["line_index"] - line_index)
            <= _INDEXED_STRUCT_POINTER_SHAPE_WINDOW
            for expected in expected_indexed_ops
            for line_index in materialized_lines
        )

    current_materialized = [
        materialized
        for materialized in current_materialized
        if _near_expected_indexed(materialized)
    ]
    if not current_materialized:
        return None
    return {
...
def read_pcdump_text_arg(path: Path) -> str:
    try:
        data = path.read_bytes()
    except OSError as exc:
        raise PcdumpInputError(str(exc)) from exc
    sample = data[:4096]
    if data.startswith(b"\x7fELF"):
        raise PcdumpInputError(PCDUMP_OBJECT_INPUT_HINT)
    if b"\x00" in sample:
        raise PcdumpInputError(
            f"{PCDUMP_OBJECT_INPUT_HINT} The selected file contains binary "
            "NUL bytes, so it is not a text pcdump."
        )
...
def add_stack_slot_pcdump_bridge(
    ...
    *,
    ...
    if pcdump_text is None and pcdump_path is not None:
        try:
            pcdump_text = read_pcdump_text_arg(pcdump_path)
        except PcdumpInputError as exc:
...
def format_pad_stack_probe_diagnostic(classification: dict) -> Optional[str]:
    ...
    if manual_padding and not pad_stack_bytes:
        ...
        diagnostic_label = f"diagnostic {pad_label}"
    if _diagnostic_pad_stack_is_irrelevant_register_window(classification):
        return None
    if _diagnostic_pad_stack_frame_already_matches(classification):
        frame = classification["stack_frame_sizes"]["expected_frame_size"]
        primary = classification.get("primary") or "unknown"
        return (
            "source-level stack reservation guidance: "
            f"{diagnostic_label} is present, but frame size already matches "
            f"target/current at {frame} bytes. Do not treat this as "
            "frame-reservation evidence unless a no-padding comparison proves "
            "the frame shrinks; for this run, prioritize the remaining "
            f"{primary} source shape instead."
        )
    source_guidance = _format_source_reservation_guidance(
        _source_reservation_guidance(
            int(total),
...
def _diagnostic_pad_stack_frame_already_matches(classification: dict) -> bool:
...
def _diagnostic_pad_stack_is_irrelevant_register_window(
    classification: dict,
) -> bool:
    backend = classification.get("backend_ceiling")
    return (
        isinstance(backend, dict)
        and backend.get("subclass") == "register-window-rotation"
        and _diagnostic_pad_stack_frame_already_matches(classification)
    )
...
def _is_callee_gpr(reg: str) -> bool:
    return _is_gpr(reg) and 14 <= _gpr_num(reg) <= 31
...
def classify_asm_diff(ref_lines: list[str], our_lines: list[str]) -> dict:
    ...
    our_mnemonics = _mnemonics(our_lines)
    ref_rendered_calls = _call_targets(ref_lines)
    our_rendered_calls = _call_targets(our_lines)
    ref_calls, our_calls, call_target_source = _paired_call_targets_for_shape(
        ref_lines,
        our_lines,
    )
    ...
    rendered_call_shape_differs = _call_target_multiplicity_differs(
        ref_rendered_calls,
        our_rendered_calls,
    )
    ...
    register_token_only_rotation = (
        ref_mnemonics == our_mnemonics
        and _line_diffs_are_register_token_only(ref_lines, our_lines)
    )
    ...
    if (
        call_target_source == "rel24"
        and rendered_call_shape_differs
        and not _call_target_multiplicity_differs(ref_calls, our_calls)
    ):
        reasons.append(
            "R_PPC_REL24 call target multiset matches; rendered bl labels "
            "differ, so this is not prototype/inline-boundary call-shape "
            "evidence"
        )
    ...
    if register_token_only_rotation:
        reasons.append(
            "opcode sequence is identical and all paired differences are "
            "register-token-only; classify as source-steerable register "
            "allocation, not a backend ceiling"
        )
    ...
    stack_frame_sizes = _stack_frame_size_summary(ref_lines, our_lines)
    ...
    if stack_frame_sizes:
        result["stack_frame_sizes"] = stack_frame_sizes
    if stack_slot_localizer:
    ...
    if backend_ceiling:
        ...
        if backend_ceiling.get("subclass") == "register-window-rotation":
            result["register_window_rotation"] = {
                "saved_gpr_window": backend_ceiling.get("saved_gpr_window"),
                "normalized_diff_lines": backend_ceiling.get(
                    "normalized_diff_lines"
                ),
                "callee_swap_pairs": backend_ceiling.get("callee_swap_pairs"),
            }
...
def format_summary(
    ...
    *,
    ...
    frame_sizes = classification.get("stack_frame_sizes")
    if isinstance(frame_sizes, dict):
        expected_frame = frame_sizes.get("expected_frame_size")
        current_frame = frame_sizes.get("current_frame_size")
        frame_delta = frame_sizes.get("frame_growth")
        if (
            expected_frame is not None
            and current_frame is not None
            and frame_delta is not None
        ):
            fields.append(f"expected_frame={expected_frame}")
            fields.append(f"current_frame={current_frame}")
            fields.append(f"frame_delta={frame_delta}")
    ...
    if pad_probe:
        ...
        register_window_pad = _diagnostic_pad_stack_is_irrelevant_register_window(
            classification,
        )
        if register_window_pad:
            fields.append("source_guidance=register-window-rotation")
        else:
            fields.append(
                f"diagnostic_pad_stack={pad_probe.get('total_pad_stack_bytes')}"
            )
        if reserved_low_spill:
            fields.append("source_guidance=reserved-low-spill-ceiling")
            fields.append("closability_tier=ceiling")
        elif _diagnostic_pad_stack_frame_already_matches(classification):
            if not register_window_pad:
                fields.append("source_guidance=source-shape-not-frame-reservation")
        else:
...
def _cleanup_wine_preloader(*, no_build: bool) -> None:
    """Best-effort cleanup for Wine processes spawned by a local build."""
    if no_build:
        return
    ...
    try:
        ...
        # Host cleanup is optional and must not replace the diff verdict.
        return
...
def main() -> int:
    ...
    pcdump_text = None
    if args.pcdump is not None and not args.dry_run:
        try:
            pcdump_text = read_pcdump_text_arg(args.pcdump)
        except PcdumpInputError as exc:
            parser.error(f"--pcdump: {exc}")
    ...
    _cleanup_wine_preloader(no_build=args.no_build)
