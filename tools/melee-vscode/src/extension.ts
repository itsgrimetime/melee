...
export function activate(context: vscode.ExtensionContext) {
    ...
    // Register decoration provider for inline match badges
    decorationProvider = new DecorationProvider(diffProvider);
    ...
    const docChangeDisposable = vscode.workspace.onDidChangeTextDocument((event) => {
        const editor = vscode.window.activeTextEditor;
        if (editor && event.document === editor.document && editor.document.languageId === 'c') {
            decorationProvider.triggerUpdate(editor);
        }
    });

    // Watch report.json for changes to refresh decorations
    const reportWatcher = vscode.workspace.createFileSystemWatcher(
        new vscode.RelativePattern(workspaceRoot, 'build/GALE01/report.json')
    );

    const reportChangeDisposable = reportWatcher.onDidChange(() => {
        diffProvider.invalidateCache();
        decorationProvider.clearLiveOverrides();
...
async function showDiffForFunction(functionName: string, context: vscode.ExtensionContext) {
    ...
    try {
        ...
        // Update inline badge for this function with live match percentage
        decorationProvider.setLiveMatch(functionName, diffResult.matchPercent);
