...
## PERMUTER ROUND (wave 2) — fleet rotation re-submit (2026-06-12, fleet-rotation agent wave 2) [CANONICAL job table]

Mechanical bootstrap + submit only (NO matching, NO triage). Branch `claude/mndiagram-802427B4-investigation`, HEAD `402ccf98c` (triage-3 wins committed; tree clean). **Allocation rationale**: orchestrator-decided per THIS round's ROTATION RANKING (above) — the top-2 ranked fns + the just-improved 8024227C re-bootstrapped. Both top picks are in the SAME residual class as a triage-3 win (UpdateHeader's CSE-hoist for AggRank's instruction-sequence; 8024227C's u64-temp for GetRankedName's indexed-pointer-materialization), i.e. the wave proved those classes are permuter-crackable. 3 builds (the three #558 base-verifies; budget held). Re-bootstrap if any source commit lands on these fns (#558).

### NEW JOBS (all three, canonical)
| Rank | Function | base % | Host | Job ID | Base score (#558) | Weights (`[weight_overrides]`) | Residual the job hunts |
|------|----------|--------|------|--------|-------------------|--------------------------------|------------------------|
| #1 | **mnDiagram2_GetAggregatedFighterRank** | 94.11 | **coder1** (intended dedicated — see host note) | `mnDiagram2_GetAggregatedFighterRank-coder1-20260612-044302` | **775** (local & remote agree) | reorder-heavy + temp/alias: `perm_reorder_decls=35, perm_reorder_stmts=30, perm_dummy_comma_expr=25, perm_temp_for_expr=22, perm_split_assignment=18, perm_refer_to_var=15, perm_pad_var_decl=12` | two adjacent callee-save transpositions (r26↔r27 base/zero, r30↔r31 type/funcTable) rooted in one register choice (D2COMPLETION:1342). instruction-sequence class = UpdateHeader's CSE-hoist channel. |
| #2 | **mnDiagram2_GetRankedName** | 97.87 | coder3 (shared) | `mnDiagram2_GetRankedName-coder3-20260612-044336` | **470** (local & remote agree) | temp + type-width + reorder: `perm_temp_for_expr=30, perm_randomize_internal_type=22, perm_reorder_decls=25, perm_dummy_comma_expr=25, perm_split_assignment=20, perm_reorder_stmts=18, perm_pad_var_decl=12` | the +1 `subf.`+`mr` j-guard (D2COMPLETION:1285), indexed-struct-pointer-materialization. The width-change arm (`randomize_internal_type` + `temp_for_expr`) is the axis 8024227C's u64 win used. Pair-shares class with GetRankedFighter (94.58) — a win transfers. |
| — | **mnDiagram_8024227C** (re-bootstrap @ 96.03) | 96.03 | coder3 (shared) | `mnDiagram_8024227C-coder3-20260612-044424` | **1345** (local & remote agree; = triage-3 best, base.c reflects the committed u64 source) | KEPT existing tuned settings.toml (proven 802427B4 reorder-heavy template + `perm_temp_for_expr=25` coalesce bump): `perm_dummy_comma_expr=35, perm_reorder_decls=25, perm_temp_for_expr=25, perm_split_assignment=22, perm_reorder_stmts=18, perm_pad_var_decl=15, perm_cast_simple/inline/add_self_assignment/duplicate_assignment=10` | +284/+288 GetNameTotalKOs-preheader scheduling transposition + r3/r6 value-home-vs-arg COALESCE order; post-u64 residual reclassified `stack-layout` (PAD_STACK 24). |

All three verified ALIVE via `remote tail` + `remote ps` (descending; each beat its base within 1 min — AggRank 600<775 @1343it, GetRankedName 435<470 @561it, 8024227C 1330<1345 @103it). Remote base scores match the local #558 figures exactly (injection-clean on the remote).

### #424 / #575 NOTES
- **8024227C**: 7 inline twins re-injected as BODIES (`randomize_funcs` = self + all 7: SumNameFalls, GetVisibleNameCursorFrom, CountUnlockedFightersInline, SumFighterFalls, GetVisibleFighterCursorFrom, GetNameTotalKOs, SumFighterKOsClamped); base 1345 (NOT 10k+) = #424-safe. Bootstrap auto-applied the **#575** `#pragma _permuter define NULL 0`; iter-1 compile, 0 errors. Existing tuned settings.toml correctly KEPT.
- **AggRank + GetRankedName**: both `injected_inline_callees: []` (callees `bl`-emitted in retail) — no #424 risk; no #575 hand-fix needed. Fresh settings.toml written with the weights above.
...
## PERMUTER ROUND (wave 3) — fleet rotation re-submit (2026-06-12, fleet-rotation agent wave 3) [CANONICAL job table]

Mechanical bootstrap + submit only (NO matching, NO triage). Branch `claude/mndiagram-802427B4-investigation`, HEAD `70baf3673` (triage-4 wins committed: AggRank 95.72 `cd47d4d41`, GetRankedName 98.62 `db5baee4e`, 8024227C 96.09 `561dcedf9`; tree clean, NO source edits this round). Allocation orchestrator-decided. **4 jobs submitted; 4 builds used (the four #558 base-verifies); budget held.** Re-bootstrap if any source commit lands on these fns (#558).

`DECOMP_AGENT_ID=fleet-wave3` set for claim stability. All bootstraps pinned `--melee-root <this worktree>` so base.c imports the worktree's committed source (NOT main checkout — the worktree gotcha). Both hosts confirmed free at start (triage-4 stopped wave-2 + the legacy posts; the lingering coder3 tmux windows are dead `fish`-pane orphans, zero threads, ignored per #591).

### NEW JOBS (all four, canonical)
| # | Function | base % | Host | Job ID | Base score (#558) | Scorer mode | Weights (`[weight_overrides]`) | Residual the job hunts |
|---|----------|--------|------|--------|-------------------|-------------|--------------------------------|------------------------|
| 1 | **mnDiagram_OnFrame** | 99.72 | **coder1** | `mnDiagram_OnFrame-coder1-20260612-080530` | **45** (local `permuter.py --seed 0`; low ⟹ #424-inline-injection-safe; standard objdiff scorer reaches **0=byte-match** at iters 12/106 in the verify run) | **standard match-objective (FALLBACK — force-phys NOT remotely supported, #595)** | move-axis-heavy per the what-if oracle: `perm_reorder_stmts=35, perm_reorder_decls=30, perm_temp_for_expr=22, perm_dummy_comma_expr=20, perm_split_assignment=18, perm_pad_var_decl=12` | the SINGLE node ig49 (class 0) → needs r28 (we give r29); `--force-phys 0:49:28` = byte-match proven. Oracle (ONFRAME IG49 ROUND, D1COMPLETION:1066) says the flip is a **dispense-position (move-axis)** outcome (Case C: shift ig49 simplify-order one slot later) → reorder-heavy weights seed that axis. |
| 2 | **mnDiagram2_GetAggregatedFighterRank** (re-bootstrap @95.72) | 95.72 | coder1 (shared) | `mnDiagram2_GetAggregatedFighterRank-coder1-20260612-081320` | **600** (local; base.c reflects committed comma-expr win — slightly above wave-2's 525 best because base IS the improved source) | standard | wave-2 reorder-heavy + temp/alias: `perm_reorder_decls=35, perm_reorder_stmts=30, perm_dummy_comma_expr=25, perm_temp_for_expr=22, perm_split_assignment=18, perm_refer_to_var=15, perm_pad_var_decl=12` | two adjacent callee-save transpositions rooted in one register choice (instruction-sequence class = UpdateHeader CSE-hoist channel). Wave-2's win (94.11→95.72) came in ~10 min — new substrate may yield the next rung fast. |
| 3 | **mnDiagram2_GetRankedName** (re-bootstrap @98.62) | 98.62 | coder3 (shared) | `mnDiagram2_GetRankedName-coder3-20260612-081335` | **370** (local; = wave-2 best, base.c reflects committed operand-swap win) | standard | wave-2 temp + type-width + reorder: `perm_temp_for_expr=30, perm_randomize_internal_type=22, perm_reorder_decls=25, perm_dummy_comma_expr=25, perm_split_assignment=20, perm_reorder_stmts=18, perm_pad_var_decl=12` | the +1 `subf.`+`mr` j-guard on the indexed-struct-pointer-materialization path. Width-change arm = the axis 8024227C's u64 win used. |
| 4 | **mnDiagram_80241E78** | 98.94 | coder3 (shared) | `mnDiagram_80241E78-coder3-20260612-081349` | **555** (local; low ⟹ injection-clean, see #424 note) | standard | reorder-heavy + temp/alias (the 4-win class): `perm_reorder_decls=35, perm_reorder_stmts=30, perm_dummy_comma_expr=25, perm_temp_for_expr=22, perm_split_assignment=18, perm_refer_to_var=15, perm_pad_var_decl=12` | the ranking tail's head; stack-layout class; residual = r25↔r26 data/row swap + FP shadow. |

(mnDiagram_8024227C @96.09 PARKED this wave — marginal yield; scheduler-tiebreak wall needs a different device.)

**ALL FOUR verified ALIVE** via `remote tail` (list+ps both broken, #591/#593 — tmux/tail evidence only): OnFrame iter 15841 base=45 descending; AggRank iter 488 base=600 descending; GetRankedName iter 649 base=370 (exploring 475/1295 above base); 80241E78 iter 179 base=555 descending. Remote base scores match local #558 figures.

### THE ONFRAME FORCE-PHYS DECISION (audit-first, the headline of this wave)
The brief's job-1 objective was the force-phys custom scorer with the proven `0:49:28` target. **Audit outcome: the custom scorer EXISTS and is LOCALLY working, but is NOT remotely supported → fell back to standard match-objective per the brief.**

- **`debug target score-force-phys` EXISTS** (the colorgraph-dispense companion to score-simplify-order, lex-encoded force-phys assignment hits). Wired locally via `setup-simplify-order-scorer --scorer-mode force-phys --force-phys 49:28 --class 0 --auto-baseline-dump` (note the flag form is `IG_IDX:PHYS`, i.e. `49:28`, NOT `class:ig:phys`; `--class 0` carries the GPR class). Generated `simplify_order_target.yaml` (`class_id: 0`, `force_phys: {49: 28}`), a 4.5MB baseline pcdump, a compile.sh wrapped to use `mwcceppc_debug.exe` + emit pcdump sidecars, and a `[scorer]` settings.toml section.
- **LOCALLY VALIDATED:** base.o scored **1000000** with "Force-phys hits: 0/1" — correctly polarized (base gives ig49 r29 NOT r28; any candidate that flips to r28 scores far lower). The full scorer path (compile.sh → pcdump sidecar → score-force-phys) works.
- **REMOTE BLOCKER (#595):** `remote submit` fails preflight because (a) the stale remote `melee-agent` on coder1 AND coder3 (`/home/discord/.local/bin/melee-agent`) **predates the `score-force-phys` command** — `debug target --help` lists score-dump/score-source/score-simplify-order but NOT score-force-phys; (b) the doctor checks PATH via non-login `sh`/`bash` but melee-agent only resolves via `fish -lc` (false "not found"); (c) `setup-simplify-order-scorer` writes ABSOLUTE `/Users/...` paths into settings.toml `[scorer].command --target` AND yaml `baseline_dump`, tripping `_validate_remote_ready_perm_dir` (`_rewrite_settings_toml_for_remote` only rewrites `objdump_command`). Hand-fixed (c) to relative paths (works; doctor accepts relative `--target` inside the function dir + score-force-phys resolves `baseline_dump` relative to `target.parent`), but (a) is the hard wall: remote melee-agent must be re-installed from current source before ANY force-phys remote job. simplify-order custom scorer WOULD work remotely (score-simplify-order present with `--strict-polarity`); force-phys does not.
- **FALLBACK rationale:** the standard objdiff scorer reaches 0=byte-match on OnFrame in the local verify (iters 12/106), and the move-axis weights are exactly the oracle's prescribed axis. So the standard-objective job IS targeting the right lever; it just can't use the lex-encoded force-phys ranking to keep placeholder-row candidates rankable. Re-submit with the custom scorer once #595 (remote melee-agent update) lands.

### #424 / #575 NOTES
- **OnFrame (#424 + #575):** bootstrap injected `mnDiagram_UpdateScrollArrowVisibility` as a BODY (`randomize_funcs` = [OnFrame, UpdateScrollArrowVisibility]) — correct, since ig49 lives INSIDE that inlined callee (the move-axis mutations must reach it). #575 hand-fix applied: bootstrap re-injects the source's `#define PAD_STACK(bytes) do { UNUSED unsigned char _[(bytes)]; } while(0)` but `UNUSED` is undefined in the permuter context → "before: unsigned" syntax error. Removed the duplicate `#define` (the permuter's `latedefine` PAD_STACK at base.c:2, `do { unsigned char _[(bytes)]; } while (0)`, is the sole correct definition; `UNUSED` is a no-op attribute, codegen-irrelevant). base.c then compiled, base score 45. **Re-applied after the re-bootstrap to standard objdiff** (the fix doesn't survive `bootstrap --force`).
- **AggRank + GetRankedName (#424 N/A):** both `injected_inline_callees: []` (callees bl-emitted in retail) — base 600/370 (low) confirms injection-clean. No #575 hand-fix.
- **80241E78 (#424 verified clean — the brief's precaution did NOT apply):** bootstrap reported `injected_inline_callees: []`. The brief warned 80241E78 "calls GetVisibleNameCursorFrom family + GetDigitCount-era helpers — verify bodies injected." **Verified: it does NOT.** Its retail asm `bl`s ONLY `__assert` + `mn_GetDigitCount`; its base.c body references only `mn_GetDigitCount` (a genuine `bl`, correctly emitted) + data symbols (`mnDiagram_804A07F4/804D4FA0/803EE728/74C/768`). NO same-TU inline callee to inject. Base score **555** (low, NOT 10k+) confirms injection-clean. The precaution was about neighboring functions, not this body.

### TOOLING ISSUES FILED
- **#595 (NEW):** remote force-phys custom scorer blocked — stale remote `melee-agent` lacks `score-force-phys` (hard blocker, both hosts) + doctor PATH false-negative (fish vs bash login) + `setup-simplify-order-scorer` emits absolute `/Users/` paths not remote-ready (papercut, hand-fixed to relative). Full repro + the 3 gaps in the issue body. Blocked OnFrame's force-phys objective this wave.

### STATE COMMIT
Recorded in both CAMPAIGN-STATE-D2COMPLETION.md (this section, canonical) and CAMPAIGN-STATE-D1COMPLETION.md (cross-ref). Commit hash: see the `docs(mn): permuter fleet wave 3` commit.
