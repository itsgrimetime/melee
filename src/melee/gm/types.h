...
/// Per-fighter statistics for VS Records.
/// Stores head-to-head KO counts and aggregate stats for each character.
/// @note Stats fields (0x34-0x77) mirror NameTagData (0xF0-0x133) with -0xBC
///       offset. Could potentially share a common VsRecordStats struct.
struct FighterData {
    /* 0x00 */ u16 fighter_kos[25]; ///< KOs scored against each fighter index
    /* 0x32 */ u8 padding_0x32[2];
    /* 0x34 */ u16 sd_count; ///< Self-destructs
    /* 0x36 */ u8 padding_0x36[2];
    /* 0x38 */ u32 attacks_hit;        ///< Attacks that connected
    /* 0x3C */ u32 attacks_total;      ///< Total attacks (hit% = hit/total)
    /* 0x40 */ s32 damage_dealt;       ///< Total damage dealt
    /* 0x44 */ s32 damage_taken;       ///< Total damage taken
    /* 0x48 */ s32 damage_recovered;   ///< Damage recovered
    /* 0x4C */ u16 peak_damage;        ///< Highest % reached
    /* 0x4E */ u16 match_count;        ///< Total matches played
    /* 0x50 */ u16 victories;          ///< Wins
    /* 0x52 */ u16 losses;             ///< Losses
    /* 0x54 */ u32 play_time;          ///< Play time (seconds)
    /* 0x58 */ u32 total_player_count; ///< Sum of player counts per match (for
                                       ///< avg calculation)
    /* 0x5C */ s32 walk_distance;      ///< Ground distance
    /* 0x60 */ s32 run_distance;       ///< Jump distance
    /* 0x64 */ s32 fall_distance;      ///< Drop distance
    /* 0x68 */ s32 peak_height;        ///< Flight distance
    /* 0x6C */ s32 coins_collected;    ///< Coin battle coins collected
    /* 0x70 */ s32 coins_swiped;       ///< Coin battle coins stolen
    /* 0x74 */ s32 coins_lost;         ///< Coin battle coins lost
    /* 0x78 */ s8 x78;
    /* 0x79 */ s8 x79;
...
/// Per-name statistics for VS Records.
/// Stores head-to-head KO counts and aggregate stats for each name tag.
/// @note Stats fields (0xF0-0x133) mirror FighterData (0x34-0x77) with +0xBC
///       offset. Could potentially share a common VsRecordStats struct.
struct NameTagData {
    /* 0x000 */ u16 vs_kos[120]; ///< KOs scored against each name index
    /* 0x0F0 */ u16 sd_count;    ///< Self-destructs
    /* 0x0F2 */ u8 padding_0xF2[2];
    /* 0x0F4 */ u32 attacks_hit;        ///< Attacks that connected
    /* 0x0F8 */ u32 attacks_total;      ///< Total attacks (hit% = hit/total)
    /* 0x0FC */ s32 damage_dealt;       ///< Total damage dealt
    /* 0x100 */ s32 damage_taken;       ///< Total damage taken
    /* 0x104 */ s32 damage_recovered;   ///< Damage recovered
    /* 0x108 */ u16 peak_damage;        ///< Highest % reached
    /* 0x10A */ u16 match_count;        ///< Total matches played
    /* 0x10C */ u16 victories;          ///< Wins
    /* 0x10E */ u16 losses;             ///< Losses
    /* 0x110 */ u32 play_time;          ///< Play time (seconds)
    /* 0x114 */ u32 total_player_count; ///< Sum of player counts per match
                                        ///< (for avg calculation)
    /* 0x118 */ s32 walk_distance;   ///< Ground distance (units / 30.5 = feet)
    /* 0x11C */ s32 run_distance;    ///< Jump distance (units / 30.5 = feet)
    /* 0x120 */ s32 fall_distance;   ///< Drop distance (units / 30.5 = feet)
    /* 0x124 */ s32 peak_height;     ///< Flight distance (units / 30.5 = feet)
    /* 0x128 */ s32 coins_collected; ///< Coin battle coins collected
    /* 0x12C */ s32 coins_swiped;    ///< Coins battle coins stolen from others
    /* 0x130 */ s32 coins_lost;      ///< Coins battle coins lost to others
    /* 0x134 */ u32
        play_time_by_fighter[25]; ///< Play time per fighter (0x19 entries)
    /* 0x198 */ char namedata[8]; ///< Name tag text (Shift-JIS encoded)
    /* 0x1A0 */ s8 x1A0;          ///< Unknown/unused
    /* 0x1A1 */ u8 rumble;        ///< Rumble toggle (0=off, 1=on)
    /* 0x1A2 */ s8 handicap;      ///< Handicap level (1-9, default=5)
...
struct NameData {
    ...
    /* x0F4 */ u32 attacks_hit;
    /* x0F8 */ u32 attacks_total;
