...
void mnDiagram3_80245BA4(HSD_GObj *gobj)
{
  ...
  base = (char *) (&mnDiagram3_803EEC10);
  {
    ...
    u8 offset = (((((((((data->scroll_offset & 0xFFFFFFFFu) & 0xFFFFFFFFu) & 0xFFFFFFFFu) & 0xFFFFFFFFu) & 0xFFFFFFFFu) & 0xFFFFFFFFu) & 0xFFFFFFFFu) & 0xFFFFFFFFu) & 0xFFFFFFFFu) & 0xFFFFFFFFu;
    int limit;
    if (is_name_mode != 0)
    {
      limit = 0x18;
    ...
    {
      int val = (long long) (scroll + offset);
      if (val >= limit)
      {
        val = val - limit;
      }
  ...
  row1 = data->jobjs[7];
  {
    ...
    (row1) ? ((void) 0) : (__assert(mnDiagram3_804D4FD8, 0x3EE, mnDiagram3_804D4FE0));
    {
      ...
      for (i = 0; i < 5; i++)
      {
        ...
        if (mnDiagram2_IsIconOnlyStat(stat_type))
        {
          mnDiagram2_GetAggregatedFighterRank(sp48, stat_type, (u8) i);
          if (sp48[0] == 0x19)
          {
            goto next;
          }
        ...
        else
        {
          ...
          {
            int val = mnDiagram2_GetStatValue(data->is_name_mode, stat_type, entity);
            if (((u32) val) > max_time)
            {
              val = max_time;
            }
        ...
        }
        {
          ...
          lb_8000B1CC(data->jobjs[6], (Vec3 *) (base + 0x30), &sp6C);
          {
            ...
            if (mnDiagram2_IsDistanceStat(stat_type))
            {
                unsigned long long stat_val = mnDiagram2_GetStatValue(
...
void fn_802461BC(HSD_GObj *gobj)
{
  ...
  if (buttons & 0x20)
  {
    ...
    ;
    gmMainLib_8015CC34()->xD = ((Diagram3 *) mnDiagram3_804D6C20->user_data)->is_name_mode;
  ...
  if (buttons & 0xC0)
  {
    ...
    for (i = 0; i < 10; i++)
    {
      labels_a = ptr->row_labels;
      if (d->row_labels[i] != 0L)
      {
        HSD_SisLib_803A5CC4(labels_a[i]);
        d->row_labels[i] = 0L;
      }
    }
  ...
  if (buttons & 0xC00)
  {
    ...
    do
    {
      ...
      text = HSD_SisLib_803A5ACC(0, 1, sp_pos1.x - row_spacing, (neg_spacing * fi) + (-sp_pos1.y), sp_pos1.z, font_size_b, mnDiagram3_804DBFFC);
      d->row_labels[i] = text;
      if (d->is_name_mode != 0)
      {
        limit = 0x18;
  ...
  if (buttons & 1)
  {
    if (data->saved_selection != 0)
    {
      ...
      row0 = d->jobjs[8];
      sel = data->saved_selection;
      (row0) ? ((void) 0) : (__assert(mnDiagram3_804D4FD8, 0x3EE, mnDiagram3_804D4FE0));
      row1 = d->jobjs[9];
      row_spacing = row0->translate.y;
      (row1) ? ((void) 0) : (__assert(mnDiagram3_804D4FD8, 0x3EE, mnDiagram3_804D4FE0));
      row_spacing = row1->translate.y - row_spacing;
      (d->jobjs[8]) ? ((void) 0) : (__assert(mnDiagram3_804D4FD8, 0x3E1, mnDiagram3_804D4FE0));
      tx = row0->translate.x;
      (popup_jobj) ? ((void) 0) : (__assert(mnDiagram3_804D4FD8, 0x3A4, mnDiagram3_804D4FE0));
      ...
      if (!(popup_jobj->flags & 0x02000000))
      {
        ...
        {
          int is_dirty;
          (popup_jobj) ? ((void) 0) : (__assert(mnDiagram3_804D4FD8, 0x234, mnDiagram3_804D4FE0));
          is_dirty = 0;
          if ((!(popup_jobj->flags & 0x800000)) && (popup_jobj->flags & 0x40))
          {
            is_dirty = 1;
          }
          if (is_dirty == 0)
          {
            HSD_JObjSetMtxDirtySub(popup_jobj);
          }
        }
      }
      row0 = d->jobjs[8];
      (row0) ? ((void) 0) : (__assert(mnDiagram3_804D4FD8, 0x3EE, mnDiagram3_804D4FE0));
      ty = (row_spacing * ((f32) sel)) + row0->translate.y;
      (popup_jobj) ? ((void) 0) : (__assert(mnDiagram3_804D4FD8, 0x3B3, mnDiagram3_804D4FE0));
      ...
      tz = row0->translate.z;
      (popup_jobj) ? ((void) 0) : (__assert(mnDiagram3_804D4FD8, 0x3C2, mnDiagram3_804D4FE0));
    ...
    if (data->scroll_offset != 0)
    {
      ...
      d = mnDiagram3_804D6C20->user_data;
      row0 = d->jobjs[8];
      scroll = d->scroll_offset & 0xFFFFFFFFFFFFFFFFu;
      font_size_c = font_size_b;
      (row0) ? ((void) 0) : (__assert(mnDiagram3_804D4FD8, 0x3EE, mnDiagram3_804D4FE0));
      row1 = d->jobjs[9];
      row_spacing = row0->translate.y;
      (row1) ? ((void) 0) : (__assert(mnDiagram3_804D4FD8, 0x3EE, mnDiagram3_804D4FE0));
      ...
      do
      {
        ...
        text = HSD_SisLib_803A5ACC(0, 1, sp_pos2.x - row_spacing, (neg_spacing * fi) + (-sp_pos2.y), sp_pos2.z, mnDiagram3_804DBFFC, font_size_c);
        d->row_labels[i] = text;
        if (d->is_name_mode != 0)
        {
          limit = 0x18;
    ...
    if (buttons & 2)
  {
    ...
    if (((u32) (data->scroll_offset + 10)) < limit)
    {
      ...
      data->scroll_offset = data->scroll_offset + 1;
      ptr = (d = mnDiagram3_804D6C20->user_data);
      labels_c = ptr->row_labels;
      for (i = 0; i < 10; i++)
      {
        if (d->row_labels[i] != 0L)
        {
          HSD_SisLib_803A5CC4(labels_c[i]);
          d->row_labels[i] = 0L;
        }
      }
      ...
      do
      {
        ...
        text = HSD_SisLib_803A5ACC(0, 1, sp_pos3.x - row_spacing, (neg_spacing * fi) + (-sp_pos3.y), sp_pos3.z, mnDiagram3_804DBFFC, font_size);
        d->row_labels[i] = text;
        if (d->is_name_mode != 0)
        {
          limit = 0x18;
...
void mnDiagram3_80246D40(HSD_GObj* gobj)
{
    ...
    jobj = data->jobjs[4];
    jobj_alias = jobj;
    mn_8022ED6C(jobj_alias, &mnDiagram3_803EEC1C);

    if (data->scroll_offset != 0) {
        HSD_JObjClearFlagsAll(jobj_alias, 0x10);
    } else {
        HSD_JObjSetFlagsAll(jobj_alias, 0x10);
    }
}
...
void mnDiagram3_8024714C(void *arg0)
{
  ...
  data = gobj->user_data;
  {
    ...
    GObj_SetupGXLink(popup, HSD_GObj_JObjCallback, new_var, 0x80);
    ...
    data = (gobj_alias = gobj)->user_data;
    row0 = data->jobjs[8];
    ...
    row_spacing = row1->translate.y - row_spacing;
    row0 = data->jobjs[8];
    (row0) ? ((void) 0) : (__assert(mnDiagram3_804D4FD8, 0x3E1, mnDiagram3_804D4FE0));
    {
    ...
    (row0) ? ((void) 0) : (__assert(mnDiagram3_804D4FD8, 0x3EE, mnDiagram3_804D4FE0));
    {
      ...
      (popup_alias) ? ((void) 0) : (__assert(mnDiagram3_804D4FD8, 0x3B3, mnDiagram3_804D4FE0));
      popup_alias->translate.y = ty;
    }
    if (!(inline_fn(popup_alias) & 0x02000000))
    {
      ftCo_800C6AFC(popup_alias);
    }
    row0 = data->jobjs[8];
  ...
  }
  {
    ...
    {
      f32 fi = (f32) i;
      HSD_Text *text = HSD_SisLib_803A5ACC(0, 1, (0, sp48.x - row_spacing), (neg_spacing * fi) + (-sp48.y), sp48.z, font_size, mnDiagram3_804DBFFC);
      d->row_labels[i] = text;
      {
        u8 type_idx = (u8) i;
        int val;
        ...
        if (val >= ((u8) limit_alias))
        {
          val = val - ((u8) limit_alias);
        }
        else
        {
          val = (u8) val;
