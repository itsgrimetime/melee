...
void mnDiagram3_80245BA4(HSD_GObj *gobj)
{
  ...
  base = (char *) (&mnDiagram3_803EEC10);
  {
    ...
    u8 offset = (u32) data->scroll_offset;
    int limit;
    if (is_name_mode != 0)
    {
      limit = 0x18;
  ...
  row1 = data->jobjs[7];
  {
    ...
    (row1) ? ((void) 0) : (__assert(mnDiagram3_804D4FD8, 0x3EE, mnDiagram3_804D4FE0));
    {
      ...
      for (i = 0; i < 5; i++)
      {
        ...
        {
            entity = mnDiagram2_GetRankedFighter(stat_type, fighter_idx);
        ...
        if (mnDiagram2_IsIconOnlyStat(stat_type))
        {
          ...
          rank_idx = (u8) i;
          mnDiagram2_GetAggregatedFighterRank(sp38, stat_type, rank_idx);
        ...
        else
        {
          ...
          {
              int val = mnDiagram2_GetStatValue(data->is_name_mode, stat_type,
                                                entity);
              if (val > max_distance) {
                  val = max_distance;
              }
        ...
        }
        {
          ...
          lb_8000B1CC(data->jobjs[6], (Vec3 *) (base + 0x30), &sp6C);
          {
            ...
            if (mnDiagram2_IsDistanceStat(stat_type))
            {
              unsigned long long stat_val = mnDiagram2_GetStatValue(data->is_name_mode, stat_type, entity);
...
void fn_802461BC(HSD_GObj* gobj)
{
    ...
    if (buttons & 0xC00) {
        ...
        if ((data->is_name_mode == 0) &&
            (((s32) (data->scroll_offset + 10)) >= 0x15))
        {
            data->scroll_offset = 0;
        }
        ptr = (d = mnDiagram3_804D6C20->user_data);
        labels_b = ptr->row_labels;
        for (i = 0; i < 10; i++) {
            if (d->row_labels[i] != 0L) {
                HSD_SisLib_803A5CC4(labels_b[i]);
                d->row_labels[i] = 0L;
            }
        }
        ...
        do {
            ...
            text = HSD_SisLib_803A5ACC(0, 1, sp_pos1.x - row_spacing,
                                       (neg_spacing * fi) + (-sp_pos1.y),
                                       sp_pos1.z, font_size_b,
                                       mnDiagram3_804DBFFC);
            d->row_labels[i] = text;
            if (d->is_name_mode != 0) {
                limit = 0x18;
        ...
        if (data->saved_selection != 0) {
            ...
            if (!(popup_jobj->flags & 0x02000000)) {
                if (popup_jobj != 0L) {
                    int is_dirty;
                    (popup_jobj) ? ((void) 0)
                                 : (__assert(mnDiagram3_804D4FD8, 0x234,
                                             mnDiagram3_804D4FE0));
                    is_dirty = 0;
...
void mnDiagram3_8024714C(void *arg0)
{
  ...
  }
  {
    ...
    (row0) ? ((void) 0) : (__assert(mnDiagram3_804D4FD8, 0x3EE, mnDiagram3_804D4FE0));
    row1 = d->jobjs[9];
    font_size = row0->translate.y;
    row_spacing = font_size;
    (row1) ? ((void) 0) : (__assert(mnDiagram3_804D4FD8, 0x3EE, mnDiagram3_804D4FE0));
    row_spacing = row1->translate.y - row_spacing;
