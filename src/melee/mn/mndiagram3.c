    ...
    if (buttons & 0xC00) {
        ...
        do {
            ...
            text = HSD_SisLib_803A5ACC(0, 1, sp_pos1.x - row_spacing,
                                       (neg_spacing * fi) + (-sp_pos1.y), sp_pos1.z,
                                       font_size_b, mnDiagram3_804DBFFC);
            d->row_labels[i] = text;
            if (d->is_name_mode != 0) {
                limit = 0x18;
            } else {
        ...
        if (data->saved_selection != 0) {
            ...
            if (!(popup_jobj->flags & 0x02000000)) {
                if (popup_jobj != 0L) {
                    int is_dirty;
                    ((popup_jobj) ? ((void) 0)
                                  : __assert(mnDiagram3_804D4FD8, 0x234, mnDiagram3_804D4FE0));
                    is_dirty = 0;
        ...
        if (data->scroll_offset != 0) {
            ...
            do {
                ...
                text = HSD_SisLib_803A5ACC(0, 1, sp_pos2.x - row_spacing,
                                           (neg_spacing * fi) + (-sp_pos2.y), sp_pos2.z,
                                           mnDiagram3_804DBFFC, font_size_c);
                d->row_labels[i] = text;
                if (d->is_name_mode != 0) {
                    limit = 0x18;
                } else {
        ...
        if (((u32) (data->scroll_offset + 10)) < limit) {
            ...
            do {
                ...
                text = HSD_SisLib_803A5ACC(0, 1, sp_pos3.x - row_spacing,
                                           (neg_spacing * fi) + (-sp_pos3.y), sp_pos3.z,
                                           mnDiagram3_804DBFFC, font_size);
                d->row_labels[i] = text;
                if (d->is_name_mode != 0) {
                    limit = 0x18;
                } else {
