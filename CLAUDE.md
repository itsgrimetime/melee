...
Treat native ports as diagnostics for the decomp, not as patch sets to import
wholesale. The durable boundary is: abstractions and source-correctness fixes
belong here; host-specific implementations belong in the port.

Classify each finding before editing:

- Correct incorrect types, pointer truncation, undefined behavior, and false
  target-layout assumptions in the main decomp when the change expresses the
  program's actual semantics.
- Add portable types, accessors, or interfaces here when ports need a shared
  abstraction, but keep DAT conversion, relocation/pointer widening, hardware
  services, headless stubs, and PPC-semantic emulation outside this repository.
- Keep fixed-format words fixed-width. Use pointer-width types only for values
  whose domain actually includes host pointers. Values that mix guest addresses,
  offsets, and native pointers need an explicit domain abstraction or resolver,
  not a blind conversion to `uintptr_t`.

Equal-width C types can still produce different MWCC code. When the portable
representation is functionally correct but exact matching requires the original
type or spelling, guard that representation with `MUST_MATCH`; use compiler or
architecture guards only for genuine compiler or architecture behavior. Test
reviewer-suggested alternatives against both compilers instead of reasoning from
width or ABI alone.

Do not solve endian-sensitive bitfields by accumulating unrelated conditional
declarations. Prefer masks, accessors, or a shared endian-aware policy supported
by native runtime evidence. Keep PPC floating-point compatibility in a designed
boundary rather than scattering host builtins through matched source.

Every portability change must preserve the expected GALE01 DOL and all match
metrics and must also be checked in the relevant native build or runtime. Keep
upstream PRs narrow. Portability-focused PRs opened with AI assistance must carry
both the `ai-assisted` and `portability` labels. Use `/portability` for the full
triage, implementation, and review workflow.
...
- `/portability` — triage native-build and native-runtime findings, decide what
  belongs upstream, and validate match-preserving portability changes.
