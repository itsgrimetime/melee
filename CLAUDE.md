...
- `/mwcc-debug` - For confirmed register-allocation mismatches, especially when you have a desired `IG:phys` target, use `melee-agent debug inspect lifetime-pressure -f <func> --force-phys "IG:PHYS,..."` before speculative source edits. It is read-only by default and emits source-actionable hypotheses plus exact validation commands.
