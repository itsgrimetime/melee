...
Keep the prompt layer short and follow the detailed references when needed:

| Resource | Use it for |
|----------|------------|
| `docs/MATCHING_GUIDE.md` | Measured MWCC matching behavior, residual classification, source levers, and terminal-verdict discipline |
| `/Users/mike/code/decomp-search` | Whole-function/window donor search and type-layout similarity; start with its `.claude/skills/decomp-search/SKILL.md` |
| `/Users/mike/code/decomp-scripts` | Standalone compiler/binary archaeology, artifact verification, and Ghidra export helpers; start with its `README.md` |
| `docs/mwcc-retro.md` | Retail compiler tracing and the issue #1240 candidate/probe toolset; preview artifacts are diagnostic, not production proof |

The standalone repositories are not fully covered by `melee-agent capabilities
search`; see `docs/agent-tool-manifest.md` for the shortest command examples.
