...
melee-agent patterns wrappers <func>       # Find wrapper functions
melee-agent patterns anti-patterns <func>  # Detect anti-patterns
melee-agent patterns inlines <func>        # Find inline candidates
melee-agent patterns similar <func>        # Find similar matched functions

# Ghidra integration (alternative decompiler)
melee-agent ghidra decompile <func>        # Ghidra decompilation
melee-agent ghidra xrefs <func>            # Cross-references (callers/callees)
melee-agent ghidra types <func>            # Type inference

# State and analytics
melee-agent state show                     # Show agent state DB
melee-agent analytics summary              # Session performance stats
melee-agent audit check                    # Audit tracked work
```
...
When running a team of agents for parallel decompilation work:

- **Build serialization**: Only one `ninja` at a time. A PreToolUse hook wraps ninja commands with a file lock (`/tmp/melee-build.lock`) so builds are automatically serialized.
- **Git discipline**: Only the team lead stages and commits. Agents are blocked from `git add`, `git commit`, and `git push` by a hook.
- **Header-first workflow**: Agents must update `.h` files (replace `UNK_RET`/`UNK_PARAMS` with real signatures) before implementing `.c` function bodies. Pre-commit hooks enforce matching prototypes.
- **Tool usage**: Use `tools/checkdiff.py` only. Direct `objdiff-cli`, `wine`, and `wibo` invocations are blocked by a hook.
- **Type discovery**: Search existing headers (`include/melee/gm/types.h`, module headers, `.static.h`) before creating new types. Use `melee-agent struct` commands.
- **Extern placement**: Extern declarations go in `.h` or `.static.h` files, never in `.c` files.
