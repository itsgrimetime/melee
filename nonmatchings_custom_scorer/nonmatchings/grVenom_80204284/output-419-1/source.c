...
inline u32 inline_fn(struct grVenom_GroundVars arg0)
{
  return arg0.xC8;
}

inline HSD_JObj *inline_fn2(HSD_GObj *arg0)
{
  return (HSD_JObj *) arg0->hsd_obj;
}
...
void grVenom_80204284(Ground_GObj *gobj)
{
  ...
  dst_jobj = inline_fn2((HSD_GObj *) gp->gv.venom.xC4);
  HSD_JObjGetTranslation(src_jobj, &pos);
  HSD_JObjSetTranslate(dst_jobj, &pos);
  Ground_801C39C0();
  Ground_801C3BB4();
  new_var = &gp->gv.venom;
  timer = inline_fn(*new_var);
  if (timer > 0)
  {
    gp->gv.venom.xC8 = timer + 1;
    if (((s32) inline_fn(gp->gv.venom)) >= 0x3C)
    {
      if (((s32) inline_fn(gp->gv.venom)) == 0x3C)
      {
        ifStatus_802F6898();
