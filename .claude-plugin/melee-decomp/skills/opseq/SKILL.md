...
```bash
cd melee && tools/table-typer/table-typer opseq <comma,separated,opcodes>
```

### Find undecompiled candidates

```bash
cd melee && tools/table-typer/table-typer opseq -candidates <comma,separated,opcodes>
```
...
cd melee && tools/table-typer/table-typer opseq beq,mr,bl
```
...
cd melee && tools/table-typer/table-typer opseq lfs,fcmpo,bge
```
...
cd melee && tools/table-typer/table-typer opseq addi,cmpwi,blt
```
...
2. Find similar already-matched functions:
   ```bash
   cd melee && tools/table-typer/table-typer opseq <pattern>
   ```
...
```bash
cd melee && tools/table-typer/table-typer opseq -candidates <pattern_from_matched_func>
```
