...
When working as a team agent (spawned by a team lead), follow these rules strictly:

- **Always update headers first**: Before implementing a function body, update the corresponding `.h` file to replace `UNK_RET`/`UNK_PARAMS` with the concrete signature. Pre-commit hooks enforce this.
- **Never use `git add/commit/push`**: The team lead handles all git operations. A hook will block these commands.
- **Never put extern declarations in .c files**: Put them in the corresponding `.h` file or a module `.static.h` file.
- **Use `tools/checkdiff.py` only**: Never use `objdiff-cli`, `wine`, or `wibo` directly. A hook will block these.
- **Type discovery before type creation**: Before creating new types, search existing headers (see Type Discovery below). Don't reinvent types that already exist.
- **One build at a time**: Ninja builds are automatically serialized by a lock, but be aware that builds may wait if another agent is building.
...
### Type Discovery (Before Creating New Types)

Before defining a new struct or typedef:
1. Search existing headers: `grep -rn "StructName\|field_pattern" include/melee/ src/melee/`
2. Check the struct tool: `melee-agent struct offset <offset>` or `melee-agent struct show <type>`
3. Check `include/melee/gm/types.h` - many game-global types live there
4. Check the module's own header and `.static.h` files
5. Only create a new type if nothing exists - and put it in the `.static.h` file, not inline in `.c`
