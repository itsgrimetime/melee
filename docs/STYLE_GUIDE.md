...
### Don't invent locals or derivations to preserve codegen

**Rule:** A local, cast, or pointer derivation that exists only to reproduce
the original instructions is "fake" and will be rejected in review. Prefer the
plain array or field access. If the plain form does not match, keep the
previously accepted form (or leave the code as it was) and say so in the PR
with the asm diff, so the maintainers choose the shape. Do not swap one fake
construct for a new one.

```c
// REJECTED (#3451) - `base` exists only to make the compiler address the
// name array from the fighter array's base
...
// PREFERRED - plain access; if this regresses the match, report the diff
// instead of inventing an intermediate
u8* p = &mnDiagram_NameDisplayOrder[max_idx];
u8 temp = *p;
```

**Why:** Maintainer feedback on #3451: "The array access is better; `base` is
fake." Readability of the source is the goal; matching constraints are
explained, not hidden behind new helper variables.
...
**Rule:** Documentation lives at the declaration; `.dox` files are retired
(#2938, #3461). Function docs go above the prototype in the header, or above
the `static` forward declaration in the C file. Struct-field docs go on the
field. Anything longer than a short paragraph goes in `docs/*.md` and is
linked with `@ref`.

**Spacing (ribbanya, #3461):** a block-documented declaration or field gets a
blank line before the block and after the declaration. One-line trailing
`///<` comments stay packed with their neighbours; if a trailing comment would
wrap past 80 columns, make it a block above the field instead.
...
/* +0E */ s16 state_timer; ///< Auto-state lockout counter.
```

`@file` blocks go above the include guard. `@dir` blocks may live in a
markdown file under `docs/`. The old `@at{}`/`@sz{}` aliases are gone; offsets
belong in the `/* +NN */` comments and sizes in `ASSERT_SIZE`.
