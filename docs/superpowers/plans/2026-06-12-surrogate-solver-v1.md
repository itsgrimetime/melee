...
def solve_coloring(*, function: str, class_id: int,
    ...
    if worksheet.window_order and not worksheet.candidates and not worksheet.tooling_leads:
...
**T10b outcome → BINDING design amendment for the calibration fixtures (orchestrator ruling):**
Context-B run froze 0/5 (provenance b354f7d69): win BASE states are FULLNORM-0 but carry a uniform
±8B frame delta vs the dol after 114 commits of TU-context drift → stack-layout primary → excluded;
the POST states reproduce faithfully today (CursorProc 100 instruction-identical). RULING — the
win-recovery fixtures are SELF-CONTAINED pairs:
1. `phys_target` := the POST-win build's actual assignments (extracted from the post IG in the same
   context), NOT the dol target. The win's outcome IS the post state; this strengthens the recovery
   check (the solver must reach the exact coloring the winning alias produced).
2. The register-only admission precondition for a win fixture is judged on the PRE-vs-POST pair
   (their diff is the win itself — pure coloring), not PRE-vs-dol.
3. Reject/flag fixtures still admit against their live targets; their current aborts are expected to
   clear via (i) the #611 identical-multiset classifier fix (reject_a), (ii) the CreateStatRow
   one-extra-call source fix making it genuinely register-only (flag_c), (iii) re-examination of
   reject_b's 2-line near-admission under the fixed classifier. NO fixture is forced; per-fixture
   honesty stands. T12 still cannot write GATE: PASS until n≥5 freeze and behave.

**Calibration admission amendment 2 (orchestrator ruling, triage-11 window):** the reject/flag
fixtures' admission must not depend on the checkdiff PRIMARY label (the #611 identical-multiset FP
mislabels two of them inline-boundary while open). The generator admits a reject/flag fixture on
DIRECT EVIDENCE: (i) bl-target multisets byte-identical between current and target (the verify-ib
REL24 method), AND (ii) every normalized diff line is register-class (no instruction-count,
call-shape, or reloc-symbol deltas beyond the known-sanctioned ones). A fixture failing (i) or (ii)
is honestly excluded with the evidence recorded — reject_b's 2 near-admission lines get this
direct-evidence verdict rather than a label; if structurally excluded, the provenance flags the
substitution question (a different real function with the caller-invisible-split property) back to
the orchestrator rather than freezing anything synthetic.

**Calibration class ruling (orchestrator, A1 rev 2 clause 6, post mine-rejex):** the corpus query
(mine-rejex, /tmp/rejex-mine-report.md; 196/354 ≥97% partials passed direct evidence) resolves the
three reject-confirmations as:
- rejected_a → REAL exemplar `gm_80164504` (99.52%; contested ig=38 first-def `li r38,0`,
  is_runtime_value=False; 2/2 reg-class lines, bl-multiset equal). Backup `it_802C4D10`.
- flagged_c → REAL exemplar `ftPp_SpecialS_0_Coll` (99.88%; is_window_order_residual True for
  r28→r27 delta=1, False on caller-save control; 6/6 reg-class). Backup `grVenom_80203B18`.
- rejected_b → PLANTED over a frozen win IG (NEGATIVE COVERAGE recorded: no register-only
  caller-invisible-split exemplar exists in the ≥97% pool — source_object is None only for
  untraced first-defs, but every real inlined-block residual node attributes to a pcode expression
  → caller_visible_source=True; even fn_803AC7DC's 15 contested nodes all attribute). The plant
  follows A1 rev 2 §1-7 (no-oracle, recompute-equality audit, paired-trace invariance,
  broken-filter control).
T10e freezes the two real exemplars as whole-function fixtures (stronger than plants — independent
function + IG + force-phys target + whole-solver reject assertion) and builds the one Class-B plant.
Gate then = 5/5.

## T19 punch-list (accumulated from per-task reviews — fold into T19's self-review pass)
- **(rev-t17 robustness MINOR)** `fpr_sweep.run_fpr_sweep`: the per-function `subprocess.run(..., timeout=600)` is unguarded — one hung/erroring `debug dump local` during the full ~17.9k-fn time-boxed sweep aborts the WHOLE run and leaves a stale `.<fname>.fprsweep.<pid>.pcdump.txt` in the source tree. Wrap the per-function dump+parse in `try/except (TimeoutExpired, parse errors)` that `unlink`s the temp `out` and `continue`s (skip the fn), so the full sweep is resilient. Relates to #620 (the same hang class). Lock already releases on exception.
- **(rev-t14 nit 1)** the T14 CLI factory test's runtime `_VA` stub exercises the `source is None` reject but not the A2-distinctive `implicit-temp`/`copy-coalesce-product` KIND reject (covered by the calibration gate + probe.py units, but not in the CLI test). Optional: add a `kind="implicit-temp"` source case asserting `caller_visible_source is False`.
- **(rev-t17 DRY minor)** `fpr_sweep._unit_source_path` re-inlines the `structure_scoring` helpers; import+delegate or relabel the comment "mirrors" (it claims "reuses").
