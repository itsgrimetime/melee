...
- **REJECT-CONFIRMATION (3):** each a whole-solver run whose enumeration trace must
  ...
  **HandleInput-S2 constant-zero** → the `var_r28` node-add is `rejected_a` at
  ...
  > **SUPERSEDED for the `rejected_b` FIXTURE (see Amendment A2 + provenance
  > run-4/run-5/run-6):** 80242C0C is NOT register-only at any reachable build
  > state (structurally excluded by direct evidence, run-4) and has no `source=None`
  > node, so it never froze. Amendment A2 re-grounds `rejected_b` on provenance
  > KIND and freezes it on a REAL synthetic-intermediate node (flag_c IG **ig55**,
  > `implicit-temp addi`). The 80242C0C description above is retained as the
  > original L2(b) MOTIVATION (the intra-inline web is exactly the
  > `copy/coalesce-product`/`implicit-temp` class A2 rejects), not the frozen
  > fixture.
...
- **§1.5 calibration spike (BLOCKING — must pass before enumeration is wired to a
  ...
  REJECT-CONFIRMATION (HandleInput-S2/gm_80164504 → `rejected_a`; `rejected_b` via
  provenance KIND on flag_c IG ig55 [Amendment A2, NOT 80242C0C — superseded];
  CreateStatRow/ftPp_SpecialS_0_Coll → `flagged_c` + exit-4 `window-order`);
...
## Amendment A1 rev 2 (2026-06-12, owner-approved direction; codex-hardened): planted reject-confirmations over the frozen win IGs

Empirical basis (FIXTURE_PROVENANCE.md, T10b context-B / T10c run-3 / T10d run-4): the three named
reject/flag exemplar FUNCTIONS carry structural residuals beyond their reject-class sites and are
excluded by DIRECT-EVIDENCE admission (bl-multiset parity + all-register-class normalized lines) —
independent of the #611-FP labels. Whole-function freezes of them would test preconditions, not the
filter. This amendment INTENTIONALLY REDUCES function-level reject coverage and compensates as below.

**Amended gate definition (n>=5 arithmetic unchanged: 2 win-recoveries + 3 reject-confirmations):**

1. **Plant mechanism (no-oracle rule — normative).** A plant is an ORDINARY candidate node-add
   targeting a chosen virtual in a frozen win pre-IG, appended via a documented forced-injection
   seam into the candidate stream of the production `solve_coloring` enumerate+filter path. Its
   ProbeContext MUST be produced by the production probe derivations (probe.py) over the frozen raw
   artifacts (IG + source-attribution bridge captured at freeze time). Candidate and ProbeContext
   objects MUST NOT carry oracle fields (`expected_token`, `reject_class`, pre-labeled reasons, or
   any equivalent); expected assertions live ONLY in fixture metadata outside the solver input. The
   harness MUST assert the returned reason token is computed by the normal predicate path (recompute
   `passes_1_5_filter` over the same derived context and assert equality with the solver-returned
   verdict).
2. **Plant selection per class (raw-field derivation).** Class A (`rejected_a`): target a virtual
   whose frozen first-def opcode is li/lis (real constant site in the win IG). Class B
   (`rejected_b`): target a virtual whose frozen attribution has no caller-visible source object
   (source=None in the real bridge). Class C (`flagged_c`): target a virtual for which the
   production window classifier over the frozen artifacts yields the window-order signal. Each
   signal derives from raw fields; if a win IG lacks a qualifying site for a class, that class runs
   on the other win IG (mapping + justification recorded in provenance). Where qualifying sites
   exist in both IGs, the class is planted on BOTH.
3. **Broken-filter control (gate-level).** The planted suite MUST include a control run with a
   deliberately permissive filter stub (admit-everything): every planted test MUST FAIL under it.
   A filter that cannot fail cannot pass the gate.
4. **Non-perturbation (paired-trace invariance).** Each planted run is paired with the unplanted
   baseline on the same IG: non-plant candidate identities and filter outcomes UNCHANGED; the
   winning-alias candidate identity UNCHANGED with rank no worse; `enumeration_truncated` UNCHANGED;
   per-kind eval counts identical except the plants' own accounted evals; no pair-escalation,
   frontier, or early-stop transition caused by the plants. (The two unplanted win-recovery fixtures
   remain mandatory and unchanged.)
5. **flagged_c / exit-4 split.** The class-C plant is a CANDIDATE-LEVEL quarantine test only: it
   must emit `flagged_c` and land in the worksheet `window_order` bucket, never in candidates. The
   exit-4 `reason: window-order` negative control is NOT claimed by the planted run (the win IG has
   admitted candidates); it remains where it lives unweakened — T15's N-series isolated all-window
   scenario — and T11 must cite that control rather than duplicate it.
6. **Real-exemplar replacement (binding).** The corpus query for register-only real exemplars
   (option (a)) MUST be recorded (command + coverage + result) in the provenance BEFORE planted
   substitutes are accepted. Any class with a qualifying real-function exemplar found MUST use the
   real frozen fixture instead of the plant (real > planted); classes falling back to plants record
   the query's negative coverage statement.
7. **Trace freezing.** Each planted run's full production enumeration trace (worksheet + filter
   tallies + the plant's verdict) is frozen alongside the fixtures for audit.

Pass criteria per planted test: the exact T3 schema token from the production predicate path, the
paired-trace invariance of (4), and (for class C) the quarantine placement of (5). The gate's
n>=5 counts: 2 win-recoveries (unplanted) + 3 class-confirmations (real exemplar or planted per (6)).

---

## Amendment A2 (2026-06-12): provenance-based caller-visibility — closing the dead L2(b) branch

**Empirical basis (T10e calibration + corpus mine, confirmed against the frozen
`bridge.json` artifacts).** The L2(b) caller-visibility check was
`caller_visible_source = (source_object is not None)`, where `source_object`
(`probe.source_object_of`) returns `name or expression`. EVERY real traced node
carries at least an `expression` (a param `nana_gobj`, a local `data`, a
call-return `mn_GetDigitAt(...)`, even a synthetic `implicit-temp` with
`expression="rlwinm r34,..."`). So `caller_visible_source` is **always True on
real input**, the `rejected_b` branch is **DEAD**, and the solver would ADMIT
unrealizable intra-inline node-adds (e.g. 80242C0C's `+13c` copy inside an
inlined `HSD_JObjSetTranslateX` — a node no caller-level C alias can route
through). This is a latent correctness hole, not merely a missing fixture.

**Why the brief's literal `source_file`+`source_line` line-range is UNSOUND, and
the corrected SOUND signal (provenance KIND, not raw line).** Two facts kill a
bare line-range check, demonstrated against the frozen bridges:
- `explain_virtuals` threads a SINGLE TU `source_file` into EVERY attribution, so
  "different source_file" never fires for a same-TU inline (the
  `ftPp_SpecialS_0_Coll` inlinees live in the same `ftNn_Init.c` as the matched
  fn).
- the attribution `source_line` is the imprecise output of a source-text
  heuristic (`_find_source_expression` greps for `base_var->field` / the call
  expression and takes a match). A line-range REJECT trigger therefore produces
  **false rejections**: a real in-body local `popo_gobj` and named call-return
  locals `temp_r31`/`stage_idx` get attributed to an EARLIER identical call
  elsewhere in the TU (lines 466/2215/2216), and an in-body call
  `ftPp_SpecialS_8011F964(popo_gobj)` (matched-fn line 957) is attributed to the
  identical call in a SIBLING function at line 851 — all caller-visible, all
  wrongly rejected by a naive out-of-span test. A line-range check mixes genuine
  out-of-body inlinees with these heuristic misses, so it is NOT a sound REJECT
  signal.

The corrected derivation makes the **provenance KIND** the reject trigger (it
encodes whether a value HAS a source-level variable at all) and uses
name/line/span only as ADMIT (never reject) signals, guaranteeing **zero false
rejections / win-preservation**:

- **REJECT (`rejected_b`) iff the node is a compiler-synthesized intermediate
  with NO source-level variable:** `source.kind in {"implicit-temp",
  "copy/coalesce-product"}`, OR (`source.kind == "first-def"` with `name is None`
  AND `source_line is None`). These are `mr`/`addi`/`rlwinm`/li-product values
  the back-end created mid-expression — there is provably NO caller-level C
  expression boundary an alias can split (exactly 80242C0C's `+13c` copy inside
  the inlined `HSD_JObjSetTranslateX` parameter web). This is the dead branch's
  true population; it is large and stable in both frozen bridges (≈30 nodes
  each).
- **ADMIT (caller-visible) otherwise** — any node with a source-level NAME
  binding (`param`/`local`/named `call-return`: the win levers `gp/flow/data/
  digit/new_var`, params `stage_id/nana_gobj`), and any `field-load` (the
  field-access aliases `nana_gobj->field_at_0x2C`, `name=None` — the brief's
  explicit caution that these are alias-introduction levers, NOT rejects). A
  conservatively-admitted genuinely-inlinee field-load (one whose base chains
  into an inlinee) is caught downstream by §3 re-extraction; admitting it cannot
  cause a false WIN (it must still survive the surrogate + §3), whereas REJECTING
  a realizable alias would break a win. Soundness (zero false reject) is chosen
  over completeness (catching every inlinee node at enumeration).

The matched-function `(source_file, sig_line, end_line)` span is still threaded
in (below) and recorded for diagnostics / a future tightened in-span ADMIT
refinement, but the v1 REJECT decision rests on `kind`, which needs no line.

Note interaction with L2(a): a `first-def li/lis` constant is `rejected_a`
(checked FIRST in `passes_1_5_filter`), so the constant case never reaches L2(b).
The `rejected_b` residual is the runtime no-source-variable synthetic class.

**Threading the matched-function provenance.** `derive_probe_context` /
`build_probe_ctx_fn` gain an optional `source` argument: the node's RAW
attribution `SourceAttribution` (the `bridge.json` entry), from which the
provenance KIND is read. `source_object_of` is kept (legacy fallback /
`tooling_leads` naming); a new `caller_visible_source_of(report, ig_idx,
matched_fn=None)` computes the verdict. The matched-function
`(source_file, sig_line, end_line)` span is threaded for the recorded-diagnostic
/ future in-span ADMIT tightening:
- **Calibration/test path:** carried in fixture metadata (`fixture.json`
  `matched_fn_span`) and re-derivable from the frozen `bridge.json` (each
  `source` records `source_file`/`source_line`); the harness passes the raw
  source into the probe closure.
- **Live CLI path:** `symbol_bridge._extract_function_text(source, fn)` already
  returns `(params_text, body_text, sig_line)`; the TU path is the `source_file`.
  Same extraction `explain_virtuals` uses — no new parsing risk.
The KIND-based REJECT needs no provenance span; when the raw `source` is absent
(the legacy `source_object is not None` signal only), the check **falls back to
legacy** so a not-yet-wired live path cannot regress to spurious `rejected_b`
(conservative: admit, let §3 catch). The calibration gate ALWAYS supplies the
raw source.

**Win-preservation invariant (HARD, verified).** The win levers are
named `param`/`local`/named-`call-return` bindings (gp/flow, data/digit, new_var)
→ ADMITTED (they are NOT a synthetic-intermediate kind), independent of any line
heuristic. Audited against the frozen bridges: the corrected REJECT set is
EXACTLY the `implicit-temp`/`copy-coalesce`/no-source `first-def` class (≈30 nodes
in each of flag_c + reject_a) — every named binding and every field-load is
admitted; **zero false rejections**. The two win-recovery fixtures do not
exercise this branch (surrogate-ranking assertion, no bridge), so the invariant
is enforced on the LOGIC: a rule that rejects only no-source-variable synthetic
intermediates cannot reject a named caller-level alias. If implementation reveals
the check rejects ANY win-recovery node, the fix is WRONG → revert + report.

**rejected_b refreeze (closes the gate to 5/5).** With the corrected derivation a
genuinely caller-invisible node in a frozen IG now yields `rejected_b` at
enumeration. The flag_c `ftPp_SpecialS_0_Coll` bridge contains many qualifying
SYNTHETIC-intermediate nodes (e.g. **ig55** `implicit-temp`
`expression="addi r55,r54,1"`, **ig67** `copy/coalesce-product`,
**ig52** `first-def` `expression="lis r52,3148"` with no name/line). The refreeze
targets such a node in the FROZEN flag_c IG (real artifact, no new compile),
running the whole-solver assertion: production `enumerate_single` GENERATES
node-add candidates → `passes_1_5_filter` returns `rejected_b` → 0 survivors into
full/partial/window. NOTE the L2 ordering — pick a node that is `is_runtime_value`
(an `implicit-temp` like ig55 whose first-def opcode `addi` is NOT li/lis, so
L2(a) passes and the verdict is genuinely `rejected_b`, not `rejected_a`); a
`first-def lis` node (ig52) would be `rejected_a` and is unsuitable. Freeze as
`reject_b_<fn>`; broken-filter control + recompute-equality audit apply
unchanged. This replaces the run-5 "Class-B plant INFEASIBLE (no source=None
node)" finding: the hole was that `source=None` was the WRONG admission key;
provenance KIND makes the branch reachable on a REAL frozen node.

**Test updates (TDD).** `test_probe.py` and `test_validity.py` `rejected_b` cases
move from the `source_object=None` premise to the provenance premise (a node with
a present source whose KIND is a synthetic intermediate → caller-invisible; a
named local / field-load → caller-visible). `test_calibration_t10e.py`'s
`_report_rejected_b` (currently `41: None`) becomes a node with a real
synthetic-intermediate source. The strict-bool `test_probe.py` guarantees and the
admit/field-load cases stay green; L2(a)/L2(c)/L1 are unchanged.

**Gate arithmetic after run-6:** 2 win-recoveries + reject_a (gm_80164504) +
flag_c (ftPp_SpecialS_0_Coll) + reject_b (provenance refreeze) = **5/5**.
