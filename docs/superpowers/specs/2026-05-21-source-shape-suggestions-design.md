...
- `pcdump-local`, `force-phys`, `force-phys-iter`, and `force-coalesce`
  can prove whether a target allocation is reachable.
- `guide`, `score-source`, `suggest-coalesce-source`, and
  `tier3-search` can explain many register-cascade and lifetime
  hypotheses.
...
- Make `mutate insert-alias` and `enumerate-decl-orders` scope-aware so
  source-shape tools can work inside nested blocks without illegal
  cross-scope edits.
- Feed compiler-temp facts from `guide` and `suggest-coalesce-source`
...
- No automatic upstream decomp-permuter patching. Existing
  `verify-perm` guards remain the local protection layer.
...
- `--target PATH`: optional target spec for `score-source`; implies the
...
- optionally run `pcdump-local` plus `score-source` when a target is
  supplied;
...
- `tier3_search.py`: add an optional seed planner path that can consume
  compiler-temp anchors from `guide`/`suggest-coalesce-source`.
- `cli/debug.py`: add `suggest-inlines` and expose scope controls on
...
### Seed source 3: `suggest-coalesce-source --discover`
...
- consume anchors from `suggest-coalesce-source --discover` and
  `guide`;
...
6. If `--target` is supplied, run `pcdump-local` and `score-source`.
...
- non-applying `suggest-inlines -f fn_80247510`;
- non-applying `suggest-inlines -f fn_80248A78`;
- non-applying `suggest-inlines` on one `mndiagram3.c` function;
...
- compiler-temp source anchoring from `guide` and
...
- `suggest-casts` multi-hop signedness recommendations.
- Upstream decomp-permuter `inline_fn` placeholder fix.
- Pre-match calibration case for `suggest-coalesce-source`.
...
- Scope-aware `enumerate-decl-orders` can enumerate swaps within a
