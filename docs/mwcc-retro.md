...
## Issue #1240 preview boundary

The three `probe-backend-*` commands and `backend-candidate` are the issue
#1240 preview toolset. Their v1 outputs are useful diagnostic partial/candidate
evidence, but they do not promote the exhaustive retail PCode lifetime proof.
The explicit `backend-candidate --one-pass --trace-version v2` path must remain
fail-closed until the exact proof registry and hooks are independently promoted.
See `tools/mwcc_retro/README.md` for the artifact-level trust contract.
