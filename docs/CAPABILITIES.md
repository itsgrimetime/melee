...
- `melee-agent backtest build-corpus` — Enumerate verified small/singular match commits into the backtest store.
- `melee-agent backtest calibrate` — Run the Phase-0 two-sided calibration gate and exit non-zero if it fails.
- `melee-agent backtest close-sandbox` — Tear down the sandbox worktree for a case.
- `melee-agent backtest discover` — Scan master for single-function match commits (cheap, no builds).
- `melee-agent backtest escalation` — List case IDs that should be escalated to the blind-agent tier.
- `melee-agent backtest judge-input` — Print the label-blinded judge input (function, ground_truth_diff, lever_class, tool_outputs) the LLM advisory judge consumes for a case.
- `melee-agent backtest open-sandbox` — Build an isolated sandbox worktree for a blind matching attempt.
- `melee-agent backtest pending-judge` — Print the JSON list of case_ids that have a result row (ready for judging).
- `melee-agent backtest report` — Print the coverage matrix.
- `melee-agent backtest run` — Run the cheap tiers over the stored corpus.
- `melee-agent backtest set-advisory` — Record the LLM advisory-judge verdict for a case.
- `melee-agent backtest set-agent` — Record the blind-agent outcome verdict for a case.
- `melee-agent backtest status` — Report harness readiness.
...
- `melee-agent claim add` — Claim a function to prevent other agents from working on it.
- `melee-agent claim list` — List all currently claimed functions.
- `melee-agent claim release` — Release a claimed function.
...
- `melee-agent complete document` — Mark a function as documented (naming, comments, struct fields).
- `melee-agent complete list` — List all completed/attempted functions.
- `melee-agent complete mark` — Mark a function as completed/attempted.
- `melee-agent debug artifacts prune` — Preview, or explicitly apply, conservative artifact-run pruning.
- `melee-agent debug artifacts report` — Report retained diagnostic evidence without modifying it.
...
- `melee-agent debug inspect causal-diff` — Compare two immutable compiler-artifact frontiers without generating artifacts.
...
- `melee-agent debug measure inline-leverage` — Measure whether real inline boundaries are structural codegen levers.
...
- `melee-agent debug permute local-orphans` — Detect abandoned local permuter compiler and Python worker processes.
...
- `melee-agent debug permute remote local-prune` — Plan or safely apply retention for locally fetched remote runs.
- `melee-agent debug permute remote local-retain` — Explicitly retain one owned locally fetched remote run.
...
- `melee-agent debug retro ghidra-setup` — Create or validate the exact GC/1.2.5n compiler Ghidra audit project.
...
- `melee-agent debug retro verify-backend` — Compare a retail backend trace to mwcc-debug pcdump facts.
- `melee-agent debug search baseline-escape` — Generate post-ceiling baseline escape candidates for retained allocator stalls.
- `melee-agent debug search combine` — Recombine complementary directed-search candidate source hunks.
- `melee-agent debug search delta-minimize` — Exhaustively minimize the closed source-delta lattice between two parents.
- `melee-agent debug search delta-namespace-review seal` — Seal explicit namespace-review authority into a provenance-bound sidecar.
...
- `melee-agent debug search source-family-continuation` — Summarize post-meta source-family continuation evidence.
- `melee-agent debug search source-model-synthesis` — Generate and classify post-meta-ceiling source-family probes.
...
- `melee-agent debug suggest expression-interferer-repair` — Rank expression-scored FPR repair candidates without losing protected hits.
- `melee-agent debug suggest frame` — Suggest source/probe levers for stack-frame/local-area residuals.
- `melee-agent debug suggest inline-boundary-continuation` — Generate or close strict inline-leverage helper-boundary probes.
- `melee-agent debug suggest inlines` — Suggest hidden inline/helper/source-shape candidates.
- `melee-agent debug suggest protected-expression-reconcile` — Generate/rank dual-frontier protected expression reconciliation probes.
