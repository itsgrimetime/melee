...
Bridge retained select-order/window-order evidence into remotely scored probes:

```bash
melee-agent debug search plan-transforms \
  -f mnDiagram_SortNamesByKOs -u melee/mn/mndiagram \
  --force-phys 34:27,44:25 \
  --source-file retained_case_c_ig34_hit.c \
  --select-order-json select_order.json \
  --transform-family retained_gpr_case_c_window_order_continuation \
  --write-probes build/diagnostics/retained_ig44_window_order/probes \
  --json
```

The `retained_gpr_case_c_window_order_continuation` family is fed by
`plan-transforms --select-order-json`. It converts
`window-order-source-steering` `LifetimeLayoutProbe`s from
`debug select-order-search --json` into normal transform-corpus probes, with
IG34 recorded as a protected target and IG44 as the attempted continuation.

The `retained_gpr_case_c_target_live_range_repair` family consumes the same
retained SortNames frontier context, or an explicit
`retained_case_c_repair_goals` payload, and materializes bounded IG44/r39
live-range, interference-shape, implicit indexed-expression spelling,
index/base alias, address-side, value-side, and coupled address/value probes
while preserving protected IG34. For the SortNames Case-C frontier this covers
both `sorted_names[(max_idx)]` vs `sorted_names[max_idx]` spellings and the
duplicated `sorted_names[j]` value temps around `GetNameText`.
