...
def intervene(ctx):
    ...
    def snapshot(stage):
        if stage != "final_scheduler":
            return
        state["stage_counts"][stage] = state["stage_counts"].get(stage, 0) + 1
        snap = {"stage": stage, "sequence": state["sequence"]}
        sample_frame_state(snap, stage)
        state["events"].append(snap)
        with open(ctx.out_dir + "/frame-only-live.json", "w") as f:
    ...
    class CodegenStart(gdb.Breakpoint):
        def stop(self):
            ...
            with open(ctx.out_dir + "/functions-live.jsonl", "a") as f:
    ...
    for stage, va in candidates.items():
        if stage != "final_scheduler" or not isinstance(va, int):
            continue
